...
def 信号已中止(信号):#读中止状态
    ...
    if 信号 is None:#无信号
        return False#未中止
    if callable(getattr(信号,'throwIfAborted',None)):#有 throwIfAborted
        try:#探测
            信号.throwIfAborted()#未中止则返回
            return False#未中止
        except Exception:#已中止
            return True#已中止
    return bool(getattr(信号,'aborted',False))#aborted 属性
