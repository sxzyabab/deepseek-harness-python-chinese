...
class _控制队列:
    ...
    def 迭代(自身,信号):
        """直到取消或结束。"""
        while (not 自身._已结束) and (not 已中止(信号)):#活跃
            帧=自身._缓冲.头弹()#取帧
            if 帧 is not None:#有帧
                yield 帧#产出
                continue#继续
            自身._事件.clear()#清事件
            if 自身._缓冲.大小>0 or 自身._已结束 or 已中止(信号):#竞态
                continue#重试
            自身._事件.wait(0.05)#短等
        while 自身._缓冲.大小>0 and (not 已中止(信号)):#排空
            帧=自身._缓冲.头弹()#取
            if 帧 is not None:#有
                yield 帧#产出
    ...
    def __init__(自身,上下文):
        ...
        def 挂任务(任务上下文):
            ...
            def 任务变化(所有者):
                """任务变化。"""
                自身._任务变化(所有者)#委托
            任务上下文.jobs.onJobsChanged(任务变化)#监听
        def 会话创建(会话):
            """新会话。"""
            自身._会话创建(会话)#委托
        def 拆除控制():
        ...
        上下文.副作用(拆除控制,'session-controller.control')#拆除
