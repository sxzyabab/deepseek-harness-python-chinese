...
"""
import os#目录
from .远程错误与并发 import 远程错误,远程错误消息,操作任务#远程错误与任务
...
class 会话智能体控制器:
    ...
    def 解析智能体(自身,会话标识):
        ...
        恢复=自身._恢复中[会话标识] if 会话标识 in 自身._恢复中 else None#已有任务
        if 恢复 is None:#新恢复
            恢复=操作任务()#共享任务
            自身._恢复中[会话标识]=恢复#登记
            def 跑():
    ...
    def 确保会话(自身,会话标识,工作目录,检查持久身份,预设标识=None):
        """解析请求的身份，必要时创建或恢复一次。"""
        创建=自身._创建中[会话标识] if 会话标识 in 自身._创建中 else None#已有
        if 创建 is None:#新创建
            创建=操作任务()#共享任务
            自身._创建中[会话标识]=创建#登记
            def 跑():
                """创建或采用并结算。"""
                try:
                    创建.兑现(自身._创建或采用(会话标识,工作目录,检查持久身份,预设标识))#兑现
                except BaseException as 错误:
                    创建.拒绝(错误)#拒绝
                finally:
