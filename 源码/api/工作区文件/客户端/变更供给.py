...
随后按该路径过滤（`\\` 归一为 `/`）。资源地址只用于刷新查找，从不决定通知路径。
"""
import json#断言帧
...
class _跟随者:
    ...
    def __init__(自身,地址,离开):
        """记下资源地址与注销回调。"""
        自身.地址=地址#刷新查找键
        自身._离开=离开#注销
        自身._待发=[]#待发 (key|None, notice)
        ...
        try:
            while True:#拉取
                if len(自身._待发)>0:#有待发
                    下一条=自身._待发.pop(0)#取头
                    键=下一条['key']#路径键
                    if 键 is None or 自身._宿主键 is None or 键==自身._宿主键:#匹配或尚未绑定/刷新
                        yield 下一条['notice']#通知
                    continue#下一条
                if 自身._已结束:#结束
                    return#停
...
class _会话供给:
    ...
    def 请求重stat(自身,地址):
        """刷新一地址及同 Session 同宿主路径的同伴。"""
        键集合=set()#同路径键
        for 跟随者 in 自身._跟随者集合:#收集
            if 跟随者.地址==地址 and 跟随者.键 is not None:#命中地址且已绑定
                键集合.add(跟随者.键)#记下
        for 跟随者 in 自身._跟随者集合:#投递
            if 跟随者.地址==地址 or (跟随者.键 is not None and 跟随者.键 in 键集合):#地址或同键
                跟随者.推({'kind':'restat'})#重 stat
...
class 变更供给:
    ...
    def 请求重stat(自身,会话标识,地址):
        """请一地址及其同 Session 宿主路径同伴再次 stat。"""
        供给=自身._会话表[会话标识] if 会话标识 in 自身._会话表 else None#取
        if 供给 is not None:#有
            供给.请求重stat(地址)#委托
