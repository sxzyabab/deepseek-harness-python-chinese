...
class 客户端终端(服务):#侧栏出现与后台清理
    ...
    def __init__(自身,上下文,远程):#构造
        ...
        def 拆除效果():#fiber
            ...
            def 清理():#拆除器
                ...
                自身._视图.clear()#清
                自身._绑定.清空()#清绑定
                for 会话保持 in list(自身._保持.values()):#保持
                    for 保持 in list(会话保持.values()):#各
                        保持.dispose()#拆
                自身._保持.clear()#清
                for 视图 in 待拆:#拆
                    视图.释放()#放流
    ...
    def _取保持(自身,会话标识,标识):
        ...
        if 会话保持 is None:#新建
            ...
            自身._保持[会话标识]=会话保持#登记
        保持=会话保持.get(标识)#已有
        if 保持 is None or 保持.已失败:#新建或重试
            if 保持 is not None:#旧失败
                自身._释放保持(保持)#释放
    ...
    def _对账保持(自身):
        ...
        for 标签 in 自身._打开标签:#标签
            ...
            if 会话表 is not None:#有
                视图=会话表.get(标签['tabId'])#出现
            if 视图 is None or 视图.状态.getSnapshot().get('info') is not None:#需保持
                if 自身._保持.get(标签['sessionId']) is None or 标识 not in 自身._保持[标签['sessionId']]:#无
    ...
    def _释放保持(自身,保持):
        ...
        任务=object()#标记
        自身._释放中.add(任务)#登记
        线=threading.Thread(target=跑)#后台
        线.daemon=True#守护
        线.start()#启
...
def 应用(上下文):
    """安装客户端终端模型。"""
    客户端终端(上下文,上下文.remote.terminal)#挂
