...
class 浏览器终端:#一路 PTY、有界仿真与可拆跟随者
    ...
    def 监视(自身,策略,关闭中,已关,失败汇):
        """提交后开始无人值守监视。"""
        def 观察():
            """活动。"""
            return 自身.句柄.检查活动() if hasattr(自身.句柄,'检查活动') else 自身.句柄.inspectActivity()#活动
        def 终止():
            """关进程。"""
            关闭中()#关 id
            自身._关进程()#关
            已关()#摘
        自身._保持器=终端保持(策略,观察,终止,失败汇)#保持

    def 保持(自身,信号):
        """独立于输出附着保持进程。"""
        if 自身._保持器 is None:#未提交
            raise RuntimeError('Terminal has not been committed')#拒绝
        yield from 自身._保持器.retain(信号)#保持
    ...
    def 关闭(自身):#先终止进程范围再放屏幕
        """失败可重试。"""
        if 自身._保持器 is not None:#有保持
            return 自身._保持器.close()#经保持
        return 自身._关进程()#直接

    def 释放(自身):
        """停止无人值守并等进程清理。"""
        if 自身._保持器 is not None:#有
            return 自身._保持器.dispose()#经保持
        return 自身._关进程()#直接
