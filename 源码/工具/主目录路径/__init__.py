...
__all__=[#仅中文公开名
    ...
    '默认主目录','展开家目录路径','解析主目录','主目录路径','缓存路径','主目录展示',
]#公开面结束
...
def 缓存路径(选项或分段=None,*分段):#缓存目录下的拼接路径
    """把路径分段拼到已解析 Harness 主目录的 cache 目录上，不创建该目录；无参数则返回该目录本身。第一参可以是显式主目录覆盖 dict（键 dshHome）或第一段路径。"""
    if 选项或分段 is None:#省略则默认主目录下的 cache
        return os.path.join(解析主目录(),'cache',*分段)#cache 目录
    if isinstance(选项或分段,str):#第一参是路径段
        return 主目录路径('cache',选项或分段,*分段)#cache 下拼接
    主目录覆盖=选项或分段['dshHome'] if 'dshHome' in 选项或分段 else None#显式主目录
    return os.path.join(解析主目录(主目录覆盖),'cache',*分段)#覆盖主目录下的 cache
