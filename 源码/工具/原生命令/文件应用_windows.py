...
def windows文件应用程序(路径,应用程序,信号,运行):
    ...
    if 应用程序 is None:
        动作="ConvertTo-Json -InputObject @([DshFileAssociations]::List($path)) -Depth 4 -Compress"
    else:
        动作="[DshFileAssociations]::Open($path, $application)"
    脚本=("$ErrorActionPreference = 'Stop'\n"
        "[Console]::OutputEncoding = New-Object System.Text.UTF8Encoding($false)\n"
        "Add-Type -ReferencedAssemblies System,System.Core,System.Drawing -TypeDefinition @'\n"
        +WINDOWS关联+
        "'@\n"
        "$path = [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('"+编码路径+"'))\n"
        "$application = [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('"+编码应用+"'))\n"
        +动作+"\n")
    结果=运行('powershell.exe',[
        '-NoProfile','-NonInteractive','-STA','-EncodedCommand',
        base64.b64encode(脚本.encode('utf-16le')).decode('ascii'),
    ],信号)
    return 结果['stdout']

__all__=['windows文件应用程序']
