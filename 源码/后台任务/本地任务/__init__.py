...
注册表比生产者和控制器寿命更长。智能体或服务拆除会取消在线工作并等待合规生产者；抛错的拆除取消只强制失败记录，并报告可能的孤儿。
"""
...
class 本地任务注册表(任务注册表):#进程内任务注册表
    """内存里的 jobs 注册表。所有权、隔离与生命周期语义由所属服务定义约定，本实现遵守它们。"""
    Config=配置#配置模式
