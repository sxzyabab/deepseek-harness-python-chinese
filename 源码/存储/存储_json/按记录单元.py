...
def _加载表记录(记录映射,版本,目录):#加载一张表的记录
    ...
    for 名 in 条目:#每个文件
        ...
        if 安全键正则.fullmatch(键) is None:#键不安全
            continue#跳过
        值=_读记录(os.path.join(目录,名),版本)#读记录
...
def _引导遗留单元(描述符,目录,状态):#从 legacy 整文件引导
    ...
    if not isinstance(文档,dict):#不是对象
        return#不解释
    单元头=文档['unit'] if 'unit' in 文档 else None#头
    if not isinstance(单元头,dict) or ('name' not in 单元头) or 单元头['name']!=描述符.name:#外单元
        return#不引导
    表映射=文档['tables'] if 'tables' in 文档 else None#表
    if not isinstance(表映射,dict):#不是对象
        return#不引导
    for 表名,记录 in 表映射.items():#每张表
        if 表名 not in 状态.tables:#未声明
            continue#跳过
        目标=状态.tables[表名]#声明表
        if not isinstance(记录,dict):#表不是对象
            continue#跳过
        for 键,值 in 记录.items():#每条记录
            路径=os.path.join(目录,表名,键+'.json')#目标路径
            os.makedirs(os.path.dirname(路径),mode=0o700,exist_ok=True)#建父目录
            原子写(路径,序列化记录(描述符.version,值))#写记录
            目标[键]=值#记入状态
    ...
    if 条目 is not None:#目录存在
        for 名 in 条目:#每项
            ...
            if os.path.isdir(完整) and 名 in 状态.tables:#声明表目录
                if _加载表记录(状态.tables[名],描述符.version,完整):#加载表
                    有新文档=True#见到文档
            elif 名=='global.json' and 描述符.hasGlobal:#全局文件
                全局=_读记录(完整,描述符.version)#读全局
                if 全局 is not None:#有效
                    状态.全局值=全局#记下
                    有新文档=True#见到文档
    if not 有新文档:#可跑 legacy 引导
        _引导遗留单元(描述符,目录,状态)#引导
    return 状态#权威状态
