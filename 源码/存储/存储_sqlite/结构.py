...
def 配置数据库(连接,路径,日志模式值):
    ...
    if 盘上版本 not in (0,存储_SQLITE_结构版本):#不兼容
        raise 存储错误(
            'version-mismatch',
            'storage database at "'+路径+'" has schema version '+str(盘上版本)+', incompatible with this build ('+str(存储_SQLITE_结构版本)+')',
        )#拒绝
    连接.executescript('''
    ...
    ''')#元数据表
    if 盘上版本==0:#新库
        连接.execute('PRAGMA user_version = '+str(存储_SQLITE_结构版本))#盖戳
    连接.commit()#提交
