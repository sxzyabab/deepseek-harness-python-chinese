...
__all__=['公开工具名','同步工具','MCP结果']

公开名最大长度=64#DeepSeek 函数名最大长度
非法名字符=re.compile(r'[^A-Za-z0-9_-]+',re.ASCII)#非法公开名字符
...
def 公开工具名(服务器名,原始名):
    ...
    if 归一==拼接 and len(归一.encode('utf-8'))<=公开名最大长度:
        return 归一
    摘要=hashlib.sha256((服务器名+'\0'+原始名).encode('utf-8')).hexdigest()[:哈希长度]#身份哈希后缀
    前缀预算=公开名最大长度-哈希长度-1#截断预算（字节）
    前缀=归一.encode('utf-8')[:前缀预算].decode('utf-8','ignore')#按 UTF-8 字节切在字符边界
    while len((前缀+'_'+摘要).encode('utf-8'))>公开名最大长度 and len(前缀)>0:
        前缀=前缀[:-1]
    return 前缀+'_'+摘要
...
def 创建执行器(客户端,原始名,要求任务式,选项):
    ...
    def 执行(参数,执行上下文):
        ...
        参数=参数 if isinstance(参数,dict) else {}#非对象则空对象
        结果=无缓存调工具(客户端,原始名,参数,执行上下文,选项)#无缓存调用
