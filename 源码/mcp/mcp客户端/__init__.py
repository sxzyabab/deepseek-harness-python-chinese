...
from ...内核.作用域 import 获取作用域#登记范围所有者
from ...工具.超时 import 定时器延迟上限毫秒#定时器延迟上限
from .连接 import 默认最大指令字节,重连默认值,解析重连策略,启动连接#重连与监督
from .工具 import 公开工具名,同步工具,MCP结果#工具桥接再导出
from .传输 import MCP错误#本包异常
from .服务器上下文 import 登记服务器上下文#资源与指令
...
配置=复合类型字段(#插件配置模式
    {#stdio 分支
        ...
        'maxInstructionBytes':数字字段(最小=1,默认值=默认最大指令字节),#指令字节上限
...
def 应用(上下文,配置值):
    ...
    def 预留名():
        ...
        名称集=已占用服务器名.get(拥有者)#取出本范围的已占用名
        if 名称集 is None:#尚无集合
            名称集=set()#新建占用集合
            已占用服务器名[拥有者]=名称集#挂到所有者
