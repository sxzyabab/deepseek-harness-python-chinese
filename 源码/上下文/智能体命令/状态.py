...
def 工作区上下文钩子(文本,变更表):#构造带变更记录的工作区上下文消息
    """组装带来源与变更的用户消息。"""
    return 创建用户消息({#组装用户消息
        'content':[{'type':'text','text':文本}],#单文本块
        'source':{'kind':'agent-instructions','form':'instructions','changes':变更表},#带来源与变更
    })#createUserMessage结束
...
def 是否工作区上下文来源(来源):#判断来源是否为带changes数组的工作区指令
    ...
    if 来源 is None:#空来源
        return False#否
    return 来源['kind']=='agent-instructions' and 'changes' in 来源 and isinstance(来源['changes'],list)#判别与changes

def 是否记录(值):#判断是否为普通对象记录
    """非None对象且非数组。"""
    return isinstance(值,dict) and 值 is not None#映射即记录

def 抽出工作区指令变更(来源):#从未知changes抽出合法变更
    """逐项校验并收集合法变更。"""
    变更表=[]#收集合法项
    for 值 in (来源['changes'] if 'changes' in 来源 else []):#逐项校验
        if not 是否记录(值):#非对象则跳过
            continue#跳过
        if 'action' not in 值:#无动作
            continue#跳过
        动作=值['action']#动作
        if 动作 not in ('set','replace','remove'):#动作非法
            continue#跳过
        if 'scope' not in 值 or 'path' not in 值:#缺字段
            continue#跳过
        if not isinstance(值['scope'],str) or not isinstance(值['path'],str):#scope与path必须是字符串
            continue#跳过
        摘要=值['digest'] if 'digest' in 值 else None#可选摘要
        if 摘要 is not None and not isinstance(摘要,str):#digest若出现必须是字符串
            continue#跳过
        项={'action':动作,'scope':值['scope'],'path':值['path']}#收下合法变更
...
def 同一指令变更(甲,乙):#比较两次变更是否同一转移
    """动作、作用域、路径、摘要都相同。"""
    return 甲['action']==乙['action'] and 甲['scope']==乙['scope'] and 甲['path']==乙['path'] and (甲['digest'] if 'digest' in 甲 else None)==(乙['digest'] if 'digest' in 乙 else None)#四字段相同
...
def 基线指令状态(文件列表):#由基线文件得到变更与版本
    ...
    return {'changes':变更表,'versions':版本表}#返回两张表
...
def 保留指令版本更新(更新列表,已渲染变更):#过滤出渲染真正代表的缓存更新
    """只保留已被渲染变更代表的缓存更新。"""
    return [更新 for 更新 in 更新列表 if any(同一指令变更(更新['change'],变更) for 变更 in 已渲染变更)]#变更完全匹配才保留

def 应用指令版本更新(会话,更新列表,缓存):#提交版本缓存更新
    """应用元数据缓存转移，不保留指令正文。"""
    if len(更新列表)==0:#没有更新则不动
        return
    表=会话版本表(会话,缓存)#取该会话表
    for 更新 in 更新列表:#按顺序应用
        if ('state' not in 更新) or 更新['state'] is None:#无state表示删除
...
def 调和指令上下文(智能体,已解析,版本缓存,文件系统,选项):#调和可见状态与提供方文件
    ...
    for 消息 in 选项['scopeMessages']:#待处理提示里的作用域
        if not 是否工作区上下文来源(消息['source'] if 'source' in 消息 else None):#非工作区来源跳过
            continue#跳过
        for 变更 in 抽出工作区指令变更(消息['source']):#抽出变更作用域
            if (not ('includeBaselineScopes' in 选项 and 选项['includeBaselineScopes'])) and 变更['scope'] in 基线作用域:#不参与基线时跳过基线作用域
                continue#跳过
            作用域集合.add(变更['scope'])#纳入提示作用域
    ...
    for 目录,目录作用域集合 in 按目录.items():#按目录处理，一组内失败则整组回滚
        ...
        for 作用域 in 探测作用域集合:#逐个探测
            ...
            项列表.append({'change':变更,'file':文件})#排队渲染
            版本更新.append({'change':变更,'state':下一版本})#排队写入缓存
    if len(项列表)==0:#没有任何转移
        return None#无
    渲染=渲染指令变更(项列表,已解析['maxBytes'])#按预算渲染
