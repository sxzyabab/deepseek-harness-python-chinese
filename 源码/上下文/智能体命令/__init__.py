...
__all__=['名称','注入','应用','配置','解析配置','渲染工作区上下文']#仅中文公开名
...
def 可见基线来源(智能体,权威消息列表):#找当前可见的基线来源
    """最新可见基线，没有则为 None。"""
    for 消息 in reversed(list(权威消息列表)):#权威消息从新到旧
        来源=消息['source'] if 'source' in 消息 else None#消息来源
        if isinstance(来源,dict) and 来源['kind']=='agent-instructions' and 'baseline' in 来源 and 来源['baseline'] is True:#找到基线标记
            return 来源#权威基线优先
    for 序号 in reversed(list(智能体.session.surface.nodes)):#表面节点从新到旧
        ...
        if 事件['type']=='user/message' and isinstance(来源,dict) and 来源['kind']=='agent-instructions' and 'baseline' in 来源 and 来源['baseline'] is True:#表面基线
            return 来源#表面基线
    return None#没有可见基线
...
def 是否工作区上下文(消息):#判断是否为工作区指令上下文
    ...
    return isinstance(来源,dict) and 来源['kind']=='agent-instructions'#来源判别标签

def 同上下文载荷(左,右):#比较两条上下文载荷是否相同
    """内容与来源深相等。"""
    return 深相等(左['content'] if 'content' in 左 else None,右['content'] if 'content' in 右 else None) and 深相等(左['source'] if 'source' in 左 else None,右['source'] if 'source' in 右 else None)#内容与来源

文件触及工具名=set(('read','write','edit'))#会触及文件路径的工具名

def 执行中的文件路径(执行):#从工具执行抽出file_path
    """不是读写改或参数非法则无路径。"""
    if 执行['name'] not in 文件触及工具名:#不是读写改则无路径
        return None#无
    参数=执行['arguments'] if 'arguments' in 执行 else None#参数
    if not isinstance(参数,dict):#参数必须是对象
        return None#无
    if 'file_path' not in 参数:#无路径键
        return None#无
    路径=参数['file_path']#file_path
...
class 投影生命周期信号:
    ...
    def 触发拆除(自身):
        ...
        if 自身._事件.is_set():#只触发一次
            return#已触发
        自身._异常=智能体命令错误('agent-instructions disposed')#拆除原因
        自身._事件.set()#置位
...
def 应用(上下文,配置值):#注册工作区指令插件
    ...
    def 组装(智能体,信号,已声明,待处理,触及路径列表=None):#组装期望的工作区上下文消息
        ...
        已准备=基线准备[智能体.session] if 智能体.session in 基线准备 else None#上次准备
        排除基线作用域=None#本轮排除集
        ...
        if (not 基线已在) or (not 保留可见基线) or 排除基线作用域 is None:#需要加载或替换基线
            ...
            纳入=指令集['included'] if 指令集 is not None and 'included' in 指令集 and 指令集['included'] is not None else []#纳入文件
            ...
            if 版本表 is not None:#有表
                for 作用域,状态 in 基线['versions'].items():#用基线版本播种缓存
                    版本表[作用域]=状态#播种
            渲染文本=指令集['rendered']['text'] if 指令集 is not None and 'rendered' in 指令集 else ''#基线正文
            if (not 保留可见基线) and 指令集 is not None and len(渲染文本)>0:#需要发出新基线正文
                基线内容=工作区上下文消息(渲染文本)['content']#基线文本块
                内容.extend(基线内容)#先放入基线内容
                替换作用域=set(基线['changes'].keys())#新基线覆盖的作用域
                替换移除=[]#替换时要显式移除旧基线里不再出现的作用域
                if 替换先前基线:#替换
                    for 变更 in (可见基线['changes'] if 'changes' in 可见基线 else []):#扫描旧基线变更
        ...
        if 更新 is not None:#有增量上下文
            上下文消息=更新['context']#增量消息
            内容.extend(上下文消息['content'] if 'content' in 上下文消息 and 上下文消息['content'] is not None else [])#追加增量文本
            来源=上下文消息['source'] if 'source' in 上下文消息 else None#增量来源
            if isinstance(来源,dict) and 来源['kind']=='agent-instructions':#来源必为本包
                变更列表.extend(来源['changes'] if 'changes' in 来源 and 来源['changes'] is not None else [])#追加增量变更
    ...
    def 同步收件箱(智能体,已声明,期望):#把期望上下文同步进收件箱
        """写入或清理下一步工作区上下文。"""
        待处理=[消息 for 消息 in 智能体.inbox.下一步队列 if 是否工作区上下文(消息)]#下一步里已有的工作区上下文
        ...
        if 期望 is not None:#有期望
            ...
            if not 已供给:#再看表面
                for 序号 in 智能体.session.surface.nodes:#或已在表面
                    ...
                    if 事件['type']=='user/message' and 同上下文载荷(事件['data'],期望):#载荷相同
                        已供给=True#已供给
                        break#停
        if 期望 is None or 已供给:#不需要待处理上下文
            for 消息 in 待处理:#清掉多余待处理
                智能体.inbox.移除(消息['id'])#删除
            return#同步结束
        可复用=None#能否复用已有待处理
        ...
        if 可复用 is not None:#已有相同载荷
            for 消息 in 待处理:#只留这一条
                if 消息 is not 可复用:#其他
                    智能体.inbox.移除(消息['id'])#删掉其他
            return#复用完成
        被替=待处理[0] if len(待处理)>0 else None#第一条待处理
        if 被替 is None:#没有则插入下一步
            智能体.inbox.前置('next-step',期望)#插入
        else:#有则替换第一条
            智能体.inbox.替换(被替['id'],期望)#替换
        for 消息 in 待处理[1:]:#删掉其余待处理
            智能体.inbox.移除(消息['id'])#删除
    def 组装并同步(智能体,信号,已声明,触及路径列表=None):#组装并同步收件箱
        """完成后无返回。"""
        if 触及路径列表 is None:#缺省
            触及路径列表=[]#空
        待处理=[消息 for 消息 in 智能体.inbox.下一步队列 if 是否工作区上下文(消息)]#当前待处理
        期望=组装(智能体,信号,已声明,待处理,触及路径列表)#组装期望
        若已中止则抛出(信号)#同步前检查取消
    ...
    def 排队投影(智能体,触及路径):#排队一次文件触及投影
        ...
        if 已中止(投影信号):#已拆除
            return#不再投影
        try:#投影可能失败
            组装并同步(智能体,投影信号,[],[触及路径])#叠在串行路径上组装同步
        except Exception as 错误:#任意刷新失败不得断链
            if not 已中止(投影信号):#拆除以外的失败记警告
                上下文.日志.警告('workspace instruction refresh failed: %o',错误)#记警告
        try:#清掉占位尾，避免弱引用表泄漏
            del 投影尾[智能体]#清掉
        except KeyError:#本无尾
            pass#放过
    ...
    def 投影触及(触及):#按步骤边界决定立刻投影还是暂存
        ...
        if not 步骤已打开(会话):#步骤已关闭
            ...
            return#处理完
        待存=步骤触及[会话] if 会话 in 步骤触及 else None#打开步骤内的暂存
        if 待存 is None:#新建列表
            步骤触及[会话]=[触及]#新建
        else:#追加
            待存.append(触及)#追加
    ...
    上下文.监听('session/event',会话事件)#跟踪步骤开关
    def 预步骤(参数,下一环):#预步骤：先委托再把工作区上下文折进本步
        """返回进入或拒绝。"""
        智能体=参数['agent']#会话所有者
        消息列表=参数['messages']#已声明消息
        步骤=参数['step']#步骤号
        信号=参数['signal'] if 'signal' in 参数 else None#取消
        决定=下一环()#先让后续监听器决定
        等待投影(智能体)#等文件触及投影排空
        待处理=[消息 for 消息 in 智能体.inbox.下一步队列 if 是否工作区上下文(消息)]#当前待处理上下文
        期望=组装(智能体,信号,消息列表,待处理)#组装期望
        若已中止则抛出(信号)#同步前检查取消
        决定消息=决定['messages'] if 'messages' in 决定 and 决定['messages'] is not None else []#决定消息
        if 决定['kind']=='reject' or (步骤==1 and len(决定消息)==0):#拒绝或空首步
            同步收件箱(智能体,消息列表,期望)#只同步收件箱，不进入本步
            return 决定#原样返回
        for 消息 in 待处理:#前进的步骤会结清待处理上下文
            智能体.inbox.移除(消息['id'])#清掉待处理
        if 期望 is None or any(同上下文载荷(消息,期望) for 消息 in 决定消息):#无期望或已覆盖
            return 决定#不插入
        最后声明=-1#最后一条已声明消息下标
        for 下标,消息 in enumerate(决定消息):#找最后声明
            if 消息 in 消息列表:#属于已声明批次
                最后声明=下标#更新
        进入=list(决定消息)#拷贝
    ...
    def 工具结果(执行,结果):#工具结果：收集文件触及并在根执行提交
        ...
        令牌=执行['token']#执行令牌
        触及列表=执行触及.pop(令牌,[])#本执行及其子执行累积的触及
        有智能体='agent' in 执行 and 执行['agent'] is not None#有调用方
        未取消=not 已中止(执行['signal'] if 'signal' in 执行 else None)#未取消
        if (not ('isError' in 结果 and 结果['isError'])) and 有智能体 and 未取消:#成功且未取消
            自身路径=执行中的文件路径(执行)#自身file_path
            if 自身路径 is not None:#有路径
                触及列表.append({'agent':执行['agent'],'path':自身路径})#记入触及
        父令牌=执行['parent'] if 'parent' in 执行 else None#父执行令牌
        if 父令牌 is not None:#嵌套执行：交给父令牌，等根执行再投影
            if len(触及列表)>0:#有触及要上交
                if 父令牌 not in 执行触及:#新建
                    执行触及[父令牌]=触及列表#新建
                else:#追加
                    执行触及[父令牌].extend(触及列表)#追加
            return#非根执行不投影
        for 触及 in 触及列表:#根执行按步骤边界投影
            投影触及(触及)#投影
    上下文.监听('tools/result',工具结果)#工具结果
