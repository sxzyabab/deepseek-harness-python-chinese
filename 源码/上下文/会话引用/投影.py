...
def 投影会话对话(快照):#从表面快照投影对话
    ...
    对话=[]#收集投影项
    for 事件 in 快照['events']:#遍历表面事件
        种类=事件['type']#事件类型
        if 种类=='user/message':#用户消息
            数据=事件['data']#事件数据
            出处=数据['source'] if 'source' in 数据 else None#出处
            检查点=是否压缩检查点来源(出处)#是否压缩检查点
            if (not 检查点) and (not isinstance(出处,dict) or 出处['kind']!='user'):#非检查点且非用户来源则跳过
                continue#跳过
            文本=抽出文本(数据['content'] if 'content' in 数据 else None)#抽出文本块
            if 文本!='':#非空才入列
...
def 保留引用会话(快照,标签,最大字节):#按字节预算保留引用会话
    ...
    def 数据():#按当前保留集组装序列化对象
        ...
        会话=快照['session']#源会话头
        return {#序列化对象
            'sessionId':会话['id'],#源会话id
            'label':标签,#展示标签
            'cwd':会话['cwd'] if 'cwd' in 会话 else None,#缺cwd则为null
            'capturedThroughSeq':快照['capturedThroughSeq'],#捕获序号
            'conversation':[{'role':项['role'],'text':项['text']} for 项 in 保留],#只输出角色与文本
        }#对象结束
    ...
    while 尺寸()>最大字节:#先丢掉非检查点、非最新的整条消息
        ...
        for 下标,项 in enumerate(保留):#找可丢的旧非检查点
            if (not 项['checkpoint']) and 下标!=最新下标:#可丢
                丢弃下标=下标#记下
                break#找到即停
    ...
    while 尺寸()>最大字节:#再截断当前最长文本
        ...
        保留[最长下标]=下一#写回
    含压缩=any(项['checkpoint'] for 项 in 原始)#原投影是否含检查点
    截断省略字节=sum(项['omittedBytes'] for 项 in 保留)#截断省略字节
...
def 附通知截断(文本,最大输出字节):#头尾截断并附省略通知
    ...
    while 下界<=上界:#二分寻找最大可装下的头尾保留
        ...
        保留器.推入(文本)#推入完整原文
        结果=保留器.收尾()#结束并取出保留文本
        省略量=结果['omittedBytes']#省略元数据
        if 省略量['kind']!='exact':#省略量必须精确
