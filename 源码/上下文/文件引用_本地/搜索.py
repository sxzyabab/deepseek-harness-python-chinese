...
__all__=[#仅中文公开名
    ...
    '工作区文件搜索','文件引用本地错误','已中止','若已中止则抛出',
    '光标处活动令牌','格式化文件提及',
]#公开面结束

class 文件引用本地错误(Exception):
    """本地文件引用包的异常基类。"""
...
class 工作区文件搜索:
    ...
    def __init__(自身,工作区根,配置=None):
        """解析配置并记住根目录。"""
        if 配置 is None:#默认空配置
            配置={}#空配置
        自身.根=os.path.abspath(工作区根)#绝对根
        自身.最大结果=配置['maxResults'] if 'maxResults' in 配置 else 默认最大结果数#结果上限
        自身.最大条目=配置['maxEntries'] if 'maxEntries' in 配置 else 默认最大条目数#索引上限
        排除=配置['excludedDirectories'] if 'excludedDirectories' in 配置 else None#排除目录
    ...
    def _遍历(自身,目录,条目列表,计数):
        """在条目预算内收集文件与目录路径。"""
        if 计数[0]>=自身.最大条目:#已达上限
            return#停止
        try:#列举目录
            名称列表=os.listdir(目录)#目录项
        except OSError:#不可读
            return#跳过
        for 名称 in sorted(名称列表):#稳定顺序
            ...
            条目列表.append({'path':相对,'kind':种类})#收录
            计数[0]+=1#计数
            if 是目录:#递归子目录
                自身._遍历(完整,条目列表,计数)#深入
    ...
    def 列举(自身,查询,信号):
        ...
        排名=[]#评分结果
        for 候选 in 索引:#扫描索引
            若已中止则抛出(信号)#构建后仍要尊重取消
            分=评分(查询串,候选['path'])#评分
            if 分 is None:#未命中
                continue#跳过
            排名.append((分,候选))#记下
        排名.sort(key=排名键)#按分排序
