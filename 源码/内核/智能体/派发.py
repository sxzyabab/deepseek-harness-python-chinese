    ...
    def 发出(名称,载荷):
        ...
        for 回调 in 获取内部数据(事件总线,'解析监听器')(事件总线,'emit',参数):#逐个监听器
            try:
                ...
                上下文.日志.警告('智能体事件 "'+名称+'" 监听器抛错: '+str(错误))#记抛错
    def 串行(名称,载荷):
        """在 Agent 作用域内按序等待派发。"""
        return 上下文.广播(载体,名称,融合(载荷))#经载体按序广播
    def 瀑布(名称,载荷,*剩余):
        """在 Agent 作用域内做环绕中间件派发。"""
        return 上下文.链式拦截(载体,名称,融合(载荷),*剩余)#经载体瀑布派发
    return {'发出':发出,'串行':串行,'瀑布':瀑布}#公开键仅中文

def 发出智能体事件(上下文,智能体,名称,载荷):
    """发出一条受控 Agent 通知，不分配常驻派发器。"""
    智能体事件(上下文,智能体)['发出'](名称,载荷)#临时派发器发即忘
