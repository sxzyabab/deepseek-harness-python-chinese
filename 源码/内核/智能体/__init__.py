...
class 智能体注册表(服务):#Agent 注册表
    ...
    def __init__(自身,ctx):#登记 agents 服务
        ...
        def 状态监听(纤程对象,*剩余):#本服务生命周期祖先正在卸载则关闭
            """本服务生命周期祖先正在卸载则关闭新发起边界。"""
            if 纤程对象.state==纤程状态.卸载中 and 自身.有生命周期祖先(纤程对象):#正在卸载
                自身.关闭发起方()#关闭新发起边界
        ctx.监听('internal/status',状态监听)#监听纤程状态
        def 发起方生命周期():#先排空再失效
            """先排空再失效，再拒绝新边界。"""
    ...
    def 有生命周期祖先(自身,候选):#是否生命周期祖先
        """一个正在卸载的纤程是否拥有本服务的生命周期。"""
        纤程对象=自身.ctx.纤程#从本服务纤程起
        while True:#上溯
            if 纤程对象 is 候选:#命中
                return True#命中
            父=纤程对象.父上下文.纤程#父纤程
