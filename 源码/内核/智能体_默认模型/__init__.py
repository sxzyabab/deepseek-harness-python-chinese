...
智能体默认模型设置模式=模式.对象({#Settings 分节模式；含可选推理力度，与智能体默认模型设置字段对齐
    'provider':模式.字符串().必填(),#提供方路由必填（Settings 文档键字面量）
    'model':模式.字符串().必填(),#模型 id 必填（Settings 文档键字面量）
    'reasoningEffort':模式.字符串(),#推理力度可选；故意不进插件配置，便于整节替换时清除
})#设置模式结束
...
class 智能体默认模型配置(服务):#独立于 Host 或传输拥有默认模型选择
    ...
    配置=模式.对象({#运行时插件配置；字段与插件配置对齐，不含 reasoningEffort
        'provider':模式.字符串().必填(),#提供方路由必填（组合配置键字面量）
        'model':模式.字符串().必填(),#模型 id 必填（组合配置键字面量）
    })#运行时配置模式
