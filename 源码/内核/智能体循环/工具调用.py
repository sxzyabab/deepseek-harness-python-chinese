...
from .中止与并发 import 已中止,赛跑,全部结算,操作任务,循环错误
...
def 追加工具结果(会话,轮次,步骤,块,结果,调用序号):
    ...
    元=结果['meta'] if 'meta' in 结果 else None#展示元数据
    if 元 is not None:
        载荷['meta']=元#有元数据则带上
...
def 执行一组(上下文对象,轮次,步骤,组,模式,信号,接受上下文):
    """跑一道独占屏障或并行池。"""
    ...
    def 填池():
        ...
        while (not 已中) and 下一启动<len(组) and len(在飞)<上限:
            ...
            if 下一启动>0 and 模式=='parallel':
                下模式=上下文对象.tools.执行模式(下一条['exec'])['kind']#再分类
                if 下模式!='parallel':
                    break#再分类成独占则停
            启动调用(下一启动)#启动
            下一启动+=1#前进
...
def 执行工具调用(上下文对象,轮次,步骤,工具调用列表,信号,接受上下文):
    """按在线并发模式调度一步助手工具调用。"""
    ...
    while 下一<len(已规划):
        首条=已规划[下一]#本组首条
        模式=上下文对象.tools.执行模式(首条['exec'])['kind']#当前执行模式
        组=已规划[下一:] if 模式=='parallel' else [首条]#并行吃剩余，独占只吃一条
        结局=执行一组(上下文对象,轮次,步骤,组,模式,信号,接受上下文)#执行一组
