...
def 生成管道进程(接口表,选项):#生成带管道 stdout/stderr 的进程
    ...
    try:#创建管道与进程
        ...
        启动=编码启动信息(标准输入['read'],标准输出['write'],标准错误['write'])#STARTUPINFOW
        进程信息=ctypes.create_string_buffer(进程信息大小)#PROCESS_INFORMATION
        命令行=构建命令行(选项['command'],选项['args'])#命令行
        if 创建受限进程(接口表,选项,命令行,0,启动,进程信息)==0:#创建失败
        ...
        if 是否空指针(进程句柄) or 是否空指针(线程句柄):#空句柄
            ...
            尽力关闭(接口表,线程句柄);尽力关闭(接口表,进程句柄)#清理
            raise Win32错误('CreateProcessAsUserW',0,'succeeded but returned null process/thread handles (pid '+str(进程id)+')')#空句柄
        for 句柄 in (标准输入['read'],标准输入['write'],标准输出['write'],标准错误['write']):#关闭写端/多余读端
            if 句柄 in 拥有:拥有.remove(句柄);尽力关闭(接口表,句柄)#关闭
        尽力关闭(接口表,线程句柄)#线程句柄不再需要
        return {'pid':进程id,'process':进程句柄,'stdoutRead':标准输出['read'],'stderrRead':标准错误['read']}#调用方拥有
    except BaseException:#失败清理
        for 句柄 in list(拥有):尽力关闭(接口表,句柄)#关闭全部拥有句柄
        raise#再抛
    ...
    try:#读直到 EOF
        while True:#循环窥视/读取
            if 接口表['peekNamedPipe'](句柄,None,0,None,ctypes.byref(计数槽),None)==0:#窥视失败
                ...
                抛上次错误(接口表,'PeekNamedPipe','drain failure after '+str(len(块列表))+' chunk(s)')#失败
            可用=解码uint32(计数槽)#可读字节
            if 可用>0:#有数据
                缓冲=ctypes.create_string_buffer(可用)#读缓冲
                if 接口表['readFile'](句柄,缓冲,可用,ctypes.byref(计数槽),None)==0:#读失败
                    抛上次错误(接口表,'ReadFile','drain failure after '+str(len(块列表))+' chunk(s)')#失败
                块列表.append(缓冲.raw[:解码uint32(计数槽)])#记下块
            time.sleep(0.001)#退避 1ms
        return b''.join(块列表)#完整字节
    finally:#总是关闭
        尽力关闭(接口表,句柄)#关闭读端
...
def 创建关闭即杀作业(接口表):#创建 kill-on-close 作业
    ...
    if 接口表['setInformationJobObject'](作业,作业扩展限制类,缓冲,len(信息))==0:#设置失败
        码=接口表['getLastError']();尽力关闭(接口表,作业);抛win32错误(接口表,'SetInformationJobObject',码)#抛出
    return 作业#作业句柄
...
def 生成继承作业进程(接口表,选项):#挂起创建、加入作业、再恢复
    ...
    try:#暂时恢复继承位并创建
        ...
        尽力关闭(接口表,作业);raise 错误#清理作业并抛出
    finally:#恢复继承位
        for 句柄 in 已启用:接口表['setHandleInformation'](句柄,句柄可继承,0)#尽力恢复
    if 创建结果==0:#创建失败
        尽力关闭(接口表,作业);抛win32错误(接口表,'CreateProcessAsUserW',失败码,'command: '+选项['command']+', cwd: '+选项['cwd'])#抛出
    进程句柄,线程句柄,进程id,线程id=struct.unpack_from('PPII',进程信息.raw)#解码
    if 是否空指针(进程句柄) or 是否空指针(线程句柄):#空句柄
        if not 是否空指针(进程句柄):接口表['terminateProcess'](进程句柄,1)#终止
        尽力关闭(接口表,作业);尽力关闭(接口表,线程句柄);尽力关闭(接口表,进程句柄)#清理
        raise Win32错误('CreateProcessAsUserW',0,'succeeded but returned null process/thread handles (pid '+str(进程id)+')')#空句柄
    if 接口表['assignProcessToJobObject'](作业,进程句柄)==0:#加入作业失败
        码=接口表['getLastError']();接口表['terminateProcess'](进程句柄,1);尽力关闭(接口表,线程句柄);尽力关闭(接口表,进程句柄);尽力关闭(接口表,作业);抛win32错误(接口表,'AssignProcessToJobObject',码,'pid '+str(进程id))#失败
    if 接口表['resumeThread'](线程句柄)==0xFFFFFFFF:#恢复失败
        码=接口表['getLastError']();尽力关闭(接口表,线程句柄);尽力关闭(接口表,进程句柄);尽力关闭(接口表,作业);抛win32错误(接口表,'ResumeThread',码,'pid '+str(进程id))#失败
    尽力关闭(接口表,线程句柄)#线程句柄不再需要
