...
def 启动子进程(规格,内部=None):#本地spawn
    ...
    溢出目录=内部['spillDir'] if 'spillDir' in 内部 else None#测试覆盖溢出根
    ...
    环境=子环境(规格['env'] if 'env' in 规格 else None)#合并后的子环境
    ...
    if 信号对象 is not None:#有取消信号
        def 盯中止():#等到中止再终止
            ...
            if not 已摘中止[0]:#仍挂着才终止
                终止()#终止
        盯线程=threading.Thread(target=盯中止)#监听线程
