...
class 本地终端句柄:#本地 PTY 会话
    ...
    def __init__(自身,终端,检查器,宽限毫秒):#钉 pid、身份、输出与退出
        ...
        自身.输出=贯通流()#用户可见输出
        自身.done=操作任务()#顶层进程退出任务；字段名对齐上游 done
        ...
        def 在退出时(事件):#进程退出
            ...
            自身.已退出=True#记下
            自身.输出.结束()#关掉用户可见流
            退出码=事件['exitCode'] if 'exitCode' in 事件 else None#退出码
            退出信号=事件['signal'] if 'signal' in 事件 else None#信号编号
            自身.done.兑现({#交出结局
