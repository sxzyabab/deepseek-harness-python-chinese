...
__all__=['客户端测试运行时错误','客户端名册行','客户端名册','组装计划','名册转启动图','断言计划']
...
class 客户端名册:#客户端名册
    ...
    def 依赖闭包(自身,包名表):#依赖闭包
        """具名行加上它们依赖的每一行（传递），按名册顺序。"""
        自身._已知(包名表,'closure')
        按名={行.包名:行 for 行 in 自身.行表}
        保留=set()
        def 访问(包名,来自):
            """收下该包及其依赖边。"""
            if 包名 in 保留 or 包名 in 平台种子:
                return
            if 包名 not in 按名:
                raise 客户端测试运行时错误('client-test-runtime: '+str(来自)+' 依赖 '+包名+'，该包不在名册内')
            行=按名[包名]
            保留.add(包名)
            for 依赖名 in 行.依赖:
                访问(依赖名,包名)
