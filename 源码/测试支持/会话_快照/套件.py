...
def 唯一消息标识(日志列表):#唯一消息 id
    ...
    唯一={}#唯一
    for 标识,指纹集合 in 指纹按标识.items():#逐 id
        if len(指纹集合)!=1:#多指纹
            continue#跳过
        指纹=next(iter(指纹集合))#唯一指纹
...
def 列出打包纪元毫秒(记录):#打包行成员时间
    ...
    数据=记录.get('data') or {}#数据
    间隙=数据.get('dt') or []#间隙
    纪元毫秒列表=[记录.get('time0') or 0]#起点
    for 间隙值 in 间隙:#累加
        纪元毫秒列表.append(纪元毫秒列表[-1]+间隙值)#追加
    return 纪元毫秒列表#返回
...
def 刷新夹具替换(日志列表,夹具列表):#刷新 fixture 替换
    ...
    for 索引,日志 in enumerate(日志列表):#逐日志
        ...
        新鲜行=解析JSONL记录(内容)#新鲜
        已有行=解析JSONL记录(夹具列表[索引] if 索引<len(夹具列表) else '')#已有
        ...
        已有溢出=提取快照溢出路径(夹具列表[索引] if 索引<len(夹具列表) else '')#已有溢出
...
def 保留打包成员时间(记录,已有成员列表):#保留打包成员时间
    ...
    if not 已有成员列表:#无成员
        return#跳过
    首时间=已有成员列表[0].get('time')#首时间
    if not isinstance(首时间,int) or isinstance(首时间,bool):#非安全整数近似
        return#跳过
    记录['time0']=首时间#写入起点
    if len(已有成员列表)!=len(数据['dt'])+1:#长度不对齐
        return#跳过
    纪元毫秒列表=[]#时间
    for 成员 in 已有成员列表:#逐成员
        时间=成员.get('time')#时间
        if not isinstance(时间,int) or isinstance(时间,bool):#非法
            return#跳过
        纪元毫秒列表.append(时间)#追加
    间隙=[纪元毫秒列表[索引+1]-纪元毫秒列表[索引] for 索引 in range(len(纪元毫秒列表)-1)]#间隙
...
def 构建归一化字符串映射(记录列表,新鲜记录列表,已有记录列表,新鲜上下文,已有上下文):#日志级双射
    ...
    反向={}#反向
    已有索引=0#已有游标
    for 记录索引,记录 in enumerate(记录列表):#逐记录
        已有记录=已有记录列表[已有索引] if 已有索引<len(已有记录列表) else None#已有
        成员数=len(列出打包纪元毫秒(记录) or [None])#成员数
        if 记录.get('type')=='session/title' and (已有记录 is None or 已有记录.get('type')!='session/title'):#插入标题
            continue#不推进
        if 成员数>1:#打包
            已有成员=已有记录列表[已有索引:已有索引+成员数]#切片
...
def 稳定刷新日志(新鲜,已有,替换列表,新鲜上下文):#稳定刷新日志
    ...
    记录列表=解析JSONL记录(稳定)#稳定记录
    已有上下文=夹具上下文(已有)#已有上下文
    字符串映射=构建归一化字符串映射(记录列表,新鲜记录列表,已有记录列表,新鲜上下文,已有上下文)#双射
    已有索引=0#游标
    先前事件时间=None#先前时间
    for 索引 in range(len(记录列表)):#逐记录
        记录=记录列表[索引]#记录
        已有记录=已有记录列表[已有索引] if 已有索引<len(已有记录列表) else None#已有
        成员数=len(列出打包纪元毫秒(记录) or [None])#成员数
        ...
        if 插入标题:#插入标题
            ...
            if 字符串映射 is not None and 成员数==1 and 已有记录 is not None and 已有记录.get('type')==记录.get('type'):#可复用
                记录=保留归一化易变(#保留
                    记录,已有记录,
                    归一化刷新记录(新鲜记录列表[索引],新鲜上下文),
                    归一化刷新记录(已有记录,已有上下文),
                    字符串映射,
                )#结束
                记录列表[索引]=记录#写回
            保留打包成员时间(记录,已有记录列表[已有索引:已有索引+成员数])#打包时间
            保留夹具易变(记录,已有记录)#易变字段
            已有索引+=成员数#推进
...
def 定义ACP快照套件(选项):#定义 ACP 快照套件
    ...
    def 运行场景用例(场景):#单场景用例
        ...
        if 比较日志:#比较会话日志
            ...
            夹具上下文列表=[夹具上下文(项) for 项 in 夹具内容]#上下文
            夹具归一={'sessionIds':[标识 for 项 in 夹具上下文列表 for 标识 in 项['sessionIds']],'cwd':夹具上下文列表[0]['cwd']}#合并
            实际快照=归一化会话快照列表(收获,上下文)#实际
            期望快照=归一化会话快照列表(夹具内容,夹具归一)#期望
        ...
        钉头列表=[恢复钉住工具模式(头,钉模式集[索引]) for 索引,头 in enumerate(钉住)]#恢复
        子钉模式={}#子 schema
        for 索引 in 子模式钉:#逐索引
    ...
    def 断言恰一钉():#每类恰一钉
        ...
        断言相等({类:len(名称列表) for 类,名称列表 in 钉.items()},{类:1 for 类 in 钉按类},'exactly one pin per header class')#计数
        for 场景 in 场景列表:#每场景有类钉
    ...
    def 断言钉组合():#钉组合 sidecar
        ...
        for 场景 in 钉按类.values():#逐钉
            ...
            断言相等(len(模式集),len(请求头列表),f"{模式源['name']}: tool-schema sequence must match {场景['name']}'s header sequence")#对齐
            for 索引,头 in enumerate(请求头列表):#逐头
