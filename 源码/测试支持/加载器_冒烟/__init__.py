...
def 解析示例模式(原始=None):#解析启动模式
    ...
    if 原始=='lib':#已构建
        return 'lib'#已构建
    raise Exception(f"{示例模式环境名} must be 'src' or 'lib', got {原始!r}.")#非法值
...
def 派生库入口(源入口):#从源 bin 派生 lib bin
    ...
    if 切点==-1:#路径非法
        raise Exception(f'resolveExampleLaunch: expected a "/src/" segment or Windows equivalent in bin path {源入口!r}.')#路径非法
...
def 运行加载器冒烟(选项):#运行真实 Loader 冒烟
    ...
    try:#主路径
        ...
        if 结果.returncode!=期望退出:#退出码不符
            raise Exception(f"{选项['label']} exited {结果.returncode} (expected {期望退出}). stdout:\n{结果.stdout}\nstderr:\n{结果.stderr}")#退出码不符
        ...
        raise Exception(f"{选项['label']} did not exit within {进程超时/1000}s. stdout:\n{标准出}\nstderr:\n{标准错}") from 超时错误#超时
    finally:#无论成败
