...
class 已编译会话格式目录:#已编译目录
    ...
    def __init__(自身,选项):#构造
        """记下链、编解码器映射与当前版本编码器。"""
        自身.链=创建会话格式链(选项)#创建链
        自身.当前版本=自身.链.当前版本#当前版本
        自身.编码当前版本产物=选项['encodeCurrentArtifact']#编码当前版本
        编解码器映射={}#编解码器映射
    ...
    def 解码产物(自身,头值,行值列表):#解码产物
        """经其冻结版本编解码器分发一份完整物理 JSON 产物。"""
        已存版本,编解码器=自身.产物编解码器(头值)#取编解码器
        return 快照会话格式产物(#快照
            编解码器['decodeArtifact'](头值,行值列表),#解码
            f'format v{已存版本} decoded artifact',#标签
        )#snapshot结束

    def 解码可恢复产物(自身,头值,行值列表):#解码可恢复
        """经其已发布行前缀恢复规则分发一份物理产物。"""
        已存版本,编解码器=自身.产物编解码器(头值)#取编解码器
        return 快照会话格式产物(#快照
            编解码器['decodeRecoverableArtifact'](头值,行值列表),#解码
    ...
    def 编码当前版本(自身,产物):#编码当前版本
        """编码 migrate 返回或活会话产出的当前版本产物；此处不再校验。"""
        if 检查会话格式版本(产物['header'])!=自身.当前版本:#非当前版本
            raise 会话格式错误(f'encodeCurrent requires Session format v{自身.当前版本}')#错误
        已编码=自身.编码当前版本产物(产物)#编码
        头=快照会话格式json(已编码['header'],'encoded current Session header')#快照头
        行列表=[]#行快照
        for 下标,行 in enumerate(已编码['rows']):#逐行
            行列表.append(快照会话格式json(行,f'encoded current Session row {下标}'))#行快照
        if 检查会话格式版本(头)!=自身.当前版本:#头非当前版本
            raise 会话格式错误('current Session codec returned a non-current header')#错误
        return 深冻结({'header':头,'rows':tuple(行列表)})#冻结返回
