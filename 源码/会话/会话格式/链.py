...
class 已编译会话格式链:#已编译链
    ...
    def __init__(自身,选项):#构造
        """记下当前版本、有序迁移与当前版本恢复器。"""
        自身.当前版本=会话格式版本(选项['currentVersion'],'current Session format version')#当前版本
        自身.恢复当前版本=选项['restoreCurrent']#恢复当前版本
        ...
        for 版本 in range(自身.当前版本):#填满链
            迁移=按源[版本] if 版本 in 按源 else None#取迁移
            if 迁移 is None:#缺失
    ...
    def 迁移(自身,源):#迁移产物
        ...
        已存版本=检查会话格式版本(源['header'])#已存版本
        当前=快照会话格式产物(源,f'format v{已存版本} source')#快照源
        if 已存版本==自身.当前版本:#已是当前版本
            当前=快照会话格式产物(自身.恢复当前版本(当前),'current Session restoration')#恢复
            自身.校验当前版本(当前)#校验当前版本
            return 当前#返回
        for 迁移项 in 自身.计划(已存版本):#逐步迁移
            名称=迁移项['name']#名称
            目标版本=迁移项['toVersion']#目标版本
            try:#尝试迁移
                已迁移=迁移项['migrate'](快照会话格式产物(当前,名称+' input'))#迁移
            except BaseException as 错误:#失败
                抛出不支持拒绝(迁移项,错误)#包装拒绝
            当前=快照会话格式产物(已迁移,f'{名称} output')#快照输出
            if 当前['header']['version']!=目标版本:#版本不符
                raise 会话格式错误(名称+' returned v'+str(当前['header']['version'])+'; expected v'+str(目标版本))#错误
            try:#校验目标
                迁移项['validateTarget'](当前)#校验
            except BaseException as 错误:#失败
                抛出不支持拒绝(迁移项,错误)#包装拒绝
        当前=快照会话格式产物(自身.恢复当前版本(当前),'current Session restoration')#最终恢复
        自身.校验当前版本(当前)#校验当前版本
        return 当前#返回
    ...
    def 迁移头(自身,源):#迁移头
        ...
        for 迁移项 in 自身.计划(当前['version']):#逐步迁移
            名称=迁移项['name']#名称
            目标版本=迁移项['toVersion']#目标版本
            try:#尝试
                已迁移=迁移项['migrateHeader'](快照会话格式头(当前,名称+' header input'))#迁移头
            except BaseException as 错误:#失败
            ...
            try:#校验目标头
                迁移项['validateTargetHeader'](当前)#校验
            except BaseException as 错误:#失败
                抛出不支持拒绝(迁移项,错误,'Session header')#包装
    ...
    def 校验当前版本(自身,产物):#校验当前版本
        """断言产物头已是当前版本。"""
        if 产物['header']['version']!=自身.当前版本:#不符
            raise 会话格式错误(#错误
                'current Session restorer returned v'+str(产物['header']['version'])+'; expected v'+str(自身.当前版本),#消息
            )#Error结束
