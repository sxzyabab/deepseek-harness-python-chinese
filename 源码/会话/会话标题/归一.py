...
_空白=re.compile(r'\s+',re.ASCII)#ASCII 空白

def _断言正整数(名称,值):
    """断言正整数配置。"""
    if isinstance(值,bool) or not isinstance(值,int) or 值<=0:#非法
        raise 会话标题错误(名称+' must be a positive integer')#拒绝

def _清洗标题(输入):
    """去掉控制并压成一行。"""
    文本=输入#起点
    文本=_OSC.sub('',文本,count=0)#去 OSC
    文本=_CSI.sub('',文本,count=0)#去 CSI
    文本=_ESC.sub('',文本,count=0)#去 ESC
    文本=_控制.sub('',文本,count=0)#去控制
    文本=_方向.sub('',文本,count=0)#去方向控制
    文本=_空白.sub(' ',文本,count=0).strip()#压空白
    return 文本#干净文本

def 截断标题utf8(输入,最大字节):
...
def 回退会话标题(输入,最大词数,最大字节):
    """从首条人类消息派生回退标题。"""
    _断言正整数('maxWords',最大词数)#校验
    词列表=_清洗标题(输入).split(' ')#分词
    词列表=[词 for 词 in 词列表 if 词!=''][:最大词数]#取前 N 词
    return 截断标题utf8(' '.join(词列表),最大字节).rstrip()#截断
