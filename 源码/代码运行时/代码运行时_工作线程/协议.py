"""无版本号、结构化克隆线路协议，连接同发行的宿主与工作线程代码。宿主把入站流量当敌对，因为模型代码可以伪造父端口消息；工作线程信任宿主应答。"""

class 工作线程启动数据:#宿主在spawn时经workerData交给工作线程的启动载荷
    """工作线程启动数据字段约定（运行时为dict）。"""
    代码='code'#已剥类型（纯JS）或Python程序体
    命名空间列表='namespaces'#要物化的绑定命名空间；函数本身留在宿主侧
    最大输出字节='maxOutputBytes'#外层序列化日志加上完成值或失败诊断的硬上限

工作线程启动数据字段=('code','namespaces','maxOutputBytes')#程序体、命名空间声明、外层输出字节上限

# 命名空间声明项约定：
# { global: str, names: list[str], errorClass?: { name: str, memberNameProperty: str } }

class 调用消息:#工作线程→宿主：一次桥接的绑定调用
    """绑定调用消息字段约定。"""
    类型='type'#消息标签，取值'call'
    编号='id'#工作线程签发的相关id；宿主对每个id至多应答一次，重复则忽略
...
class 日志消息:#工作线程→宿主：捕获的文本，急切流式发送
    """日志消息字段约定。"""
    类型='type'#消息标签，取值'log'
    文本='text'#捕获的文本
...
class 输出超限消息:#工作线程→宿主：工作线程侧捕获或完成值计量已超过外层上限
    """输出超限消息字段约定。"""
    类型='type'#消息标签，取值'output-limit'

输出超限消息字段=('type',)#标签output-limit

class 完成消息:#工作线程→宿主：程序已结算；日志不在此携带
...
class 工作线程到宿主类型:#工作线程发出的全部消息标签
    """工作线程→宿主联合标签（线路值就是标签字符串）。"""
    调用='call'#绑定调用
    日志='log'#捕获文本
    输出超限='output-limit'#工作线程侧触顶
    完成='done'#程序已结算

工作线程到宿主类型元组=('call','log','output-limit','done')#工作线程→宿主联合标签元组

class 应答消息:#宿主→工作线程：对一条调用消息的应答
