...
def 快照代码json值(值):#脱离为无损JSON
    """校验并脱离一个工作线程边界值；无效则为None。迭代遍历不增加调用栈深度上限。"""
    活跃=set()#环检测：当前路径上的容器
    根=None#根快照槽
    ...
    任务列表=[{'kind':'visit','value':值,'destination':{'kind':'root'}}]#从根访问开始
    while len(任务列表)>0:#栈式迭代
        ...
        if 任务['kind']=='array-item':#排队的数组元素
            追加(任务列表,{'kind':'visit','value':任务['source'][任务['index']],'destination':{'kind':'array','target':任务['target'],'index':任务['index']}})#改成visit
            continue#下一任务
        if 任务['kind']=='object-property':#排队的对象属性
            追加(任务列表,{'kind':'visit','value':任务['source'][任务['key']],'destination':{'kind':'object','target':任务['target'],'key':任务['key']}})#改成visit
            continue#下一任务
        候选=任务['value']#当前候选
        ...
        if isinstance(候选,list):#数组
            ...
            活跃.add(身份)#进入环检测
            追加(任务列表,{'kind':'leave','source':身份})#退出时解除
            for 下标 in range(len(候选)-1,-1,-1):#逆序入栈以正序写出
                追加(任务列表,{'kind':'array-item','source':候选,'index':下标,'target':目标})#排队元素
            continue#下一任务
        键列表=list(候选.keys())#JSON可见键
        for 键 in 键列表:#逐键检查
        ...
        活跃.add(身份)#进入环检测
        追加(任务列表,{'kind':'leave','source':身份})#退出时解除
        for 下标 in range(len(键列表)-1,-1,-1):#逆序入栈
            键=键列表[下标]#当前键
            追加(任务列表,{'kind':'object-property','source':候选,'key':键,'target':目标})#排队属性
    return 根#返回根快照

def 编码工作线程json(值):#编码为扁平线路
    """把已校验的JSON值压平，供工作线程消息端口传输。"""
    线路=[]#输出token
    待办=[值]#待编码栈
    while len(待办)>0:#栈式迭代
        当前=弹出末项(待办)#弹出
        ...
        if isinstance(当前,dict):#对象
            键列表=list(当前.keys())#可枚举字符串键
            追加(线路,{'kind':'object','keys':键列表})#先写容器标记
            for 下标 in range(len(键列表)-1,-1,-1):#逆序入栈
                键=键列表[下标]#当前键
                追加(待办,当前[键])#排队属性值
            continue#下一值
...
def 解码工作线程json(输入):#解码扁平线路
    """从扁平工作线程线路格式重建一个无损JSON值；畸形或不完整返回None。"""
    try:#任何抛错都视为无效线路
        ...
        for 记号 in 输入:#逐token
            ...
            if 记号 is None or isinstance(记号,bool) or isinstance(记号,str):#简单标量
                ...
                if 'kind' not in 记号:#无类别
                    return None#无效
                种类=记号['kind']#标记类别
                if 种类=='array':#数组容器
                    if 'length' not in 记号:#无长度
                        return None#无效
                    长度=记号['length']#长度字段
                    if isinstance(长度,bool) or not isinstance(长度,int) or 长度<0:#非法长度，排除布尔
                        return None#无效
                    目标=[]#新数组
                    值=目标#本token的值是空数组
                    if 长度>0:#非空则开帧
            ...
            if 帧 is not None:#非空容器
                帧列表.append(帧)#压栈
            while len(帧列表)>0:#弹出已填满的帧
                当前=帧列表[-1]#栈顶
                上限=当前['length'] if 当前['kind']=='array' else len(当前['keys'])#容量
                if 当前['index']<上限:#尚未填满
                    break#停弹
                帧列表.pop()#弹出完成帧
        return 根 if len(帧列表)==0 else None#还有打开帧则不完整
    except Exception:#畸形线路可能抛任意类型；解码契约是返回 None 而不是抛给调用方
        return None#视为无效线路
