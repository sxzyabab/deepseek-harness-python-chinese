...
def 应用(上下文,配置值=None):#登记全部终端工具与最少用法说明
    ...
    def 执行打开(参数,执行元数据):#执行打开
        ...
        请求={'type':类型}#搭建请求
        if 'name' in 参数 and 参数['name'] is not None:#有名则带上
            请求['name']=参数['name']#显示名
        if 'cwd' in 参数 and 参数['cwd'] is not None:#有cwd则带上
            请求['cwd']=参数['cwd']#工作目录
        return 上下文.terminals.搭建(#搭建会话
    ...
    def 执行发送(参数,执行元数据):#执行发送
        ...
        请求={'text':参数['text'],'submit':提交}#写入请求
        if 'run_in_background' in 参数 and 参数['run_in_background'] is True:#后台发送
            if not 后台启用:#配置禁止
                raise 终端工具错误('background terminal sends are disabled by tool-terminal configuration')#配置禁止
    ...
    def 发送展示元(_参数,值):#前台才给元数据
        ...
        if 值['kind']!='foreground':#后台无元数据
            return None#无元数据
        return {#前台元数据
            'viewport':值['viewport'],#视口
            'waitReason':值['waitReason'],#等待原因
            'sessionStatus':值['sessionStatus'],#会话状态
    ...
    def 呈现发送(参数):#调用卡片
        ...
        if 'run_in_background' in 参数 and 参数['run_in_background'] is True:#后台
            return {#通用卡片
                'card':'generic',#通用
                'title':'Send to terminal '+str(参数['sessionId'])+' in background',#标题
                'kind':'execute',#执行
                'rawInput':参数['text'],#原文
            }#后台卡片结束
        标题=参数['text'] if 'text' in 参数 and 参数['text'] is not None and len(参数['text'])>0 else '(send input)'#标题
        return {#终端卡片
            'card':'terminal',#终端
            'title':标题,#标题
