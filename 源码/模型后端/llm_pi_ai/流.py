...
def 分类派爱错误(消息):
    ...
    if re.search(r'\b429\b|rate.?limit',消息,re.I|re.ASCII):#429 或速率限制措辞
        return 'RATE_LIMIT'#速率限制
    if re.search(r'\b400\b|invalid.?request',消息,re.I|re.ASCII):#400 或非法请求措辞
        return 'INVALID_REQUEST'#坏请求
    if re.search(r'\b5\d\d\b',消息,re.ASCII):#5xx 归服务端
        return 'SERVER'#服务端错误
    if re.search(r'\btime(?:d)?\s*out\b|timeout',消息,re.I|re.ASCII):#超时措辞
        return 'TIMEOUT'#超时
    if re.search(r'stream ended (?:before|without)\b',消息,re.I|re.ASCII):#流在 done 前结束，当传输截断
        return 'TRANSPORT'#流截断
    if re.search(r'\b(?:network|connection|socket|fetch)\b|\bECONN[A-Z]+\b',消息,re.I|re.ASCII):#网络/连接/套接字失败
        return 'TRANSPORT'#传输失败
    if re.search(r'\b(?:other side closed|HTTP2 request did not get a response|WebSocket closed unexpectedly)\b',消息,re.I|re.ASCII):#对端关闭、HTTP2 无响应、WebSocket 意外关闭
        return 'TRANSPORT'#对端关闭、HTTP2无响应、WebSocket意外关闭
    if re.search(r'\bterminated\b|premature close',消息,re.I|re.ASCII):#进程被终止或过早关闭
...
def 转流块(事件列表,上下文窗口=None):
    ...
    for 事件 in 事件列表:#按派爱事件顺序翻译；start 无 harness 块，done/error 终止
        ...
        if 类型=='text_delta':#文本增量
            yield {'type':'text-delta','index':下标,'text':事件.delta}#文本增量
            continue#text_delta结束
        if 类型=='text_end':#文本块结束，带已组装正文
            yield {'type':'block-end','index':下标,'block':{'type':'text','text':事件.content}}#带已组装文本
            continue#text_end结束
        ...
        if 类型=='thinking_delta':#推理增量
            yield {'type':'reasoning-delta','index':下标,'text':事件.delta}#推理增量
            continue#thinking_delta结束
        if 类型=='thinking_end':#推理块结束，带已组装推理正文
            yield {'type':'block-end','index':下标,'block':{'type':'reasoning','text':事件.content}}#带已组装推理
            continue#thinking_end结束
        if 类型=='toolcall_start':#工具调用开始；从 partial 记下 id 与名字，供后续增量用
            部分=事件.partial#部分消息
            部分内容=部分.content if 部分 is not None else None#部分内容
            部分块=None#该下标的部分块
            if 部分内容 is not None and 下标 is not None and 下标<len(部分内容):#partial 里有这一块才取
                部分块=部分内容[下标]#该下标的部分块
            调用=''#调用id或空
            名字=''#工具名或空
            部分类型=部分块.type if 部分块 is not None else None#部分块类型
        ...
        if 类型=='toolcall_delta':#工具调用参数增量；有非空名字才带上
            已知=工具标识[下标] if 下标 in 工具标识 else None#已记下的id与名字
            增量={'type':'tool-call-delta','index':下标,'id':llm.调用标识('' if 已知 is None else 已知['id']),'argumentsDelta':事件.delta}#工具调用增量
            if 已知 is not None and 'name' in 已知 and len(已知['name'])>0:#判 length：有非空名字才写进增量
                增量['name']=已知['name']#有非空名字才带上
            yield 增量#工具调用增量
            continue#toolcall_delta结束
        if 类型=='toolcall_end':#工具调用结束，参数序列化回原始 JSON
            工具调用=事件.toolCall#已组装工具调用
            参数=工具调用.arguments#已解析参数
            yield {
                'type':'block-end',#块结束
                'index':下标,#块下标
                'block':{
        ...
        if 类型=='error':#流内错误：先发用量再发 finish，不带回放状态
            错误=事件.error#错误助手消息
            yield {'type':'usage','usage':映射用量(错误.usage)}#先发用量
