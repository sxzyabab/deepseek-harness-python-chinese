...
from .目录 import 模态列表,解析路由模型,受支持思考格式,思考档位列表,配置错误#目录词表与本包异常
...
def 断言可服务(配置):#拒绝本适配器无法服务的设置段
    """拒绝本适配器无法服务的设置段。配置为插件设置 dict。"""
    提供方表=配置['providers'] if 'providers' in 配置 else None#原始路由字典；缺席则解析空表
    解析配置表(提供方表)#解析即校验
...
def 解析配置表(提供方表):#校验并拆离按路由键控的配置映射
