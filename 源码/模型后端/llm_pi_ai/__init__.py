...
from .认证 import 建认证注入,记录键于#认证
from .登录 import 登记派爱登录流#登录流

__all__=(#仅中文公开名；无英文别名
    '名称','依赖','设置空间','配置模式','派爱适配器','受支持协议',
    '路由键','登记事实','目录条目','应用','默认','记录键于',
)#公开面结束
...
def 目录条目(配置表):#可配置提供方目录
    ...
    目录集合=set(目录路由)#成员资格
    条目表={}#去重后的条目
    def 声明(提供方,展示名,错误=None):#声明一条目录条目
        """声明一条目录条目。"""
        条目={'provider':提供方,'displayName':展示名,'settingsNs':设置空间,'settingsPath':['providers',提供方],'declared':提供方 not in 目录集合}#目录条目
        if 错误 is not None:#有诊断
            条目['error']=错误#带上
        条目表[提供方]=条目#写入
    ...
    for 提供方,配置项 in 配置表.items():#配置已声明的路由一律进目录，覆盖同名目录项的展示名
        声明(提供方,配置项['displayName'],配置项['catalogError'] if 'catalogError' in 配置项 else None)#声明
...
def 应用(上下文,配置):#为所有已配置提供方路由注册通用 pi-ai 适配器
    ...
    def 配置表():#当前配置的已解析配置，按原始快照身份记住
        ...
        下一份=解析配置表(提供方表,'deferred')#延迟目录校验
        上次原始=原始#记下原始
        已记住=下一份#记下成功
        return 下一份#新配置
    配置表()#加载时先解析一次
    ...
    def 挂授权(授权上下文):#有 authorization 才登记登录
        """登记 pi-ai 登录流。"""
        登记派爱登录流(授权上下文,认证面)#登记
    上下文.依赖启动(['authorization'],挂授权)#作用域绑授权缝
    目录句柄=None#目录句柄
    目录事实=None#上次目录事实
    def 确保目录():#目录变了则就地替换
        """目录变了则就地替换。"""
    ...
    安装设置段(上下文,设置空间,配置模式,配置,{
        'validate':(lambda 值:解析配置表(值['providers'] if 'providers' in 值 else None,'deferred') if 装段中[0] else 断言可服务(值,当前())),#装段延迟，其后仅校验变更
