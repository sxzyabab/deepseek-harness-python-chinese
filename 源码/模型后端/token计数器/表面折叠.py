...
def 折叠表面令牌(节点列表,事件):#把一条表面事件折到已计价表面上
    """把一条表面事件折到已计价表面上。事件与节点均为 dict。"""
    消息=事件派生消息(事件)#导出消息
    令牌数=0 if 消息 is None else 计价消息(消息)#无消息则0
    操作=事件['surfaceOp']#表面操作
    if 操作=='append':#追加
        return {'tokens':令牌数,'nodes':[*节点列表,{'seq':事件['seq'],'tokens':令牌数}],'deltaTokens':令牌数}#接到尾部
    起点下标=-1#范围起点
    终点下标=-1#范围终点
    下标=0#扫描下标
    起点序号=操作['start']#声明起点
    终点序号=操作['end']#声明终点
    for 节点 in 节点列表:#逐个节点
        if 起点下标==-1 and 节点['seq']==起点序号:#命中起点
            起点下标=下标#记下起点
        if 终点下标==-1 and 节点['seq']==终点序号:#命中终点
            终点下标=下标#记下终点
        下标+=1#前进一步
    ...
    被遮蔽=0#被替换价格
    for 节点 in 节点列表[起点下标:终点下标+1]:#含端切片
        被遮蔽+=节点['tokens']#合计被遮蔽价格
    下一=[*节点列表[:起点下标],{'seq':事件['seq'],'tokens':令牌数},*节点列表[终点下标+1:]]#用本事件替换该范围
