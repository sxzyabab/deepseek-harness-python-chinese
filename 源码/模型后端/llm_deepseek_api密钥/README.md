---
description: "DeepSeek API 密钥鉴权与模型发现。"
...
为 `deepseek-official` 注册独立的鉴权和模型发现插件。通过 llm_deepseek 共用 Messages 请求实现；本包拥有凭据解析和目录可用性判断。
...
`deepseek-official` 请求不包含本插件添加的模型可见文本；请求由共享 Messages 传输序列化。鉴权和目录过滤不增加输入 token。
