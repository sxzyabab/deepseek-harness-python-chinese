...
__all__=(#仅中文公开名
    '重试政策错误','默认最大重试次数','默认初始延迟毫秒','默认最大延迟毫秒','默认抖动比例',
...
普通政策模式={
    ...
    'maxRetries':整数字段(最小=0,最大=9007199254740991,默认值=默认最大重试次数),#非负整数次数
...
def 解析退避(配置,路径):#解析并校验退避
    ...
    最大=配置['maxDelayMs'] if 配置 is not None and 'maxDelayMs' in 配置 else None#最大延迟
    抖动=配置['jitterRatio'] if 配置 is not None and 'jitterRatio' in 配置 else None#抖动
    if 初始 is None:#缺省初始
        初始=默认初始延迟毫秒#默认初始
    if 最大 is None:#缺省最大
        最大=默认最大延迟毫秒#默认最大
    ...
    if not (isinstance(初始,(int,float)) and not isinstance(初始,bool) and math.isfinite(初始) and 初始>0) or 初始>定时器延迟上限毫秒:#初始越界
        raise 重试政策错误(路径+'.initialDelayMs must be a positive finite number no greater than '+str(定时器延迟上限毫秒))#初始延迟越界
    if not (isinstance(最大,(int,float)) and not isinstance(最大,bool) and math.isfinite(最大) and 最大>0) or 最大>定时器延迟上限毫秒:#最大越界
        raise 重试政策错误(路径+'.maxDelayMs must be a positive finite number no greater than '+str(定时器延迟上限毫秒))#最大延迟越界
    if 初始>最大:#初始超过最大
        raise 重试政策错误(路径+'.initialDelayMs must be less than or equal to maxDelayMs')#初始不得超过最大
    if not (isinstance(抖动,(int,float)) and not isinstance(抖动,bool) and math.isfinite(抖动)) or 抖动<0 or 抖动>1:#抖动非法
        raise 重试政策错误(路径+'.jitterRatio must be between 0 and 1')#抖动必须在 0 到 1
...
def 解析重试政策(配置,路径):#校验、填默认并拆离重试政策
    ...
    if 配置 is None:#缺省普通政策
        ...
        政策.update(退避)#并入退避字段
        return 深冻结(政策)#冻结
    模式值=配置['mode'] if 'mode' in 配置 else None#模式
    if 模式值=='normal':#普通模式
        ...
        if not 是安全整数 or 次数<0:#次数非法
            raise 重试政策错误(路径+'.maxRetries must be a non-negative safe integer')#必须是非负安全整数
        if len(码列表)==0:#码列表空
            raise 重试政策错误(路径+'.retryableCodes must not be empty')#不可为空
        for 码 in 码列表:#逐码
            if not isinstance(码,str) or len(码)==0:#码非法
                raise 重试政策错误(路径+'.retryableCodes must contain only non-empty strings')#必须全是非空字符串
        if len(set(码列表))!=len(码列表):#有重复
            raise 重试政策错误(路径+'.retryableCodes must not contain duplicates')#不可重复
        政策={'mode':'normal','maxRetries':次数,'retryableCodes':深冻结(list(码列表))}#普通政策
        政策.update(解析退避(配置['backoff'] if 'backoff' in 配置 else None,路径+'.backoff'))#解析退避
    ...
    raise 重试政策错误(路径+'.mode must be "normal" or "always"')#模式必须是 normal 或 always
