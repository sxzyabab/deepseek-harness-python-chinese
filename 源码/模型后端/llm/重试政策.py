...
退避模式=模式.对象({
    'initialDelayMs':模式.数字().最大(定时器延迟上限毫秒).默认(默认初始延迟毫秒),#初始延迟
    'maxDelayMs':模式.数字().最大(定时器延迟上限毫秒).默认(默认最大延迟毫秒),#最大延迟
    'jitterRatio':模式.数字().最小(0).最大(1).默认(默认抖动比例),#抖动比例
})#退避配置模式
普通政策模式=模式.对象({
    'mode':模式.常量('normal').必填(),#必须是 normal
    'maxRetries':模式.数字().步进(1).最小(0).最大(9007199254740991).默认(默认最大重试次数),#非负整数次数
    'retryableCodes':模式.数组(模式.字符串()).默认(list(默认可重试码)),#可重试码列表
    'backoff':退避模式,#嵌套退避
})#普通政策模式
始终政策模式=模式.对象({
    'mode':模式.常量('always').必填(),#必须是 always
    'backoff':退避模式,#嵌套退避
})#始终政策模式
重试政策模式=模式.联合([普通政策模式,始终政策模式])#政策配置联合模式
