...
class 线路文本内容部件(TypedDict):#多模态用户消息里的文本部件
    type:Literal['text']#文本类型
    text:str#文本

class 线路文件内容部件(TypedDict):#多模态用户消息里的 Files 引用
    type:Literal['file']#文件类型
    file_id:str#文件 id

class 线路图片网址内容部件(TypedDict):#多模态用户消息里的内联 data URL
    type:Literal['image_url']#图片网址类型
    image_url:dict#url 字段
...
class 线路用量(TypedDict):#线路 token 记账
    ...
    total_tokens:NotRequired[int]#可选合计
