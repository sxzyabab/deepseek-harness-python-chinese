...
def 映射用量(用量):
    ...
    命中数=0 if 缓存命中 is None else int(缓存命中)#减法用 0
    提示=int(用量['prompt_tokens'])#提示
    补全=int(用量['completion_tokens'])#补全
    合计=提示+补全#合计
    线都合计=用量.get('total_tokens')#可选线路合计
    有精确合计=(
        not isinstance(用量['prompt_tokens'],bool) and isinstance(用量['prompt_tokens'],int) and 用量['prompt_tokens']>=0
        and not isinstance(用量['completion_tokens'],bool) and isinstance(用量['completion_tokens'],int) and 用量['completion_tokens']>=0
        and (线都合计 is None or 线都合计==合计)
    )#精确合计
    结果={
        'inputTokens':提示-命中数,#去掉命中后的输入
        'outputTokens':补全,#补全
    }#互不相交计数
    if 有精确合计:#有精确合计
        结果['totalTokens']=合计#带上
...
def 接受身份(当前,到来):#接受流式身份字段
    """id/name 是身份不是累积：空串或 null 表示不变。"""
    if isinstance(到来,str) and len(到来)>0:#非空串才更新
        return 到来#新身份
    return 当前#不变
    ...
    for 载荷 in 载荷序列:#逐条载荷
        ...
        for 选择 in 块.get('choices') or []:#逐个选择
            ...
            for 调用 in 增量.get('tool_calls') or []:#工具调用增量
                ...
                工具['callId']=接受身份(工具.get('callId'),调用.get('id'))#身份
                函数=调用.get('function') or {}#可选函数片段
                工具['name']=接受身份(工具.get('name'),函数.get('name'))#名字
