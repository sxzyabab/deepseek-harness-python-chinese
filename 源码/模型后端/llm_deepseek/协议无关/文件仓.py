...
最大图片字节=32*1024*1024#每张请求图上限，含文件 id 引用
自有文件前缀='dsh-'#自有文件前缀
最大聊天图字节=最大图片字节#旧名兼容，公开面只挂 最大图片字节
...
class _仓中止控制器:#仓内中止控制器
    ...
    def __init__(自身):
        ...
        自身.中止原因值=None#原因
        自身.信号=_仓中止信号(自身._事件,lambda:自身.中止原因值)#信号
    def 中止(自身,原因=None):
        """发出中止；重复忽略。"""
...
def 文件名(版本):#文件名
    ...
    return 自有文件前缀+附件+'-'+变体+'.'+扩展名(版本['mediaType'])#文件名
...
class 深求文件仓:#文件存储
    ...
    def __init__(自身,选项=None):#构造
        ...
        自身.取传输=选项.get('fetch')#可选 fetch
        自身.飞行中={}#飞行中上传
        自身.锁=threading.Lock()#飞行表锁

    def 客户端(自身,连接):#客户端
        """按连接事实构造 Files 客户端。"""
        选项={'baseURL':连接['baseURL'],'apiKey':连接['apiKey'],'protocol':连接['protocol']}#连接
        if 自身.取传输 is not None:#测试传输
            选项['fetch']=自身.取传输#注入
        return 深求文件客户端(选项)#客户端
    ...
    def _确保已上传一次(自身,版本,连接,政策,信号):#单次确保上传
        """单次确保上传路径。"""
        字节=版本.get('bytes',len(版本.get('data') or b''))#字节
        if 字节>最大图片字节:#超限
            raise 大模型错误('DeepSeek image exceeds the 32 MiB per-image limit.','INVALID_REQUEST')#超限
        作用域=文件作用域(连接)#作用域
        现在=自身.现在()#当前毫秒
        ...
        if not 已提交['accepted']:#落败
            ...
            except (大模型错误,OSError,RuntimeError):#忽略
                pass#配额由恢复重试
        return {'record':已提交['record'],'uploaded':已提交['accepted']}#结果

    def 失效(自身,版本,文件标识,连接):#失效
        """模型请求拒绝远程 id 后失效精确本地映射。"""
        自身.索引.移除(文件作用域(连接),版本['variantId'],文件标识)#移除
    ...
    def 回收最旧自有(自身,连接,数量,信号=None):#回收最旧自有
        ...
        之后=None#分页
        自有=[]#待删
        while 连接['protocol']=='messages' or len(自有)<数量:#消息扫完全部页，对话补全凑满即停
            ...
            for 文件 in 页['data']:#逐文件
                if not 文件['filename'].startswith(自有文件前缀):#非自有
                    continue#跳过
                自有.append({'id':文件['id'],'createdAt':文件['createdAt']})#记下
                if 连接['protocol']=='chat-completions' and len(自有)==数量:#对话补全凑满
                    break#停
            if 连接['protocol']=='messages':#消息无升序查询
                自有.sort(key=lambda 项:项['createdAt'])#按创建时间
                自有=自有[:数量]#只留最旧
            if not 页.get('hasMore') or 'lastId' not in 页 or 页['lastId']==之后:#无更多
                break#停
            之后=页['lastId']#续
        for 文件 in 自有:#删除
            客户端.删除(文件['id'],信号)#删
