...
def 缺文件(错误):#是否缺文件
    """是否 ENOENT。"""
    return isinstance(错误,OSError) and getattr(错误,'errno',None)==2#ENOENT
...
def 解析索引(文本):#解析索引
    ...
    try:#JSON
        值=json.loads(文本)#解析
    except (json.JSONDecodeError,UnicodeDecodeError,TypeError,ValueError) as 错误:#非法 JSON
...
class 深求上传索引:#上传索引
    ...
    def 加载(自身):#加载
        ...
        except (OSError,非法上传索引错误) as 错误:#失败
