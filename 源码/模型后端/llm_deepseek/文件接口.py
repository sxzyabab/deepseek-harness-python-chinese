"""OpenAI 兼容的 DeepSeek Files API 传输。

对齐上游 `llm-deepseek/src/files-api.ts`。公开面仅中文名；无英文别名。
"""
import json,re,uuid#JSON、配额正则与 multipart 边界
from http.client import HTTPSConnection as 安全连接,HTTPConnection as 明文连接,HTTPException as 超文本异常#HTTP
from urllib.parse import urlparse as 解析网址,urlencode as 编码查询#URL
...
配额措辞=re.compile(r'(?:quota|storage|stored files|file count|too many files)',re.I)#配额措辞
...
def 非法响应(操作):#非法响应
    """构造非法响应错误。"""
    return 大模型错误(f'DeepSeek Files API returned an invalid {操作} response.','INVALID_RESPONSE')#非法
...
def 组multipart(字段,文件字段):#组 multipart 体
    ...
    块.append(b'\r\n')#换行
    块.append(f'--{边界}--\r\n'.encode('utf-8'))#收尾
    return b''.join(块),f'multipart/form-data; boundary={边界}'#体与类型

class 深求文件客户端:#Files 客户端
    """OpenAI 兼容 `/files` 端点的直接客户端。"""
    ...
    def 请求(自身,路径,方法,体=None,头覆盖=None,信号=None):#请求
        ...
        try:#传输
            ...
            if 信号 is not None and getattr(信号,'已中止',False):#中止
                raise 错误#原样
            raise 大模型错误(f'DeepSeek Files API request to {自身.基址} failed','TRANSPORT',{'cause':错误}) from 错误#传输
        ...
        细节=提供方错误细节(解析体)#细节
        raise 深求文件错误(
            细节.get('message') or f'DeepSeek Files API error (HTTP {状态})',#消息
            状态,#状态
            细节['detail'],#细节
        )#抛出
    ...
    def 上传(自身,输入):#上传
        ...
        体,类型=组multipart(
            ...
            ('file',数据,输入['mediaType'],输入['filename']),#文件
        )#multipart
        _,原文=自身.请求('/files','POST',体,{'content-type':类型},输入.get('signal'))#上传
        try:#解析
            文件=解析文件对象(json.loads(原文.decode('utf-8')),'upload')#文件
        except 大模型错误:#原样
            raise#原样
        except Exception:#非法
    ...
    def 列出(自身,选项=None):#列出
        ...
        _,原文=自身.请求('/files?'+编码查询(查询),'GET',信号=选项.get('signal'))#GET
        try:#解析
        ...
        if 值 is None or not isinstance(值,dict):#形态
            raise 非法响应('list')#非法
        数据=值.get('data')#数据
        if (值.get('object')!='list' or not isinstance(数据,list)
            or not isinstance(值.get('has_more'),bool)
            or (值.get('first_id') is not None and not isinstance(值.get('first_id'),str))
            or (值.get('last_id') is not None and not isinstance(值.get('last_id'),str))):#非法
            raise 非法响应('list')#非法
        页={
            'data':[解析文件对象(项,'list') for 项 in 数据],#文件表
            'hasMore':值['has_more'],#是否还有
        }#页
        if isinstance(值.get('first_id'),str):#首页 id
            页['firstId']=深求文件标识(值['first_id'])#品牌
        if isinstance(值.get('last_id'),str):#末页 id
            页['lastId']=深求文件标识(值['last_id'])#品牌
        return 页#页

    def 取回(自身,文件标识,信号=None):#取回
        """取回一个文件对象。"""
        from urllib.parse import quote as 百分编码#编码路径段
        _,原文=自身.请求('/files/'+百分编码(文件标识,safe=''),'GET',信号=信号)#GET
        try:#解析
            return 解析文件对象(json.loads(原文.decode('utf-8')),'retrieve')#文件
        except 大模型错误:#原样
            raise#原样
        except Exception:#非法
    ...
    def 删除(自身,文件标识,信号=None):#删除
        """删除一个提供方文件。"""
        from urllib.parse import quote as 百分编码#编码路径段
        _,原文=自身.请求('/files/'+百分编码(文件标识,safe=''),'DELETE',信号=信号)#DELETE
        try:#解析
            值=json.loads(原文.decode('utf-8'))#JSON
        except Exception:#非法
            raise 非法响应('delete')#非法
        if (值 is None or not isinstance(值,dict)
            or 值.get('id')!=文件标识 or 值.get('object')!='file' or 值.get('deleted') is not True):#非法
            raise 非法响应('delete')#非法
