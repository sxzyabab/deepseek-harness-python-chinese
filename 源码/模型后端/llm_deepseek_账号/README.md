---
description: "DeepSeek 账号鉴权与模型发现。"
...
为 `deepseek-account` 注册独立的鉴权和模型发现插件。通过 llm_deepseek 共用 Messages 请求实现；本包拥有凭据解析和目录可用性判断。

鉴权解析返回 `x-dsh-auth-token` 和捕获同一 token 的失败回调。HTTP 401 的分类和凭据失效处理由此提供方负责；被拒绝的旧请求不能清除替换后的登录。

## 使用此包

账号 token 仅由 `deepseekAccount.解析令牌(baseURL)` 提供，目标地址必须满足账号服务的来源校验。未登录或目标不允许账号鉴权时，请求以 `ACCOUNT_SIGN_IN_REQUIRED` 失败，目录为空；不会回退到 API key。其他凭据查询错误继续向调用者报告。
...
`deepseek-account` 请求不包含本插件添加的模型可见文本；请求由共享 Messages 传输序列化。鉴权和目录过滤不增加输入 token。
