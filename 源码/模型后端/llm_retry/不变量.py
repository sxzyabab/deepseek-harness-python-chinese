...
对齐上游 `llm-retry/src/invariant.ts`。公开面仅中文名。事件与失败事实为 dict。
"""
...
def 校验失败载荷(值,失败):
    ...
    if 值 is None or not isinstance(值,dict):#不是对象
        失败('llm/retry failure must be an object')#必须是对象
    if 'message' not in 值 or not isinstance(值['message'],str) or len(值['message'])==0:#消息非法
        失败('llm/retry failure.message must be a non-empty string')#消息必须非空
    if 'code' not in 值 or not isinstance(值['code'],str) or len(值['code'])==0:#code非法
        失败('llm/retry failure.code must be a non-empty string')#code必须非空
    if 'status' in 值:#有状态
        状态=值['status']#可选HTTP状态
        if isinstance(状态,bool) or not isinstance(状态,int) or 状态<100 or 状态>599:#不是合法HTTP码
            失败('llm/retry failure.status must be an integer from 100 through 599 when present')#状态越界
    if 'providerRetryAfterMs' in 值:#有提供方等待
        建议等待=值['providerRetryAfterMs']#可选提供方等待
        if isinstance(建议等待,bool) or not isinstance(建议等待,(int,float)) or not math.isfinite(建议等待) or 建议等待<=0:#不是正有限数
            失败('llm/retry failure.providerRetryAfterMs must be a positive finite number when present')#等待非法
    if 'requestId' in 值:#有请求id
        请求号=值['requestId']#可选请求id
...
def 校验重试(历史,事件,失败):
    ...
    载荷=事件['data']#本条重试载荷
    链身份=载荷['retryId'] if 'retryId' in 载荷 else None#重试链身份
    回合=载荷['turn'] if 'turn' in 载荷 else None#回合
    步=载荷['step'] if 'step' in 载荷 else None#步
    提供方=载荷['provider'] if 'provider' in 载荷 else None#提供方
    模式=载荷['mode'] if 'mode' in 载荷 else None#普通或始终
    政策键=载荷['policyKey'] if 'policyKey' in 载荷 else None#政策指纹
    序号=载荷['retry'] if 'retry' in 载荷 else None#本次重试序号
    延迟毫秒=载荷['delayMs'] if 'delayMs' in 载荷 else None#等待毫秒
    if not isinstance(链身份,str) or len(链身份)==0:#链身份非法
        失败('llm/retry retryId must be a non-empty string')#retryId必须非空
    失败事实=载荷['failure'] if 'failure' in 载荷 else None#失败载荷
    校验失败载荷(失败事实,失败)#校验失败事实
    if isinstance(序号,bool) or not isinstance(序号,int) or 序号<1 or 序号>外来JSON上限:#序号非法
    ...
    if 模式=='normal':#普通
        上限=载荷['maxRetries'] if 'maxRetries' in 载荷 else None#上限
        if isinstance(上限,bool) or not isinstance(上限,int) or 上限<1 or 上限>外来JSON上限 or 序号>上限:#上限非法或序号超出
            失败('llm/retry retry '+转JS字符串(序号)+' must not exceed a positive safe maxRetries '+转JS字符串(上限))#超出上限
    elif 模式=='always':#始终
        if 'maxRetries' in 载荷:#始终模式带了上限
            失败('llm/retry always mode must omit maxRetries')#始终模式不得带上限
    else:#未知模式
        失败('llm/retry mode must be normal or always, got '+转JS字符串(模式))#模式非法
    if isinstance(延迟毫秒,bool) or not isinstance(延迟毫秒,(int,float)) or not math.isfinite(延迟毫秒) or 延迟毫秒<0 or 延迟毫秒>定时器延迟上限毫秒:#延迟越界
    ...
    if 回合边界 is None or 回合边界['type']!='turn/start':#不在打开回合内
        失败('llm/retry must be appended inside an open turn')#必须在打开回合
    打开回合=回合边界['data']['turn']#打开回合号
    if 回合!=打开回合:#点名的回合对不上
    ...
    if 步边界 is None or 步边界['type']!='step/start':#不在打开步内
        失败('llm/retry must be appended inside an open step')#必须在打开步
    步载荷=步边界['data']#打开步载荷
    打开步=步载荷['step']#打开步号
    打开步回合=步载荷['turn']#打开步所属回合
    ...
    if 先前政策重试 is not None and 先前政策重试['data']['retryId']!=链身份:#链身份变了
    ...
    if 先前政策重试 is None:#新链
        def 占用链身份(先前):
            ...
            if 类型!='llm/retry' and 类型!='llm/retry-started':#不是本包事件
                return False#不是本包事件
            return 先前['data']['retryId']==链身份#同一身份
        if 从后找(历史,占用链身份) is not None:#已被占用
            失败('llm/retry retryId '+json.dumps(链身份,ensure_ascii=False,separators=(',',':'),allow_nan=False)+' is already owned by another chain')#retryId冲突
...
def 校验会话(会话,失败):
    ...
    事件列表=会话.events#会话事件
    下标=0#当前下标
    for 事件 in 事件列表:#按顺序回放
        类型=事件['type']#事件类型
        if 类型=='llm/retry':#重试记录
            校验重试(事件列表[:下标],事件,失败)#重试记录
        elif 类型=='llm/retry-started':#过渡记录
            校验已开始(事件列表[:下标],事件,失败)#过渡记录
        下标+=1#前进
...
def 安装(上下文,失败):
    ...
    def 新会话(会话):
        """新会话也回放。"""
        校验会话(会话,失败)#回放
    上下文.监听('session/created',新会话,{'全局':True})#新会话也回放
    ...
    上下文.监听('internal/dispatch',分派钩子,{'全局':True})#全局监听分派
