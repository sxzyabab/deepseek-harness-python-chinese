...
def 渲染目标(标题,目标):
    ...
    if 原因 is None:#无阻塞行
        ...
        阻塞行=['Blocker: '+原因['code']+': '+原因['message']]#可选阻塞行
    行=[#按行拼给用户
        标题,#操作标题
        'Status: '+阶段标签(阶段),#阶段
        *阻塞行,#阻塞原因，无则空
        'Objective: '+目标['objective'],#目标陈述
        'Rounds: '+str(目标['roundsStarted'])+'/'+str(目标['maxGoalRounds']),#已接纳轮次与上限
...
def 执行目标命令(上下文,调用):
    ...
    try:#域边界可能抛 目标错误
        ...
        if 种类=='pause':#暂停
            ...
            return 渲染目标('Goal paused',上下文.goals.pause(调用['agent'],目标引用(当前)))#暂停并渲染
        if 种类=='resume':#恢复
            if 当前 is None:#没有可恢复的目标
                return 缺少目标('resume')#缺目标
            return 渲染目标('Goal resumed',上下文.goals.resume(调用['agent'],目标引用(当前)))#恢复并渲染
        if 种类=='clear':#清除
            if 当前 is None:#本来就没有
                return {'kind':'success','text':'No goal to clear.'}#空清除
            上下文.goals.clear(调用['agent'],目标引用(当前))#留下墓碑
