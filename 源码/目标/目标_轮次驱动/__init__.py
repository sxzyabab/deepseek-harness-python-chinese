...
def 应用(上下文):#安装自动续跑
    ...
    def 当前目标(状态):#只在精确实例仍在线时读
        """精确 Agent 仍在线时读当前目标。"""
        if 上下文.agents.获取(状态['agent'].id) is not 状态['agent']:#已换实例
            return None#无
        return 上下文.goals.获取(状态['agent'])#当前视图

    def 可以驱动(状态):#静止且无竞争
        """本生命周期是否静止且无竞争提示。"""
        return (上下文.纤程.state==纤程状态.已激活
            and not 状态['stopping']
            and 上下文.agents.获取(状态['agent'].id) is 状态['agent']
    ...
    def 解除武装(状态):#去掉自动权限
        ...
        try:#解除可能失败
            ...
            if 目标 is not None and 目标['activation']=='armed':#仍武装
                上下文.goals.解除武装(状态['agent'])#解除
        except Exception as 错误:#失败只记日志
            上下文.日志.警告('goal-round-driver: could not disarm agent "'+str(状态['agent'].id)+'": '+渲染抛出(错误))#警告
    ...
    def 驱动(状态):#处理已接纳工作再预订下一轮
        ...
        if 状态['needsCheckpoint']:#需要落盘
            ...
            try:#落盘
                上下文.sessions.flush(智能体.session)#刷会话
            except Exception as 错误:#失败
                上下文.日志.警告('goal-round-driver: durability checkpoint failed for agent "'+str(智能体.id)+'": '+渲染抛出(错误))#警告
                解除武装(状态)#解除
                return
                ...
                def 循环():#请求循环
                    ...
                    while 状态['requested'] and not 状态['stopping']:#还有请求
                        ...
                        try:#一次驱动
                            ...
                            上下文.日志.警告('goal-round-driver: driver failed for agent "'+str(状态['agent'].id)+'": '+渲染抛出(错误))#警告
                            解除武装(状态)#解除
                上下文.agents.无发起方(循环)#隐藏发起方
    ...
    def 安装():#监听与拆除
        ...
        def 目标已变更(载荷,*位置参数):#变更边
            ...
            if 变更['operation']=='pause' and 智能体.状态=='running' and 上下文.agents.当前发起方() is not 智能体:#宿主暂停
                智能体.取消({'kind':'user'},{'keepInbox':True})#取消本轮
            请求驱动(状态)#再跑
        上下文.监听('goal/changed',目标已变更)#目标变更
        def 收件箱插入(载荷,*位置参数):#插入边
            ...
            if 尝试 is not None and 尝试['phase']=='queued':#排队中
                尝试['stale']=True#过期
        上下文.监听('agent/inbox/inserted',收件箱插入)#插入
        def 收件箱领取(载荷,*位置参数):#领取边
            ...
            if 尝试 is not None and 同一排队(消息['content'],消息['source'],尝试):#本预订
                尝试['phase']='claimed'#领取
        上下文.监听('agent/inbox/claimed',收件箱领取)#领取
        ...
        上下文.监听('agent/inbox/discarded',收件箱丢弃)#丢弃
        def 会话事件(会话,事件,*位置参数):#日志边
            """接纳与中止边。"""
            智能体=上下文.agents.获取(会话.id)#按会话
        ...
        def 预步骤(载荷,下一):#步骤围栏
            ...
            try:#检查
                ...
                上下文.日志.警告('goal-round-driver: pre-step check failed for agent "'+str(智能体.id)+'": '+渲染抛出(错误))#警告
                解除武装(状态)#解除
            if not 有效:#无效
                尝试=状态['attempt']#预订
            ...
            try:#决定后再核
                ...
                上下文.日志.警告('goal-round-driver: post-decision check failed for agent "'+str(智能体.id)+'": '+渲染抛出(错误))#警告
                解除武装(状态)#解除
                有效=False#无效
            if not 有效:#无效
                状态['attempt']=None掉
    ...
    上下文.副作用(安装,'goal-round-driver lifecycle')#复合副作用
