...
对齐上游 `@deepseek-ai/dsh-host-open-in-app`。安全围栏在本文件：每条路由先问
connection.requestRejection；open 路由再校验媒体类型、64 KiB 上限、可用目录 id
...
from .内部缝 import 内部#测试缝
from .共享 import 应用列表路由,图标前缀,打开路由#线路径

名称='open-in-app'#Cordis 插件名（字面量）
注入=['webServer','connection','subprocess']#路由载体、信任围栏、PATH 解析
...
def 应用(上下文,配置值):#登记三条路由
    ...
    def 打开处理(请求,响应):#POST open
        ...
        try:#读正文
            文本=读取有界正文(请求)#有界
        except Exception:#连接中断等
            发送json(响应,400,{'code':'bad-request','message':'request body unreadable'})#400
            return#结束
        if 文本 is None:#超限
        ...
        if not 是目录:#不是现存目录
            ...
            return#结束
        结局=启动已解析(已解析,解析体['path'],配置值['launchWatchMs'],目录内部())#启动
