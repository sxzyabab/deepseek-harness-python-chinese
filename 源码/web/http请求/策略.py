...
媒体类型去参=re.compile(r';.*\Z',re.S)#去掉 Content-Type 参数，行尾用 \\Z
...
def 校验抓取网址(输入,最大网址长度):#校验并解析请求 URL
    ...
    if (网址.username is not None and len(网址.username)>0) or (网址.password is not None and len(网址.password)>0):#判 length：内嵌用户名或密码
...
def 字符集解码器(字符集):#按 charset 建解码标签
    ...
    if 字符集 is None:#未声明则 UTF-8
        return 'utf-8'#默认编码
    try:#按标签探测
        ''.encode(字符集)#不认识的标签抛 LookupError
        return 字符集#声明编码
    except LookupError as 错误:#未知 charset
