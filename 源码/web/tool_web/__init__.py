...
配置模式=模式.对象({#插件配置：注册哪些 web 工具、来源上限、各工具预算、抓取输出上限
    'search':模式.布尔().默认(True),#是否注册 web_search
    'fetch':模式.布尔().默认(True),#是否注册 web_fetch
    'searchMaxResults':模式.数字().默认(网络搜索最大结果数),#一次 web_search 返回的来源上限
    'fetchTimeoutMs':模式.数字().默认(默认网络工具超时毫秒),#web_fetch 的协作超时预算
    'searchTimeoutMs':模式.数字().默认(默认网络工具超时毫秒),#web_search 的协作超时预算
    'fetchMaxOutputChars':模式.数字().默认(默认抓取最大输出字符),#同步转换源字符与完整输出上限
})#配置模式结束
