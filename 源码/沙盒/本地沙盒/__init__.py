...
class 本地沙箱提供方(沙箱提供方):#本地进程沙箱提供方
    ...
    def __init__(自身,上下文对象,配置):#构造本地提供方
        ...
        if 运行器 is None:#缺席
            运行器=[]#空
        失败签名=配置['runnerFailureSignatures'] if 'runnerFailureSignatures' in 配置 else []#失败签名
        if 失败签名 is None:#缺席
            失败签名=[]#空
        if len(运行器)==0 and len(失败签名)>0:#有签名没有运行器
            raise 本地沙箱错误('sandbox-local: runnerFailureSignatures requires runnerCommand')#签名依赖运行器
        if len(运行器)>0 and len(失败签名)==0:#有运行器没有签名
            raise 本地沙箱错误('sandbox-local: runnerCommand requires at least one runnerFailureSignatures entry')#运行器需要签名
        for 签名 in 失败签名:#逐条检查形态
            if len(签名.strip())==0 or re.search(r'[\r\n]',签名) is not None:#空或含换行
                raise 本地沙箱错误('sandbox-local: runnerFailureSignatures entries must be non-empty single-line strings')#必须是非空单行
        自身.runnerCommand=运行器 if len(运行器)>0 else None#空则未配置
        自身.configuredRunnerFailureSignatures=list(失败签名)#记下签名
        if 'probeTimeoutMs' in 配置:#配置给了超时
            自身.probeTimeoutMs=配置['probeTimeoutMs']#记下超时
        else:
            自身.probeTimeoutMs=5000#回落 5 秒
        断言正有限数('probeTimeoutMs',自身.probeTimeoutMs)#必须正整数
    ...
    def 隔离(自身,参数表,政策):#包装为隔离 argv
        ...
        已选=自身.选择运行器(政策['mode'])#选择运行器
        运行器名=已选['runner']#所选档
        运行器参数表=自身.运行器参数表(运行器名,政策)#运行器调用
        return {#平台运行器
            'argv':[*运行器参数表,'--',*参数表],#运行器加调用方 argv
            'enforcement':已选['enforcement'],#所选强制程度
            'denialSignatures':list(拒绝签名[运行器名]),#该运行器拒绝签名
            'runnerFailureRules':[{键:list(值) if isinstance(值,tuple) else 值 for 键,值 in 规则.items()} for 规则 in 运行器失败规则[运行器名]],#该运行器失败规则
        }#返回结束
        ...
        if 会话号 is None or 政策['mode']=='read-only':#无会话或只读
            return [#不物化授权
                *自身.windowsAcl运行器调用(),#运行器前缀
                '--workspace',政策['workspaceRoot'],#工作区
                '--temp',tempfile.gettempdir(),#环境临时根
                '--mode',政策['mode'],#模式
            ]#返回结束
        临时=自身.物化ACL授权(会话号,政策['workspaceRoot'])#物化授权
        return [#seam 管理的授权
            *自身.windowsAcl运行器调用(),#运行器前缀
            '--workspace',政策['workspaceRoot'],#工作区
            '--temp',临时['dir'],#私有临时目录
            '--mode',政策['mode'],#模式
            '--write-sid',工作区写入SID(政策['workspaceRoot']),#工作区 SID
            '--temp-write-sid',临时['writeSid'],#临时 SID
        ]#返回结束
    ...
    def 撤销ACL授权(自身):#撤销 ACL 授权
        ...
        if len(自身.workspaceGrants)==0 and len(自身.tempCapabilities)==0:#没有授权
            return#跳过
        失败列表=[]#清理失败
        授权列表=[*自身.workspaceGrants.values(),*[能力['grant'] for 能力 in 自身.tempCapabilities.values()]]#每份授权
        for 授权 in 授权列表:#逐份拆除
            try:#拆除
        ...
        if len(失败列表)>0:#有清理失败
            自身.ctx.日志.警告('sandbox-local: windows-acl grant cleanup completed with '+str(len(失败列表))+' failure(s)')#警告文案
            for 错误 in 失败列表:#逐条错误
                自身.ctx.日志.警告(错误)#记日志
    ...
    def 链裁决(自身):#链裁决
        ...
        if 钩子链 is not None:#有钩子链
            ...
            平台=自身.internals['platform'] if 'platform' in 自身.internals else None#钩子平台
            if 平台 is None:#未注入
                平台=规范平台(sys.platform)#宿主平台
            链=list(平台链[规范平台(平台)] if 规范平台(平台) in 平台链 else ())#平台链或空
        if len(链)==0:#没有链
    ...
    def 探测运行器(自身,运行器):
        ...
        if 运行器=='bwrap':#bwrap
            ...
            return 'full' if 默认探测Bwrap(自身.probeTimeoutMs) else 'unusable'#默认探测
        if 运行器=='landlock':#Landlock
            if 'probeLandlock' in 自身.internals:#有钩子
                return 自身.internals['probeLandlock'](自身.landlock启动器())#钩子探测
            return 默认探测Landlock(自身.landlock启动器(),{'timeoutMs':自身.probeTimeoutMs})#默认探测
        if 运行器=='seatbelt':#Seatbelt
            if 'probeSeatbelt' in 自身.internals:#有钩子
                探测=自身.internals['probeSeatbelt']#钩子
                return 'full' if 探测(自身.seatbelt可执行()) else 'unusable'#通过则完整
            return 'full' if 默认探测Seatbelt(自身.seatbelt可执行(),自身.probeTimeoutMs) else 'unusable'#默认探测
        if 运行器=='windows-acl':#windows-acl
            if 'probeWindowsAcl' in 自身.internals:#有钩子
                探测=自身.internals['probeWindowsAcl']#钩子
                return 'partial' if 探测() else 'unusable'#通过则部分
            return 'partial' if 默认探测WindowsAcl(自身.windowsAcl运行器调用(),自身.probeTimeoutMs) else 'unusable'#默认探测
        return 断言永不(运行器,'SelectedRunner.runner')#封闭联合穷尽
    ...
    def windowsAcl运行器调用(自身):
        ...
        if 'windowsAclRunnerEntry' in 自身.internals:#测试注入入口
            入口覆盖=自身.internals['windowsAclRunnerEntry']#假入口
            return [sys.executable,入口覆盖]#解释器 + 入口
        入口=os.path.normpath(os.path.join(os.path.dirname(__file__),'..','沙盒_windows访问控制','运行器.py'))#本树运行器入口
        return [sys.executable,入口]#解释器 + 运行器.py

__all__=['名称','配置模式','本地沙箱提供方','本地沙箱错误']#仅中文公开名
name=名称#Cordis 插件名
Config=配置模式#Cordis 配置模式
default=本地沙箱提供方#Cordis 默认导出
