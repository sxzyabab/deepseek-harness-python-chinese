...
from .诊断 import 分类运行器失败,是否运行器派生失败,匹配签名#再导出运行器失败分类
...
class 沙箱提供方(服务):#沙箱提供方
    ...
    def 隔离(自身,参数表,政策,信号=None):#包装为隔离argv
        """包装 argv 使它在本宿主上按政策隔离执行；调用方用返回的 argv 代替自己的去 spawn。参数表是调用方即将 spawn 的精确 argv，不是 shell 字符串；政策是本次执行所处的文件效果政策，按次携带。信号在解析政策与运行器期间取消。"""
        若已中止则抛出(信号)#解析前检查取消
