...
def 查找登录SID(接口,令牌):#抽出登录SID
    ...
    组字节=bytearray(所需)#组缓冲
    组缓冲=(ctypes.c_ubyte*所需).from_buffer(组字节)#可写ctypes视图
    if 接口.getTokenInformation(令牌,abi.令牌组信息,组缓冲,所需,所需槽)==0:#真正读取失败
        抛上次错误(接口,'GetTokenInformation','TokenGroups')#带码抛出
    组数=int.from_bytes(组字节[0:4],'little')#组数
    for 下标 in range(组数):#逐组
        基=abi.令牌组偏移+下标*abi.SID与属性大小#本组偏移
        sid指针=解码缓冲内指针(组字节,基)#本组SID
        属性=int.from_bytes(组字节[基+8:基+12],'little')#本组属性
...
def 设令牌默认DACL授予(接口,令牌,sid指针):#合并默认DACL授权
    ...
    if 当前DACL is None:#没有默认DACL
        raise 访问控制错误('setTokenDefaultDaclGrant: the token carries no default DACL to extend')#无法扩展
...
def 构建限制SID数组(SID列表):#打包限制SID数组
    ...
    for 下标,条目SID in enumerate(SID列表):#逐个SID
        ...
        缓冲[偏移:偏移+8]=指针地址(条目SID).to_bytes(8,'little')#写入SID指针
    return 缓冲#已打包数组
...
def 创建受限令牌(接口,当前令牌,登录SID,写入SID列表,已知,模式):#创建写入受限令牌
    ...
    if 模式=='read-only':#只读
        ...
        raise 访问控制错误('createRestrictedToken: workspace-write restricting list requires at least one write SID')#必须至少一条
    else:#workspace-write
        限制列表=[登录SID,已知['world'],*写入SID列表]#保活组加能力SID
    限制缓冲=构建限制SID数组(限制列表)#打包
