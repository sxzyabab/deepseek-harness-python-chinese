...
def 断言临时根在工作区外(工作区根,临时根):#断言临时根在工作区外
    ...
    if 包含目录(工作区根,临时根):#临时根落在工作区内
        raise 访问控制错误('Windows ACL temp root must be outside the workspace')#拒绝重叠

def 断言私有临时不相交(可写目录列表,临时目录):#断言私有临时与可写目录不相交
    """拒绝实际私有临时目录与任一可写目录重叠：任一方向的继承都会把两种能力合并。"""
    for 可写目录 in 可写目录列表:#逐个可写目录
        if 包含目录(可写目录,临时目录) or 包含目录(临时目录,可写目录):#任一方向包含
            raise 访问控制错误('AclSandbox private temp directory must be disjoint from writable directories')#拒绝重叠
