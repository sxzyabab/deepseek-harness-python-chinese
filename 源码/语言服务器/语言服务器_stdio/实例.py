...
class 语言服务器实例:#一条已初始化的语言服务器实例
    ...
    def 跑查询(自身,请求,源,信号=None):#跑瞬时打开→请求→关闭
        ...
        try:#打开文档、发请求、归一结果
            ...
            载荷=自身.发送请求(操作,网址,请求['position'],信号)#发送语义请求
            return 自身.归一(操作,载荷)#归一成seam结果
        finally:#无论成败都尝试关闭文档
            if 已打开 and not 自身.已死:#仍活着且已打开
                try:#发送didClose
                    自身.连接.通知('textDocument/didClose',{'textDocument':{'uri':网址}})#关闭瞬时文档
    ...
    def 回答服务器请求(自身,方法,参数):#回答一条服务器→客户端请求
        ...
        if 方法=='workspace/configuration':#配置请求
            ...
            return [自身.规格['configuration'] if 'configuration' in 自身.规格 else None for _ in 项列表]#每项都回同一静态值
