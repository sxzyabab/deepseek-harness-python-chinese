"""消费沙箱的 PowerShell 执行器。经 ctx.sandbox 包装本地 pwsh argv，并报告模式、强制与拒绝事实。"""
from ...沙盒.沙盒 import (
    沙箱不可用错误,
    分类运行器失败,
    是否运行器派生失败,
...
from ..本地powershell import 本地PowerShell执行器

__all__=['沙盒PowerShell执行器']

class 沙盒PowerShell执行器(本地PowerShell执行器):
    ...
    def 隔离(自身,规格,政策,信号=None):
        """经 sandbox 包装本次 pwsh argv。"""
        return 自身.ctx.sandbox.隔离(自身.参数表(规格),政策,信号)

default=沙盒PowerShell执行器
