...
def 断言正有限(名称,值):#断言值为正有限数
    ...
    if isinstance(值,bool) or not isinstance(值,(int,float)) or not math.isfinite(值) or 值<=0:#不是正有限数
        raise Exception('bash-local: '+名称+' must be a positive finite number')#用字段名报配置不可用
...
def 断言可用Bash配置(配置):#断言配置能拿来跑
    ...
    if 宽限>定时器延迟上限毫秒:#宽限超过定时器上限
        raise Exception('bash-local: graceMs must be no greater than '+str(定时器延迟上限毫秒))#拒绝过大的宽限
...
def 取出已收集(句柄):#取出收集模式的两路读取器
    ...
    if 标准输出 is None or 标准误 is None:#实现丢掉了请求的收集流
        raise Exception('bash-local: subprocess implementation dropped a requested collect stream')#按缝约定这两路必须在
...
class 本地Bash执行器(外壳执行器):#本地 bash 执行器
    ...
    本执行器在每次 spawn 时提供它们的配置预算。公开方法仅中文：解析、运行、启动、按参数表运行/启动。
    """
    inject=['subprocess']#Cordis 注入子进程服务（协议槽）
    Config=配置模式#Cordis 配置模式（协议槽）
    def __init__(自身,上下文对象,配置):#用上下文和配置构造执行器
        ...
        def 变更时():#每个字段都在每条命令经 getter 读取
            """文档变化时没有从源派生、需要重建的东西。"""
            return#空变更钩子
        安装设置段(上下文对象,外壳设置命名空间,本地Bash执行器.Config,配置,{
    ...
    def 解析(自身,请求):#把请求解析成完整规格
        ...
        超时毫秒=夹取超时(取字段(请求,'timeoutMs'),取字段(自身.配置,'timeoutMs'),取字段(自身.配置,'maxTimeoutMs'),'bash-local: request.timeoutMs')#夹取本次超时
    ...
    def 拉起规格(自身,规格,标准输出上限,信号,参数表):#把规格和argv映射成子进程spawn规格
        """把一条已解析 bash 规格加上显式 argv 映射成完整的子进程 spawn。"""
    ...
    def 运行(自身,规格):#前台跑一条已解析规格
        """前台跑一条已解析规格。"""
        return 自身.按参数表运行(规格,['bash','-c',取字段(规格,'command')])#用bash -c前台跑
    ...
    def 启动(自身,规格):#后台拉起一条已解析规格
        """后台拉起一条已解析规格。"""
        return 自身.按参数表启动(规格,['bash','-c',取字段(规格,'command')])#用bash -c后台拉起
    ...
    def 按参数表启动(自身,规格,参数表):#按给定argv后台启动
        ...
        进程={
            ...
            'signal':None,#尚未被信号打死
            'done':完成,#结算承诺
            'readOutput':读输出,#增量读取（句柄载荷键字面量，对齐 shell 消费方）
            'kill':杀死,#终止（句柄载荷键字面量）
        }#后台进程句柄
...
Config=配置模式#Cordis 配置模式再导出（协议槽，与类上 Config 同权威）
默认=本地Bash执行器#中文默认导出
