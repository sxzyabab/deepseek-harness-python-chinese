"""bash 工具面向模型的结果渲染。对齐上游 `tool-bash/src/render.ts`。公开面仅中文名。
...
__all__=['解析退出状态','解析退出状态字段','渲染结果','渲染进程读取']#仅中文公开名
...
def 渲染结果(结果,升级模式=None):#渲染前台 bash 结果
    ...
    if 取字段(结果,'signal') is not None:#被信号杀死
        标记们.append('[killed by signal: '+str(取字段(结果,'signal'))+']')#信号标记
...
def 渲染进程读取(读取,沙箱=None,升级模式=None):#渲染后台进程增量
    """把一次后台进程读取收成模型可见的 job_output 增量。

    增量正文，并在内存截断丢掉未读字节时附上有损读取通知（含完整流溢出路径）。
    空增量渲染（`(no new output)`）是通用任务控制器的事。
    """
    if 升级模式 is None:#缺省空表
        升级模式=[]#无升级提示
    通知们=[]#附加通知
    if 取字段(读取,'lossy'):#内存有损
        路径们=[]#溢出路径
        出溢出=取字段(读取,'stdoutSpillPath')#标准输出溢出
        错溢出=取字段(读取,'stderrSpillPath')#标准错误溢出
        if 出溢出 is not None:#有标准输出溢出
            路径们.append(出溢出)#收下
        if 错溢出 is not None:#有标准错误溢出
            路径们.append(错溢出)#收下
        if len(路径们)>0:#有路径
            路径文=', '.join(路径们)#拼接
        else:#不可用
