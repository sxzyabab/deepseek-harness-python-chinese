...
def 经语言模型摘要(上下文,配置,输入,智能体,信号=None):
    """跑默认的复用缓存 ctx.llm.stream() 摘要调用：重放对话前缀，再把压缩指令追加为最后一条用户消息。"""
    会话=智能体.session#目标会话
    请求头=会话.请求头()#最近请求头
    最近头=请求头['config'] if 请求头 is not None and 'config' in 请求头 else None#最近一次已路由请求配置
    if len(配置['summarizationProvider'] if 'summarizationProvider' in 配置 and 配置['summarizationProvider'] is not None else '')==0:#未配置摘要提供方
        已配置=None#则不强制覆盖
    else:#成对覆盖
        已配置={'provider':配置['summarizationProvider'],'model':配置['summarizationModel']}#成对
    选项=智能体.options if 智能体.options is not None else {}#智能体选项
    选项提供方=选项['provider'] if 'provider' in 选项 else None#选项提供方
    选项模型=选项['model'] if 'model' in 选项 else None#选项模型
    ...
    组装器=块组装器()#组装流式块
    消息列表=list(输入['messages'] if 'messages' in 输入 and 输入['messages'] is not None else [])+[创建用户消息({#重放消息加指令
        'content':[{'type':'text','text':压缩指令}],#指令文本
        'source':{'kind':'plugin','plugin':'dsh-compaction-basic'},#插件出处
    })]#messages 结束
    选项表={#一次性生成选项
        'provider':目标['provider'],#提供方
        'model':目标['model'],#模型
        'messages':消息列表,#含指令的消息
        'maxTokens':配置['maxTokens'],#生成上限
    ...
    摘要=摘要文本(原始输出)#只留文本块
    if not any(len((块['text'] if 'text' in 块 else '').strip())>0 for 块 in 摘要):#没有任何非空白文本
        raise 基础压缩错误('summarization produced no text summary content')#空摘要失败
    结果={#已标记 LLM 调用结果
        'summary':摘要,#安全文本
        'rawOutput':原始输出,#完整输出
