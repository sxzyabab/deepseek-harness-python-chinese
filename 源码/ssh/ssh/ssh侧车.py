...
def 运行ssh辅助(传输):#跑到通道关闭或租期到期
    ...
    def 处理(方法,原始,请求信号):#入站
        ...
        if 方法=='executable':#可执行
            ...
            环境=None if 'env' not in 原始 else {键:值 for 键,值 in 环境模式(原始['env']).items() if 值 is not None}#去掉墓碑
            try:#解析
        ...
        if 方法=='fs.write' or 方法=='fs.edit':#变更
            ...
            if 方法=='fs.write':#写
                ...
                期望=None if 'expected' not in 原始 else 意图模式(原始['expected'])#意图
