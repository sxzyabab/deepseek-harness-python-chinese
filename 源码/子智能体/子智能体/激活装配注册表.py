...
class 子智能体激活装配注册表:#装配注册表
    ...
    def 登记(自身,贡献):#登记贡献
        ...
        def 撤销():#撤销登记
            ...
            if 登记 in 自身._登记列表:#仍在活集
                自身._登记列表.remove(登记)#移出活集
            自身._全部拆除(list(登记['installations']),'contribution removal')#释放全部安装
        return 撤销#撤销器
    ...
    def 应用(自身,子上下文):#安装全部活贡献
        ...
        try:#安装可能抛
            ...
            # 保持安装器失败为权威，但尝试每一次回滚。
            try:#回滚不得掩盖原失败
                自身._全部拆除(list(状态['installations']),'setup rollback')#回滚本批
        ...
        def 子拆除工厂():#子作用域拆除时释放
            ...
            def 子拆除():#子作用域拆除时释放
                ...
                自身._拆除子(子上下文)#释放一子
            return 子拆除#拆除器
        子上下文.副作用(子拆除工厂,'subagents.activationSetup()')#子作用域拆除时释放
        def 提交():#发布边界提交
            """发布边界提交。"""
    ...
    def _拆除子(自身,子上下文):#释放一子
        """释放一个已拆除子作用域拥有的每个剩余安装。"""
        索引=自身._按子.get(子上下文) or []#该子安装
        自身._全部拆除(list(索引),'child scope disposal')#全部释放

    def _全部拆除(自身,安装列表,期间):#释放一批
        ...
        if len(失败列表)==0:#全部成功
            return#成功
        raise 子智能体错误(#聚合失败
            'continuable-subagent setup '+期间+' failed to release '+str(len(失败列表))+' installation(s): '#文案前
            +'; '.join([错误链(失败) for 失败 in 失败列表]),#文案后
            'ACTIVATION_SETUP_RELEASE_FAILED',#错误码
        )#SubagentError结束

    def _拆除一条(自身,安装):#释放一条
        """从两个索引丢掉一次安装并恰好拆除它一次。"""
