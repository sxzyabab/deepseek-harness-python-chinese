...
def 断言已知键(值,键集,路径):#断言无未知键
    ...
    for 键 in 值.keys():#逐键
        if 键 not in 键集:#未知字段
            raise 子智能体描述符错误('persisted subagent descriptor '+路径+' has unknown field "'+键+'"')#拒绝
...
def 可选字符串(值,键):#读可选字符串
    ...
    if not isinstance(字段,str):#类型不对
        raise 子智能体描述符错误('persisted subagent descriptor '+键+' must be a string')#拒绝
    return 字段#字符串
...
def 可选字符串数组(值,键):#读可选字符串数组
    ...
    if not isinstance(字段,list):#不是数组
        raise 子智能体描述符错误('persisted subagent descriptor toolFilter.'+键+' must be an array of strings')#拒绝
    for 项 in 字段:#逐元素
...
def 解析工具过滤(值):#解析工具过滤
    """校验并重建持久化的工具限制。"""
    if not 是否记录(值):#必须是对象
        raise 子智能体描述符错误('persisted subagent descriptor toolFilter must be an object')#拒绝
    断言已知键(值,工具过滤键,'toolFilter')#只允许allow/deny
    允许=可选字符串数组(值,'allow')#可选允许表
    拒绝=可选字符串数组(值,'deny')#可选拒绝表
    if 允许 is None and 拒绝 is None:#两者都缺
        raise 子智能体描述符错误('persisted subagent descriptor toolFilter must declare allow and/or deny')#必须声明其一
    结果={}#重建限制
...
def 解析子智能体描述符(值):#解析描述符
    """为当前运行时校验一份持久化描述符载荷。"""
    if not 是否记录(值):#必须是对象
        raise 子智能体描述符错误('persisted subagent descriptor payload must be an object')#拒绝
    版本=值['version'] if 'version' in 值 else None#版本字段
    if not isinstance(版本,(int,float)) or isinstance(版本,bool):#必须是数字
        raise 子智能体描述符错误('persisted subagent descriptor version must be a number')#拒绝
    if 版本!=子智能体描述符版本:#他版本无法分类
        return None#无法分类
    模式=值['mode'] if 'mode' in 值 else None#模式字段
    if 模式!='one-shot' and 模式!='continuable':#必须是已知模式
        raise 子智能体描述符错误('persisted subagent descriptor mode must be "one-shot" or "continuable"')#拒绝
    断言已知键(值,一次性描述符键 if 模式=='one-shot' else 可续跑描述符键,'payload')#按模式校验键
    提供方=值['provider'] if 'provider' in 值 else None#提供方字段
    if not isinstance(提供方,str):#必须是字符串
        raise 子智能体描述符错误('persisted subagent descriptor provider must be a string')#拒绝
    if 模式=='one-shot':#一次性载荷
        ...
        if 标签 is not None:#有标签
            结果['label']=标签#展开
        return 结果#一次性
    标签=值['label'] if 'label' in 值 else None#可续跑标签必填
...
def 快照子智能体描述符(输入):
    """在任何 Task 或提供方工作开始之前，把描述符输入校验并分离成耐久载荷——与会话日志自身强制的同一分离无损 JSON 边界。输入为 dict。"""
    模式=输入['mode']#生命周期模式
    提供方=输入['provider']#提供方名
    if 模式=='one-shot':#一次性候选
        候选={'version':子智能体描述符版本,'mode':模式,'provider':提供方}#一次性候选
        if 'label' in 输入 and 输入['label'] is not None:#有标签
            候选['label']=输入['label']#展开
    else:#可续跑候选
        标签=输入['label']#标签
    ...
    if 快照 is None:#不能无损序列化
        raise 子智能体描述符错误('subagent descriptor is not losslessly JSON-serializable')#拒绝
    return 快照#已分离载荷

def 折叠子智能体描述符(事件列表):
    """把持久化子日志折叠成其受支持的描述符。第一条 subagent/descriptor 事件是权威——建立提供方恰好追加一条，因此后来的同类型事件不能改写已声明组合。事件为 dict。"""
    事件=None#第一条描述符
    for 候选 in 事件列表:#找第一条描述符事件
        类型=候选['type'] if isinstance(候选,dict) and 'type' in 候选 else None#类型
