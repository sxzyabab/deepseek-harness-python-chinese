...
from ..子智能体.深度 import 断言子智能体最大深度#深度上限断言
...
class 进程内跑:
    """持有者所有的进程内一次性跑。载荷字段 id/localAgent/result 为线协议键；拆除入口仅 销毁。"""
...
def 启动进程内运行(请求,选项=None):
    ...
    if 已中止(信号):#已取消
        raise 子智能体错误('子智能体请求在子体发布前已中止','CANCELLED')#拒绝
    父=请求['parent']#父智能体
    ...
    return 驱动已发布运行(句柄,信号,请求['prompt'],子标识,激活边界,状态['结构化'])#驱动

def 驱动已发布运行(句柄,信号,提示,子标识,边界,结构化):
    """包装已发布子体的单回合生命周期。句柄为已发表句柄对象。"""
    子=句柄.智能体#子智能体
    取消标志={'cancelled':False}#取消
    def 中止():
        """取消子体当前回合。"""
        取消标志['cancelled']=True#标记
    ...
    if 信号 is not None:#有信号
        def 监视中止():
            ...
            信号._事件.wait()#阻塞到中止
            中止()#取消
        threading.Thread(target=监视中止,daemon=True).start()#听中止
        if 已中止(信号):#已中止
            中止()#立刻
    结果任务=操作任务()#结果
    def 工作者():
