...
致命工作流错误——坏的钩子参数、不受支持的模式/选项、上限、启动失败、取消——经组合子传播。只有子失败和普通阶段错误变成按条 null。
"""
import copy,threading#克隆 args 与槽位等待
from concurrent.futures import Future as 原生结果#单次操作结果
from ...内核.会话 import 会话标识#会话标识铸造
from ...内核.工具 import 断言对象json模式,json模式错误#智能体 schema 子集
...
class 执行观察者:#执行观察者协议
    ...
    def 智能体开始(自身,信息):#智能体开始
        """智能体开始。信息是 dict。"""
        raise NotImplementedError('ExecutionObserver.agentStart')#由会话实现

    def 智能体结束(自身,信息):#智能体结束
        """智能体结束。信息是 dict。"""
        raise NotImplementedError('ExecutionObserver.agentEnd')#由会话实现

def 输出文本(块列表):#抽取文本块并拼接
    """把子运行最终输出块压成文本（无 schema 时的 agent() 结果）。块列表是 list[dict]。"""
    文本列表=[]#收集文本
    for 块 in 块列表:#遍历内容块
        if 块['type']=='text':#只留文本块
            文本=块['text'] if 'text' in 块 else ''#取出文本
            文本列表.append(文本 if isinstance(文本,str) else str(文本))#收入
    return ''.join(文本列表)#拼成一段

def 默认标签(提示词):#从提示词取默认标签
    """脚本未传标签时，从提示词推导的短展示标签。按码点计宽度，不是字节预算。"""
    换行=提示词.find('\n')#第一行换行位置
    行=提示词 if 换行==-1 else 提示词[0:换行]#取第一行
    return 行 if len(行)<=48 else 行[0:47]+'…'#超长则截断；判 length
...
class 工作流执行:#worker 侧一次脚本执行
    """worker 内一次活着的脚本执行。由会话按次构造；驱动() 恰好调用一次且永不拒绝——每次失败都变成带非 completed 停止原因的工作流结果。宿主拥有取消，以及任何被丢掉的子工作的清理。"""
    def __init__(自身,元数据,正文,参数,上限,观察者,子端口):#按次构造一次执行
        """按次构造一次执行。元数据与上限是 dict。"""
        自身.上限=上限#本次运行上限
        自身.观察者=观察者#进度观察者
        自身.子端口=子端口#子 RPC 端口
        自身.已启动=0#已启动智能体计数
        ...
        自身.取消错误=None#对应的 CANCELLED 错误
        自身.当前阶段=None#当前阶段标题
        自身.槽锁=threading.Lock()#并发槽锁
        自身.元数据名=元数据['name']#工作流名
        try:#编译包装后的正文
            自身.已编译=编译脚本正文(正文,自身.元数据名)#按 worker 包装编译
        ...
        自身.参数值=copy.deepcopy(参数) if 参数 is not None else None#克隆 args，以免脚本改写初始化数据
        自身.脚本全局={#注入脚本全局
            'agent':自身.智能体,#agent 钩子
            'parallel':自身.并行,#parallel 钩子
            'pipeline':自身.管道入口,#pipeline 钩子
            'phase':自身.阶段,#phase 钩子
            'log':自身.日志,#log 钩子
            'args':自身.参数值,#脚本输入
        }#结束全局表

    def 管道入口(自身,条目,*阶段列表):#pipeline 变参入口
        """pipeline(items, ...stages) 钩子入口。"""
        return 自身.管道(条目,list(阶段列表))#同步跑完

    def 是否已取消(自身):#查询是否已取消
        """运行是否已被取消。这是方法，不是内联属性读取：取消() 会并发改写 取消原因。"""
        return 自身.取消原因 is not None#有取消原因即为已取消

    def 若已取消则抛(自身):#已取消则抛 CANCELLED
        """共享的钩子入口守卫：取消之后，每个钩子在下一次调用时抛 CANCELLED。"""
        if 自身.是否已取消():#已取消
            raise 自身.取取消错误()#抛规范错误

    def 取消(自身,原因):#取消本次执行
        """取消本次运行：正在等的 agent() 槽拒绝，此后每次钩子调用都抛 CANCELLED。幂等；第一条原因获胜。"""
        if 自身.取消原因 is not None:#已取消则忽略
            return#幂等
        自身.取消原因=原因#先到的原因获胜
        自身.取消错误=工作流错误('workflow run cancelled: '+自身.取消原因,'CANCELLED')#构造规范取消错误
        with 自身.槽锁:#拒绝所有排队等槽的人
            等待列表=list(自身.等槽者)#快照
            自身.等槽者.clear()#清空
        for 等待 in 等待列表:#逐个拒绝
            等待.拒绝(自身.取取消错误())#拒绝
    ...
    def 驱动(自身):#驱动脚本直到结算
        ...
        try:#执行正文并物化返回值
            if 自身.是否已取消():#启动前已取消则不跑
                raise 自身.取取消错误()#抛取消
            原始=执行已编译脚本(自身.已编译,自身.脚本全局)#跑脚本
            if 自身.是否已取消():#结算后发现已取消
                raise 自身.取取消错误()#抛取消
            值=None if 原始 is None else 自身.物化结果(原始)#None 当成 null，其余物化
            return {'value':值,'stopReason':'completed','agentsStarted':自身.已启动}#干净完成
        except Exception as 错误:#脚本领域可抛任意类型
            if 自身.是否已取消():#已取消
                取消错=自身.取取消错误()#规范取消错误
                取消消息=str(取消错) if str(取消错) else 'workflow run cancelled'#错误消息
                return {'value':None,'stopReason':'cancelled','error':取消消息,'agentsStarted':自身.已启动}#报取消
            return {'value':None,'stopReason':'error','error':渲染抛出(错误),'agentsStarted':自身.已启动}#报错误

    def 取取消错误(自身):#取出规范取消错误
        """取消() 在任何调用方能观察到已取消之前就武装 取消错误；回退只护住类型。"""
        if 自身.取消错误 is not None:#已武装
            return 自身.取消错误#返回已武装错误
    ...
    def 取槽(自身):#取得一个并发槽
        ...
        等待=None#可能排队
        with 自身.槽锁:#检查空槽
            if 自身.占用槽<自身.上限['maxConcurrentAgents']:#还有空槽
                自身.占用槽+=1#立即占用
                return#无需排队
            等待=操作任务()#排队等释放
            自身.等槽者.append(等待)#登记等待者
        等待.等待()#锁外等待，由放槽兑现
    ...
    def 放槽(自身):#释放一个并发槽
        ...
        下一个=None#下一个等待者
        with 自身.槽锁:#减占用或转交
            if len(自身.等槽者)>0:#有人排队；判 length
                下一个=自身.等槽者.pop(0)#取出下一个等待者
            else:#无人排队
                自身.占用槽-=1#减占用
        if 下一个 is not None:#有人排队则转交槽并唤醒
            下一个.兑现(None)#唤醒等待者

    def 智能体(自身,原始提示,原始选项=None):#跑一个子智能体直到完成
        """agent(prompt, opts) 钩子。同步跑完一个子智能体。"""
        自身.若已取消则抛()#入口取消守卫
        if not isinstance(原始提示,str) or len(原始提示)==0:#提示不是非空字符串；判 length
            raise 工作流错误('agent() requires a non-empty prompt string','INVALID_ARGUMENT')#参数无效
        选项=自身.读智能体选项(原始选项)#读取并校验选项袋
        if 自身.已启动>=自身.上限['maxTotalAgents']:#已达总数上限
            raise 工作流错误(#失控循环挡板
                'this run reached its total agent cap ('+str(自身.上限['maxTotalAgents'])+') — a runaway-loop backstop; raise the applicable maxTotalAgents limit if the scale is intentional',#说明已达上限
                'AGENT_CAP',#上限错误码
            )#结束抛错
        自身.已启动+=1#计入一次启动
        序号=自身.已启动#本次成员序号
        标签=选项['label'] if 'label' in 选项 else 默认标签(原始提示)#展示标签
        阶段=选项['phase'] if 'phase' in 选项 else 自身.当前阶段#所属阶段
        自身.取槽()#取得并发槽
        try:#在持有槽期间跑子运行
            自身.若已取消则抛()#取得后取消守卫
            try:#向宿主请求启动子运行
                启动请求={'prompt':原始提示}#经子端口启动
                if 'schema' in 选项:#有模式才传入
                    启动请求['schema']=选项['schema']#写入模式
                if 'provider' in 选项:#有提供方覆盖才传入
                    启动请求['provider']=选项['provider']#写入提供方
                if 'model' in 选项:#有模型覆盖才传入
                    启动请求['model']=选项['model']#写入模型
                运行=自身.子端口.启动子(启动请求)#等待已发布句柄
            except Exception as 错误:#启动子可抛工作流错误或宿主渲染的失败
            ...
            if 自身.是否已取消():#启动后发现已取消
                运行.销毁()#立即销毁子运行
                raise 自身.取取消错误()#报取消
            信息={'seq':序号,'label':标签,'childId':会话标识(运行.标识)}#组装开始信息
            if 阶段 is not None:#有阶段
                信息['phase']=阶段#写入阶段
            自身.观察者.智能体开始(信息)#报告智能体开始
            try:#等待子运行结算并配对结束
                try:#等待子结果
                    子结果=运行.结果.等待()#等待子运行兑现
                except Exception as 错误:#子结果拒绝（基础设施故障）
                    if 自身.是否已取消():#拒绝时已取消
                        自身.观察者.智能体结束({**信息,'outcome':'cancelled'})#配对为取消
                        raise 自身.取取消错误()#报取消
                    自身.观察者.智能体结束({**信息,'outcome':'failed'})#配对为失败
                    raise 工作流错误('child agent run failed: '+渲染抛出(错误),'AGENT_RESULT',{'cause':错误})#致命的结果故障
                停止原因=子结果['stopReason']#停止原因
                if 停止原因=='completed':#子运行干净完成
                    if 'schema' in 选项:#要了结构化输出
                        结构化=子结果['structured'] if 'structured' in 子结果 else None#结构化值
                        if 结构化 is None:#缺少结构化值
                            自身.观察者.智能体结束({**信息,'outcome':'failed'})#配对为失败
                            return None#按条 null
                        自身.观察者.智能体结束({**信息,'outcome':'completed'})#配对为完成
                        return 结构化#返回结构化对象
                    输出=子结果['output'] if 'output' in 子结果 else []#输出块
                    自身.观察者.智能体结束({**信息,'outcome':'completed'})#配对为完成
                    return 输出文本(输出)#返回拼接文本
                if 自身.是否已取消():#运行已取消
                    自身.观察者.智能体结束({**信息,'outcome':'cancelled'})#配对为取消
                    raise 自身.取取消错误()#杀死脚本
                自身.观察者.智能体结束({**信息,'outcome':'failed'})#配对为失败
    ...
    def 并行(自身,原始形参):#并发跑零参函数
        """parallel(thunks) 钩子：每个 thunk 被捕获则变成 null；致命错误传播。本端口按顺序等待以保确定性。"""
        自身.若已取消则抛()#入口取消守卫
        ...
        for 形参 in 形参列表:#顺序等待
            try:#跑一个 thunk
                输出.append(形参())#取其返回值
            except Exception as 错误:#thunk 可抛任意类型
                if 是否致命工作流错误(错误):#致命错误杀死脚本
                    raise#再抛
                输出.append(None)#普通错误变成 null
        return 输出#返回全部

    def 管道(自身,原始条目,原始阶段):#按条跑多阶段
        ...
        if len(原始阶段)==0:#没有阶段；判 length
            raise 工作流错误('pipeline() requires at least one stage function','INVALID_ARGUMENT')#至少要一个阶段
        阶段列表=[]#校成阶段函数列表
        for 索引,阶段函数 in enumerate(原始阶段):#校验
            if not callable(阶段函数):#该项不是函数
        ...
        for 索引,条目 in enumerate(原始条目):#每条独立跑完全部分阶段
            值=条目#当前阶段输入，起步为条目本身
            try:#按顺序跑阶段
                for 阶段函数 in 阶段列表:#逐个阶段
                    值=阶段函数(值,条目,索引)#本阶段输出成为下一阶段输入
                输出.append(值)#该条的最终值
            except Exception as 错误:#某阶段可抛任意类型
                if 是否致命工作流错误(错误):#致命错误杀死脚本
                    raise#再抛
    ...
    def 断言条目上限(自身,长度,钩子):#检查组合子条目上限
        """检查组合子条目上限。"""
        if 长度>自身.上限['maxItemsPerCall']:#超过每调用上限
            raise 工作流错误(#条目过多
                钩子+' received '+str(长度)+' items — over the per-call cap ('+str(自身.上限['maxItemsPerCall'])+'); split the work or raise maxItemsPerCall in the engine config',#说明超限
                'ITEM_CAP',#条目上限错误码
            )#结束抛错

    def 阶段(自身,标题):#开始一个进度阶段
        """phase(title) 钩子：为随后的 agent() 调用设置当前标签并通知观察者。"""
        自身.若已取消则抛()#入口取消守卫
        if not isinstance(标题,str) or len(标题)==0:#标题不是非空字符串；判 length
...
def 缩进(正文):#给正文每行加一层缩进
    """给正文每行加一层缩进。"""
    行列表=正文.splitlines() or ['']#至少一行
    return '\n'.join('    '+行 for 行 in 行列表)+('\n' if 正文.endswith('\n') else '')#缩进

def 执行已编译脚本(已编译,上下文):#在领域上下文中执行已编译脚本
    """在领域上下文中执行已编译脚本。真正强制终止由宿主宽限负责。"""
    本地=dict(上下文)#可写领域
    本地['__name__']='workflow'#模块名
    exec(已编译,本地,本地)#执行包装
    return 本地['工作流返回值'] if '工作流返回值' in 本地 else None#取出返回值
