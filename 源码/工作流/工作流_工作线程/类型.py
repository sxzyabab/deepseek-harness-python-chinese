...
class 工作线程上限(TypedDict):#worker 侧上限
...
class 工作线程初始化(TypedDict):#worker 初始化载荷
...
class 子句柄(Protocol):#worker 侧子句柄
    ...
    @property#只读属性
    def 标识(自身):#子智能体 id（由宿主侧子智能体缝铸造）
        """子智能体 id。"""
        ...#协议桩

    @property#只读属性
    def 结果(自身):#兑现为子的终态子结果；仅在宿主报告基础设施故障时拒绝
        """子结果任务。"""
        ...#协议桩

    def 销毁(自身):#请宿主销毁该子；在宿主确认时返回
        """销毁子运行。"""
        ...#协议桩

class 子端口(Protocol):#worker 侧子端口
    """运行时用来启动子智能体的 worker 侧端口——这条缝让执行核心不必知道线程边界。"""
    def 启动子(自身,请求):#在宿主上启动一个子智能体（agent() 钩子的启动半边）
        """提示词与已校验选项；返回已发布的子句柄；同步启动或提供方异步启动失败时拒绝。请求是 dict。"""
        ...#协议桩
