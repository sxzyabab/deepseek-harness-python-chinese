...
class 工作流运行:#持有者所有的存活工作流句柄协议；结果永不拒绝；消费方可取消，且必须调用幂等的 销毁() 以等待脚本与子运行静止
    """持有者所有的存活工作流。提供方对象实现本协议（字段由实例赋值；方法由提供方覆写）。"""
    id=None#运行标识（工作流运行标识品牌；载荷键字面量）
    meta=None#脚本正文运行前即可取得的已校验 meta 块（载荷键字面量）
    结果=None#结算结果任务（兑现值为工作流结果）
