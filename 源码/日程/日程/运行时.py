...
def 一次性排序键(项):
    """到期一次性按目标时刻再创建序。"""
    return (解析纪元毫秒(项['record']['scheduledAt']),项['index'])#键
...
def 到期决定(折叠,现在):
    ...
    for 项 in 带序:#筛选
        ...
        if 记录['kind']!='every' and 解析纪元毫秒(记录['scheduledAt'])<=现在:#非 every 且已到期
            一次性列表.append(项)#收入
    一次性列表.sort(key=一次性排序键)#排序
    if len(一次性列表)>0:#有到期一次性
        return {'kind':'one-shot','record':一次性列表[0]['record']}#优先一次性
    固定列表=[]#固定频率候选
    for 项 in 带序:#筛选
        记录=项['record']#记录
        if 记录['kind']=='every' and 解析纪元毫秒(记录['scheduledAt'])<=现在:#every 且已到期
            固定列表.append(项)#收入
    固定列表.sort(key=固定排序键)#按目标再创建序
    if len(固定列表)>0:#有到期固定频率
        提醒列表=[]#一批最新出现
        for 项 in 固定列表:#逐条
            记录=项['record']#活动记录
            提醒列表.append({'record':记录,'occurrenceAt':解析固定频率出现(记录,现在)['occurrenceAt']})#最新到期出现
        return {'kind':'every','acceptedAt':纪元转规范UTC(现在),'reminders':提醒列表}#批次
    目标=None#最早未来目标
...
class 日程运行时:#日程运行时
        ...
        def 循环体():#新循环
            ...
            try:#在无发起方作用域启动
                def 执行无发起方():
                    """执行合并请求。"""
                    自身.执行合并请求()#串行排空
                自身.上下文.agents.withoutInitiator(执行无发起方)#无发起方
            except Exception as 错误:#循环失败
                if 自身.仍权威():#仍是权威所有者
                    自身.上下文.日志.警告('schedule: runtime failed for agent "'+str(自身.智能体.id)+'": '+渲染抛出(错误))#记警告
                自身.已故障=True#永久故障
            finally:#退休该循环
                自身.退休(线程)#退休
        try:#启动线程
            ...
            if 自身.仍权威():#仍是权威所有者
                自身.上下文.日志.警告('schedule: could not start runtime for agent "'+str(自身.智能体.id)+'": '+渲染抛出(错误))#记警告
            自身.运行=None#清空
    ...
    def 执行合并请求(自身):#串行排空合并触发
        """串行排空合并触发。"""
        while 自身.已请求 and (not 自身.停止中) and (not 自身.已故障):#还有请求且可执行
            自身.已请求=False#吃掉当前请求
            执行日程事务(自身.智能体,自身.驱动一轮)#独占执行一轮
    ...
    def 仍权威(自身):#此精确根生命周期是否仍权威
        """此精确根生命周期是否仍权威。"""
        标识=自身.智能体.id#智能体 id
        return 自身.上下文.agents.获取(标识) is 自身.智能体 and 自身.智能体 in 自身.上下文.agents.roots()#仍指向且仍是根

    def 可运行(自身):#此运行时是否可以开始或继续日程工作
        """此运行时是否可以开始或继续日程工作。"""
            ...
            try:#空闲或拆除
                while not 自身.停止事件.is_set():#未拆除
                    try:#尝试空闲
                        ...
                        if 自身.仍权威():#仍权威
                            自身.上下文.日志.警告('schedule: idle wait failed for agent "'+str(自身.智能体.id)+'": '+渲染抛出(错误))#记警告
                        break#停等
            finally:#清空槽
                自身.空闲等待=None#清空
                自身.请求驱动()#再驱
    ...
    def 读折叠(自身):#折叠当前精确运行时后缀，并把损坏的持久流关在里面
        ...
        try:#折叠当前日志
            ...
            自身.上下文.日志.警告('schedule: corrupt schedule log for agent "'+str(自身.智能体.id)+'": '+细节)#记警告
            return None#无法继续
    ...
    def 安全决定(自身,折叠,现在):#把非法墙钟决定关在里面，不永久故障此运行时
        ...
        try:#派生决定
            ...
            自身.上下文.日志.警告('schedule: fixed-rate decision failed for agent "'+str(自身.智能体.id)+'": '+渲染抛出(错误))#记警告
            return None#本轮放弃

    def 驱动一轮(自身):#预检、折叠、武装，或派发下一条一次性或固定频率批次
        """预检、折叠、武装，或派发下一条一次性或固定频率批次。"""
        自身.清计时器()#先清计时器
        if not 自身.可运行():#不可运行则停
            return#停
        try:#预检持久屏障
            冲洗日程持久(自身.上下文,自身.智能体.session)#flush 当前前缀
        except Exception as 错误:#预检失败
            if 自身.仍权威():#仍权威
                自身.上下文.日志.警告('schedule: preflight failed for agent "'+str(自身.智能体.id)+'": '+渲染抛出(错误))#记警告
            return#本轮放弃
        if not 自身.可运行():#flush 期间可能拆除
            return#停
        折叠=自身.读折叠()#折叠当前流
        if 折叠 is None:#损坏则停
            return#停
        ...
        def 维护体():#维护体内派发
            ...
            try:#组装并投递提醒
                ...
                自身.智能体.followup(消息)#投递 followup
            except Exception as 错误:#成帧或投递失败
                if 自身.仍权威():#仍权威
                    自身.上下文.日志.警告('schedule: framing or followup failed for agent "'+str(自身.智能体.id)+'": '+渲染抛出(错误))#记警告
                return False#未派发
            try:#追加派发变更
                会话=自身.智能体.session#会话
                if 决定['kind']=='one-shot':#一次性
                    会话.追加('schedule/change',{'version':1,'operation':'dispatch','id':决定['record']['id']})#追加派发
                else:#固定频率批次
                    for 提醒 in 决定['reminders']:#逐条追加
                        会话.追加('schedule/change',{'version':1,'operation':'dispatch','id':提醒['record']['id'],'acceptedAt':决定['acceptedAt']})#追加派发
            except Exception as 错误:#追加失败
                自身.已故障=True#永久故障
                自身.清计时器()#清计时器
                自身.上下文.日志.警告('schedule: dispatch append failed for agent "'+str(自身.智能体.id)+'": '+渲染抛出(错误))#记警告
                return False#未成功
            return True#已派发
        try:#在空闲相位认领维护
            维护=自身.智能体.runMaintenance(维护体)#维护回合
        ...
        if not 维护:#未派发则停
            return#停
        try:#派发后耐久屏障
            冲洗日程持久(自身.上下文,自身.智能体.session)#flush 派发事件
        except Exception as 错误:#屏障失败
            if 自身.仍权威():#仍权威
                自身.上下文.日志.警告('schedule: dispatch barrier failed for agent "'+str(自身.智能体.id)+'": '+渲染抛出(错误))#记警告
            return#本轮结束
        if 自身.可运行():#继续下一轮
            自身.请求驱动()#再驱
