...
def 行走(根,节点,值,路径,密钥列表):
    ...
    引用=节点['$ref'] if isinstance(节点,dict) and '$ref' in 节点 else None#本地引用
    if isinstance(引用,str) and 引用.startswith('#/'):#本地 $ref
        当前=根#从根走
        for 片段 in 引用[2:].split('/'):#逐段
    ...
    if '角色' in 节点 and 节点['角色']=='secret':
        密钥列表.append({'path':list(路径),'set':值 is not 缺席})#记录位置与是否有值
        return 缺席#从结果中移除
    属性=节点['properties'] if 'properties' in 节点 else None#对象字段
    if isinstance(属性,dict):
        ...
        for 键,孩子 in 属性.items():
            if 源 is 缺席 or 键 not in 源:
                ...
                子值=源[键]#含 JSON null
            剥掉=行走(根,孩子,子值,list(路径)+[键],密钥列表)#递归擦除
            if 剥掉 is not 缺席:
                重建[键]=剥掉#写入
        if 源 is 缺席 and len(重建)==0:
            return 值#原值
        return 重建#重建对象
    额外=节点['additionalProperties'] if 'additionalProperties' in 节点 else None#字典值模式
    if isinstance(额外,dict) and 是否记录(值):
        重建={}#重建字典
        for 键,条目 in 值.items():
            剥掉=行走(根,额外,条目,list(路径)+[键],密钥列表)#按值模式擦除
            if 剥掉 is not 缺席:
                重建[键]=剥掉#写入
        return 重建#重建字典
    项=节点['items'] if 'items' in 节点 else None#数组元素
    if isinstance(项,dict) and isinstance(值,list):
        结果=[]#重建数组
        下标=0#元素下标
        for 条目 in 值:
            结果.append(行走(根,项,条目,list(路径)+[str(下标)],密钥列表))#按下标走
            下标+=1#前进
        return 结果#重建数组
    return 值#其余类型原样返回
