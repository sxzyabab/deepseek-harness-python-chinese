...
def 压平纯文本(内容):#全文本内容压成一个UTF-8字符串
    ...
    文本=''#累积文本
    for 块 in 内容:#逐块
        if 块['type']!='text':#有非文本则放弃
            return None#放弃
        文本+=(块['text'] if 'text' in 块 else '') or ''#拼接文本
    return 文本#纯文本
...
def 预览(文本,预算):#为text构造有界头/尾预览
    ...
    头字节=(预算+1)//2#头预算，对齐 Math.ceil
    尾字节=预算//2#尾预算，对齐 Math.floor
    保留器=文本保留器({'kind':'headTail','headBytes':头字节,'tailBytes':尾字节})#头尾保留器
    保留器.推入(文本)#推入全文
    留下=保留器.收尾()#完成保留
    return {'text':留下['text'],'omitted':留下['omittedBytes']}#预览与省略

def 溢出通知(省略,引用):#给定省略与已存引用的溢出通知行
    """给定省略与已存引用的溢出通知行（无预览、无前导空行）。"""
    省略句=描述省略(省略,'bytes')#省略描述
    return '('+省略句+' Full formatted result stored at: '+str(引用['locator'])+'. '+str(引用['retrievalHint'])+')'#通知正文

def 应用(上下文,配置):#安装溢出策略
    """安装溢出策略：省略上限则空操作；否则校验后挂前置 post-execute 与 code-dispatch-log 臂。"""
    if 'maxInlineBytes' not in 配置:#省略⇒无自动溢出策略
        return#什么也不注册
    内联上限=配置['maxInlineBytes']#取出上限
    if isinstance(内联上限,bool):#布尔不是数字
        raise 溢出策略错误('spill-policy: maxInlineBytes must be a non-negative integer (got '+str(内联上限)+')')#加载失败
    if isinstance(内联上限,int):#整型
        合法=内联上限>=0#非负
    elif isinstance(内联上限,float) and 内联上限.is_integer():#整值浮点
        合法=内联上限>=0#非负
    else:#其余非法
        合法=False#非法
    if not 合法:#上限非法
    ...
    def 溢出替换(文本,总字节,会话标识,工具名,调用标识,标签):#构造溢出替换
        ...
        if 会话标识 is None:#没有会话所有者
            上下文.日志.警告('spill-policy: no session owner for '+工具名+' '+标签+'; keeping the inline content')#记警告
            return None#保留内联
        溢出存储=上下文.获取服务('spillStore',False)#取溢出后端
        if 溢出存储 is None:#未加载后端
            上下文.日志.警告('spill-policy: no ctx.spillStore backend loaded; keeping the inline content')#记警告
            return None#保留内联
        保存={#保存请求
        ...
        try:#持久全文
            引用=溢出存储.保存文本(保存)#保存
        except (OSError,UnicodeEncodeError) as 错误:#保存失败
            #尽力而为：存储失败（权限、ENOSPC、后端宕）绝不能让调用失败或藏起内容——保留原内联。
            上下文.日志.警告('spill-policy: saveText failed for '+工具名+': '+str(错误)+'; keeping the inline content')#记警告
            return None#保留内联
        ...
        通知=溢出通知(预览结果['omitted'],引用)#真实通知
        预览文本=预览结果['text'] or ''#预览正文
        ...
        if 字节长(替换文本)>上限:#替换仍超上限
            上下文.日志.警告('spill-policy: spill notice for '+工具名+' exceeds maxInlineBytes; keeping the inline content')#记警告
            return None#保留内联
        return 替换文本#上限内替换
    ...
    def 面向模型臂(执行,结果,下一步,*剩余):#面向模型的post-execute臂
        ...
        if 决策['kind']!='accept' or 'value' in 决策 or 执行.parent is not None or 执行.name=='read':#非纯内容接受、嵌套或read则透传
            return 决策#透传
        内容=决策['content'] if 'content' in 决策 else None#决策内容
        ...
        if 总字节<=上限:#未超上限
            return 决策#原样
        替换文本=溢出替换(文本,总字节,所有者会话标识(执行),执行.name,执行.callId,'result')#尝试溢出
        if 替换文本 is None:#放弃则原样
            return 决策#原样
        接受={'kind':'accept','content':[{'type':'text','text':替换文本}]}#接受替换
        if 'additionalContexts' in 决策:#保留下游附加上下文
    ...
    def 派发日志臂(派发,下一步,*剩余):#持久dispatch-log臂
        ...
        if 总字节<=上限:#未超上限
            return 内容#原样
        替换文本=溢出替换(文本,总字节,所有者会话标识(派发['exec']),派发['name'],派发['subCallId'],'dispatch')#尝试溢出日志副本
        if 替换文本 is None:#放弃则原样
            return 内容#原样
