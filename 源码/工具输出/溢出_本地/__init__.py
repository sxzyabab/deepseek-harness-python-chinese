...
class 本地溢出存储(溢出存储):#本地文件系统溢出后端
    ...
    def __init__(自身,上下文,配置):#构造本地存储
        """已解析的绝对溢出根（配置 root，否则私有默认），构造时固定。"""
        super().__init__(上下文)#注册spillStore
        if 'root' in 配置:#有配置根
            自身.根=os.path.abspath(配置['root'])#解析为绝对路径
        else:#省略则私有默认
            自身.根=私有根()#惰性私有根
    ...
    def 保存文本(自身,输入):#保存文本
        ...
        已存=保存文本文件({#写入会话作用域文件
            'root':自身.根,#溢出根
            'sessionId':所有者['sessionId'],#会话id
            'suggestedName':输入['suggestedName'],#建议名
            'content':输入['content'],#全文
        })#保存结束
        return {#溢出引用
