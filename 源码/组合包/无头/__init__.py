...
def 汇总(事件列表,起始序号):
    ...
    for 事件 in 事件列表:#按事件扫描
        ...
        if 种类=='assistant/message':#助手消息
            数据=事件['data']#数据
            消息=数据['message']#消息
            内容=消息['content'] if 'content' in 消息 else []#内容块；?? 缺席才空
            拼=''#拼接
            for 块 in 内容:#逐块
                if 块['type']=='text':#文本块
...
def 跑(上下文,任务,出入):#跑一次性任务
    ...
    句柄=智能体服务.创建({#创建一次性智能体
        ...
        'agentOptions':{'provider':选择['provider'],'model':选择['model']},#默认模型
        'setup':安装,#setup
    })#create
    智能体=句柄.智能体#循环驱动
    智能体.等到空闲()#等到空闲
    ...
    结局=汇总(智能体.session.events,起始)#汇总
    出入['stdout'].write(结局['text']+'\n')#打印最终文本
    原因=结局['reason']#结束原因
    if 原因['kind']=='error':#错误结束
        错误体=原因['error']#错误
        出入['stderr'].write('dsh: '+str(错误体['code'])+': '+str(错误体['message'])+'\n')#打印
    出入['exit'](0 if 原因['kind']=='completed' else 1)#完成则 0 否则 1
