...
class 控制台导出器:
    ...
    def 渲染(自身,消息):
        ...
        if 自身.时间模板:
            ...
            输出+=日志器.着色(自身,8,时长单位.格式化时间(自身.时间模板))#灰色时间
        ...
        if 自身.显示间隔:
            输出+=日志器.着色(自身,色号,' +'+时长单位.格式化(消息.时刻-自身.上次时刻))#与上一条的间隔
        自身.上次时刻=消息.时刻#记住本条时刻
        return 输出#整行

控制台导出器.name='logger-console'#Cordis name 槽
控制台导出器.Config={
