...
class 服务实现:
    ...
    def __init__(自身,名称,值,纤程,检查):
        """保存服务名、实现值、提供方纤程与可用性判断。"""
        自身.名称=名称#服务名
        自身.值=值#实现值
        自身.纤程=纤程#提供方纤程
        自身.检查=检查#提供方自报可用性的判断
...
class 反射服务:
    ...
    def 获取服务(自身,服务名,严格=True):
        ...
        if 实现 is None:
            return None#还没有提供方
        return 套上调用方上下文(自身.所属上下文,实现.值)#套上本上下文

    def 获取服务实现(自身,服务名,严格=True):
        """按当前隔离标签取该服务的实现记录。"""
        标签=获取内部数据(自身.所属上下文,'属性链')['隔离'].get(服务名)#隔离标签
    ...
    def 设置服务(自身,服务名,值,错误=None):
        ...
        实现=自身.存储.get(获取内部数据(自身.所属上下文,'属性链')['隔离'].get(服务名))#按标签取
        if 实现 is None:
            raise 错误 or AttributeError(f'没有 提供服务() 过就不能写入服务 "{服务名}"')#未提供
        if 实现.纤程 is not 自身.所属上下文.纤程:
    ...
    def 提供服务(自身,服务名,值=None,检查=None):
        ...
        def 登记服务():
            ...
            根隔离=获取内部数据(自身.所属上下文.根,'属性链')['隔离']#根隔离表
            if 服务名 not in 根隔离:
                根隔离[服务名]=object()#该服务的默认隔离标签
            标签=获取内部数据(自身.所属上下文,'属性链')['隔离'].get(服务名)#当前隔离标签
            已有实现=自身.存储.get(标签)#同标签上的实现
            if 已有实现 is not None:
                raise TypeError(f'服务 "{服务名}" 已经由 <{已有实现.纤程.名称}> 提供')#重复提供
            实现=服务实现(服务名,值,自身.所属上下文.纤程,检查)#实现记录
            自身.存储[标签]=实现#写入存储
            if 自身.所属上下文.纤程.快照 is not None:
                自身.所属上下文.纤程.快照[服务名]=实现#提供方自己也能读到
            if 自身.所属上下文.纤程.状态==纤程状态.已激活:
                自身.通知([服务名])#立刻唤醒依赖方
            def 注销服务():
                ...
                for 纤程 in 自身.通知([服务名]):
                    try:
                        ...
                        纤程.所属上下文.日志.错误(原因)#依赖方自身的失败不该打断本次注销
                if 自身.所属上下文.纤程.快照 is not None:
                    自身.所属上下文.纤程.快照.pop(服务名,None)#从加载快照摘掉
            return 注销服务#拆除器
        return 自身.所属上下文.纤程.副作用(登记服务,f'上下文.提供服务({服务名!r})')#作为纤程副作用

    def 通知(自身,服务名列表,过滤器=None):
        """重新核对依赖这些服务的纤程，返回被刷新的纤程。"""
        if 过滤器 is None:
            def 过滤器(上下文,服务名):
                """默认只通知隔离标签与本上下文一致的依赖方。"""
                对方=获取内部数据(上下文,'属性链')['隔离'].get(服务名)#对方标签
                本方=获取内部数据(自身.所属上下文,'属性链')['隔离'].get(服务名)#本方标签
                return 对方 is 本方#同一隔离作用域
        被刷新=[]#受影响的纤程
        for 运行时 in list(自身.所属上下文.注册表.全部运行记录()):
            for 纤程 in list(运行时.纤程表):
                命中=False#本纤程是否受影响
                for 服务名 in 服务名列表:
                    if 服务名 not in 纤程.依赖表:
                        continue#不依赖该服务
                    if not 过滤器(纤程.所属上下文,服务名):
                        continue#隔离作用域不匹配
                    命中=True#需要重算
        ...
        for 服务名 in 服务名列表:
            ...
            派发上下文=自身.所属上下文.扩展()#派发接收者
            设置内部数据(派发上下文,'过滤器',派发过滤)#按隔离判定过滤监听
            自身.所属上下文.事件.广播(派发上下文,'internal/service',服务名,实现.值 if 实现 else None)#广播
        return 被刷新#被刷新的纤程
    ...
    def 定义访问器(自身,属性名,读取,写入=None):
        ...
        def 登记访问器():
            ...
            return 移除访问器#拆除器
        return 自身.所属上下文.纤程.副作用(登记访问器,f'上下文.定义访问器({属性名!r})')#作为纤程副作用

    def 混入成员(自身,源服务名,成员表):
        """把源服务的选定成员直接暴露到上下文上。"""
        def 逐个登记():
            """逐个成员登记访问器。"""
            映射项列表=[(键,键) for 键 in 成员表] if isinstance(成员表,list) else list(成员表.items())#成员到挂名
            for 源成员名,挂名 in 映射项列表:
                yield 自身.定义访问器(挂名,_生成混入读钩子(源服务名,源成员名),_生成混入写钩子(源服务名,源成员名))#登记访问器
        return 自身.所属上下文.纤程.副作用(逐个登记,f'上下文.混入成员({源服务名!r})')#作为纤程副作用

    def 套上本上下文(自身,值):
        "把本上下文套到值上"
        return 套上调用方上下文(自身.所属上下文,值)#套壳
