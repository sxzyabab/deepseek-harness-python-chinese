...
class 网页套接字下行:#mux 与 host 两条下行
    ...
    def _升级(自身,请求,套接字,头,种类):#把 HTTP 升级变成一条帧泵
        """协商完成后泵帧。"""
        事件面=自身.网关.events#事件面
        if 事件面 is None:#网关无事件面
            拒绝网页套接字升级(套接字)#无法泵则拒绝
            return#完
        打开=getattr(事件面,种类)#mux 或 host
        if not callable(打开):#无该方法
            拒绝网页套接字升级(套接字)#拒绝
            return#完
        自身.客户集.add(套接字)#登记
        中止器=threading.Event()#套接字结束则取消
        ...
        try:#打开事件流
            帧列表=打开({'rpcId':rpc标识,'payload':{}},中止器)#按信号打开
        except Exception:#打开失败
            拒绝网页套接字升级(套接字)#拒绝
            return#完
        泵={'frames':帧列表,'abort':中止器}#泵状态
        自身.泵集.add(id(泵))#登记
        if hasattr(帧列表,'__iter__'):#可迭代
            try:#泵帧
                for 帧 in 帧列表:#逐帧
                    if 中止器.is_set():#已中止
                        break#停
                    信封={#服务端请求信封
                        ...
                        'rpcId':帧['rpcId'] if 'rpcId' in 帧 else rpc标识,#关联 id
