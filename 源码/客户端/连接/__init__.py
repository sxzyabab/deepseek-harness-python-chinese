...
def 断言图像正文容量(上下文,最大请求正文字节):#正文上限必须装得下合计图像
    ...
    if 附件 is None:#没有附件服务则不检查
        return#跳过
    图像上限=附件.imageLimits.maxMessageImageBytes#合计图像上限
...
def 应用(上下文,配置值=None):#安装连接插件
    ...
    if 配置值 is None:#无配置
        ...
        受信任主机表=配置值['trustedHosts'] if 'trustedHosts' in 配置值 else []#受信任 Host
        最大请求正文字节=配置值['maxRequestBodyBytes'] if 'maxRequestBodyBytes' in 配置值 else 默认最大请求正文字节#正文上限
    for 条目 in 受信任主机表:#逐条校验权威形态
        断言受信任权威(条目)#畸形条目加载失败
    if 上下文.获取服务('apiProxy') is not None:#有网关才核图像容量
    ...
    def 挂下行(网关上下文):#等 apiProxy 出现再挂 WebSocket 下行
        ...
        def 下行效应():#下行生命周期
            ...
            def 拆除():#拆除
                ...
                下行.close()#关掉下行
            return 拆除#拆除器
        网关上下文.副作用(下行效应,'client-connection: WebSocket downlinks')#拆除时关掉下行
