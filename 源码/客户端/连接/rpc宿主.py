...
通道规则=re.compile(r'^/[A-Za-z0-9._~-]+\Z')#独占通道：单段绝对路径
...
def rpcFetch处理(通道,处理函数):#把 RPC handler 适配成 FetchHandler
    ...
    def fetch(请求):#处理一条 POST
        ...
        请求头表=请求['headers'] if 'headers' in 请求 and 请求['headers'] is not None else {}#头
        内容类型=请求头表['content-type'] if 'content-type' in 请求头表 else (请求头表['Content-Type'] if 'Content-Type' in 请求头表 else '')#内容类型
        ...
        体字节=请求['body'] if 'body' in 请求 else None#正文
        try:#正文可能不是 JSON
            if 体字节 is None:#空
                体=None#空
                ...
                体=体字节#原样
        except (json.JSONDecodeError,UnicodeDecodeError):#JSON 语法错误
        ...
        rpc标识=体['rpcId'] if 'rpcId' in 体 else None#关联 id
        方法名=体['method'] if 'method' in 体 else None#方法
        载荷=体['payload'] if 'payload' in 体 else None#载荷
        if not isinstance(rpc标识,str) or not isinstance(方法名,str):#字段不合格
...
class 宿主连接服务(服务):#提供 ctx.connection
    ...
    def createSharedFetchHandler(自身,通道,回退):#组合共享通道
        ...
        def fetch(请求):#按路径选拦截器或回退
            ...
            选项=拦截器['options'] if 'options' in 拦截器 else {}#信任政策
            if 'authority' in 选项 and 选项['authority']==连接权威_回环 and (not 是否受信任接口请求(请求,[])):#拦截器钉回环
