...
def 是压缩检查点(事件):#是否 compact 插件的替换检查点
    ...
    return ('kind' in 来源 and 来源['kind']=='plugin' and 'plugin' in 来源 and 来源['plugin']=='compact')#compact
...
def 消息开始(_上下文,匹配项,读取器):#按来源与收件箱认领分类
    ...
    数据=事件['data'] if 'data' in 事件 else {}#载荷
    来源=数据['source'] if 'source' in 数据 and 数据['source'] is not None else {}#来源
    if 'kind' not in 来源 or 来源['kind']!='user':#非用户 → 上下文
        return {#上下文消息
            'kind':'context',#上下文
            'seq':事件['seq'],#序号
            'time':事件['time'] if 'time' in 事件 else None,#时刻
            'content':数据['content'] if 'content' in 数据 else None,#内容
            'source':来源,#来源
            'provenance':上下文出处(来源),#出处
            'form':上下文形态(来源),#形态
        }#结束
    上一=读取器.previous('inbox-next-step')#收件箱
    已认领=False#默认
    if 上一 is not None:#有
        态=上一['state'] if 'state' in 上一 and 上一['state'] is not None else {}#态
        集=态['claimed'] if 'claimed' in 态 and 态['claimed'] is not None else set()#集
        已认领=str(数据['id']) in 集#是否认领
    if 已认领:#插话
        return {#插话
            'kind':'steering',#插话
            'messageId':数据['id'] if 'id' in 数据 else None,#消息 id
            'seq':事件['seq'],#序号
            'time':事件['time'] if 'time' in 事件 else None,#时刻
            'content':数据['content'] if 'content' in 数据 else None,#内容
            'source':来源,#来源
...
def 消息建视图(上下文):#按分类 kind 组装 Chat 节点
    ...
    if 态 is None:#无
        return None#不渲染
    return 聊天节点(上下文,态['kind'],态['seq'],态)#锚在 seq
