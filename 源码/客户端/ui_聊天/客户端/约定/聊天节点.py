...
def 助手聊天数据(状态,回合,步骤,块列表,时间,用量=None,最终节点=None):#Assistant 行数据
    """流式与已落定态共用。"""
    数据={'status':状态,'turn':回合,'step':步骤,'blocks':块列表,'time':时间}#基
...
def 重试聊天数据(尝试列表,当前):#重试行数据
    """一条持久重试链。"""
    return {'attempts':尝试列表,'current':当前}#载荷

def 回合令牌用量路由(提供方,模型):#用量路由
    """贡献了计费请求尝试的一条提供方/模型路由。"""
    return {'provider':提供方,'model':模型}#路由

def 回合令牌用量(未缓存输入,输出,总量,缓存读=None,缓存写=None,推理=None,路由列表=None):#轮次 token 用量
...
def 是运行中工具(块):
    """根是否尚无最终结果。"""
    return 是已结算工具(块) is False#非落定即运行中
