...
class 模型重试行:#重试披露
    ...
    def 渲染(自身):#结构树
        ...
        return {#重试
            ...
            'delayLabel':翻译('message.retry.delay'),'delayMs':取字段(节点,'delayMs'),#延迟
            'failureLabel':翻译('message.retry.failure'),'failure':取字段(取字段(节点,'failure'),'message'),#失败
...
class 回合错行:#回合失败
    ...
    def 渲染(自身):#结构树
        ...
        return {'type':'turn-error','title':翻译('message.turnError'),'message':取字段(节点,'message'),'code':取字段(节点,'code'),'dot':'error','cssModule':'消息项.module.css'}#错
...
class 待提交泡:#pending submission 回声
    ...
    def 渲染(自身):#结构树
        """回声泡。"""
        提交=取字段(自身.属性,'submission') or {}#提交
        文=取字段(提交,'text') or ''#文本
        内容=[] if 文=='' else [{'type':'text','text':文}]#块
        预览=[]#图
        for 图 in 取字段(提交,'images') or []:#扫
            项={'preview':{'url':取字段(图,'previewUrl')}}#预览
            if 取字段(图,'name') is not None:#名
                项['preview']['name']=取字段(图,'name')#名
            if 取字段(图,'width') is not None:#宽
                项['preview']['width']=取字段(图,'width')#宽
            if 取字段(图,'height') is not None:#高
                项['preview']['height']=取字段(图,'height')#高
            预览.append(项)#收
        return {'type':'pending-submission','text':文,'previewImages':预览,'pending':取字段(提交,'placement')=='steering','echo':True,'time':取字段(提交,'time'),'cssModule':'消息项.module.css'}#泡
