...
class 上下文消息节点视图:#context 键
    ...
    def 渲染(自身):#结构
        ...
        return 自身.行({'content':取字段(数据,'content'),'source':取字段(数据,'source'),'provenance':取字段(数据,'provenance'),'form':取字段(数据,'form'),'t':取字段(属性,'t',lambda 键,_=None:键)})#渲
...
class 系统提示节点视图:#system-prompt 键
    """委托系统提示行。"""
...
class 未知节点视图:#unknown
    ...
    def 渲染(自身):#结构
        ...
        return {'type':'unknown-node','label':翻译('message.unknownSurface',{'type':取字段(数据,'type') or 取字段(节点,'kind')}),'payload':载荷,'cssModule':'消息项.module.css'}#视图
