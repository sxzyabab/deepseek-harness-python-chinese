...
def 派生统计(节点们):#窗口折叠回退
    ...
    for 节点 in (节点们 or []):#遍历
        ...
        读数=助手步骤读数(节点)#读数
        if 读数.get('ttftMs') is not None:#TTFT
            首令牌毫秒+=读数['ttftMs']#加
            首令牌步+=1#步
        if 读数.get('decodeMs') is not None and 读数.get('outputTokens') is not None:#吞吐样本
            解码毫秒+=读数['decodeMs']#加
...
def 缓存命中百分(用量):#缓存命中文本
    """委托格式化；无计费输入则 None。"""
    return 格式化缓存命中百分比(取字段(用量,'cacheReadTokens') or 0,计费输入令牌(用量))#百分
...
class 统计行:#composer.dock 读数带
    ...
    def 渲染(自身):#结构
        ...
        用聊天=取字段(属性,'useChat')#快照钩
        用投影=取字段(属性,'useProjection')#投影
        节点们=用聊天(lambda s:取字段(取字段(s,'legacy'),'nodes') or []) if callable(用聊天) else []#节点
        ...
        if 取字段(统计,'steps',0)>0:#有步
            ...
            if 取字段(统计,'llmMs',0)>0:#LLM
                时长们.append(翻译('stats.llm',{'duration':格式化时长(取字段(统计,'llmMs'),翻译)}))#LLM
            if 取字段(统计,'toolMs',0)>0:#工具
                时长们.append(翻译('stats.toolCall',{'duration':格式化时长(取字段(统计,'toolMs'),翻译)}))#工具
            if 时长们:#有
                组们.append(' · '.join(时长们))#时长组
            速们=[]#速
            首步=取字段(统计,'ttftSteps',0)#TTFT 步
            if 首步>0:#平均 TTFT
                速们.append(翻译('stats.ttftAverage',{'duration':格式化时长(取字段(统计,'ttftMs')/首步,翻译)}))#TTFT
        ...
        if 用量 is not None and (计费输入令牌(用量)>0 or (取字段(用量,'outputTokens') or 0)>0):#计费
            ...
            组们.append(翻译('stats.tokens',{'input':格式化令牌(计费输入令牌(用量),翻译),'output':格式化令牌(取字段(用量,'outputTokens') or 0,翻译)}))#令牌
        if len(组们)==0:#空
            return None#不画
        return {'type':'stats-line','groups':组们,'line':' | '.join(组们),'cssModule':'统计行.module.css'}#行
