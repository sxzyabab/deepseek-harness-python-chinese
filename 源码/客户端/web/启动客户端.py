...
对齐上游 `web/src/boot-client.ts`。公开面仅中文名。
在模块系统上挂 Loader、为清单每一行建条目、等待静默并审计激活。
Promise.all 译为线程加 Future 的全部并发；`加载器.等待()` 必须显式调用。
加载器插件类对应上游 `@deepseek-ai/cordis-plugin-loader`，由宿主写入本模块名称后再启动。
"""
...
def 启动客户端(选项):
    ...
    if 加载器插件 is None:#未绑定
        raise 网页错误('web boot: cordis Loader plugin class is not bound')#失败
    上下文=选项['ctx']#根上下文
    清单=选项['manifest']#启动清单
...
def 断言条目已激活(上下文):
    ...
    for 条目 in 上下文.loader.entries():#逐条目
        ...
        if 条目.fiber is None:#导入失败
            失败表.append(名+': import failed (see console for the import error)')#记失败
            continue#下一条
        ...
        if 态=='pending':#等服务
            ...
            for 服务名 in 条目.fiber.inject:#逐服务
                if 上下文.get(服务名) is None:#缺席
                    缺失.append(服务名)#记下
            复数='' if len(缺失)==1 else 's'#单复数
            缺失文=', '.join(缺失) if len(缺失)>0 else 'unknown'#名单
            失败表.append(名+': pending (waiting for service'+复数+': '+缺失文+')')#记 pending
        else:#其它非激活态
            失败表.append(名+': '+态)#记状态
    if len(失败表)>0:#有失败
        单复='y' if len(失败表)==1 else 'ies'#单复数
        raise 网页错误('web boot: '+str(len(失败表))+' entr'+单复+' did not activate\n'+'\n'.join(失败表))#汇总抛错
