"""预览自有状态：已读页，以及读者如何查看它们。

对齐上游 `ui-sidebar-documentpreview/src/client/store.ts`。公开面仅中文名。
`file` 资源只携带元数据，因此文本由本类型拉取并持有——按页、以每页起始的 1 起算行键控。
视图状态必须比正文活得久。按标签 id 分桶。跨线 / 跨包值为 dict。
"""
