"""压缩落点标记行：披露摘要，不替换被遮蔽历史。

对齐上游 `ui-conversation/src/client/chat/CompactionItem.tsx`。公开面仅中文名。
"""
...
class 压缩项:#compaction 流标记
    ...
    def 渲染(自身):#结构树
        ...
        return {#视图
            'type':'compaction-item',#类型
            'title':标题 if 标题 is not None else 翻译('message.compaction'),#题
            'summary':摘要,#摘要
            'expandable':可展,#可展
            'open':打开,#开
            'body':取字段(节点,'summary') if 打开 else None,#体
            'onToggle':自身.切换,#切换
