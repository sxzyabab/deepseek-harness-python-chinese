...
class 悬停卡:#悬停预览
    ...
    def 渲染(自身):
        ...
        return {#视图
            ...
            'content':属性['content'] if 'content' in 属性 else None,#卡内容
            'open':自身.已打开,#开
            'pos':自身.位置,#位
            'copied':自身.已复制,#反馈
            'copyable':可复制,#可复制
            'copyLabel':属性['copyLabel'] if 'copyLabel' in 属性 else '复制',#复制前缀
            'copiedLabel':属性['copiedLabel'] if 'copiedLabel' in 属性 else '复制成功',#成功文案
            'disabled':禁用,#禁
            'openDelayMs':延迟,#延迟
            'onOpen':自身.打开卡,#开
            'onClose':自身.关闭,#关
