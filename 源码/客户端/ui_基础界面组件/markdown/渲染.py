...
def 收集引用目标(节点列表,目标):#深度优先收定义
    ...
    for 节点 in 序列:#逐节点
        ...
        子=节点['children'] if 'children' in 节点 else None#子树
        if 子 is not None:#有子；空表仍递归（JS 空数组为真）
            收集引用目标(子,目标)#递归
...
def 渲染块列表(块列表,上下文):#顶层块→视图列表
    ...
    for 块 in 序列:#逐块
        节点=块['node'] if 'node' in 块 else None#节点
        键=块['key'] if 'key' in 块 else None#稳定 key
        元=渲染节点(节点,键,上下文)#渲染
        if 元 is not None:#非空
            出.append(元)#收下
    return 出#列表
...
def 包块子节点(元素列表,边):#块间插入换行文本
    ...
    包=[]#结果
    for 元 in 序列:#逐元素
        if 边 is True or len(包)>0:#需要前导换行
            包.append('\n')#换行
        包.append(元)#元素
    if 边 is True and len(序列)>0:#尾换行
        包.append('\n')#尾
    return 包#包好
...
def 渲染节点(节点,键,上下文):#单节点→视图
    ...
    if 种=='text':#文本
        return 节点['value'] if 'value' in 节点 else None#字面
    if 种=='paragraph':#段
        return {'type':'element','tag':'p','key':键,'children':渲染子节点(子,上下文)}#段
    if 种=='heading':#标题
        深=节点['depth'] if 'depth' in 节点 and 节点['depth'] is not None else 1#级；缺席 ?? 1
        return {'type':'element','tag':'h'+str(深),'key':键,'children':渲染子节点(子,上下文)}#标题
    if 种=='blockquote':#引用
        包=包块子节点([c for c in 渲染子节点(子,上下文) if c is not None],True)#包
        return {'type':'element','tag':'blockquote','key':键,'children':包}#引用
    if 种=='thematicBreak':#分隔
        return {'type':'element','tag':'hr','key':键}#hr
    if 种=='break':#硬换行
        return {'type':'fragment','key':键,'children':[{'type':'element','tag':'br'},'\n']}#br+换行
    if 种=='strong':#粗
        return {'type':'element','tag':'strong','key':键,'children':渲染子节点(子,上下文)}#strong
    if 种=='emphasis':#斜
        return {'type':'element','tag':'em','key':键,'children':渲染子节点(子,上下文)}#em
    if 种=='delete':#删
        return {'type':'element','tag':'del','key':键,'children':渲染子节点(子,上下文)}#del
    if 种=='inlineCode':#行内码
    ...
    if 种=='math':#块数学
        值=节点['value'] if 'value' in 节点 and 节点['value'] is not None else ''#TeX
        return {'type':'fragment','key':键,'children':渲染TeX到树(值,True)}#展示
    if 种=='inlineMath':#行内数学
        值=节点['value'] if 'value' in 节点 and 节点['value'] is not None else ''#TeX
        return {'type':'fragment','key':键,'children':渲染TeX到树(值,False)}#行内
    if 种=='list':#列表
    ...
    if 种=='link':#链接
        址=节点['url'] if 'url' in 节点 and 节点['url'] is not None else ''#url
...
def 渲染行内码(节点,键,上下文):#行内 code
    ...
    提及=None#文件提及
    if ('inLink' not in 上下文) or (上下文['inLink'] is not True):#不在锚内
        面=上下文['fileMentions'] if 'fileMentions' in 上下文 else None#提及面
        解析=面['resolve'] if 面 is not None and 'resolve' in 面 else None#解析器
        if 解析 is not None:#有；上游 resolve 为函数
            提及=解析(值)#解析
    if 提及 is not None:#命中文件；提及为 dict
        return {'type':'element','tag':'code','key':键,'children':[{#按钮
            'type':'element','tag':'button','props':{#属性
                'type':'button','className':'fileMention',#类
                'title':提及['title'] if 'title' in 提及 else None,#无障碍
                'aria-label':提及['label'] if 'label' in 提及 else None,#aria
                'onClick':提及['open'] if 'open' in 提及 else None,#打开
            },'children':[值],#正文
        }]}#结束
...
def 渲染代码(节点,键,上下文):#围栏代码
    ...
    if 语言 is not None:#有 lang
        命=re.match(r'^[\w-]+',str(语言),re.ASCII)#截断；\w 对齐 JS ASCII
        语=命.group(0) if 命 is not None else None#id
    流式=上下文['streaming'] is True if 'streaming' in 上下文 else False#流式
    if 流式 is False and 语=='math':#定稿 math 围栏
        return {'type':'fragment','key':键,'children':渲染TeX到树(值+'\n',True)}#展示 TeX
    文案=上下文['codeLabels'] if 'codeLabels' in 上下文 and 上下文['codeLabels'] is not None else {}#复制文案
    复制标=文案['copyLabel'] if 'copyLabel' in 文案 else None#复制
    已复制标=文案['copiedLabel'] if 'copiedLabel' in 文案 else None#已复制
    块=代码块({'code':值+'\n','lang':None if 流式 is True else 语,'copyLabel':复制标,'copiedLabel':已复制标})#块
    视=块.渲染()#视图
    视['key']=键#稳定 key
    return 视#代码块
...
def 渲染列表(节点,键,上下文):#ul/ol
    ...
    if isinstance(起,(int,float)) and not isinstance(起,bool) and 起!=1:#非 1
        属性['start']=int(起)#起始
    子=节点['children'] if 'children' in 节点 and 节点['children'] is not None else []#项
    if any(isinstance(项['checked'] if 'checked' in 项 else None,bool) for 项 in 子):#任务
        属性['className']='contains-task-list'#类
    有序=节点['ordered'] is True if 'ordered' in 节点 else False#有序
    标签='ol' if 有序 is True else 'ul'#有序/无序
    项视=[渲染列表项(项,松,下标,上下文) for 下标,项 in enumerate(子)]#项
...
def 渲染块条目(块列表,上下文):#区分段与其它块
    ...
    for 下标,块 in enumerate(序列):#逐块
        if ('type' in 块) and 块['type']=='paragraph':#段
            子=块['children'] if 'children' in 块 else None#子
            条.append({'paragraph':渲染子节点(子,上下文)})#段
        else:#其它
            元=渲染节点(块,下标,上下文)#渲
            if 元 is not None:#非空
...
def 渲染列表项(项,松散,键,上下文):#li
    ...
    任务=isinstance(勾选,bool)#任务
    if 任务 is True:#勾选
        勾={'type':'element','tag':'input','key':'task-checkbox','props':{'type':'checkbox','checked':勾选 is True,'disabled':True}}#框
        if len(条)>0 and 'paragraph' in 条[0]:#首段
            段=条[0]['paragraph']#段子
    ...
    for 下标,入 in enumerate(条):#逐条
        是段='paragraph' in 入#段?
        if 松散 is True or 下标!=0 or 是段 is False:#前导换行
            片.append('\n')#换行
        if 是段 is False:#元素
    ...
    尾=条[-1] if len(条)>0 else None#尾
    if 尾 is not None and (松散 is True or 'paragraph' not in 尾):#尾换行
        片.append('\n')#换行
    类='task-list-item' if 任务 is True else None#类
...
def 渲染表行(行,格标,对齐,键,上下文):#tr
    ...
    for 下标 in range(长):#逐列
        格=格列表[下标] if 下标<len(格列表) else None#格
        齐=对齐[下标] if 对齐 is not None and 下标<len(对齐) else None#对齐
        样式={'textAlign':齐} if 齐 is not None else None#style
        子=渲染子节点(格['children'] if 'children' in 格 else None,上下文) if 格 is not None else []#子
        胞.append({'type':'element','tag':格标,'key':下标,'props':{'style':样式} if 样式 is not None else {},'children':子})#胞
...
def 引用后缀(节点):#缺定义时回退括号文
    ...
    if 形=='collapsed':#空方
        return '][]'#后缀
    if 形=='full':#全
        标=节点['label'] if 'label' in 节点 and 节点['label'] is not None else None#label
        if 标 is None:#回退 identifier
            标=节点['identifier'] if 'identifier' in 节点 and 节点['identifier'] is not None else ''#identifier
        return ']['+str(标)+']'#后缀
    return ']'#短
...
def 渲染链接引用(节点,键,上下文):#linkReference
    ...
    子=节点['children'] if 'children' in 节点 else None#子
    if 定义 is None:#缺
        return {'type':'fragment','key':键,'children':['[']+渲染子节点(子,上下文)+[引用后缀(节点)]}#回退
    址=定义['url'] if 'url' in 定义 and 定义['url'] is not None else ''#url
    return 渲染锚(址,渲染子节点(子,dict(上下文,inLink=True)),键)#锚
...
def 渲染脚注引用(节点,键,上下文):#footnoteReference
    ...
    次=已见 if 已见 is not None else 0#?? 0
    上下文['footnoteCounts'][标识]=次+1#累计
...
def 渲染脚注区(上下文):#文末脚注 section
    ...
    项列表=[]#li 列表
    for 标识 in 序:#按序
        脚注表=上下文['targets']['footnotes']#表
        定义=脚注表[标识] if 标识 in 脚注表 else None#定义
        if 定义 is None:#无
            continue#跳
        次=上下文['footnoteCounts'][标识] if 标识 in 上下文['footnoteCounts'] else 0#引用次数
        回=[]#↩ 标记
        ...
        子=定义['children'] if 'children' in 定义 else None#体
        条=渲染块条目(子,上下文)#体条目
        尾=条[-1] if len(条)>0 else None#尾
        体=[]#body
        for 下标,入 in enumerate(条):#逐条
            if 'paragraph' in 入:#段
                段子=list(入['paragraph'])#拷
                if 入 is 尾:#尾段挂回指；身份 is
                    段子.extend([' ']+回)#空格+回指
                体.append({'type':'element','tag':'p','key':'p-'+str(下标),'children':段子})#p
            else:#元素
                体.append(入['element'])#元素
        if 尾 is None or 'paragraph' not in 尾:#无尾段
            体.extend(回)#回指挂块末
        项列表.append({'type':'element','tag':'li','key':标识,'props':{'id':'user-content-fn-'+归一化URI(标识.lower())},'children':包块子节点(体,True)})#li
    if len(项列表)==0:#无
        return None#空
    return {'type':'element','tag':'section','key':'footnotes','props':{'data-footnotes':True,'className':'footnotes'},'children':[#区
        ...
        {'type':'element','tag':'ol','children':项列表},#有序列表
    ]}#section
