...
def 应用(上下文):#安装浏览选目录浏览器半边
    ...
    def 登记词表():#登记中英文案
        ...
        拆除列表=[]#已成功登记
        try:#按语言登记
            拆除列表.append(上下文.locale.register(词表命名空间,'zh',中文))#中文
            拆除列表.append(上下文.locale.register(词表命名空间,'en',英文))#英文
        except Exception:#locale.register 异常契约未定，第二次失败需回滚
            for 拆 in reversed(拆除列表):#逆序回滚
                拆()#拆除
            raise#继续抛
        def 拆除():#卸载
            """卸掉已登记词表。"""
            for 拆 in 拆除列表:#逐个
                拆()#拆除
        return 拆除#拆除器
    上下文.副作用(登记词表,'directory-picker-browse: dialog dictionaries')#词表生命周期
    def 列举目录(路径=None,信号=None):#列举
        """转调 workspaces.listDirectory。"""
        return 上下文.workspaces.listDirectory(路径,信号)#列举
    def 创建目录(路径,名):#创建
        """转调 workspaces.createDirectory。"""
        return 上下文.workspaces.createDirectory(路径,名)#创建
    def 注入面():#浏览流注入面
        """列举、创建与文案。"""
        return {#注入
            'listDirectory':列举目录,#列举
            'createDirectory':创建目录,#创建
