...
class 检索行:#grep/glob 行
    ...
    def 渲染(自身):#结构树
        ...
        检索=检索卡模型(块)#卡 dict 或 None
        标题=检索标题表[工具名] if 工具名 in 检索标题表 else None#专用标题
        if 标题 is None:#无
            标题=模型['title'] if 'title' in 模型 else None#回退
        摘要=None#摘要
        if 检索 is not None:#有卡
            摘要=检索['title'] if 'title' in 检索 else None#卡标题优先
        if 摘要 is None:#无
            摘要=模型['summary'] if 'summary' in 模型 else None#模型摘要
        载荷={#行 props
