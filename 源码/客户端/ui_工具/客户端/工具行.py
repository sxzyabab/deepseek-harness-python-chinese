...
class 工具行:#单行工具摘要
    ...
    def 可展开(自身):#是否可展开
        ...
        卡=(
            ...
            or ('diff' in 属性 and 属性['diff'] is not None)
            or ('read' in 属性 and 属性['read'] is not None)
            or ('search' in 属性 and 属性['search'] is not None)
            or ('web' in 属性 and 属性['web'] is not None)
        )#任一卡
    ...
    def 建展开体(自身):#展开体结构树
        ...
        状态=属性['state'] if 'state' in 属性 and 属性['state'] else 'ok'#态
        体=属性['body'] if 'body' in 属性 else None#输入
        出=属性['output'] if 'output' in 属性 else None#输出
        终端=属性['terminal'] if 'terminal' in 属性 else None#终端卡
        差异=属性['diff'] if 'diff' in 属性 else None#diff 卡
        读=属性['read'] if 'read' in 属性 else None#读卡
        检索=属性['search'] if 'search' in 属性 else None#检索卡
        网页=属性['web'] if 'web' in 属性 else None#网页卡
        if 终端 is not None:#终端
            return {#终端体
                'kind':'terminal',#种
                'card':终端['card'] if 'card' in 终端 else None,#卡
        ...
        if 差异 is not None:#diff
            return {#diff 体
                'kind':'diff',#种
                'card':差异['card'] if 'card' in 差异 else None,#卡
        ...
        if 检索 is not None:#检索
            return {#检索体
                'kind':'search',#种
                'card':检索['card'] if 'card' in 检索 else None,#卡
                'recovery':检索['recovery'] if 'recovery' in 检索 else None,#恢复定位
    ...
    def 渲染(自身):#结构树
        ...
        失败行=属性['errorSummary'] if 状态=='error' and 'errorSummary' in 属性 else None#失败首行
        摘要=失败行 if 失败行 is not None else (属性['summary'] if 'summary' in 属性 else None)#摘要
        后缀=None if 失败行 is not None else (属性['summarySuffix'] if 'summarySuffix' in 属性 else None)#后缀
        路径=属性['filePath'] if 'filePath' in 属性 else None#路径
        开文件=属性['onOpenFile'] if 'onOpenFile' in 属性 else None#开文件
        ...
        图标=属性['icon'] if 'icon' in 属性 else None#图标
        return {#结构树
            'type':'tool-row',#类型
            'variant':属性['variant'] if 'variant' in 属性 else None,#变体
            'toolName':属性['toolName'] if 'toolName' in 属性 else None,#工具名
            'icon':图标,#图标名/节点（宿主解析）
            'title':属性['title'] if 'title' in 属性 else None,#标题
            'summary':摘要,#摘要
            'summarySuffix':后缀,#后缀
            'state':状态,#态
            'leading':前导态(状态,图标),#前导
