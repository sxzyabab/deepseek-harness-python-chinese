...
def 适配产出文件(可用宽,间隙,芯片宽表,更多宽按展示):#选最大能装下的前缀
    ...
    for 展示,宽 in enumerate(前缀):#候选 shown
        ...
        需要=宽+(0 if 更多 is None else 更多)+max(0,项数-1)*间隙#所需宽
        if 需要<=可用宽:#装得下
            最大适配=展示#记下
    return 最大适配#返回
...
def 读开路径(描述):#宿主描述能否开路径
    """canOpenPath 为 True 才算能开。"""
    return 'canOpenPath' in 描述 and 描述['canOpenPath'] is True#能力
...
class 产出文件行:#回合尾产出行
    ...
    def 视图(自身):#读视图模型
        """投影芯片行。"""
        匹配=自身.属性['matched'] if 'matched' in 自身.属性 else None#匹配路径
        路径表=list(匹配) if 匹配 is not None else []#空列表保留
        翻译=自身.属性['t']#文案
        打开文件=自身.属性['openFile']#打开
        回环=自身.属性['isLoopback'] is True if 'isLoopback' in 自身.属性 else False#回环
        用宿主描述=自身.属性['useHostDescription'] if 'useHostDescription' in 自身.属性 else None#宿主描述钩子
        能开路径=False#默认不能
        if 回环 and 用宿主描述 is not None:#回环且有钩子
            能开路径=用宿主描述(读开路径) is True#读能力
        上限=min(len(路径表),展示上限)#上限；判的是 length
        可见=上限 if 自身.展示数 is None else min(自身.展示数,上限)#可见数
        显示=路径表[:可见]#可见路径
        隐藏=len(路径表)-len(显示)#隐藏数；判的是 length
        芯片=[{#芯片
            'path':路径,#全路径
            'name':路径末段(路径),#末段
