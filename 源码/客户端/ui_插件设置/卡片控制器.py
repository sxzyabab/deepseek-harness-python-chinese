...
def 引用于(快照):
    """有非空 apiKeyEnv 则用，否则默认。快照与 value 都是 dict。"""
    if 'value' not in 快照 or 快照['value'] is None:#无有效层
        return 默认密钥引用#默认
    值=快照['value']#有效层
    if 'apiKeyEnv' not in 值:#未配引用
        return 默认密钥引用#默认
    声明=值['apiKeyEnv']#声明
    if not isinstance(声明,str) or len(声明)==0:#空串当未配
        return 默认密钥引用#默认
    return 声明#引用
...
class 网页搜索卡片控制器:#web-search 卡片
    ...
    def 读凭证(自身):#向凭证域询问当前引用
        ...
        try:#describe
            应答=自身.接口.credentials.describe({'refs':[引用]}).等待()#描述
        except Exception:#读取失败不让卡片不可用；RPC 异常契约未定
            return#保留上次
        结果=应答['result']#业务结果
        if not 结果['ok'] or 引用!=引用于(自身.作用域.getSnapshot()):#失败或引用已变
            return#丢弃
        值=结果['value'] if 'value' in 结果 else None#值
        视图=值['credentials'] if 值 is not None and 'credentials' in 值 and 值['credentials'] is not None else {}#凭证图
        项=视图[引用] if 引用 in 视图 else None#该项
        已配=bool(项['configured']) if 项 is not None and 'configured' in 项 else False#已配置
        可写=bool(项['writable']) if 项 is not None and 'writable' in 项 else True#可写
        下一={'ref':引用,'configured':已配,'writable':可写}#新状态
        if 下一['configured']==自身.凭证['configured'] and 下一['writable']==自身.凭证['writable']:#无变化
            return#不投影
        自身.凭证=下一#采纳
        自身.store.set(自身.投影())#发布

    def 刷新凭证(自身,引用):#Host 报告监视引用有变时重读
        """不是本卡片监视的引用则忽略。"""
        if 引用!=自身.凭证['ref']:#无关
            return#忽略
        自身.读凭证()#重问
    ...
    def 写密钥(自身,值):#写出暂存密钥并重读
        """Host 是密钥是否存在的唯一权威。"""
        try:#set
            自身.接口.credentials.set({'ref':引用于(自身.作用域.getSnapshot()),'value':值}).等待()#写入
        except Exception:#拒绝经重读浮出；RPC 异常契约未定
            pass#重读
        自身.读凭证()#重问
