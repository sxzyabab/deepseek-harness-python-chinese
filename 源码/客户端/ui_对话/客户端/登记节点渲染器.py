...
class 上下文消息节点视图:
    ...
    def 渲染(自身):
        ...
        return 自身.行({#注入
            ...
            'source':数据['source'] if 'source' in 数据 else None,#源
            'provenance':数据['provenance'] if 'provenance' in 数据 else None,#出处
            'form':数据['form'] if 'form' in 数据 else None,#形态
            't':翻译,#文案
        })#渲
...
class 未知节点视图:
    ...
    def 渲染(自身):
        ...
        return {#视图
            ...
            'label':翻译('message.unknownSurface',{'type':面种}),#标
            'payload':载荷,#载荷
