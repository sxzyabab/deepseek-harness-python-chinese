...
def 上下文占用(压力):
    ...
    if 已用 is None:#回退样本
        已用=压力['pressureTokens'] if 'pressureTokens' in 压力 else None#样本
    窗=压力['contextWindow'] if 'contextWindow' in 压力 else None#容量
...
class 统计行:
    ...
    def 渲染(自身):
        ...
        节点列表=用会话(取遗留节点) if 用会话 is not None else []#节点
        ...
        if 步>0:#有步
            ...
            if llm>0:#LLM
                时长列表.append(翻译('stats.llm',{'duration':格式化时长(llm)}))#LLM
            工具=统计['toolMs'] if 'toolMs' in 统计 and 统计['toolMs'] is not None else 0#工具
            if 工具>0:#工具
                时长列表.append(翻译('stats.toolCall',{'duration':格式化时长(工具)}))#工具
            if len(时长列表)>0:#有
                组列表.append(' · '.join(时长列表))#时长组
            速率列表=[]#速
            首步=统计['ttftSteps'] if 'ttftSteps' in 统计 and 统计['ttftSteps'] is not None else 0#TTFT 步
            if 首步>0:#平均 TTFT
                ttft=统计['ttftMs'] if 'ttftMs' in 统计 and 统计['ttftMs'] is not None else 0#TTFT
                速率列表.append(翻译('stats.ttftAverage',{'duration':格式化时长(ttft/首步)}))#TTFT
            解码=统计['decodeMs'] if 'decodeMs' in 统计 and 统计['decodeMs'] is not None else 0#解码
            if 解码>0:#吞吐
                令牌=统计['decodeTokens'] if 'decodeTokens' in 统计 and 统计['decodeTokens'] is not None else 0#令牌
        ...
        if 用量 is not None:#有用量
            ...
            if 计费输入令牌(用量)>0 or 输出>0:#计费
                ...
                组列表.append(翻译('stats.tokens',{'input':格式化令牌(计费输入令牌(用量)),'output':格式化令牌(输出)}))#令牌
        if len(组列表)==0:#空
            return None#不画
