...
def 附件错误文案(翻译,原因,限额=None):
    ...
    if 原因=='TOO_MANY_IMAGES' and 限额 is not None:#张数
        上限=限额['maxImagesPerMessage'] if 'maxImagesPerMessage' in 限额 else None#张数
        return 翻译('image.tooMany',{'count':上限})#上限
    if 原因=='IMAGE_TOO_LARGE' and 限额 is not None:#单张
        上限=限额['maxImageBytes'] if 'maxImageBytes' in 限额 else None#字节
        return 翻译('image.fileTooLarge',{'size':图像尺寸文案(上限)})#体积
    if 原因=='IMAGES_TOO_LARGE' and 限额 is not None:#合计
        上限=限额['maxMessageImageBytes'] if 'maxMessageImageBytes' in 限额 else None#字节
