...
对齐上游 `ui-conversation/src/client/conversation-nodes/turn-tail.ts`。公开面仅中文名。
事件与状态为 dict。
"""
...
def 收口锚(上下文):
    ...
    for 匹配项 in 匹配列表:#扫
        ...
        if 种=='llm/retry':#重试
            ...
            continue#下
        if 种=='step/end' and 先前['streamedText'] is True and 先前['finalized'] is False:#中断收口
...
def 尾载荷(上下文):
    ...
    终态列表=[]#终态列表
    for 候 in 已结:#扫
        终态列表.append(候['finalNode'])#收下
    指标表=派生回合指标(终态列表)#按终态算
