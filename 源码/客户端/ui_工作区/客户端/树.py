...
def 会话标题(会话):#会话行标题
    """空白用规范英文标题，否则用已存展示标题。"""
    return 'New Session' if 会话['blank'] else 会话['displayTitle']#标题

def 近因键(项):#最新在前
    """(-updatedAt, id)。"""
    return (-项['updatedAt'],项['id'])#近因

def 组装分组(键,工作区标识,工作目录,创建于,标签,成员列表,顺序):#组装内部分组
    """组装一个分组，不把会话谱系投影成展示。"""
    会话=list(成员列表)#拷贝成员
    if 顺序=='recency':#近因序
        会话.sort(key=近因键)#最新在前
    return {'key':键,'workspaceId':工作区标识,'cwd':工作目录,'createdAt':创建于,'label':标签,'sessions':会话}#内部分组

def 有序未分组(成员列表,已存):#按已存键序排未分组会话
    """套用已存的未分组顺序，新散落的会话按近因追加。"""
    按标识={会话['id']:会话 for 会话 in 成员列表}#id → 摘要
    已纳入=set()#已纳入
    有序=[]#结果
    for 键 in 已存:#先走已存顺序
        if 键 not in 按标识 or 键 in 已纳入:#未知或已纳入
            continue#跳过
        有序.append(按标识[键])#追加
        已纳入.add(键)#记下
    剩余=sorted(成员列表,key=近因键)#近因
    for 会话 in 剩余:#收新散落
        if 会话['id'] in 已纳入:#已在
            continue#跳过
        有序.append(会话)#追加
...
def 按工作区分组(列表,工作区列表,已归档,未分组序):#按工作区账本分组
    ...
    for 工作区 in 工作区列表:#按宿主顺序
        ...
        分组列表.append(组装分组(工作区['workspaceId'],工作区['workspaceId'],工作区['path'],创建毫秒,工作区['title'],成员,'account'))#账本序
    散落=[]#未被认领且可见
    ...
    if len(散落)>0:#有散落
        ...
        分组列表.append(组装分组(未分组键,None,None,None,未分组标签,成员,'recency' if 未分组序 is None else 'account'))#未分组桶
...
def 会话行(摘要,后代):#摘要 → 展示行
    ...
    if 'pendingInteraction' in 摘要 and 摘要['pendingInteraction'] is not None:#有则带上
        行['pendingInteraction']=摘要['pendingInteraction']#待处理
    return 行#展示行

def 派生分组(列表,工作区列表,已归档会话标识,视图):#派生分组树
...
def 派生检索结果(列表,工作区列表,查询,已归档会话标识,正文,上限):#合并标题/工作区匹配与正文检索
