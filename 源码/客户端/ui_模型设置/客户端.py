...
from .欢迎存储 import 欢迎通知存储,已加载则刷新欢迎#欢迎存储
...
__all__=[#仅中文公开名
    '依赖','应用','模型设置存储','欢迎通知存储','模型分区','欢迎通知','官方引导对话框',
    '提供方编辑器','自定义提供方卡片','DeepSeek模型编辑器','模型列表编辑器','编辑器页脚','引导模态','密钥失败',
    '命名空间','中文','英文','已加载则刷新',
]

依赖=['slots','locale','connection','remote']#依赖

def 应用(上下文):#安装模型设置浏览器半边
    """登记模型分区与引导步骤，接到连接失效。"""
    def 登记词典():#登记中英文案
        """把 settings.models 写进 locale。"""
        return 上下文.locale.register(命名空间,{'zh':中文,'en':英文})#词典
    上下文.副作用(登记词典,'ui-settings-models: copy dictionaries')#词典
    连接=上下文.获取服务('connection')#连接句柄
    接口=连接.api#API
    控制器=模型设置存储(接口)#模型页存储
    翻译=上下文.locale.bind(命名空间)#绑定词表
    欢迎=欢迎通知存储(接口,'host' if 连接.isLoopback else 'memory')#欢迎存储
    def 推送失效():#设置/凭证/拓扑与重连
        ...
        拆除器表=[#拆除器
            ...
            上下文.监听('connection/reset',刷全部),#重连
        ]#拆除器表结束
        def 拆除():#拆除
            """逐个取消。"""
            for 拆除器 in 拆除器表:#每个
                拆除器()#取消
        return 拆除#拆除器
    上下文.副作用(推送失效,'ui-settings-models: pushed invalidations')#推送失效
    def 选模型快照(选):#对当前快照应用选择器
        """存储快照选择。"""
        return 选(控制器.存储.getSnapshot())#选择
    def 模型注入():#模型分区注入面
        """存储 + 快照选择器 + API + 翻译。"""
        return {'controller':控制器,'useSnapshot':选模型快照,'api':接口,'t':翻译}#注入
    def 欢迎注入():#欢迎通知注入面
        """欢迎存储 + 钩子 + 翻译。"""
        return {'controller':欢迎,'hooks':{'welcome':欢迎.存储},'t':翻译}#注入
    def 官方引导注入():#官方 DeepSeek 引导注入面
        """模型存储 + 钩子 + API + 翻译。"""
        return {'controller':控制器,'hooks':{'models':控制器.存储},'api':接口,'t':翻译}#注入
    def 导航标签():#分区导航文案
        """模型设置导航标签。"""
        return 翻译('nav')#标签
    def 登记模型分区():#登记模型设置分区
        """登记 settings.section 条目。"""
        return 上下文.slots.register({#模型分区
            'name':'settings.section','id':'models','order':10,'label':导航标签,'inject':模型注入,
        },模型分区)#组件
    def 登记欢迎():#登记欢迎通知
        """登记 settings.onboarding 欢迎步。"""
        return 上下文.slots.register({#欢迎通知
            'name':'settings.onboarding','id':'welcome-notice','order':-100,'inject':欢迎注入,
        },欢迎通知)#组件
    def 登记官方引导():#登记官方 DeepSeek
        """登记 settings.onboarding 官方步。"""
        return 上下文.slots.register({#官方 DeepSeek
            'name':'settings.onboarding','id':'deepseek-official','order':0,'inject':官方引导注入,
        },官方引导对话框)#组件
    上下文.slots.inject('settings.section',登记模型分区)#模型分区
    上下文.slots.inject('settings.onboarding',登记欢迎)#欢迎
    上下文.slots.inject('settings.onboarding',登记官方引导)#官方
