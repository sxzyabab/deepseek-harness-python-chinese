...
from .约定.种子 import 向导种类,页面地址,造向导标签#种子
from .约定.参数 import 右侧侧栏资源参数,右侧侧栏标签参数,右侧侧栏导航参数#参数占位
from .存储 import 创建表面,创建右侧侧栏存储#存储
...
__all__=[#仅中文公开名
    '注入','应用','命名空间',
    '中文','英文','右侧侧栏文案键','停靠标签',
    '右侧侧栏词表命名空间','席名右侧签正文','席名右侧签标题','席名右侧向导','席名右侧签菜单项','右侧侧栏子槽',
    '向导种类','页面地址','造向导标签',
...
def 应用(上下文):
    ...
    def 挂席与向导():
        """一存储两席，加出厂向导。"""
        规格=创建右侧侧栏存储(lambda:翻译('tab.guide.title'))#规格
        认领表=[]#释放器
        ...
        拆席=上下文.slots.inject('rightbar',挂右栏)#等洞
