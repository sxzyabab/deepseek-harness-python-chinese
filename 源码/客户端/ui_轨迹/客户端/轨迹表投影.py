...
def 首字耗时文案(指标):#TTFT
    ...
    if ('stepStartTime' not in 指标) or 指标['stepStartTime'] is None:#无起点
        return 'Step start unavailable'#无起点
    if ('firstTokenTime' not in 指标) or 指标['firstTokenTime'] is None:#无首字
...
def 吞吐文案(指标):#吞吐
    ...
    if ('completedTime' not in 指标) or 指标['completedTime'] is None:#未完
        return 'Pending'#进行中
    生成秒=(指标['completedTime']-指标['firstTokenTime'])/1000#秒
    if 生成秒<=0:#过短
...
def 展平记录(轮次列表):#轮次→表记录
    ...
    for 段下标,轮 in enumerate(表):#各轮
        ...
        for 组 in (轮['groups'] if 'groups' in 轮 and 轮['groups'] is not None else []):#各组
            格列表=组['cells'] if 'cells' in 组 and 组['cells'] is not None else []#格
            for 格下标,格 in enumerate(格列表):#各格
                轮起始=段内首 and (格['requestOnly'] if 'requestOnly' in 格 else None) is not True and 格['kind']!='system' and (格['kind']!='compacted' or ('turn' not in 轮 or 轮['turn'] is None))#轮起始
                if 轮起始:#已标
                    段内首=False#消费
                本轮.append({#表记录
                    'turn':轮['turn'] if 'turn' in 轮 else None,#轮号
                    'section':段下标,#段
                    'group':组['title'] if 'title' in 组 else None,#组标题
                    'groupStart':格下标==0,#组起
                    'turnStart':轮起始,#轮起
                    'cell':格,#格
                    'turnEnd':False,#稍后补
...
def 过滤记录(记录列表,匹配集):#按搜索匹配过滤
    ...
    for 下标,记录 in enumerate(过滤):#重算
        ...
        记录['groupStart']=前 is None or 前['section']!=记录['section'] or 前['group']!=记录['group']#组起
        格=记录['cell']#格
        记录['turnStart']=记录['section'] not in 已起段 and 格['kind']!='system' and (格['kind']!='compacted' or 记录['turn'] is None)#轮起
        if 记录['turnStart']:#已起
            已起段.add(记录['section'])#记
        记录['turnEnd']=后 is None or 后['section']!=记录['section']#轮尾
    return 过滤#结果
...
def 索引请求边界(记录列表):#请求边界下标
    ...
    for 记录 in 记录列表:#逐条
        ...
        if 键 in 边界:#已有
            continue#跳
        if 请求步号(记录['group']) is None:#非步组
            if 记录['groupStart']:#组起
                边界[键]=记录['cell']['index']#记下
            continue#下一条
        种类=记录['cell']['kind']#种类
...
def 索引请求编号(记录列表,会话编号,边界):#请求编号表
    ...
    边界记录=sorted(#边界记录
        [记录 for 记录 in 记录列表 if (边界[请求键(记录['turn'],记录['group'])] if 请求键(记录['turn'],记录['group']) in 边界 else None)==记录['cell']['index'] and 请求步号(记录['group']) is not None],
        key=记录格下标,
    )#排序
    for 记录 in 边界记录:#补号
        键=请求键(记录['turn'],记录['group'])#键
...
def 索引请求边界游程(记录列表):#边界游程下标
    ...
    for 记录 in 记录列表:#逐条
        格=记录['cell']#格
        if (格['requestOnly'] if 'requestOnly' in 格 else None) is True:#仅分隔
            索引[格['index']]=游程#记下
            游程+=1#加
            continue#下
        if 游程>0 and 记录['groupStart'] and 请求步号(记录['group']) is not None:#内容边界
            索引[格['index']]=游程#记下
        游程=0#清
    return 索引#映射
...
def 折叠轮次记录(记录列表,折叠轮次):#折叠轮次
    ...
    for 记录 in 记录列表:#投影
        ...
        if 轮 is None or 轮 not in 折叠轮次:#未折
            输出.append(记录)#原样
            continue#下
        本轮=按轮[轮] if 轮 in 按轮 else [记录]#本轮
        格=记录['cell']#格
        if (格['requestOnly'] if 'requestOnly' in 格 else None) is True or 格['kind']=='system':#分隔/系统
            输出.append(记录)#保留
            continue#下
        内容=[候 for 候 in 本轮 if (候['cell']['requestOnly'] if 'requestOnly' in 候['cell'] else None) is not True and 候['cell']['kind']!='system']#内容
        if len(内容)<=1:#不足折
            输出.append(记录)#原样
            continue#下
        if 格['index']!=内容[0]['cell']['index']:#非首
            continue#丢
...
def 助手工具调用(记录列表,助手下标):#助手后工具链
    ...
    for 下标,记录 in enumerate(记录列表):#找
        if 记录['cell']['index']==助手下标:#命中
            位=下标#记
            break#停
    if 位==-1 or 记录列表[位]['cell']['kind']!='message':#非助手
        return []#空
    调用=[]#链
    for 记录 in 记录列表[位+1:]:#后续
        种类=记录['cell']['kind']#种类
...
def 折叠助手记录(记录列表,折叠助手):#折叠助手工具
    ...
    while 下标<len(记录列表):#扫描
        ...
        输出.append(记录)#压
        格=记录['cell']#格
        if 格['kind']!='message' or 轨迹记录身份(格) not in 折叠助手:#不折
            下标+=1#前进
            continue#下
        调用=[]#工具链
        游=下标+1#探
        while 游<len(记录列表):#探链
            候=记录列表[游]#候
            if ('collapsedSummary' in 候) and 候['collapsedSummary'] is not None:#已是摘要
                break#断
            种类=候['cell']['kind']#种类
            if 种类 not in ('tool','subtool'):#断
                break#停
        ...
        输出.append(dict(记录,groupStart=False,turnStart=False,turnEnd=末['turnEnd'] if 'turnEnd' in 末 else False,collapsedSummary=摘要助手工具(调用),collapsedSummaryKind='assistant'))#摘要
        下标+=1+len(调用)#跳过链
    return 输出#结果
...
def 记录状态(记录):#complete/running/error
    ...
    if 格['isError'] if 'isError' in 格 else None:#错
        return 'error'#错
    if 格['kind']=='compacted' and ('timeSeconds' not in 格 or 格['timeSeconds'] is None):#压缩进行中
        return 'running'#跑
    if 格['kind'] in ('tool','subtool') and (格['outputDetail'] if 'outputDetail' in 格 else None) is None:#工具未果
        return 'running'#跑
    return 'complete'#完
...
def 输入合计(用量):#输入桶合计
    ...
    读=用量['cacheRead'] if 'cacheRead' in 用量 else None#读
    写=用量['cacheWrite'] if 'cacheWrite' in 用量 else None#写
    if 入 is None and 读 is None and 写 is None:#全空
        return None#无
...
def 消息来源标签(来源):#消息来源
    ...
    if 种=='user':#用户
        return 'User'#用户
    if 种=='plugin':#插件
        插件=来源['plugin'] if 'plugin' in 来源 else None#名
        return f'Plugin · {插件}' if isinstance(插件,str) and 插件!='' else 'Plugin'#插件
    if 种=='goal':#目标
        轮=来源['round'] if 'round' in 来源 else None#轮
...
def 父级记录(记录列表,记录):#工具父级
    ...
    if 种类 not in ('tool','subtool'):#非工具
        return {}#空
    位=-1#位置
    for 下标,候 in enumerate(记录列表):#找
        if 候['cell']['index']==记录['cell']['index']:#命中
            位=下标#记
            break#停
    if 位==-1:#无
        return {}#空
    工具=None#父工具
    if 种类=='subtool':#子工具
        for 候 in reversed(记录列表[:位]):#向前
            if 候['turn']!=记录['turn'] or 候['group']!=记录['group']:#出组
                break#停
            if 候['cell']['kind']=='tool':#父工具
                工具=候#记
                break#停
    父格=工具['cell'] if 工具 is not None else 记录['cell']#父格
    父调用=父格['callId'] if 'callId' in 父格 else None#调用 id
    消息=None#父消息
    if 父调用 is not None:#有调用
        for 候 in 记录列表:#找消息
            if 候['turn']!=记录['turn']:#异轮
                continue#跳
            if 候['cell']['kind']!='message':#非消息
                continue#跳
            块源=候['cell']['sourceBlocks'] if 'sourceBlocks' in 候['cell'] else None#源块
            块列表=[] if 块源 is None else 块源#?? []；TS cell.sourceBlocks ?? []，空表保留
            if any(('callId' in 块 and 块['callId']==父调用) for 块 in 块列表):#含调用
                消息=候#记
                break#停
    结果={}#父级
...
def Markdown源(记录):#Markdown 源文本
    ...
    if 种类 in ('user','context'):#入
        return (格['inputDetail'] if 'inputDetail' in 格 else None)#入详
    if 种类 in ('message','compacted'):#出
        return (格['outputDetail'] if 'outputDetail' in 格 else None)#出详
    return None#无
...
def 详情标签页(记录):#详情标签页列表
    ...
    种类=格['kind']#种类
    if 种类=='system':#系统
        return list(系统提示标签页 if ('previousPromptDetail' not in 格 or 格['previousPromptDetail'] is None) else 系统更新标签页)#系统
    if 种类=='compacted':#压缩
    ...
    if 是否Markdown记录(记录):#Markdown
        ...
        if ('messageSource' in 格) and 格['messageSource'] is not None:#有来源
...
def 记录展示文案(格):#行内展示
    ...
    if 预览Markdown is not None:#有预览
        ...
        文本=(格['text'] if 'text' in 格 else None) or ''#文
        if 文本=='':#空文
            return 预览#预览
    ...
    种类=格['kind']#种类
    if 种类 in ('user','context'):#入
        Markdown=(格['inputDetail'] if 'inputDetail' in 格 else None)#入
    elif 种类=='message':#助手
        Markdown=(格['outputDetail'] if 'outputDetail' in 格 else None)
        if Markdown is None:#回退思考
            Markdown=(格['thinkingDetail'] if 'thinkingDetail' in 格 else None)#思
    else:#其它
        Markdown=None#无
...
def 记录呈现(格):#列表呈现值
    ...
    结果=记录结果文案(格)#结果
    仅工具=是否仅工具调用(格)#仅工具
    部件=工具调用文案部件(格['kind'],展示)#部件
    if 仅工具:#仅工具
        列表文='(tool call only)'#占位
    elif 部件 is None:#非工具拆分
        列表文=展示#展示
    else:#工具
        列表文=' '.join(x for x in ((部件['name'] if 'name' in 部件 else None),(部件['args'] if 'args' in 部件 else None)) if x)#合成
    return {#呈现
        'displayText':展示,#展示
...
def 解析工具Schema(值):#解析工具 Schema
    ...
    if not isinstance(解析,dict):#非对象
        return None#无
    名=解析['name'] if 'name' in 解析 else None#名
    描述=解析['description'] if 'description' in 解析 else None#描述
    参数=解析['parameters'] if 'parameters' in 解析 else None#参数
...
def 提示词差分行(之前,之后):#统一差分行
    ...
    for 行 in 统一:#逐行
        if 行.startswith('+++') or 行.startswith('---'):#文件头
            continue#跳
        if 行.startswith('@@'):#块头
            行列表.append({'kind':'meta','text':行})#元
            continue#下
        if 行.startswith('+'):#增
            行列表.append({'kind':'added','text':行})#增
            continue#下
        if 行.startswith('-'):#删
            行列表.append({'kind':'removed','text':行})#删
            continue#下
        if 行.startswith(' '):#上下文
            行列表.append({'kind':'context','text':行})#上下文
            continue#下
