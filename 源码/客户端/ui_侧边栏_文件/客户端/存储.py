"""文件树的视图状态：哪些目录已展开，以及每一级已加载的内容。

对齐上游 `ui-sidebar-files/src/client/store.ts`。公开面仅中文名。
树不是一份资源：按级懒加载的目录列举是本类型自持的状态，落在槽位标准的独占存储
（每会话一份），再按 tab id 分桶，因为同会话两个同种 tab 各自独立展开。

写者在 `start` 与 `forget` 之间运行：拥有方的 `signal` 结束一桶的寿命，面在中止后不再派发。
