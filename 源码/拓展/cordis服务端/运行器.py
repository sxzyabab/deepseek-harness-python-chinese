...
from .类型 import 动态插件标识,动态包标识,动态运行标识,审批请求标识,动态插件错误#标识构造与本包异常
from .沙箱边界 import 是否插件,归一处理方法#插件判定与处理方法归一
...
def 格式错误细节(失败):#把细节格式成文本
    """消息行加可选栈。"""
    文本='message: '+str(失败['message'] if 'message' in 失败 else '')#消息行
    栈=失败['stack'] if 'stack' in 失败 else None#可选栈
    if 栈 is not None:#有栈
        文本+='\nstack:\n'+栈#附上
    return 文本#文本
...
def 仍缺(上下文,运行):#活动运行仍缺的服务名
    """无纤维则空。运行记录为 dict。"""
    光纤=运行['fiber'] if 'fiber' in 运行 else None#纤维
    return [] if 光纤 is None else 缺失服务(上下文,光纤)#缺失服务

def 取包定义(插件,包标识):
    """从插件包表取不可变定义，缺席为 None。"""
    表=插件['packages']#包表
    return 表[包标识] if 包标识 in 表 else None#定义
...
class 动态插件运行器服务(远程服务):#动态运行器服务
    ...
    def 定义(自身,请求):#定义或追加包
        ...
        if len(名称)==0:#名称为空
            raise 动态插件错误('cordis_define needs a non-empty `name`')#拒绝
        if len(用途)==0:#用途为空
            raise 动态插件错误('cordis_define needs a non-empty `purpose`')#拒绝
        代码=请求['code']#源码
        if 'host' not in 代码 and 'client' not in 代码:#两侧都缺
            raise 动态插件错误('cordis_define needs `code.host`, `code.client`, or both')#至少要一侧
        if 'host' in 代码:#有宿主源码
            预检代码(代码['host'],'code.host')#预检
        if 'client' in 代码:#有客户端源码
        ...
        if 插件选择['kind']=='new':#新建插件
            ...
            if 前缀模式.match(前缀) is None:#前缀非法
                raise 动态插件错误('cordis_define `plugin.idPrefix` must contain 3–6 lowercase English letters')#拒绝
            ...
            if 找到 is None or 找到['sessionId']!=请求['sessionId']:#不存在或不属于本会话
                raise 动态插件错误(插件缺失文案(插件选择['pluginId']))#报缺失
            插件=找到#沿用
        ...
        return {#定义回执
            ...
            'name':名称,#名称
            'purpose':用途,#用途
            'hasHostHalf':'hostCode' in 定义记录,#是否有宿主半
            'hasClientHalf':'clientCode' in 定义记录,#是否有客户端半
        }#回执
    ...
    def 运行(自身,智能体,插件标识,包标识,模式,信号=None):#启动或更新一个包
        ...
        if 信号 is not None and 信号.is_set():#请求已取消
        ...
        if 'clientCode' not in 计划['definition']:#没有客户端半
        ...
        自身.ctx.广播('cordis/request-run',{#发出运行请求事件
        ...
        if 'currentPackageId' in 计划['plugin']:#有当前包
    ...
    def 运行宿主半(自身,智能体,插件标识,包标识,模式,请求标识,批准后续版本):#启动宿主半
        ...
        if 请求标识 is not None:#模型驱动的请求
            ...
            最近=计划['plugin']['latestRun'] if 'latestRun' in 计划['plugin'] else None#最近尝试
            ...
            活动=计划['plugin']['run'] if 'run' in 计划['plugin'] else None#活动运行
            最近=计划['plugin']['latestRun'] if 'latestRun' in 计划['plugin'] else None#最近尝试
            ...
            if not 挂接:#新建尝试才改指针
                ...
                计划['plugin']['latestRun']=尝试#记下最近尝试
            if 'clientCode' in 计划['definition']:#有客户端半
                计划['plugin']['approvedClientPackages'].add(包标识)#视为已批准
        活动运行=计划['plugin']['run'] if 'run' in 计划['plugin'] else None#活动运行
    ...
    def 取客户端代码(自身,智能体,插件标识,运行标识):#取客户端源码
        ...
        插件=自身.本会话插件(智能体,插件标识)#本会话拥有的插件
        if 插件 is None:#不存在
            raise 动态插件错误(插件缺失文案(插件标识))#不存在
        运行=(插件['run'] if 'run' in 插件 else None)#活动运行
        if 运行 is None or 运行['pluginRunId']!=运行标识:#没有运行或不匹配
            raise 动态插件错误('dynamic plugin "'+插件标识+'" is not running activation "'+运行标识+'"')#该激活未在跑
        定义=取包定义(插件,运行['packageId'])#当前包定义
        if 定义 is None or 'clientCode' not in 定义:#没有客户端半
            raise 动态插件错误('package "'+运行['packageId']+'" has no Client half')#没有客户端半
        return {#客户端源
    ...
    def 结算运行请求(自身,请求标识,决议):#结算客户端激活请求
        ...
        活动=(插件['run'] if 'run' in 插件 else None) if 插件 is not None else None#活动运行
        if 决议['ok'] and (活动 is None or 活动['pluginRunId']!=决议['pluginRunId']):#成功决议对不上
            return {'accepted':False}#不受理
        if (not 决议['ok']) and 'pluginRunId' in 决议 and (活动 is None or 活动['pluginRunId']!=决议['pluginRunId']):#失败也对不上
            return {'accepted':False}#不受理
        自身.注册表.认领请求(请求标识)#认领
    ...
    def 停止(自身,智能体,插件标识):#停止运行
        ...
        if (插件['run'] if 'run' in 插件 else None) is None and 挂起 is None:#既没在跑也没挂起
        ...
        if (插件['run'] if 'run' in 插件 else None) is not None:#收回活动运行
            自身.收回(插件)#收回
        最近=(插件['latestRun'] if 'latestRun' in 插件 else None)#最近尝试
    ...
    def 面板停止(自身,智能体,插件标识):#面板停止
        ...
        当前=(插件['currentPackageId'] if 'currentPackageId' in 插件 else None) if 插件 is not None else None#当前包
    ...
    def 同步巡检清单(自身,提供方列表):#同步客户端清单
        """用客户端巡检提供方目录替换宿主镜像。"""
        自身.巡检注册表.同步客户端清单(提供方列表)#写入镜像
        ...
        for 插件 in 自身.注册表.全部():#每个插件一行
            行={#清单行
                ...
                'packages':[{#各包摘要
                    ...
                    'hasHostHalf':'hostCode' in 定义,#是否有宿主半
                    'hasClientHalf':'clientCode' in 定义,#是否有客户端半
                } for 定义 in 插件['packages'].values()],#包摘要
            }#行
            if (插件['currentPackageId'] if 'currentPackageId' in 插件 else None) is not None:#有当前包
                行['currentPackageId']=插件['currentPackageId']#带上
            if (插件['nextPackageId'] if 'nextPackageId' in 插件 else None) is not None:#有下一包
                行['nextPackageId']=插件['nextPackageId']#带上
            if (插件['run'] if 'run' in 插件 else None) is not None:#有活动运行
                行['activeRun']={'pluginRunId':插件['run']['pluginRunId'],'packageId':插件['run']['packageId']}#活动运行指针
            if (插件['latestRun'] if 'latestRun' in 插件 else None) is not None:#有最近尝试
    ...
    def 快照(自身,智能体):#会话快照
        ...
        for 插件 in 自身.注册表.按会话(智能体.id):#本会话每个插件
            行={'pluginId':插件['pluginId'],'packages':[{#各包摘要
                ...
                'hasHostHalf':'hostCode' in 定义,'hasClientHalf':'clientCode' in 定义,
            } for 定义 in 插件['packages'].values()]}#行
            if (插件['currentPackageId'] if 'currentPackageId' in 插件 else None) is not None:#当前包
                行['currentPackageId']=插件['currentPackageId']#带上
            if (插件['nextPackageId'] if 'nextPackageId' in 插件 else None) is not None:#下一包
                行['nextPackageId']=插件['nextPackageId']#带上
            if (插件['run'] if 'run' in 插件 else None) is not None:#活动运行
                运行=插件['run']#运行
                活动={'pluginRunId':运行['pluginRunId'],'packageId':运行['packageId'],'handlers':list(运行['handlers'].keys())}#活动
                if 'fiber' in 运行:#有纤维
                    活动['fiber']=运行['fiber']#带上
                if 'renderFailure' in 运行:#有渲染失败
                    活动['renderFailure']=运行['renderFailure']#带上
                行['activeRun']=活动#挂上
            if (插件['latestRun'] if 'latestRun' in 插件 else None) is not None:#最近尝试
                行['latestRun']=克隆尝试(插件['latestRun'])#克隆
            行列表.append(行)#收入
        return 行列表#快照
    ...
    def 引用(自身,智能体,插件标识):#引用上下文
        ...
        if 插件 is None:#不存在
            return None#无
        包标识=(插件['nextPackageId'] if 'nextPackageId' in 插件 else None) or (插件['currentPackageId'] if 'currentPackageId' in 插件 else None)#优先下一包否则当前
        if 包标识 is None:#否则最后一个包
            键列表=list(插件['packages'].keys())#包键
            包标识=键列表[-1] if len(键列表)>0 else None#最后一个
    ...
    def 巡检插件(自身,智能体,插件标识):#巡检插件
        ...
        if 引用 is None:#没有包
            raise 动态插件错误('dynamic plugin "'+插件标识+'" has no package')#没有包
    ...
    def 巡检包(自身,智能体,插件标识,包标识):#巡检一个包
        ...
        定义=取包定义(插件,包标识)#取包定义
        if 定义 is None:#没有该包
            raise 动态插件错误('dynamic package "'+包标识+'" does not exist on plugin "'+插件标识+'"')#包不存在
        代码={}#源码
        if 'hostCode' in 定义:#有宿主源码
            代码['host']=定义['hostCode']#带上
        if 'clientCode' in 定义:#有客户端源码
    ...
    def 报告渲染失败(自身,智能体,插件标识,运行标识,失败):#记录渲染失败
        ...
        插件=自身.本会话插件(智能体,插件标识)#本会话拥有的插件
        运行=(插件['run'] if 'run' in 插件 else None) if 插件 is not None else None#活动运行
        if 运行 is not None and 运行['pluginRunId']==运行标识:#仍是该活动运行
            定义=取包定义(插件,运行['packageId'])#当前包定义
            应引导='renderFailure' not in 运行#首次失败才steer
            运行['renderFailure']=失败#记下失败
            尝试=(插件['latestRun'] if 'latestRun' in 插件 else None)#最近尝试
    ...
    def 调用(自身,插件标识,运行标识,方法,参数):#调用活动宿主方法
        ...
        if 插件 is None or (插件['run'] if 'run' in 插件 else None) is None:#不存在或没在跑
        ...
        处理器=运行['handlers'][方法] if 方法 in 运行['handlers'] else None#按名取处理器
        if 处理器 is None:#未登记
            return {'ok':False,'code':'method-not-found','message':'dynamic plugin "'+插件标识+'" registered no Host method "'+方法+'"'}#没有该方法
        try:#处理器约定可能抛
            return {'ok':True,'value':处理器(参数)}#成功则装箱 JSON
        except Exception as 错误:#沙箱宿主方法可抛任意类型，契约未钉死，无法再收窄
            失败=错误细节(错误)#抽出消息与栈
    ...
    def 解析计划(自身,智能体,插件标识,包标识,模式,允许挂接=False):#解析激活计划
        ...
        当前=(插件['currentPackageId'] if 'currentPackageId' in 插件 else None)#当前成功版本
    ...
    def 激活(自身,计划,请求标识,允许挂接,尝试):#启动或去重挂接
        """去重返回同一承诺。"""
        if 计划['plugin']['pluginId'] in 自身.启动中:#已在飞
            return 自身.启动中[计划['plugin']['pluginId']]#返回同一结果
        def 执行():#新启一次
            """真正启动。"""
            try:#启动
                return 自身.全新启动(计划,请求标识,允许挂接,尝试)#新启
            finally:#结束后清掉
                if 计划['plugin']['pluginId'] in 自身.启动中:#仍记着
                    del 自身.启动中[计划['plugin']['pluginId']]#清掉
        结果=执行()#同步启动；全新启动内部已同步
        return 结果#结果
    ...
    def 全新启动(自身,计划,请求标识,允许挂接,尝试):#真正启动一次新运行
        ...
        定义=计划['definition']#定义
        模式=计划['mode']#模式
        活动=(插件['run'] if 'run' in 插件 else None)#活动运行
        ...
        if 活动 is not None:#先收回旧运行
            自身.收回(插件)#收回
        if 模式=='update' or (插件['currentPackageId'] if 'currentPackageId' in 插件 else None) is None:#记下下一包
        ...
        插件['run']=运行#挂上活动运行
        自身.ctx.广播('cordis/dynamic-package',{#发出动态包事件
            'pluginId':插件['pluginId'],#插件 id
            'packageId':定义['packageId'],#包 id
        ...
        尝试['host']={#更新宿主半状态
            'status':'absent' if 'fiber' not in 运行 else ('running' if len(等待)==0 else 'waiting'),#缺席、运行中或等待
            'waitingFor':等待,#仍缺
        }#host
        if 'clientCode' not in 定义:#没有客户端半
    ...
    def 启动宿主(自身,插件,宿主代码,运行):#求值并挂上宿主半
        ...
        def 登记(方法,函数):#登记宿主方法
            ...
            def 拆除():#拆除该登记
                """仍是自己才删。"""
                if 归一['method'] in 运行['handlers'] and 运行['handlers'][归一['method']] is 归一['handler']:#仍是自己
                    del 运行['handlers'][归一['method']]#删
            运行['拆除器列表'].append(拆除)#记下拆除器
            return 拆除#交给调用方
        try:#求值与启动可能抛
            沙箱=创建沙箱(插件['pluginId'],{'handle':登记})#带 handle 的沙箱
            超时=自身.已解析['vmTimeoutMs'] if 'vmTimeoutMs' in 自身.已解析 else 5000#超时；?? 显式 0 仍是 0
            已求值=求值宿主代码(沙箱,宿主代码,插件['pluginId'],超时)#求值宿主源码
            if not 是否插件(已求值):#不是合法插件
                raise 动态插件错误('the Host half returned `None` — did you forget `return`?' if 已求值 is None else 'the Host half must return a Plugin function or an object with apply(ctx)')#必须是插件
            def 报告门面失败(错误):#门面失败回调
                """steer 宿主门面失败。"""
                自身.引导守卫失败(插件,运行,'Host',错误细节(错误))#steer；方法名对齐线上 reportGuardFailure
            运行['fiber']=启动宿主半(自身.需要组(),已求值,报告门面失败)#启动宿主半纤维
            return None#成功
        except Exception as 错误:#求值或启动失败：沙箱与纤程.等待 可抛任意插件错误，无法再收窄
            for 拆除 in 运行['拆除器列表'][:]:#清掉已登记处理器
                拆除()#拆除
            运行['拆除器列表'].clear()#清空
    ...
    def 结算激活(自身,插件,决议,请求标识=None):#按客户端决议提交或回滚
        ...
        尝试=(插件['latestRun'] if 'latestRun' in 插件 else None)#最近尝试
        if not 决议['ok']:#决议失败
            if 'reason' in 决议 and 决议['reason']=='rejected':#用户拒绝
                拒文=决议['message'] if 'message' in 决议 and 决议['message'] is not None else 'the run request was declined'#?? 显式空串仍保留
                if 尝试 is not None:#有尝试则记账
                    尝试['status']='rejected'#标拒绝
                    尝试['error']=自身.诊断(插件,尝试,'approval',拒文)#审批诊断
                    尝试['client']={'status':'stopped','waitingFor':[]}#客户端停止
                return {'ok':False,'reason':'rejected','message':拒文}#拒绝回执
            运行=(插件['run'] if 'run' in 插件 else None)#活动运行
            决议运行=决议['pluginRunId'] if 'pluginRunId' in 决议 else None#决议运行 id
            启动来源=运行['startedForRequest'] if 运行 is not None and 'startedForRequest' in 运行 else None#启动来源
            在此启动=决议['startedHere'] if 'startedHere' in 决议 else None#是否本宿主启动
            拥有=(运行 is not None and 决议运行==运行['pluginRunId']
                and (请求标识 is None or 启动来源==请求标识)
                and 在此启动 is not False)#本运行的失败
            if 拥有:#本运行的失败则收回
                自身.收回(插件)#收回
            消息=决议['message'] if 'message' in 决议 and 决议['message'] is not None else (决议['reason'] if 'reason' in 决议 else None)#失败消息
            if 尝试 is not None and (决议运行 is None or 尝试['pluginRunId']==决议运行):#尝试对应本决议
                阶段='host-apply' if 'reason' in 决议 and 决议['reason']=='host-half-failed' else 'client-apply'#宿主或客户端应用阶段
                失败={'message':消息}#失败细节
                if 'stack' in 决议:#有栈
                    失败['stack']=决议['stack']#带上
                自身.失败尝试(插件,尝试,阶段,失败)#标记尝试失败
            回执={'ok':False,'reason':决议['reason'] if 'reason' in 决议 else None,'message':消息}#失败回执
            if 'stack' in 决议:#有栈
        ...
        if 尝试 is not None and 尝试['pluginRunId']==运行['pluginRunId']:#尝试对应本运行
            等待=决议['waitingFor'] if 'waitingFor' in 决议 else []#等待的服务；?? 缺席才空
    ...
    def 宣布已决议(自身,请求标识,决议,覆盖=None):#宣布运行请求已决议
        ...
        if 覆盖 is not None:#覆盖
            ...
            结局='approved'#批准
        elif 'reason' in 决议 and 决议['reason']=='rejected':#拒绝
            结局='rejected'#拒绝
        else:#其它失败
            结局='failed'#失败
        自身.ctx.广播('cordis/request-run-resolved',{'requestId':请求标识,'outcome':结局})#发出事件

    def 引导运行结局(自身,挂起,已结算):#把运行结局steer给模型
        """steer 文本。"""
        智能体服务=自身.根上下文.获取服务('agents')#智能体表
        智能体=None if 智能体服务 is None else 智能体服务.获取(挂起['agentId'])#原请求的智能体
        if 智能体 is None:#智能体已不在
            return#返回
        插件=自身.注册表.获取(挂起['pluginId'])#对应插件
        身份=挂起['pluginId']+'/'+挂起['packageId']+' ('+挂起['pluginRunId']+')'#身份串
        if 已结算['ok']:#激活成功
            当前包=已结算['currentPackageId'] if 'currentPackageId' in 已结算 and 已结算['currentPackageId'] is not None else 挂起['packageId']#?? 缺席才回落到挂起包
            文本='Cordis '+挂起['mode']+' '+身份+' completed successfully. currentPackageId is '+str(当前包)+'. Continue using the running Plugin.'#成功
        elif 'reason' in 已结算 and 已结算['reason']=='rejected':#用户拒绝
            ...
            当前=(插件['currentPackageId'] if 'currentPackageId' in 插件 else None) if 插件 is not None else None#当前包
            下一=(插件['nextPackageId'] if 'nextPackageId' in 插件 else None) if 插件 is not None else 挂起['packageId']#下一包
            原因=已结算['reason'] if 'reason' in 已结算 else None#失败原因
            当前文='none' if 当前 is None else str(当前)#展示
            下一文=挂起['packageId'] if 下一 is None else str(下一)#展示
            文本=('Cordis '+挂起['mode']+' '+身份+' failed after cordis_run returned '+当时+': '
                +str(原因)+'\n'+格式错误细节(已结算)+'\n'
                +'currentPackageId: '+当前文+'\n'
                +'nextPackageId: '+下一文+'\n'
    ...
    def 引导渲染失败(自身,智能体,插件,定义,运行标识,失败):#把渲染失败steer给模型
        ...
        文本=('Cordis Client UI '+插件['pluginId']+'/'+定义['packageId']+' ('+运行标识+') failed while rendering '
            ...
            +'entryAbdicated: '+str(失败['abdicated'] if 'abdicated' in 失败 else False)+'\n'
    ...
    def 引导宿主处理器失败(自身,插件,运行,方法,失败):#把宿主处理器失败steer给模型
        ...
        if not 自身.认领运行时失败(插件,运行,报告键):#已报告过
            return#跳过
        智能体服务=自身.根上下文.获取服务('agents')#智能体表
        智能体=None if 智能体服务 is None else 智能体服务.获取(插件['sessionId'])#所属会话智能体
        if 智能体 is None:#智能体已不在
            return#返回
        import json#序列化方法名
        文本=('Cordis Host handler '+插件['pluginId']+'/'+运行['packageId']+' ('+运行['pluginRunId']+') failed when the Client called '
            +'host.call('+json.dumps(方法,ensure_ascii=False,separators=(',',':'),allow_nan=False)+').\n'+格式错误细节(失败)+'\n'
    ...
    def 引导守卫失败(自身,插件,运行,平面,失败):#把守卫拒绝steer给模型
        ...
        if not 自身.认领运行时失败(插件,运行,报告键):#已报告过
            return#跳过
        智能体服务=自身.根上下文.获取服务('agents')#智能体表
        智能体=None if 智能体服务 is None else 智能体服务.获取(插件['sessionId'])#所属
        if 智能体 is None:#智能体已不在
            return#返回
    ...
    def 认领运行时失败(自身,插件,运行,键):#认领一次运行时失败报告
        """已报告过则 False。"""
        尝试=(插件['latestRun'] if 'latestRun' in 插件 else None)#最近尝试
        if (插件['run'] if 'run' in 插件 else None) is not 运行 or 尝试 is None or 尝试['pluginRunId']!=运行['pluginRunId']:#不是当前活动运行
            return False#跳过
    ...
    def 注入用户运行结局(自身,智能体,插件标识,已结算):#把用户手势运行结局注入上下文
        ...
        插件=自身.本会话插件(智能体,插件标识)#本会话拥有的插件
        if 已结算['ok']:#手势成功
            当前包=已结算['currentPackageId'] if 'currentPackageId' in 已结算 else None#当前包
            文本='The user manually ran Cordis Plugin '+插件标识+', Package '+已结算['packageId']+', as '+已结算['pluginRunId']+'. The activation succeeded; currentPackageId is '+str(当前包)+'.'#成功
        else:#手势失败
            尝试=(插件['latestRun'] if 'latestRun' in 插件 else None) if 插件 is not None else None#最近尝试
            段='' if 尝试 is None else ', Package '+尝试['packageId']+', as '+尝试['pluginRunId']#包与运行
            当前=(插件['currentPackageId'] if 'currentPackageId' in 插件 else None) if 插件 is not None else None#当前包
            下一=(插件['nextPackageId'] if 'nextPackageId' in 插件 else None) if 插件 is not None else None#下一包
            原因=已结算['reason'] if 'reason' in 已结算 else None#原因
            当前文='none' if 当前 is None else str(当前)#展示
            下一文='none' if 下一 is None else str(下一)#展示
            文本=('The user manually ran Cordis Plugin '+插件标识+段+', but it failed: '
                +str(原因)+'\n'+格式错误细节(已结算)+'\n'
                +'currentPackageId: '+当前文+'\n'
                +'nextPackageId: '+下一文)#失败
    ...
    def 取消挂起(自身,插件标识,消息):#取消挂起的运行请求
        ...
        最近=(插件['latestRun'] if 'latestRun' in 插件 else None) if 插件 is not None else None#最近尝试
    ...
    def 新建尝试(自身,计划):#按计划新建一次尝试
        ...
        return {#尝试
            ...
            'host':{'status':'absent' if 'hostCode' not in 计划['definition'] else 'pending','waitingFor':[]},#宿主半
            'client':{'status':'absent' if 'clientCode' not in 计划['definition'] else 'pending','waitingFor':[]},#客户端半
        }#尝试
    ...
    def 失败尝试(自身,插件,尝试,阶段,失败):#把尝试标为失败
        ...
        if 阶段.startswith('host'):#宿主阶段
            if isinstance(失败,str):#失败是文案
                消息=失败#文案
            elif 'message' in 失败:#失败是细节 dict
                消息=失败['message']#消息
            else:#没有 message 键
                消息=str(失败)#字符串化
            尝试['host']={'status':'failed','waitingFor':[],'error':消息}#宿主失败
        else:#客户端阶段
            if isinstance(失败,str):#失败是文案
                消息=失败#文案
            elif 'message' in 失败:#失败是细节 dict
                ...
                消息=str(失败)#字符串化
            尝试['client']={'status':'failed','waitingFor':[],'error':消息}#客户端失败
    ...
    def 收回(自身,插件):#收回活动运行
        ...
        for 拆除 in 运行['拆除器列表'][:]:#拆除全部处理器
            拆除()#拆除
        运行['拆除器列表'].clear()#清空
        if 'fiber' in 运行 and 运行['fiber'] is not None:#有纤维
            运行['fiber'].拆除()#拆除宿主纤维
        自身.ctx.广播('cordis/dynamic-retract',{#发出收回事件
            'pluginId':插件['pluginId'],#插件 id
            'packageId':运行['packageId'],#包 id
            'pluginRunId':运行['pluginRunId'],#运行 id
        })#emit
    ...
    def 需要组(自身):#取或建动态插件纤维组
        ...
        if 自身.组 is None:#尚未创建
            def 空应用(上下文,配置值=None):
                """空插件体，只用来当组。"""
                return None#无贡献
            自身.组=自身.根上下文.启动插件({'name':'cordis-dynamic','apply':空应用})#空插件当组
        return 自身.组#返回组
