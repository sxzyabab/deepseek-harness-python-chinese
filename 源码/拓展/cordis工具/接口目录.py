...
继承上下文目录=[#继承 ctx 目录（上游完整）
    {'name':'ctx.监听 / ctx.监听一次','summary':'Register an event listener (disposable).'},#监听
    {'name':'ctx.广播 / ctx.并发广播 / ctx.serial / ctx.首个结果 / ctx.链式拦截','summary':'Dispatch an event (sync / awaited / first-bail / short-circuit chain).'},#分发
    {'name':'ctx.启动插件 / ctx.依赖启动','summary':'Load a plugin / declare required services.'},#加载/声明
    {'name':'ctx.副作用','summary':'Register a disposable side effect tied to the fiber.'},#副作用
    {'name':'ctx.获取服务 / ctx.设置服务 / ctx.提供服务 / ctx.定义访问器 / ctx.混入成员','summary':'Low-level service-store access and binding.'},#服务仓
    {'name':'ctx.扩展 / ctx.隔离 / ctx.拦截','summary':'Derive a child context (scoped services / isolation / interception).'},#派生子上下文
    {'name':'ctx.根 / ctx.纤程 / ctx.注册表 / ctx.反射 / ctx.事件 / ctx.日志','summary':'Ambient handles onto the running context graph.'},#环境句柄
    {'name':'ctx.timer (+ 间隔 / 超时 / 节流 / 防抖)','summary':'Disposable timer helpers. The `timer` key is provided at runtime; the four supported helpers are mixed onto ctx directly (declared via Pick).'},#定时器
...
def 引用类型闭包(种子列表,类型列表=None):#签名引用的类型闭包
    """种子文本引用到的已编目类型形态的传递闭包。"""
    if 类型列表 is None:#缺省
        类型列表=类型目录#用模块目录
    已收=set()#已收入的类型名
    前沿=list(种子列表)#本轮要搜的文本
    while len(前沿)>0:#还有未展开的
        ...
        前沿=下一轮#换一轮
    return [条目 for 条目 in 类型列表 if 条目['name'] in 已收]#按目录顺序过滤
...
def 查询服务目录(键=None,服务列表=None):#查询服务
    ...
    return {#服务模式
        ...
        'service':{#详细服务
            ...
            'access':{#访问方式
                'optional':{'expression':'ctx.获取服务('+__import__('json').dumps(服务['key'])+')','requiresUndefinedCheck':True},#可选查找
