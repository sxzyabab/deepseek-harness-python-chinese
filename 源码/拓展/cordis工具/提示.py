...
### Code: use plain Python for Host (Client half remains browser JavaScript)

- Host code runs as the body of a Python function in the Host sandbox; it is not transformed by a bundler.
- Do not use TypeScript types, as, decorators, import, require, or JSX in Host code.
...
- Host runs in the DSH process and is appropriate for files, networking, commands, Agent/Session access, Host Events, Services, model Tools, and JSON methods callable by the Client.
