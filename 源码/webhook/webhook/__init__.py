...
def 若已中止则抛出(信号):
    """已中止则抛出本包中止异常。"""
    if 已中止(信号):#已中止
        raise Webhook已中止('webhook 规则已拆除')#抛出

class 中止控制器:
    """登记生命周期用的中止控制器。信号是 threading.Event。"""
    def __init__(自身):
        """创建配套 Event。"""
        自身.信号=threading.Event()#中止信号
...
class Webhook运行时(服务):
    ...
    def 登记(自身,规则):
        ...
        if 自身._正在关闭:#正在关闭
            raise Webhook错误('webhook 运行时正在关闭')#拒绝
        规则号=规则['id']#规则id
        if (not isinstance(规则号,str)) or 规则号.strip()=='':#非法id
            raise TypeError('webhook 规则 id 必须是非空字符串')#拒绝
        种类=规则['kind']#provider kind
        if (not isinstance(种类,str)) or 种类.strip()=='':#非法kind
            raise TypeError(f'webhook 规则 "{规则号}" 的 kind 必须是非空字符串')#拒绝
        if not callable(规则['run']):#缺少 run
            raise TypeError(f'webhook 规则 "{规则号}" 需要 run()')#拒绝
        ...
        def 挂上():
            ...
            if 自身._正在关闭:#正在关闭
                raise Webhook错误('webhook 运行时正在关闭')#拒绝
            if 规则号 in 自身._规则:#重复
                raise Webhook错误(f'webhook 规则 "{规则号}" 已登记')#拒绝
            自身._规则[规则号]=登记对象#写入
            def 拆除():
                """等待登记拆除完成。"""
                自身._拆除登记(登记对象)#拆除
    ...
    def _启动调用(自身,登记,投递):
        ...
        def 执行规则调用():
            ...
            请求=登记['rule']['run'](投递,登记['controller'].信号)#执行规则
