...
def 解析正整数(值,名):#解析正整数参数
    ...
    if isinstance(值,int) and 值>=1:#已经是正整数
        return 值#原样
    if isinstance(值,float) and math.isfinite(值) and 值.is_integer() and 值>=1:#整值浮点
        return int(值)#收成int
    raise 工具文件系统错误(名+' must be a positive integer')#参数非法
...
def 解析读参数(参数,最大行数):#校验读工具参数
    ...
    行数=最大行数 if 'limit' not in 参数 or 参数['limit'] is None else 解析正整数(参数['limit'],'limit')#缺席或null用部署上限
    if 行数>最大行数:#不得超过部署上限
        raise 工具文件系统错误('limit must be less than or equal to '+str(最大行数))#不得超过部署上限
...
def 应用读工具(上下文,上限):#注册 read 工具
    ...
    def 渲染(参数,值):#渲染模型可见信封
        ...
        if len(窗口行)>0:#有窗口行；判 length
            ...
            末行=max(0,值['offset']-1)#offset前一行
        因字节截断=len(窗口行)<输入['行数'] and 末行<值['totalLines']#行未满且未到EOF则是字节截断
        结果={'offset':值['offset'],'lines':窗口行,'totalLines':值['totalLines']}#窗口结果
        if 因字节截断:#字节截断
    ...
    def 呈现元数据(参数,值):#结果呈现用的 read meta
        ...
        元数据={#meta载荷
            'path':值['path'],#路径
            'offset':值['offset'],#起始行
            'lines':[{'number':行['number'],'text':行['text']} for 行 in 值['lines']],#行窗口
            'totalLines':值['totalLines'],#总行数
        }#meta结束
    ...
    def 执行(参数,执行上下文):#执行读取
        ...
        目标=已解析['target']#目标
        信息=已解析['info']#stat结果
        字节数=信息['size'] if 'size' in 信息 else None#文件字节数
        信号=执行上下文['signal'] if 'signal' in 执行上下文 else None#取消信号
        if 字节数 is None or 字节数>=上限['流最小字节数']:#未知字节数或达到阈值
            块列表=上下文.fs.流文本(目标,信号)#流式块
        else:#小文件
            块列表=[上下文.fs.读文本(目标,信号)]#整文件作为单块
        窗口=构建窗口(#构建有界行窗口
            块列表,#文本块
    ...
    def 呈现结果(参数,结果):#结果时 read 卡片
        ...
        文本=唯一['text'] if 唯一 is not None and 'type' in 唯一 and 唯一['type']=='text' else None#必须是文本块
        if 文本 is None:#没有文本
            return None#回退
        ...
        if 'lang' in 元数据 and 元数据['lang'] is not None:#有语言提示
            卡片['lang']=元数据['lang']#带上
        return 卡片#卡片
    def 呈现调用(参数):#调用时通用卡片
        """调用时通用卡片。窗口反映原始参数。"""
        ...
        return {#卡片
            'card':'generic',#通用卡片
            'title':'Read '+参数['file_path']+窗口,#标题含窗口
            'kind':'read',#读种类图标
