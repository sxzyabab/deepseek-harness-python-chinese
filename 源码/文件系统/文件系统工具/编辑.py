...
def 解析编辑参数(参数):#校验编辑工具参数
    ...
    if len(参数['old_string'])==0:#旧字面量不得为空
        raise 工具文件系统错误('old_string must be a non-empty string')#旧字面量不得为空
    if 参数['old_string']==参数['new_string']:#新旧必须不同
        raise 工具文件系统错误('old_string and new_string must differ')#新旧必须不同
    替换全部=参数['replace_all'] if 'replace_all' in 参数 else False#缺席则不替换全部
    if 替换全部 is None:#显式null当缺省
        替换全部=False#缺省不替换全部
    return {#转为内部输入
        '路径':参数['file_path'],#路径
        '旧字面量':参数['old_string'],#旧字面量
        '新字面量':参数['new_string'],#新字面量
        '替换全部':替换全部,#是否替换全部
    }#输入结束
...
def 应用编辑工具(上下文,沙箱):#注册 edit 工具
    ...
    def 呈现调用(参数):#调用时 diff 卡片
        ...
        return {#卡片
            'card':'diff',#diff卡片
            'title':'Edit '+参数['file_path'],#标题
            'diffs':[{'path':参数['file_path'],'oldText':旧字面量 if len(旧字面量)>0 else None,'newText':参数['new_string']}],#字面量替换片段
    ...
    def 呈现结果(参数,结果):#结果时 diff 卡片
        ...
        if 差异列表 is None:#畸形
            return None#交给通用渲染
        return {'card':'diff','title':'Edit '+参数['file_path'],'diffs':差异列表}#结果diff卡片
