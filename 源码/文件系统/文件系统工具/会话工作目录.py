"""推导文件系统工具解析相对路径所用的工作目录：调用方 agent 的每会话 workspace（exec.agent.session.header.cwd），使每个会话的 read/write/edit 作用在它自己的 workspace 上，而不是服务器启动目录——镜像 dsh-tool-bash 把 bash workdir 默认到会话 cwd。非 agent 调用返回 None，把回退留给提供方，而不是在工具边界读 process.cwd()。"""
import re#父目录段匹配
...
def 会话工作目录(执行,请求路径):#推导此次调用的会话cwd
    """此次调用的会话 workspace cwd；不适用时为 None。父目录穿越会使符号链接 cwd 的文件系统身份可观察，因此有穿越时规范化 cwd。执行是 dict，智能体是对象，会话头是 dict。"""
    智能体=执行['agent'] if 'agent' in 执行 else None#调用方智能体
    if 智能体 is None:#非agent调用
        return None#交给提供方默认
    头=智能体.session.header#会话头
    工作目录=头['cwd'] if 'cwd' in 头 else None#取出会话头上的cwd
...
def 会话解析选项(执行,请求路径,政策工作区根=None):#组装resolve所用的cwd与取消信号
    ...
    选项={'signal':执行['signal'] if 'signal' in 执行 else None}#带上取消信号
