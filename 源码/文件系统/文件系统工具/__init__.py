...
配置=模式.对象({#schemastery配置模式；全部可选，缺省由模式填入产品默认值
    'readLimit':模式.数字().默认(读行数上限),#读行数默认值
    'readMaxLineLength':模式.数字().默认(读最大行长),#单行长度默认值
    'readMaxBytes':模式.数字().默认(读最大字节),#字节上限默认值
    'readStreamMinSize':模式.数字().默认(流最小大小),#流式阈值默认值
})#配置模式结束
