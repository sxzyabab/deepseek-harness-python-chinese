...
class 文件系统沙箱控制器:#文件系统沙箱升级控制器
    ...
    def __init__(自身,上下文):#按当前挂载的文件系统能力构造
        ...
        自身.政策=None if 默认模式 is None else 上下文.获取服务('sandboxPolicy',False)#有隔离则取出策略服务
        if 默认模式 is not None and 自身.政策 is None:#隔离后端却缺少策略服务
            raise 工具文件系统错误('tool-fs: the mounted filesystem confines but ctx.sandboxPolicy is missing')#加载时大声失败
    ...
    def 解析政策(自身,工具名,参数,执行):#解析此次变更的沙箱策略
        ...
        if ('sandbox_permissions' not in 参数) or 参数['sandbox_permissions'] is None or ('justification' not in 参数) or 参数['justification'] is None:#没有升级请求
            return 常驻政策#使用常驻策略
        if len(自身.升级模式)==0:#组合未广告升级
            raise 工具文件系统错误('sandbox_permissions is not available in this composition (no sandboxing filesystem to escalate)')#无隔离文件系统可升级
        审批上下文={#审批上下文
            'approver':自身.上下文.获取服务('approval',False),#审批服务
            'agent':智能体,#调用方agent
            'callId':执行['callId'],#此次调用id
            'toolName':工具名,#工具名
            'signal':执行['signal'] if 'signal' in 执行 else None,#取消信号
        }#审批上下文结束
        批准模式=批准升级(#走审批得到更宽模式
            {'requestedMode':参数['sandbox_permissions'],'justification':参数['justification'],'effectiveMode':常驻政策['mode'],'subject':'operation'},#升级请求
            审批上下文,#审批上下文
        )#审批结束
        已批=dict(常驻政策)#拷贝常驻策略
    ...
    def 映射错误(自身,错误,政策):#把沙箱拒绝映射为面向模型的标记错误
        """为模型映射抛出的提供方错误：FS_SANDBOX_DENIED 变成共享 [sandbox: …] 拒绝标记加上本回合升级提示的 FsError，使策略拒绝读起来与 bash 相同，同时保留结构化码。其他错误原样穿过。FS_SANDBOX_DENIED 只在隔离后端下出现，而隔离后端总是广告升级字段，因此此处提示总是适用。政策是 dict。"""
