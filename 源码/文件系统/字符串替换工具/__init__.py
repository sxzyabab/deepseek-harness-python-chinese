...
class 变更政策:#变更所用的沙箱策略适配
    ...
    def __init__(自身,上下文):#按文件系统沙箱模式解析策略服务
        ...
        if 沙箱模式 is None:#非沙箱则不取策略
            ...
            自身.政策=上下文.获取服务('sandboxPolicy',False)#取已挂载的沙箱策略服务
...
def 状态已存在(上下文,目标,命令,执行上下文):#确认目标存在并取出元数据
    ...
    if 信息 is None:#目标不存在
        ...
        raise 文件系统错误(#报未找到
            'The path '+目标['displayPath']+' does not exist. Please provide a valid path.',#面向模型的缺失说明
            'FS_NOT_FOUND',#未找到码
        )#FsError结束
    if 信息['type']=='directory' and 命令!='view':#目录上只允许view
        raise 文件系统错误(#报不是普通文件
            'The path '+目标['displayPath']+' is a directory and only the `view` command can be used on directories',#目录不能替换或插入
...
def 格式化文件视图(路径,内容,最大输出字符,视图范围=None):#格式化带行号的文件视图
    ...
    if 视图范围 is not None:#指定了行范围
        ...
        if 请求起始 is None or 请求结束 is None:#起始或结束不得缺失
            raise 错误('Invalid `view_range`. It should be a list of two integers.')#要求两个整数
        for 项 in 视图范围:#入口校验每一项
            if isinstance(项,bool) or not isinstance(项,int):#排除布尔，只收int
                raise 错误('Invalid `view_range`. It should be a list of two integers.')#要求两个整数
        起始行=请求起始#采用请求的起始
        结束行=请求结束#采用请求的结束
        ...
        if 结束行==-1:#-1表示切到文件末尾
            行列表=全部行[起始行-1:]#从起始切到末尾
        else:#闭区间切到结束行
            行列表=全部行[起始行-1:结束行]#闭区间
        导语+=' with view_range=['+str(起始行)+', '+str(结束行)+']'#导语补上范围
    编号行=[]#给展示行编号
    for 序号,行 in enumerate(行列表):#逐行编号
    ...
    def 访问(目录,深度):#递归列举一层
        ...
        for 候选项 in 条目:#遍历直接子项
            ...
            if 类型=='directory':#目录
                ...
                标记='?'#未知标记
            子目标=候选项['target']#已解析子目标
            行列表.append(标记+'\t'+子目标['displayPath'])#标记加展示路径
            if 类型=='directory' and 深度<2:#未到两层则再下降
                行列表.extend(访问(子目标,深度+1))#展开子目录
        return 行列表#本层及更深层的行
...
def 查看路径(上下文,路径,视图范围,最大输出字符,执行上下文):#查看文件或目录
    ...
    if 信息['type']!='file':#既非目录也非普通文件
        raise 文件系统错误('cannot view "'+目标['displayPath']+'": not a regular file or directory','FS_NOT_REGULAR_FILE')#拒绝特殊文件
    内容=上下文.fs.读文本(目标,执行上下文['signal'] if 'signal' in 执行上下文 else None)#读出全文
...
def 创建文件(上下文,政策,路径,文件文本,执行上下文):#仅在不存在时创建文件
    ...
    except 文件系统错误 as 异常:#写入失败
        raise 政策.映射错误(异常,沙箱政策)#沙箱拒绝换成带模式标记的错误
    上下文.广播('fs/observed',目标,{'kind':'present','version':结果['version']},执行上下文)#记录存在观察
...
def 文件内替换(上下文,政策,路径,旧串,新串,执行上下文):#字面量替换恰好一处
    ...
    if 信息['type']!='file':#不是普通文件
        raise 文件系统错误('cannot edit "'+目标['displayPath']+'": not a regular file','FS_NOT_REGULAR_FILE')#拒绝特殊文件
    之前=上下文.fs.读文本(目标,信号)#读出替换前全文
    下标列表=匹配下标(之前,旧值)#找出每一处字面量
    if len(下标列表)==0:#一处都没有
        raise 文件系统错误(#报未找到
            'No replacement was performed, old_str `'+旧值+'` did not appear verbatim in '+目标['displayPath']+'.',#必须逐字匹配
            'FS_EDIT_NOT_FOUND',#未找到码
        )#未找到结束
    if len(下标列表)>1:#多于一处则拒绝，避免误替换
        行号列表=下标行号(之前,下标列表)#各处行号
        raise 文件系统错误(#报不唯一
            'No replacement was performed. Multiple occurrences of old_str `'+旧值+'` in lines ['+', '.join(str(项) for 项 in 行号列表)+']. Please ensure it is unique',#列出所有行号
            'FS_AMBIGUOUS_EDIT',#不唯一码
        )#不唯一结束
    下标=下标列表[0]#第一处（唯一）
    之后=之前[0:下标]+新值+之前[下标+len(旧值):]#拼接替换后文本
    if 意图 is None:#没有编辑意图则用stat版本
        写意图={'kind':'replaceIfVersion','version':信息['version']}#按stat版本替换
    else:#按意图版本替换
        写意图={'kind':'replaceIfVersion','version':意图['version']}#按意图版本替换
    try:#按版本守卫替换
        结果=上下文.fs.写文本(#写入替换后全文
            目标,#目标
            之后,#替换后文本
            写意图,#写意图
            信号,#中止信号
...
def 文件内插入(上下文,政策,路径,插入行,新串,执行上下文):#在指定行后插入文本
    ...
    if 信息['type']!='file':#不是普通文件
        raise 文件系统错误('cannot insert into "'+目标['displayPath']+'": not a regular file','FS_NOT_REGULAR_FILE')#拒绝特殊文件
    之前=上下文.fs.读文本(目标,信号)#读出插入前全文
    行列表=之前.split('\n')#按行拆分
    if isinstance(插入行,bool) or not isinstance(插入行,int) or 插入行<0 or 插入行>len(行列表):#行号必须落在[0, 行数]
        raise 错误(#报insert_line越界
            'Invalid `insert_line` parameter: '+str(插入行)+'. It should be within the range of lines of the file: [0, '+str(len(行列表))+']',#允许插在末尾
        )#越界结束
    之后='\n'.join(#插入后的行
        行列表[0:插入行]#插入点之前
        +值.split('\n')#插入文本按行拆开
    ...
    if 意图 is None:#没有编辑意图则用stat版本
        期望={'kind':'replaceIfVersion','version':信息['version']}#按stat版本替换
    else:#按意图版本替换
        期望={'kind':'replaceIfVersion','version':意图['version']}#按意图版本替换
    try:#按版本守卫写入
        结果=上下文.fs.写文本(目标,之后,期望,信号,沙箱政策)#写入插入后全文
    except 文件系统错误 as 异常:#写入失败
        raise 政策.映射错误(异常,沙箱政策)#沙箱拒绝换成带模式标记的错误
    上下文.广播('fs/observed',目标,{'kind':'present','version':结果['version']},执行上下文)#记录存在观察
    return 'The file '+目标['displayPath']+' has been edited successfully.'#面向模型的成功说明
...
def 呈现编辑器调用(参数):#调用中的编辑器卡片
    ...
    if 命令=='create':#创建
        文件文本=参数['file_text'] if 'file_text' in 参数 else None#create的新文件内容
        return {#diff卡片，旧文本为空
            'card':'diff',#diff卡片
    ...
    if 命令=='str_replace':#字面量替换
        旧串=参数['old_str'] if 'old_str' in 参数 else None#被替换原文
        新串=参数['new_str'] if 'new_str' in 参数 else None#替换文本
        return {#diff卡片展示替换前后
            'card':'diff',#diff卡片
    ...
    if 命令=='insert':#插入
        插入行=参数['insert_line'] if 'insert_line' in 参数 else None#insert的插入行
        定位={'path':路径}#定位到该路径
...
def 登记字符串替换编辑器(上下文,已解析配置):#注册str_replace_editor
    ...
    def 执行(参数,执行上下文):#按命令分派
        ...
        if 命令=='create':#创建
            return 创建文件(上下文,政策,参数['path'],参数['file_text'] if 'file_text' in 参数 else None,执行上下文)#仅在不存在时创建
        if 命令=='str_replace':#字面量替换
            return 文件内替换(#恰好一处替换
                上下文,#插件上下文
                政策,#变更策略
                参数['path'],#目标路径
                参数['old_str'] if 'old_str' in 参数 else None,#原文
                参数['new_str'] if 'new_str' in 参数 else None,#新文
                执行上下文,#运行上下文
            )#replaceInFile结束
        if 命令=='insert':#插入
            return 文件内插入(#在指定行后插入
                ...
                政策,#变更策略
                参数['path'],#目标路径
                参数['insert_line'] if 'insert_line' in 参数 else None,#插入行
                参数['new_str'] if 'new_str' in 参数 else None,#插入文本
                执行上下文,#运行上下文
            )#insertInFile结束
        raise 错误('unsupported str_replace_editor command: '+str(命令))#封闭联合穷尽
    上下文.tools.登记(定义工具({#定义并注册工具
...
def 解析配置(配置值):#填入默认值并校验
    ...
    if 最大 is None:#缺省
        最大=16_000#默认上限
    描述=配置值['description'] if 'description' in 配置值 else None#工具描述
    if 描述 is None:#缺省
        描述=默认描述#默认描述
    if isinstance(最大,bool):#布尔不是整数
        raise 错误('tool-str-replace-editor: maxOutputChars must be a positive safe integer')#加载时拒绝
    if isinstance(最大,float) and 最大.is_integer():#配置入口整值浮点
        最大=int(最大)#收成int
    if not isinstance(最大,int) or 最大<=0 or 最大>9007199254740991:#必须是正的安全整数
        raise 错误('tool-str-replace-editor: maxOutputChars must be a positive safe integer')#加载时拒绝
