...
配置=配置模式#沙箱后端配置与本地后端配置相同（cwd、diffBasisMaxBytes）
...
class 沙箱文件系统(本地文件系统):#沙箱强制文件系统后端
    ...
    def 检查目标(自身,目标,沙箱政策=None):#按策略检查并返回实际变更目标
        ...
        模式=政策['mode']#取出沙箱模式；政策是跨包 dict
        if 模式=='danger-full-access':#全权限则不围栏
            return 目标#原样返回调用方目标
        展示路径=目标['displayPath']#取出展示路径；目标是本包 dict
        ...
        for 根 in 可写根(政策):#遍历策略给出的可写根
            if 是否路径位于下(新鲜['targetKey'],根):#新鲜目标键是否位于该根下
                已包含=True#已包含
                break#不必再看其余根
        if 已包含 is False:#不在任何可写根下则拒绝
