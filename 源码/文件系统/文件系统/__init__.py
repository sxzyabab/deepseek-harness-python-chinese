"""文件系统服务：稳定目标身份、进程路径、file URI、包含关系、文本/字节读写与原子变更。读窗口与观察策略在消费方；编辑与版本检查共享同一临界区。"""
from ...依赖 import cordis#外部依赖胶水
服务=cordis.服务#从Cordis导入服务基类
...
__all__=[#仅中文公开名；Cordis 槽英文别名不入表
    '目标键','版本令牌','文件系统错误','文件系统观察种类',
    '文件系统观察存在字段','文件系统观察缺失字段','文件系统目标字段',
    '文件系统信息字段','文件系统信息类型','文件系统路径信息字段','文件系统路径信息类型',
    '文件系统目录项字段','文件系统目录项类型','文件系统写意图种类',
    '文件系统写意图创建字段','文件系统写意图替换字段','文件系统写结果字段',
    '文件系统写操作','文件系统编辑请求字段','文件系统编辑结果字段','文件系统错误码',
    '文件系统','默认',
]#公开面结束
...
#事件 fs/observed(target, observation, actor) @mode emit：记录一次权威的正/负观察。监听器须同步；抛错会使工具调用失败。参数 target 为被观察目标，observation 为带版本的存在或确认缺失，actor 为观察时的工具执行上下文；缺席时记录没有用处。
...
class 文件系统(服务):#文件系统服务定义抽象类
    ...
    def 解析(自身,路径,选项=None):#把路径解析为稳定目标
        """把路径解析成稳定目标。可能做 I/O。相对路径相对选项 cwd；选项 signal 可中止。同一文件同一 targetKey。"""
        raise NotImplementedError('FileSystem.resolve')

    def 进程路径(自身,目标):#返回执行世界中的规范进程路径
        """返回此文件系统执行世界中子进程可以打开的规范绝对路径。该路径有意与目标键分开：消费方可把此值传给另一 OS 能力，但必须继续把目标键当作不透明。"""
        raise NotImplementedError('FileSystem.processPath')

    def 宿主路径转进程路径(自身,宿主路径):#把宿主绝对路径映到执行世界
        """把 harness 宿主上的绝对路径映到本文件系统执行世界中的同一文件。基类无映射；宿主后端或显式共享后端覆盖。宿主路径是宿主文件系统中的绝对路径。返回同一文件的进程路径；本执行世界读不到该宿主文件时为空。"""
        return None#基类无映射

    def 文件网址(自身,目标):#返回目标的规范file URI
        """返回此文件系统执行世界中目标的规范 file: URI。后端拥有 URI 编码，因为宿主平台可能与执行平台不同。"""
        raise NotImplementedError('FileSystem.fileUrl')

    def 包含(自身,父目标,子目标):#判断子目标是否位于父目标之下
        """测试规范包含关系，不暴露也不解析后端目标键。两个目标必须来自此提供方。子目标是父目标或其后代时为真。"""
        raise NotImplementedError('FileSystem.contains')
    ...
    def 链接状态(自身,路径,选项=None,信号=None):#不跟随末段符号链接的路径元数据
        ...
        raise NotImplementedError('FileSystem.lstat')

    def 读文本(自身,目标,信号=None):#读取整个文本文件
        """把整个普通文本文件读成单个已解码字符串。"""
        raise NotImplementedError('FileSystem.readText')

    def 流文本(自身,目标,信号=None):#流式读取整个文本文件
        """以已解码文本块迭代读取整个普通文本文件，文本语义与读文本相同，用于大文件。后端拥有跨块 UTF-8 解码与二进制拒绝，因此策略层从不接触原始字节。"""
        raise NotImplementedError('FileSystem.streamText')

    def 读字节(自身,目标,信号,最大字节):#按字节上限读取原始内容
        """以原始字节读取整个普通文件，不做解码或二进制拒绝。上限放在此处，使后端永远不能缓冲无界文件：已知或发现超过最大字节的目标以 FS_TOO_LARGE 失败，而不是返回截断结果。"""
        raise NotImplementedError('FileSystem.readBytes')

    def 列目录(自身,目标,信号=None):#列举目录直接子项
        """以稳定名称顺序列举目录的直接子项。只返回已解析子目标加廉价元数据；从不读取文件内容。"""
        raise NotImplementedError('FileSystem.listDir')
    ...
    def 写文本(自身,目标,内容,期望=None,信号=None,沙箱政策=None):#原子写入整文件文本
        ...
        raise NotImplementedError('FileSystem.writeText')
    ...
    def 编辑文本(自身,目标,编辑,期望=None,信号=None,沙箱政策=None):#原子字面量编辑
        ...
        raise NotImplementedError('FileSystem.editText')
