...
行搜索最大命中数=250#内联命中默认上限
行搜索最大行字节=2000#单行预览默认字节上限
...
def 解析行搜索命中(标准输出):#解析完整--json stdout为扁平命中
...
def 格式化行搜索输出(保留,溢出引用):#格式化面向模型的grep结果
    ...
    正文=格式化行搜索命中(取字段(保留,'items') or [])#按文件分组的正文
...
def 格式化已保留行搜索(保留,溢出引用=None):#零命中与有命中的统一入口
    """为 Native 面格式化一份已保留的命中列表。"""
    if 取字段(保留,'seen')==0:#没有任何命中
        return 'No matches found'#零命中文案
    return 格式化行搜索输出(保留,溢出引用)#有命中则走完整格式化

def 呈现行搜索调用(参数):#调用中的搜索卡片
...
def 呈现行搜索结果(参数,结果):#完成调用后的搜索卡片展示
...
def 应用行搜索工具(上下文,上限):#注册grep工具与系统提示
    """注册 grep 工具及其系统提示指引。"""
    ...
    def 渲染(参数,值):#把规范值渲染成文本块
        """把规范值渲染成文本块。"""
        _=参数#渲染不依赖原始参数
        return [{'type':'text','text':格式化已保留行搜索(保留grep命中(取字段(值,'matches') or [],取字段(上限,'maxMatches'),取字段(上限,'maxLineBytes')))}]#按上限保留并格式化
    ...
    def 执行(参数,执行上下文):#执行一次grep
        """校验后拉起打包的 rg --json。"""
        输入=解析行搜索参数(参数)#校验参数
        运行=解开(跑ripgrep(上下文,执行上下文,'grep',构造行搜索命令(输入),取字段(上限,'rawOutputMaxBytes'),取字段(上限,'graceMs'),取字段(上限,'stderrMaxBytes')))#拉起打包的rg --json
        if 取字段(运行,'noMatches'):#exit 1表示成功但零命中
            return 已兑现({'matches':[]})#零结果
        全部=[]#收集工作目录相对路径的命中
        for 原始 in 解析行搜索命中(取字段(运行,'stdout') or ''):#解析全部match记录
            全部.append({#一条展示用命中
    ...
    工具=定义工具({#定义面向模型的grep工具
        ...
        'execute':执行,#执行一次grep
        'presentCall':呈现行搜索调用,#调用中卡片
        'presentResult':呈现行搜索结果,#完成后卡片
    })#defineTool结束
    ...
    def 事后溢出(执行,结果,下一步,*剩余):#超额时把完整结果溢出保存
        ...
        溢出引用=解开(尽力保存格式化结果(#尽力保存完整格式化结果
            ...
            'Found '+str(len(命中们))+' '+命中名词(len(命中们))+'\n\n'+格式化行搜索命中(已预览全部),#完整分组正文
        ))#trySaveFormattedResult结束
        接受={#接受并替换为带溢出定位的文本
            'kind':'accept',#接受此结果
            'content':[{#面向模型的文本内容
                'type':'text',#文本块
                'text':格式化已保留行搜索(保留grep命中(命中们,取字段(上限,'maxMatches'),取字段(上限,'maxLineBytes')),溢出引用),#内联保留页加溢出页脚
            }],#content结束
        }#替换结果
