"""`grep` 与 `glob` 在结果时刻的搜索卡片呈现。两种工具都落到同一种 `card: 'search'` 渲染意图，再用 `shape` 判别两种变体：`grep` 按文件分组投影命中，`glob` 投影扁平路径列表。本模块拥有各工具声明的值→`presentationMeta` 投影，以及各工具 `presentResult` 在回放时读回的防御性 `meta`→视图收窄。
