...
def 解析通配参数(参数):#校验并接受glob参数
    ...
    if len(参数['pattern'].strip())==0:#空白pattern拒绝；判 length
        raise 搜索工具错误('pattern must be a non-empty string')#空白pattern拒绝
    出={'pattern':参数['pattern']}#必填pattern
    if 'path' in 参数:#给出了path
        路径=参数['path']#取出path
        if len(路径.strip())==0:#给出的path不得空白；判 length
            raise 搜索工具错误('path must be a non-empty string when given')#空白path拒绝
        出['path']=路径#带上path
    return 出#已接受的输入
...
def 构造通配命令(输入):#构造rg --files argv
    ...
    段列表=[#固定的rg --files参数
        '--files',#列出文件而非搜索内容
        '--glob='+输入['pattern'],#模型给定的glob
        '--sort=modified',#按修改时间排序
        '--no-ignore',#不尊重ignore文件
        '--hidden',#包含隐藏文件
    ]#固定段结束
    for 名 in 通配版本控制排除:#每个VCS名展开两条排除glob
        段列表.append('--glob=!**/'+名)#任意深度目录剪枝
        段列表.append('--glob=!**/'+名+'/**')#根在目录内时仍排除内容
    if 'path' in 输入:#有path则放在--之后以免被当成旗标
        段列表.append('--')#分隔旗标与位置参数
        段列表.append(输入['path'])#搜索根
    return 段列表#完整argv（不含二进制）
...
def 相对搜索根(路径,根):#相对搜索根的展示路径
    ...
    if len(修剪根)==0:#根只剩分隔符则只剥路径前导分隔符；判 length
...
def 跨顶层抽样(路径列表,最大条数,根='.'):#跨顶层条目轮询抽样
    """用跨完整结果顶层条目的轮询选择超额结果的内联页，而不是取其头部。每个顶层条目先分到一个名额，再给第二个；耗尽的组退出。"""
    组表={}#顶层键到该组全部路径
    活跃=[]#仍有未取路径的组
    for 路径 in 路径列表:#按修改时间顺序建组
        键=顶层段(相对搜索根(路径,根))#相对搜索根的顶层段
        if 键 not in 组表:#首次见到该顶层
            条目列表=[路径]#该组的路径列表
            组表[键]=条目列表#登记新组
            活跃.append({'key':键,'items':条目列表,'index':0,'current':路径})#第一轮即可取
        else:#已有该顶层
            组表[键].append(路径)#追加到组内，组内保持修改时间顺序
    已取={}#已抽入内联页的按组路径
    计数=0#已抽路径数
    while len(活跃)>0 and 计数<最大条数:#还有组且未抽满；判 length
        下一活跃=[]#下一轮仍有剩余的组
        for 组 in 活跃:#本轮每个组取一条
            ...
            下标=组['index']+1#组内下一候选
            条目列表=组['items']#组内全部路径
            if 下标<len(条目列表):#未耗尽；判 length
                下一活跃.append({'key':键,'items':条目列表,'index':下标,'current':条目列表[下标]})#进入下一轮
        活跃=下一活跃#只保留仍有剩余的组
    页=[]#展平页
    for 键 in 已取:#保持首次出现的顶层顺序
        页.extend(已取[键])#该组已抽路径
    return {'items':页,'shown':len(已取),'total':len(组表)}#展平页并报告顶层覆盖

def 格式化通配页(条目列表,已见,溢出引用,依据):#一页路径加页脚
    """格式化一页有界路径及其完整排序结果的恢复路径。"""
    正文='\n'.join(条目列表)#路径每行一条
    if 溢出引用 is not None:#已保存则给定位与取回提示
        恢复='Full sorted result stored at: '+溢出引用['locator']+'. '+溢出引用['retrievalHint']#定位与取回
    else:#未保存则建议收窄搜索
        恢复='The complete result could not be saved; narrow pattern or path to see more.'#收窄建议
    return 正文+'\n\n(Showing '+str(len(条目列表))+' of '+str(已见)+' paths'+依据+' '+恢复+')'#正文加展示条数与恢复

def 格式化通配输出(抽样,已见,溢出引用):#格式化抽样页与恢复说明
    """格式化截断抽样页及其完整结果恢复路径。扁平结果保留普通页脚，因为其抽样就是修改时间头部。"""
    if 抽样['total']==已见:#完整结果每个路径都是独立顶层时
        依据='.'#扁平结果：页脚用句点接恢复句
    else:#跨顶层抽样说明
        依据=', sampled across '+str(抽样['shown'])+' of the '+str(抽样['total'])+' top-level entries this pattern matched instead of taken in modification-time order.'#跨顶层说明
        if 抽样['shown']<抽样['total']:#未覆盖全部顶层时建议收窄path
            依据=依据+' Narrow path to inspect a specific subtree.'#收窄建议
    return 格式化通配页(抽样['items'],已见,溢出引用,依据)#交给统一页格式化

def 渲染通配路径(路径列表,上限,根,溢出引用=None):#按上限渲染glob路径
    """相对其搜索根，为 Native 面有界并格式化一份规范路径列表。"""
    if len(路径列表)==0:#零结果；判 length
        return 'No files found'#零结果文案
    if len(路径列表)<=上限['maxResults']:#未超额则整份按修改时间列出；判 length
        return '\n'.join(路径列表)#整份列出
    if not 上限['sampleOverCapGlobResults']:#部署选择取修改时间头部
        return 格式化通配页(路径列表[0:上限['maxResults']],len(路径列表),溢出引用,'.')#取头部并带恢复页脚
...
def 通配卡片页(路径列表,上限,根):#卡片与文本共用的内联页
    ...
    if not 上限['sampleOverCapGlobResults']:#取修改时间头部
        return {'items':路径列表[0:上限['maxResults']],'truncated':True}#头部截断
...
def 呈现通配结果(参数,结果):#完成调用后的搜索卡片展示
    ...
    if 视图 is None or 视图['shape']!='paths':#必须是paths形态
        return None#回退通用卡片
    return 视图#返回搜索卡片

def 应用通配工具(上下文,上限):#注册glob工具与系统提示
    """注册 glob 工具及其系统提示指引。"""
    if 上限['sampleOverCapGlobResults']:#超额时系统提示用语
        超额指引='while a larger one is sampled across top-level entries, so it spans the tree instead of one subtree.'#跨顶层抽样
        超额描述='a larger result instead returns '+str(上限['maxResults'])+' paths sampled across top-level entries'#跨顶层抽样
    else:#取修改时间头部
        超额指引='while a larger one keeps the modification-time-ordered head.'#取修改时间头部
        超额描述='a larger result returns the first '+str(上限['maxResults'])+' paths in modification-time order'#取修改时间头部
    ...
    def 展示元(参数,值):#投影搜索卡片meta
        ...
        return glob搜索元({'items':页['items'],'truncated':页['truncated'],'seen':len(值['paths'])},上限['maxMetaBytes'])#再按字节预算截meta
    def 执行(参数,执行上下文):#执行一次glob
        """校验后拉起打包的 rg --files。"""
        输入=解析通配参数(参数)#校验参数
        运行=执行ripgrep(上下文,执行上下文,'glob',构造通配命令(输入),上限['rawOutputMaxBytes'],上限['graceMs'],上限['stderrMaxBytes'])#拉起打包的rg --files
        根='.' if 'path' not in 输入 else 改成工作目录相对(输入['path'],运行['workdir'])#展示用搜索根
        if 运行['noMatches']:#exit 1表示成功但零文件
            return {'root':根,'paths':[]}#空路径
        全部=[]#收集工作目录相对路径
        for 行 in 运行['stdout'].split('\n'):#rg --files每行一条路径
            if len(行)==0:#空行跳过；判 length
                continue#下一行
            全部.append(改成工作目录相对(行,运行['workdir']))#绝对路径改成工作目录相对
        return {'root':根,'paths':全部}#返回全部路径，截断由事后策略处理
    工具=定义工具({#定义面向模型的glob工具
        ...
        'description':'Find files whose paths match a glob pattern. Returns matching file paths — never directories — '#工具描述：按glob发现文件
            ...
            +'Up to '+str(上限['maxResults'])+' paths come back in modification-time order; '+超额描述+', '#内联上限与超额策略
    ...
    def 事后臂(执行上下文,结果,下一步,*位置参数):#超额时把完整结果溢出保存
        """超额时把完整结果溢出保存。"""
        _=位置参数#瀑布其余参数不用
        决策=下一步()#先让下游策略处理
        ...
        if 值 is None:#无权溢出则原样返回下游决策
            return 决策#原样
        路径列表=值['paths']#规范值中的全部路径
        if len(路径列表)<=上限['maxResults']:#未超额则无需溢出；判 length
            return 决策#原样
        溢出引用=尽力保存格式化结果(上下文,执行上下文,'glob-results.txt','\n'.join(路径列表))#尽力保存完整排序列表
        接受={#接受并替换为带溢出定位的文本
            'kind':'accept',#接受此结果
            'content':[{'type':'text','text':渲染通配路径(路径列表,上限,值['root'],溢出引用)}],#内联页加溢出页脚
        }#替换骨架
        if 'additionalContexts' in 决策:#下游附加上下文原样带上
            接受['additionalContexts']=决策['additionalContexts']#附加上下文
        return 接受#post-execute替换结果
    上下文.监听('tools/post-execute',事后臂)#post-execute监听
