...
依赖=['tools']#本插件包装（tools/execute）并读取（get）的工具注册表服务

__all__=['工具超时码','名称','依赖','应用']#仅中文公开名；Cordis 槽英文别名不入表
...
def 应用(上下文):
    ...
    def 执行臂(执行,下一步,*剩余):
        ...
        定义=上下文.tools.获取(执行['name'],执行['agent'] if 'agent' in 执行 else None)#读取工具声明
        ...
        try:#委托下游执行
            ...
            写信号(执行,上游)#还原调用方信号
            句柄.释放()#清除已武装定时器
    上下文.监听('tools/execute',执行臂)#安装环绕监听
