...
def 解析预览fixture清单(值):#解析并校验
    ...
    if 记录 is None or 记录.get('version')!=预览fixture清单版本 or not isinstance(记录.get('fixtures'),list):#版本或列表非法
        raise 运行时错误(f'preview fixture manifest must use version {预览fixture清单版本}')#拒绝
    条目列表=[]#输出列表
    已见标识=set()#已见id
    for 原始 in 记录['fixtures']:#逐条校验
        ...
        覆盖地址列表=[层 for 层 in 覆盖层 if isinstance(层,str) and len(层)>0] if isinstance(覆盖层,list) else []#非空串
