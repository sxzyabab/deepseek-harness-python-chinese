...
def 服务文件系统调用(文件系统,操作,参数列表):#服务FS调用
    ...
    if 操作=='list':#list
        return 文件系统['list'](参数列表[0])#list
    if 操作=='readText':#读文本
        return 文件系统['readText'](参数列表[0])#读
    if 操作=='writeText':#写文本
        文件系统['writeText'](参数列表[0],参数列表[1],参数列表[2] if len(参数列表)>2 else False)#写
        return None#无返回值
    if 操作=='mkdir':#建目录
        文件系统['mkdir'](参数列表[0],参数列表[1])#mkdir
        return None#无返回值
    if 操作=='remove':#移除
        文件系统['remove'](参数列表[0],参数列表[1])#remove
        return None#无返回值
    if 操作=='rename':#重命名
        文件系统['rename'](参数列表[0],参数列表[1])#rename
        return None#无返回值
...
def 启动worker进程(选项):#Worker进程
    ...
    def 收消息(事件):#收进程帧
        ...
        except Exception as 错误:#服务文件系统调用可能抛运行时错误/KeyError/TypeError，契约未定所以收不窄
...
def 启动内联进程(选项):#内联进程
    ...
    except Exception as 错误:#shell 程序或脚本体什么都可能抛，契约未定所以收不窄
