...
def struct(名称,字段表=None):#结构类型
    """结构类型描述符；大小与对齐按 koffi x64 填充规则。"""
    源=字段表 if isinstance(名称,str) else 名称#字段表或位置参数
    成员表={} if 源 is None else 源#??空表，空字典合法
    偏移=0#当前偏移
    对齐=1#结构对齐
    for 成员 in 成员表.values():#遍历字段
        类型=解析类型(成员)#解析字段类型
...
class _类型表:#按名取类型的代理面
    ...
    def __contains__(自身,属性):#仅原始名
        """是否为已知原始类型名。"""
        return isinstance(属性,str) and 属性 in 原始大小表#仅原始名
