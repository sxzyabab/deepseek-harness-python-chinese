...
class 事件发出器:#事件发出器
    ...
    def 监听(自身,事件,监听器):#注册监听器
        """注册监听器。"""
        列表=自身._注册表.get(事件)#已登记监听器
        if 列表 is None: 列表=[]#??空列表，判的是缺席不是 length
        列表.append(监听器)#追加
        自身._注册表[事件]=列表#写回
        return 自身#链式
    ...
    def 列出监听器(自身,事件):#取监听器副本
        """某一事件的监听器列表副本。"""
        已登记=自身._注册表.get(事件)#已登记监听器
        if 已登记 is None: 已登记=[]#??空列表，判的是缺席不是 length
        return list(已登记)#拷贝
    ...
    emit=发出#Node面
    listeners=列出监听器#Node面
    listenerCount=监听器数量#Node面
    setMaxListeners=设最大监听器#Node面

EventEmitter=事件发出器#Node面别名
__esModule=True#CJS互操作
default={'EventEmitter':事件发出器}#默认导出
