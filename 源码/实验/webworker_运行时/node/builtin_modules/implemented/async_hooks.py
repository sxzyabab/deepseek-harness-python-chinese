...
class 异步本地存储:#异步本地存储
    ...
    def run(自身,存储,回调,*参数):#进入边界运行
        """在操作整个寿命内以可见存储运行回调。"""
        条目={'store':存储}#折叠栈项
        自身._条目列表.append(条目)#压入

        def 移除():#移除折叠项
            """按项身份移除。"""
            for 下标 in range(len(自身._条目列表)-1,-1,-1):#自后向前
                if 自身._条目列表[下标] is 条目:#身份匹配
                    自身._条目列表.pop(下标)#移除
                    return#结束

        环境={'store':存储}#环境槽
        自身._环境列表.append(环境)#压入环境

        def 卸边界():#卸边界
            """卸环境与折叠项，并恢复先前恢复槽。"""
            for 下标 in range(len(自身._环境列表)-1,-1,-1):#自后向前
                if 自身._环境列表[下标] is 环境:#身份匹配
                    自身._环境列表.pop(下标)#移除环境
                    break#结束环境
    ...
    def getStore(自身):#读当前存储
        ...
        if len(自身._环境列表)>0: return 自身._环境列表[-1]['store']#最内环境
        if len(自身._条目列表)>0: return 自身._条目列表[-1]['store']#折叠栈顶
        return None#无
    ...
    def enterWith(自身,存储):#持久进入
        """进入持续到 disable 的边界。"""
        自身._条目列表.append({'store':存储})#压入折叠栈

    def disable(自身):#禁用并清空
        """丢掉每个槽。"""
        自身._条目列表.clear()#清空折叠栈
        自身._覆盖=None#清覆盖
        自身._环境列表.clear()#清空环境
        自身._已恢复=None#清恢复槽
    ...
    def 快照全部():#全实例快照
        """复制每个活动实例的有效存储。"""
        return [{'instance':实例,'store':实例.getStore()} for 实例 in 实例集合]#逐实例
    ...
    def 带上下文运行(快照,回调):#覆盖下运行
        ...
        for 项 in 快照:#安装覆盖
            ...
            先前列表.append({'instance':实例,'before':先前})#记录
        try:#执行
            return 回调()#回调
        finally:#恢复
            for 记录 in 先前列表:#还原覆盖
...
def 在异步上下文根运行(回调):#根上下文运行
    """在根处运行回调：每个实例读 None。"""
    根=[{'instance':实例,'store':None} for 实例 in 异步本地存储.列出活动实例()]#全清快照
