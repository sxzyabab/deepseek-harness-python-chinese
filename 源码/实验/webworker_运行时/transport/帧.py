...
def 解析入站帧(数据):#校验入站帧
    ...
    if not isinstance(标识,(str,int)):#id不可用
        raise 运行时错误(f'webworker tunnel: frame has no usable id: {帧.get("id")!r}')#拒绝
    ...
    if 帧.get('t')=='stream-open':#流打开
        ...
        if not isinstance(端点,str) or len(端点)==0:#端点非法
            raise 运行时错误(f'webworker tunnel: stream {标识} needs a non-empty endpoint')#拒绝
        return {'t':'stream-open','id':标识,'endpoint':端点,'payload':帧.get('payload')}#返回流打开
    if 帧.get('t')!='req':#未知类型
        raise 运行时错误(f'webworker tunnel: unknown frame type {帧.get("t")!r}')#拒绝
    if not isinstance(帧.get('method'),str) or not isinstance(帧.get('url'),str):#方法或URL非法
        raise 运行时错误(f'webworker tunnel: request {标识} needs string method and url')#拒绝
    原始头=帧.get('headers')#头
    if not isinstance(原始头,dict):#头非法
        raise 运行时错误(f'webworker tunnel: request {标识} needs a headers object')#拒绝
    头={}#规范化头
    ...
    if 正文 is not None and not isinstance(正文,(bytes,bytearray,memoryview)):#正文类型错
        raise 运行时错误(f'webworker tunnel: request {标识} body must be an ArrayBuffer')#拒绝
