...
def 转可转移(字节序列):#拷为可转移缓冲
    """把字节拷进精确大小的缓冲，以便可转移。"""
    return bytes(字节序列)#精确拷贝
...
class 缓冲汇:#缓冲汇
    ...
    def _记录(自身,调用):#记录或转发
        ...
        if 自身._目标 is None:#尚未冲刷则入队
            自身._调用列表.append(调用)#入队
        else:#已冲刷则立即
            调用()#立即

    def _记头(自身,状态,头列表):#记头
        """延后或立即发头并流式落定。"""
        def 调用():#延后调用
            """转发头。"""
            if 自身._目标 is not None:#有目标
                自身._目标['head'](状态,头列表)#发头
        自身._记录(调用)#记录
...
class 隧道服务器:#隧道服务器
    ...
    def _服务流(自身,帧):#服务开流
        ...
        except Exception as 错误:#openStream 迭代器体什么都可能抛，契约未定所以收不窄
            if not 已中止[0]:#未中止才报错
    ...
    def _为请求建汇(自身,标识):#为请求建汇
        ...
        def 发头(状态,头列表):#发流式头
            """头帧。"""
            自身._发送({'t':'res-head','id':标识,'status':状态,'headers':头列表})#头帧
        def 发块(字节序列):#发块
            """块帧。"""
            缓冲=转可转移(字节序列)#可转移
