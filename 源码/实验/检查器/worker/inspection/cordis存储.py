...
class Cordis树存储:#Cordis树存储
    ...
    def 关闭(自身,源,原因):#关闭源
        ...
        if 当前 is None or 当前['source']['generation']!=源['generation'] or 当前['connection']['state']=='disconnected':#已断或不符
            return#返回
        自身._树表[源['sourceId']]={**当前,'connection':{'state':'disconnected','reason':原因}}#写断开态
    ...
    def _安装(自身,源,快照):#安装快照
        ...
        自身._树表[源['sourceId']]={'source':源,'snapshot':快照,'connection':{'state':'connected'},'nodesByObject':索引}#写入
        return True#已变更
