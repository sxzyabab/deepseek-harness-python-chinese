...
class 检查器查询路由:#查询路由
    ...
    def 打开(自身,传输):#打开对端
        ...
        def 已登记(已接受):#是否已登记
            """代数匹配。"""
            活动=自身._按源活动[已接受['sourceId']] if 已接受['sourceId'] in 自身._按源活动 else None#取
            return 活动 is not None and 活动['peer'] is 对端 and 活动['generation']==已接受['generation']#匹配
        def 注销():#注销
            """移除对端活动。"""
            自身._对端集合.discard(对端)#移除对端
            for 源id,活动 in list(自身._按源活动.items()):#扫活动
                if 活动['peer'] is 对端:#清除
                    del 自身._按源活动[源id]#删除
    ...
    def 断开(自身,源):#断开
        """源注册表关闭一个代数时吊销查询访问。"""
        活动=自身._按源活动[源['sourceId']] if 源['sourceId'] in 自身._按源活动 else None#取活动
...
class 检查器查询对端:#查询对端
    ...
    def _执行(自身,帧,已接受):#执行查询
        ...
        try:#执行
            ...
            if _json字节长(响应)>自身._最大帧字节:#结果过大
                ...
                return#返回
            自身._投递(响应)#投递
        except (检查器错误,ValueError,KeyError,TypeError) as 错误:#执行失败
            if 自身._可回复(帧,已接受):#可回复
                自身._发送失败(帧,'internal-error',str(错误))#失败
        finally:#清理
            if 帧['requestId'] in 自身._进行中 and 自身._进行中[帧['requestId']] is 已接受:#仍进行中
    ...
    def _拒绝畸形(自身,值,错误):#拒绝畸形
        ...
        except (检查器错误,ValueError,KeyError,TypeError):#无身份
    ...
    def _可回复(自身,帧,已接受):#可否回复
        """检查代数与进行中。"""
        return not 自身._已关闭 and 自身._已接受 is 已接受 and 自身._已登记(已接受) and 帧['requestId'] in 自身._进行中 and 自身._进行中[帧['requestId']] is 已接受#条件

    def _投递(自身,帧):#投递响应
        """发送或关传输。"""
        try:#发送
            自身._传输['send'](帧)#发送
        except (检查器错误,ValueError,KeyError,TypeError,OSError) as 错误:#发送失败
            自身._拒绝传输(1011,str(错误))#关传输
    ...
    def _拒绝传输(自身,码,原因):#拒绝传输
        ...
        except (检查器错误,ValueError,KeyError,TypeError,OSError):#载体已废
