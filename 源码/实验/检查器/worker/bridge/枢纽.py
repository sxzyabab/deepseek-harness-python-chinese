...
class 检查器源注册表:#源注册表
    ...
    def __init__(自身,消费者列表,最大帧字节,每帧记录上限):#构造
        """保存消费者与上限。"""
        自身._消费者列表=消费者列表#消费者
    ...
    def 接收(自身,连接,值):#接收帧
        ...
        try:#解析应用
            ...
            自身._应用(连接,帧)#应用
        except (ValueError,检查器错误) as 错误:#畸形帧
            信息=str(错误)#信息
    ...
    def 断开(自身,连接,原因):#断开连接
        """移除已关闭连接承载的每一个代数。"""
        for 源id in list(自身._源表.keys()):#扫源
            状态=自身._源表[源id]#取状态
            if 状态['connection'] is not 连接:#非本连接
                continue#跳过
            del 自身._源表[源id]#删除
            for 消费者 in 自身._消费者列表:#通知消费者
    ...
    def 关闭(自身):#关闭注册表
        """关闭每一个源并遗忘全部状态。"""
        for 状态 in list(自身._源表.values()):#扫源
    ...
    def _替换记录(自身,状态,帧):#替换记录
        ...
        记录列表=[{**记录,'sequence':帧['nextSequence']+索引} for 索引,记录 in enumerate(帧['records'])]#带序号
        for 消费者 in 自身._消费者列表:#替换
            消费者.替换(状态['source'],记录列表)#替换记录
    ...
    def _追加记录(自身,连接,状态,帧):#追加记录
        ...
        记录列表=[{**记录,'sequence':帧['firstSequence']+索引} for 索引,记录 in enumerate(帧['records'])]#带序号
        状态['expectedSequence']=帧['firstSequence']+len(帧['records'])#推进期望
        for 消费者 in 自身._消费者列表:#追加
            消费者.追加(状态['source'],记录列表)#追加
        自身._计数(状态,帧['records'])#计数
        连接['send']({#确认追加
    ...
    def _发出(自身,事件):#发出事件
        ...
        for 监听 in list(自身._事件监听):#扫监听
            try:#隔离调用
                监听(事件)#回调
            except Exception:#消费者回调什么都可能抛，收不窄
