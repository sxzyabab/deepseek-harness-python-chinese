...
def 解析求值(参数):#解析求值
    ...
    if not isinstance(参数.get('expression'),str):#表达式
        raise 检查器错误('Runtime.evaluate expression must be a string')#抛错
    ...
    if 超时 is not None and (not isinstance(超时,(int,float)) or isinstance(超时,bool) or not (超时==超时) or 超时<0):#非法
        raise 检查器错误('Runtime.evaluate timeout must be a non-negative finite number')#抛错
    请求={#请求
...
def 解析取属性(参数):#解析取属性
    ...
    if not isinstance(参数.get('objectId'),str):#类型
        raise 检查器错误('Runtime.getProperties objectId must be a string')#抛错
    return {#结果
...
def 解析调函数(参数):#解析调函数
    ...
    if not isinstance(参数.get('functionDeclaration'),str):#函数声明
        raise 检查器错误('Runtime.callFunctionOn functionDeclaration must be a string')#抛错
    ...
    if 对象id is None and 选择器.get('executionContextId') is None and 选择器.get('uniqueContextId') is None:#要求
        raise 检查器错误('Runtime.callFunctionOn requires objectId or an execution context')#抛错
    if 对象id is not None and (选择器.get('executionContextId') is not None or 选择器.get('uniqueContextId') is not None):#互斥
        raise 检查器错误('Runtime.callFunctionOn objectId and execution context are mutually exclusive')#抛错
    参数列表=[]#参数
    if 'arguments' in 参数:#有参数
        if not isinstance(参数['arguments'],list):#类型
            raise 检查器错误('Runtime.callFunctionOn arguments must be an array')#抛错
        参数列表=[解析调用参数(项) for 项 in 参数['arguments']]#解析每项
    结果={**选择器,'arguments':参数列表,'request':{#结果
...
def 解析等Promise(参数):#解析等Promise
    ...
    if not isinstance(参数.get('promiseObjectId'),str):#类型
        raise 检查器错误('Runtime.awaitPromise promiseObjectId must be a string')#抛错
    return {#结果
...
def 解析释放对象(参数):#解析释放对象
    ...
    if not isinstance(参数.get('objectId'),str):#类型
        raise 检查器错误('Runtime.releaseObject objectId must be a string')#抛错
...
def 解析释放对象组(参数):#解析释放组
    ...
    if not isinstance(参数.get('objectGroup'),str):#类型
        raise 检查器错误('Runtime.releaseObjectGroup objectGroup must be a string')#抛错
...
def 解析调用参数(值):#解析调用参数
    ...
    if not 是否普通对象(值):#非对象
        raise 检查器错误('Runtime.callFunctionOn argument must be an object')#抛错
    ...
    if len(出现)>1:#多重
        raise 检查器错误('Runtime.callFunctionOn argument has multiple value representations')#抛错
    if len(出现)==0:#无表示
        return {'kind':'undefined'}#undefined
    if 出现[0]=='value':#JSON值
        if not 是否json值(值['value']):#非JSON
            raise 检查器错误('Runtime.callFunctionOn argument value must be JSON')#抛错
        return {'kind':'value','value':值['value']}#值
    if 出现[0]=='unserializableValue':#不可序列化
        if not isinstance(值['unserializableValue'],str):#类型
            raise 检查器错误('Runtime.callFunctionOn unserializableValue must be a string')#抛错
        return {'kind':'unserializable','value':值['unserializableValue']}#不可序列化
    if not isinstance(值['objectId'],str):#类型
        raise 检查器错误('Runtime.callFunctionOn argument objectId must be a string')#抛错
...
def 解析上下文选择器(参数,数字键):#解析上下文选择器
    ...
    if 数字 is not None and (not isinstance(数字,int) or isinstance(数字,bool) or abs(数字)>9007199254740991):#非安全整数
        raise 检查器错误(f'Runtime {数字键} must be an integer')#抛错
    if 唯一 is not None and not isinstance(唯一,str):#类型
        raise 检查器错误('Runtime uniqueContextId must be a string')#抛错
    if 数字 is not None and 唯一 is not None:#互斥
        raise 检查器错误('Runtime context selectors are mutually exclusive')#抛错
    结果={}#选择器
...
def 可选对象id(值,标签):#可选对象id
    ...
    if not isinstance(值,str):#类型
        raise 检查器错误(f'{标签} must be a string')#抛错
    return 值#返回
...
def 可选json对象(值,键):#可选JSON对象字段
    ...
    if not 是否普通对象(项) or not 是否json值(项):#非法
        raise 检查器错误(f'Runtime {键} must be a JSON object')#抛错
    return {键:项}#字段
