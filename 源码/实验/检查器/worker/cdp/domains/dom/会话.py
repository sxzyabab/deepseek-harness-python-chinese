...
class Cordis_Dom会话:#Cordis DOM会话
    ...
    def _执行(自身,方法,参数):#执行方法
        ...
        if 方法=='DOM.performSearch':#执行搜索
            ...
            节点列表=[自身._节点id(节点) for 节点 in 自身._后端.文档()['byBackendId'].values() if 节点['name']!='#document' and 查询 in 可搜索(节点)]#过滤
            搜索id=f'cordis-search-{自身._下一搜索id}'#搜索id
            自身._下一搜索id+=1#推进
            自身._搜索[搜索id]=节点列表#登记
            return {'searchId':搜索id,'resultCount':len(节点列表)}#结果
    ...
    def _选节点(自身,参数):#选择节点
        """按前端/后端/对象 id 选节点。"""
        if 'nodeId' in 参数 and 参数['nodeId'] is not None:#按前端id
            return 自身._从前端id取(参数['nodeId'])#返回
        if 'backendNodeId' in 参数 and 参数['backendNodeId'] is not None:#按后端id
            标识=cdp后端节点id(参数['backendNodeId'],'backendNodeId')#品牌
            节点=自身._后端.文档()['byBackendId'].get(标识)#查找
            if 节点 is not None:#命中
                return 节点#返回
        if 'objectId' in 参数 and isinstance(参数['objectId'],str):#按对象id
            绑定=自身._对象到节点[cdp字符串id(参数['objectId'],'objectId')] if cdp字符串id(参数['objectId'],'objectId') in 自身._对象到节点 else None#绑定
    ...
    def _修剪文档状态(自身):#修剪文档状态
        ...
        for 对象id,绑定 in list(自身._对象到节点.items()):#扫对象
            节点=文档['byBackendId'][绑定['backendNodeId']] if 绑定['backendNodeId'] in 文档['byBackendId'] else None#节点
            对象=None if 节点 is None else (节点['object'] if 'object' in 节点 else None)#对象
            源=None if 对象 is None else (对象['source'] if 'source' in 对象 else None)#源
            if 源 is not None and 源['sourceId']==绑定['sourceId'] and 源['generation']==绑定['generation']:#仍有效
                continue#跳过
    ...
    def _释放源对象(自身,源):#释放源对象
        """释放某源代数对象映射。"""
        for 对象id,绑定 in list(自身._对象到节点.items()):#扫对象
            if 绑定['sourceId']!=源['sourceId'] or 绑定['generation']!=源['generation']:#不符
                continue#跳过
            del 自身._对象到节点[对象id]#删
            for 组,集合 in list(自身._组到对象.items()):#扫组
                集合.discard(对象id)#删
...
def 呈现(节点):#节点呈现
    """节点呈现。"""
    对象=节点['object'] if 'object' in 节点 else None#对象
