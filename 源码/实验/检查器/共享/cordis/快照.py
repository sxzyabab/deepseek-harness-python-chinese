...
def 解析节点(值,状态,最大节点,深度):#解析树节点
    ...
    if 状态['count']>最大节点:#超节点数
        raise 检查器错误(f'inspector protocol: Cordis tree exceeds {最大节点} nodes')#英文诊断
    if not 是否普通对象(值) or 值.get('kind') not in ('context','fiber'):#种类未知
        raise 检查器错误('inspector protocol: Cordis tree node must have a known kind')#英文诊断
    对象句柄=线上标识(值.get('objectHandle'),'objectHandle')#对象句柄
    if 对象句柄 in 状态['handles']:#重复句柄
        raise 检查器错误('inspector protocol: Cordis tree repeats an object handle')#英文诊断
    状态['handles'].add(对象句柄)#记入句柄
    if not isinstance(值.get('children'),list):#children须数组
        raise 检查器错误('inspector protocol: Cordis tree node children must be an array')#英文诊断
    ...
    if not isinstance(uid,int) or isinstance(uid,bool) or uid<1:#uid非法
        raise 检查器错误('inspector protocol: Cordis Fiber uid must be a positive safe integer')#英文诊断
    if uid in 状态['fiberUids']:#重复uid
        raise 检查器错误('inspector protocol: Cordis tree repeats a Fiber uid')#英文诊断
    状态['fiberUids'].add(uid)#记入uid
    if len(值['children'])!=1:#须一子
        raise 检查器错误('inspector protocol: Cordis Fiber must own exactly one Context')#英文诊断
    上下文=解析节点(值['children'][0],状态,最大节点,深度+1)#解析唯一子
    if 上下文.get('kind')!='context':#子须Context
        raise 检查器错误('inspector protocol: Cordis Fiber child must be a Context')#英文诊断
...
def 解析cordis树快照(值,最大节点):#解析树快照
    ...
    if 记录['schemaVersion']!=cordis树模式版本 or not isinstance(修订,int) or isinstance(修订,bool) or 修订<1 or not isinstance(记录['truncated'],bool):#头非法
        raise 检查器错误('inspector protocol: invalid Cordis tree header')#英文诊断
    状态={'count':0,'handles':set(),'fiberUids':set()}#解析状态
    根=解析节点(记录['root'],状态,最大节点,0)#解析根
    if 根.get('kind')!='context':#根须Context
        raise 检查器错误('inspector protocol: Cordis tree root must be a Context')#英文诊断
    return cordis树快照(#快照
