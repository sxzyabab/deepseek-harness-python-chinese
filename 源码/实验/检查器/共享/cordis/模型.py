...
def 解析节点(值,状态,深度):#解析树节点
    ...
    if not 是否普通对象(值) or 值.get('kind') not in ('context','fiber'):#种类未知
        raise 检查器错误('inspector protocol: Cordis runtime node must have a known kind')#英文诊断
    ...
    if not isinstance(记录.get('children'),list):#children须数组
        raise 检查器错误('inspector protocol: Cordis runtime node children must be an array')#英文诊断
    ...
    if not isinstance(uid,int) or isinstance(uid,bool) or uid<1 or len(记录['children'])!=1:#非法
        raise 检查器错误('inspector protocol: invalid Cordis runtime Fiber')#英文诊断
    if uid in 状态['fiberUids']:#重复uid
        raise 检查器错误('inspector protocol: Cordis runtime tree repeats a Fiber uid')#英文诊断
    状态['fiberUids'].add(uid)#记入
    上下文=解析节点(记录['children'][0],状态,深度+1)#解析唯一子
    if 上下文.get('kind')!='context':#子须Context
        raise 检查器错误('inspector protocol: Cordis runtime Fiber child must be a Context')#英文诊断
...
def 解析领域(值,种类):#解析单个界域
    ...
    if 源.get('kind')!=种类 or not isinstance(标签,str) or len(标签)==0 or len(标签)>256:#源形状非法
        raise 检查器错误(f'inspector protocol: invalid {种类} Cordis runtime source')#英文诊断
    修订=记录['revision']#修订号
    if not isinstance(修订,int) or isinstance(修订,bool) or 修订<1 or not isinstance(记录['truncated'],bool):#头非法
        raise 检查器错误('inspector protocol: invalid Cordis runtime realm header')#英文诊断
    根=解析节点(记录['root'],{'fiberUids':set()},0)#解析根节点
    if 根.get('kind')!='context':#根须为Context
        raise 检查器错误('inspector protocol: Cordis runtime root must be a Context')#英文诊断
...
def 解析cordis运行时树(值):#解析运行时树
    ...
    客户端列表=[解析领域(项,'client') for 项 in 记录['clients']]#解析各Client
    源标识集合=set()#已见源标识
    for 领域 in (([宿主]+客户端列表) if 宿主 is not None else 客户端列表):#扫描全部界域
        if 领域.source.sourceId in 源标识集合:#重复sourceId
            raise 检查器错误('inspector protocol: Cordis runtime tree repeats a sourceId')#英文诊断
        源标识集合.add(领域.source.sourceId)#记入
