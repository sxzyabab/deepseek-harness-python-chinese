...
class 领域对象注册表:#界域对象注册表
    ...
    def 解析(自身,句柄):#按句柄解析
        """解析一个当前不透明句柄。"""
        return 自身.保留[句柄] if 句柄 in 自身.保留 else None#查强引用表

    def 识别(自身,值):#识别保留对象
        """识别最新快照所保留的一个对象。"""
        if 值 is None:#空
            return None#未识别
        句柄=自身.已知[id(值)] if id(值) in 自身.已知 else None#查已知句柄
        if 句柄 is not None and 句柄 in 自身.保留 and 自身.保留[句柄] is 值:#命中保留
    ...
    def 关闭(自身):#关闭注册表
        ...
        if 自身.已释放:#幂等
            return#已关
        自身.已释放=True#标记释放
        取全局注册表().pop(自身.id,None)#从全局表删除
        自身.保留.clear()#清空强引用

    def 保留对象(自身,值,下一表):#保留对象
        """在一次待定世代中分配稳定句柄并保留一个值。"""
        身份=id(值)#对象身份
        句柄=自身.已知[身份] if 身份 in 自身.已知 else None#已有句柄
        if 句柄 is None:#首次见到
