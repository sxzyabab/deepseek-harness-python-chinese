...
def 解析记录(值):#解析观测记录
    ...
    if (not 是否普通对象(值) or not isinstance(值.get('monotonicMs'),(int,float)) or isinstance(值.get('monotonicMs'),bool)
        ...
        raise 检查器错误('inspector protocol: invalid observation record')#英文诊断
...
def 解析打开(值):#解析打开帧
    ...
    if 种类 not in ('host','client'):#种类非法
        raise 检查器错误('inspector protocol: invalid source kind')#英文诊断
    if not isinstance(源.get('label'),str) or len(源['label'])==0 or len(源['label'])>256:#标签非法
        raise 检查器错误('inspector protocol: source label must contain 1 to 256 characters')#英文诊断
    if not isinstance(源.get('timeOriginMs'),(int,float)) or isinstance(源.get('timeOriginMs'),bool) or not (源['timeOriginMs']==源['timeOriginMs']):#时间原点非法
        raise 检查器错误('inspector protocol: source timeOriginMs must be finite')#英文诊断
    if not isinstance(源.get('capabilities'),list):#能力须数组
        raise 检查器错误('inspector protocol: source capabilities must be an array')#英文诊断
    能力列表=[解析源能力(项) for 项 in 源['capabilities']]#解析能力
    能力类型=set()#已见能力类型
    for 能力 in 能力列表:#逐能力
        if 能力['type'] in 能力类型:#重复类型
            raise 检查器错误(f'inspector protocol: source declares {能力["type"]} more than once')#英文诊断
        能力类型.add(能力['type'])#记入
    if 种类!='client' and len(能力列表)>0:#Host不得声明Client能力
        raise 检查器错误('inspector protocol: Host sources cannot declare Client capabilities')#英文诊断
    主题列表=[]#主题
    for 主题 in 值['topics']:#校验主题
        if not isinstance(主题,str) or len(主题)==0 or len(主题)>128:#主题非法
            raise 检查器错误('inspector protocol: every source topic must contain 1 to 128 characters')#英文诊断
        主题列表.append(主题)#合法主题
...
def 解析记录帧(值,最大记录,替换):#解析记录帧
    ...
    if not isinstance(值.get('records'),list) or len(值['records'])>最大记录:#记录超限
        raise 检查器错误(f'inspector protocol: source batch exceeds {最大记录} records')#英文诊断
    记录列表=[解析记录(项) for 项 in 值['records']]#解析记录
    公共={'v':检查器协议版本,'sourceId':源标识(值['sourceId']),'generation':世代(值['generation']),'records':记录列表}#公共字段
    if 替换:#替换
...
def 解析工作者源帧(值):#解析Worker到源帧
    ...
    if 值['t']=='source/rejected':#拒绝帧
        ...
        if 值.get('code') not in ('invalid-frame','version-mismatch','unauthorized') or not isinstance(值.get('message'),str):#码非法
            raise 检查器错误('inspector protocol: invalid source/rejected frame')#英文诊断
...
def 解析源帧(值,最大记录):#解析源到Worker帧
    ...
    if 值.get('v')!=检查器协议版本:#版本不支持
        raise 检查器错误(f'inspector protocol: unsupported version {值.get("v")!r}')#英文诊断
    类型=值.get('t')#类型
    ...
    raise 检查器错误(f'inspector protocol: unknown source frame {类型!r}')#英文诊断
