...
def 解析结果封装(值):#解析结果
    ...
    if not 是否普通对象(值) or not isinstance(值.get('ok'),bool):#须有ok
        raise 检查器错误('inspector protocol: invalid Client source outcome')#英文诊断
    if 值['ok']:#成功
    ...
    if 错误['code'] not in 错误码集合 or not isinstance(错误['message'],str):#错误非法
        raise 检查器错误('inspector protocol: invalid Client source error')#英文诊断
...
def 解析客户端源能力(值):#解析源能力
    ...
    if 记录['type']!='client-sources':#类型须匹配
        raise 检查器错误('inspector protocol: invalid Client Sources capability')#英文诊断
...
def 解析客户端源请求帧(值):#解析源请求
    ...
    if 值.get('v')!=检查器协议版本 or 值.get('t')!='client-sources/request':#信封非法
        raise 检查器错误('inspector protocol: invalid Client source request envelope')#英文诊断
...
def 解析客户端源响应帧(值):#解析源响应
    ...
    if 值.get('v')!=检查器协议版本 or 值.get('t')!='client-sources/response':#信封非法
        raise 检查器错误('inspector protocol: invalid Client source response envelope')#英文诊断
...
def 解析客户端源会话关闭帧(值):#解析会话关闭
    ...
    if 值.get('v')!=检查器协议版本 or 值.get('t')!='client-sources/session-closed':#信封非法
        raise 检查器错误('inspector protocol: invalid Client source session close envelope')#英文诊断
