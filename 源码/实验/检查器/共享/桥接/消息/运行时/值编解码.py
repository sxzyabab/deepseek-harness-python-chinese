...
大整数字面量=re.compile(r'-?(?:0|[1-9][0-9]*)n',re.ASCII)#BigInt 字面量
...
def 校验远程对象(值):#校验远程对象表示
    ...
    if 描述符.get('subtype') is not None and 描述符.get('type')!='object':#仅object可有子类型
        raise 检查器错误('inspector protocol: only object RemoteObjects may have a subtype')#英文诊断
    if 描述符.get('preview') is not None and 描述符.get('type')!='object':#仅object可有预览
        raise 检查器错误('inspector protocol: only object RemoteObjects may have a preview')#英文诊断
...
def 解析远程对象描述符(值):#解析远程对象描述符
    ...
    if 记录.get('subtype') is not None and 记录.get('subtype') not in 远程子类型集合:#子类型非法
        raise 检查器错误('inspector protocol: invalid Client RemoteObject subtype')#英文诊断
    if 记录.get('value') is not None and not 是否json值(记录['value']):#值须JSON
        raise 检查器错误('inspector protocol: Client RemoteObject value must be JSON')#英文诊断
    结果={'type':记录['type']}#描述符
    if 记录.get('subtype') is not None:#子类型
        结果['subtype']=记录['subtype']#子类型
...
def 解析属性描述符(值):#解析属性描述符
    ...
    if 数据 and 访问器:#混用非法
        raise 检查器错误('inspector protocol: property descriptor mixes data and accessor fields')#英文诊断
...
def 解析客户端运行时结果(值):#解析Runtime结果
    ...
    if 操作=='get-properties':#取属性
        ...
        if not isinstance(值.get('properties'),list):#须数组
            raise 检查器错误('inspector protocol: properties must be an array')#英文诊断
        内部=值.get('internalProperties')#内部属性
        if 内部 is not None and not isinstance(内部,list):#内部须数组
            raise 检查器错误('inspector protocol: internalProperties must be an array')#英文诊断
    ...
    if 操作=='global-lexical-scope-names':#词法名
        ...
        if not isinstance(值.get('names'),list) or not all(isinstance(名,str) for 名 in 值['names']):#须字符串数组
            raise 检查器错误('inspector protocol: lexical scope names must be strings')#英文诊断
        return {'op':'global-lexical-scope-names','names':值['names']}#词法名结果
    raise 检查器错误(f'inspector protocol: unknown Client Runtime result {操作!r}')#英文诊断
