...
def 解析检查器工作者配置(值):#解析Worker配置
    ...
    if 记录['host']!='127.0.0.1':#主机须回环
        raise 检查器错误('inspector protocol: Worker host must be 127.0.0.1')#英文诊断
    if not isinstance(记录['targetId'],str) or len(记录['targetId'])==0:#目标标识非法
        raise 检查器错误('inspector protocol: Worker targetId must be a non-empty string')#英文诊断
    if not isinstance(记录['clientToken'],str) or len(记录['clientToken'])==0:#令牌非法
        raise 检查器错误('inspector protocol: Worker clientToken must be a non-empty string')#英文诊断
    if not isinstance(记录['clientOrigins'],list) or not all(isinstance(项,str) for 项 in 记录['clientOrigins']):#来源非法
        raise 检查器错误('inspector protocol: Worker clientOrigins must be strings')#英文诊断
    起始端口=自然数(记录['startPort'],'startPort',True)#解析端口
    if 起始端口>65535:#端口上限
        raise 检查器错误('inspector protocol: Worker startPort must not exceed 65535')#英文诊断
    return {#配置对象
        'host':记录['host'],#主机
        'startPort':起始端口,#端口
...
def 解析检查器工作者控制(值):#解析Worker控制
    ...
    if 记录['type']=='ready':#就绪
        ...
        if not isinstance(记录['host'],str) or not isinstance(记录['targetId'],str):#身份非法
            raise 检查器错误('inspector protocol: invalid Worker ready identity')#英文诊断
    ...
    if 记录['type']=='failure':#失败
        ...
        if not isinstance(记录['message'],str):#须字符串
            raise 检查器错误('inspector protocol: invalid Worker failure')#英文诊断
...
def 解析检查器客户端引导(值):#解析Client引导
    ...
    if len(记录['protocol'])==0 or len(记录['protocol'])>256:#协议名长度
        raise 检查器错误('inspector protocol: Client bootstrap protocol must contain 1 to 256 characters')#英文诊断
    引导={#引导对象
    ...
    if 引导['reconnectMaxMs']<引导['reconnectBaseMs']:#重连区间非法
        raise 检查器错误('inspector protocol: reconnectMaxMs must be at least reconnectBaseMs')#英文诊断
    return 引导#引导
