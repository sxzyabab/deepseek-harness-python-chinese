...
from .栈 import 客户端错误栈,空解析脚本#错误栈|空脚本键
...
class 客户端运行时执行器:#Client Runtime执行器
    ...
    def 执行(自身,帧,信号=None,延迟对象提交=False):#执行
        ...
        try:#执行体
            ...
            if 延迟对象提交:#延迟提交
                ...
                会话.提交分配(分配)#提交
            return 响应#返回
        except Exception as 错误:#Client Runtime 命令执行/序列化什么都可能抛，契约未定所以收不窄
            会话.回滚(分配)#回滚
    ...
    def 控制台事件(自身,会话标识,类型,值列表,时间戳,栈=None):#Console事件
        ...
        分配=会话.开始分配()#分配
        try:#序列化
            事件={'type':'console-api','event':{'type':类型,'arguments':会话.全部序列化(值列表,'console',分配),'timestamp':时间戳}}#事件
            if 栈 is not None:#栈
            ...
            会话.提交分配(分配)#提交
            return 事件#返回
        except Exception:#序列化 Console/异常事件可能抛自定义 valueOf/toJSON，契约未定所以收不窄
            会话.回滚(分配)#回滚
            raise#继续抛
