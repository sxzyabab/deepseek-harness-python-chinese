...
def 获取客户端属性(对象存储,命令,最大属性,分配):#取属性
    ...
    if isinstance(值,dict):#映射
        for 名,子 in list(值.items())[:最大属性]:#逐键
            属性列表.append({'name':str(名),'value':对象存储.序列化(子,{},分配),'configurable':True,'enumerable':True,'isOwn':True})#属性
    elif isinstance(值,(list,tuple)):#序列
        for 索引,子 in enumerate(list(值)[:最大属性]):#逐项
            属性列表.append({'name':str(索引),'value':对象存储.序列化(子,{},分配),'configurable':True,'enumerable':True,'isOwn':True})#属性
    return {'properties':属性列表}#属性结果
