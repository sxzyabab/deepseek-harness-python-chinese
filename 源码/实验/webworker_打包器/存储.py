...
def 配置树列表(仓库根):#读CLI包声明的配置树
    ...
    声明=None if dsh块 is None else dsh块.get('configTrees')#可选链，缺席才空
    if 声明 is None:return []#无则空
    if not isinstance(声明,list):#须数组
        raise 打包器错误('vfs image: '+命令行包+' dsh.configTrees must be an array')#须数组
    挂载集合=set()#挂载去重
    结果=[]#输出树
...
def 描述打包(结果,仓库根,输出文件):#把一次打包渲染成构建日志行
    ...
    行列表+=['    '+str(项['bytes']).rjust(9)+' B  '+项['name']+' ('+str(项['count'])+' files)' for 项 in 最重]#最重行
    if len(结果['missing'])>0:#有缺失
        行列表+=['  unresolved dependencies:']+[('    '+项) for 项 in 结果['missing']]#缺失行
    行列表.append('')#尾空行
    return 行列表#要打印的行
