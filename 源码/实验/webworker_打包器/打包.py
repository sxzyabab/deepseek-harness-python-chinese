...
def 树名册(根):#一棵配置树下各合成所点名的包名
    ...
    def 遍历(目录):#递归遍历目录
        for 名字 in os.listdir(目录):#读取目录项
            ...
            收集模块名(yaml.load(open(绝对,'r',encoding='utf-8').read(),Loader=条目列表读取器),名称集合)#解析并收集
    遍历(根)#从根开始走
    return list(名称集合)#展开为数组
...
def 发布过滤器(模式表):#npm files白名单谓词
    ...
    def 加宽(模式):#目录名扩成整树
        return [模式,模式+'/**']#自身与子孙
    正向=[]#正向模式
    for 模式 in 字符串列表:#收集正向
        if not 模式.startswith('!'):正向+=加宽(规范化(模式))#正向
    负向=[]#负向模式
    for 模式 in 字符串列表:#收集负向
...
悬空源映射=re.compile(r'\n//# sourceMappingURL=\S+\s*\Z')#尾部sourceMappingURL注释
...
def 调试器命名器(工作区表,解析自):#构造从镜像键到入口调试器名的映射函数
    ...
    def 命名(键):#按镜像键取调试器名
        ...
        段列表=其余.split('/')#路径段
        包名=(段列表[0]+'/'+段列表[1]) if (len(段列表)>0 and 段列表[0].startswith('@') and len(段列表)>1) else (段列表[0] if len(段列表)>0 else '')#作用域或普通包名
        目录=仓库目录表.get(包名)#仓库相对目录
...
def 扫描镜像(文件表,选项,根包列表,根):#只保留工作线程可达的JavaScript并在途中变换
    ...
    虚拟文件系统=内存vfs()#内存虚拟文件系统
    for 名,字节 in 文件表.items():#把候选灌入VFS
        if 名.endswith('/'):虚拟文件系统.注水目录(根+'/'+名)#目录种子
        else:虚拟文件系统.注水(根+'/'+名,字节)#文件种子
    def 桩():#替换模块桩
        return {}#空对象
    ...
    while len(队列)>0:#BFS扫描
        ...
        导入方=条目['importer']#导入方
        try:#尝试解析
            解析结果=加载器.解析(说明符,来自)#按加载器算法解析
        except Exception as 原因:#加载器.resolve 可能抛运行时错误/KeyError，契约未定所以收不窄
            ...
            else:失败列表.append(导入方+': "'+说明符+'" — '+str(原因))#记入失败
            continue#下一队列项
        种类=解析结果['kind']#解析种类
        if 种类=='static':continue#静态模块无需体
        路径=解析结果['path']#解析到的绝对路径
        ...
        if (not js扩展.search(键)) or 是页面资源(键):#非JS或页面资源
            ...
            continue#下一队列项
        已访问+=1#计入访问
        降级=降低模块源({'filename':'/'+键,'source':解码(字节)})#降级变换
    ...
    if len(失败列表)>0:#存在不可解析请求
        raise 打包器错误(#打包失败
            'vfs image: '+str(len(失败列表))+' unresolvable module request(s); '#失败条数
    ...
    脚本入口数=0#保留的JS入口数
    丢弃=0#丢弃数
    for 名,字节 in 文件表.items():#遍历全部候选
        是js=bool(js扩展.search(名))#是否JS
...
def 丢弃可执行脚本(文件表):#从镜像丢弃可执行脚本
    ...
    def 解码(字节):#解码器
        return 字节.decode('utf-8')#解码
    已丢=[]#已丢弃列表
    for 名 in list(文件表.keys()):#遍历条目名（可删）
        字节=文件表[名]#字节
        if not js扩展.search(名):continue#非JS跳过
...
def 物化(名册,选项):#将每个名册包的依赖闭包物化进镜像
    ...
    while len(队列)>0:#BFS物化
        ...
        名=条目['name']#包名
        来自=条目['from']#来自
        if 名 in 包表 or 名 in 已替换:continue#已处理或已替换则跳过
        ...
        if 名 in 选项['workspaces']:#工作区包
            ...
            收集树(目录,文件表,前缀,工作区保留)#收集工作区树
        else:#外部包
            def 外部保留(相对路径):#按通用排除收集
                return not 已排除(相对路径)#谓词
            收集树(目录,文件表,前缀,外部保留)#收集外部树
        包表[名]=len(文件表)-之前#记录本包贡献数
    ...
    return {'files':文件表,'packages':包表,'missing':缺失}#返回物化结果
...
def 打包虚拟文件系统镜像(选项):#打包一份VFS镜像
    ...
    def 编码(文本):#编码器
        return 文本.encode('utf-8')#编码
    配置树列表=选项['configTrees'] if 选项.get('configTrees') is not None else []#配置树
    for 树 in 配置树列表:#校验每棵配置树存在
        if not os.path.exists(树['directory']):#目录缺失
            raise 打包器错误('vfs image: config tree '+树['mount']+' is missing at '+树['directory'])#抛错
    名册=list(dict.fromkeys(#合并去重名册（保序）
        配置名册(选项['config'])+[名 for 树 in 配置树列表 if 树.get('scanRoster') is True for 名 in 树名册(树['directory'])],#主配置与扫描树
    ))#名册结束
    物化结果=物化(名册,选项)#物化闭包
    文件表=物化结果['files']#镜像文件
    包表=物化结果['packages']#包计数
    缺失=物化结果['missing']#缺失
    文件表[配置路径]=编码(选项['config'])#写入合成配置
    for 树 in 配置树列表:#复制配置树
        def 配置保留(相对路径):#排除谓词
            return not 已排除(相对路径)#谓词
        收集树(树['directory'],文件表,树['mount'],配置保留)#收集
    可执行列表=丢弃可执行脚本(文件表)#丢弃可执行脚本
    根包列表=[名 for 名 in 包表.keys() if 名 in 选项['workspaces']]#工作区根包
    扫描=扫描镜像(文件表,选项,根包列表,根)#可达性扫描
    扫描后=扫描['swept']#扫描后文件
    ...
    },indent=2,ensure_ascii=False,separators=(',',':'),allow_nan=False)+'\n')#JSON格式化并编码
    for 目录 in (选项['emptyDirectories'] if 选项.get('emptyDirectories') is not None else 镜像空目录列表):#创建空目录
        扫描后[目录]=b''#空目录占位
    return {#返回打包结果
        'image':压缩镜像(打包tar(扫描后)),#压缩tar镜像
        'files':扫描后,#未压缩条目
        'packages':包表,#包计数
        'workspacePackages':len([名 for 名 in 包表.keys() if 名 in 选项['workspaces']]),#工作区包数
        'roster':名册,#名册
        'missing':缺失,#缺失
        'executables':可执行列表,#可执行脚本
...
def 打包虚拟文件系统叠加(树列表):#把不透明数据树打成有序VFS叠加层
    ...
    for 树 in 树列表:#遍历每棵树
        if not os.path.exists(树['directory']):#目录缺失
            raise 打包器错误('vfs overlay: tree '+树['mount']+' is missing at '+树['directory'])#抛错
        挂载=re.sub(r'/\Z','',re.sub(r'^\./','',树['mount'],count=1),count=1)#规范化挂载路径
        ...
        def 全保留(相对路径):#全量收集
            return True#保留
        收集树(树['directory'],文件表,挂载,全保留,True)#全量收集并保留目录
    return {'image':压缩镜像(打包tar(文件表)),'files':文件表}#压缩并返回
