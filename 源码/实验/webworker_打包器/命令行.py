...
from .存储 import (#存储辅助
    组合配置档,配置树列表,描述打包,索引工作区包,预览夹具列表,#存储函数
)#存储结束
...
def 读标志(名,缺省=None):#读一对--flag value
    ...
    if 位置 is None:#缺席
        ...
        raise 打包器错误('dsh-pack-vfs-image: --'+名+' is required')#必填
    if 位置+1>=len(sys.argv) or sys.argv[位置+1].startswith('--'):#缺值
        raise 打包器错误('dsh-pack-vfs-image: --'+名+' needs a value')#缺值
...
def 主():#命令行入口
    ...
    if len(结果['missing'])>0:#缺依赖
        raise 打包器错误('vfs image: '+str(len(结果['missing']))+' dependencies did not resolve; the image would be incomplete')#缺依赖
    ...
    for 夹具 in 夹具定义:#每个夹具
        ...
        夹具行列表.append('  fixture overlay     '+夹具['id']+' ('+str(len(已打['image']))+' B compressed)')#报告行
        夹具列表.append({#清单条目
            'id':夹具['id'],#标识
            'label':夹具['label'],#标签
    ...
    清单={#预览夹具清单
        ...
        'defaultFixture':(夹具列表[0]['id'] if len(夹具列表)>0 else None),#默认fixture
        'fixtures':夹具列表,#条目
    }#清单结束
    open(os.path.join(os.path.dirname(输出文件),预览夹具清单文件),'w',encoding='utf-8').write(#写清单
        json.dumps(清单,indent=2,ensure_ascii=False,separators=(',',':'),allow_nan=False)+'\n',#JSON正文
    )#写完
    sys.stdout.write('\n'.join(描述打包(结果,仓库根,输出文件)+夹具行列表+['']))#报告
