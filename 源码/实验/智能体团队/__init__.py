...
配置=字典字段({#配置 schema
    ...
    'disposalTimeoutMs':正整数字段(默认值=默认拆除超时毫秒),#拆除超时
})#配置结束
...
def _配置项(配置值,键,缺省):#读配置键
    """从配置映射读键，缺席用缺省。"""
    if 配置值 is None or 键 not in 配置值:#缺席
        return 缺省#缺省
    return 配置值[键]#键值
...
class 团队服务(远程服务):#团队服务
    ...
    def __init__(自身,上下文,配置值=None):#构造并接线
        ...
        自身.config={#组装配置
            'maxMembers':正限制('maxMembers',_配置项(配置值,'maxMembers',默认最大成员)),#成员上限
            'maxTasks':正限制('maxTasks',_配置项(配置值,'maxTasks',默认最大任务)),#任务上限
            'maxPendingMessagesPerMember':正限制(#待投上限
                'maxPendingMessagesPerMember',#名
                _配置项(配置值,'maxPendingMessagesPerMember',默认最大待投),#值
            ),#待投结束
            'maxMessageBytes':正限制('maxMessageBytes',_配置项(配置值,'maxMessageBytes',默认最大消息字节)),#消息字节
            'disposalTimeoutMs':正限制(#拆除超时
                'disposalTimeoutMs',#名
                _配置项(配置值,'disposalTimeoutMs',默认拆除超时毫秒),#值
            ),#超时结束
        }#配置结束
        ...
        def 提交时(根):#日志提交通知
            """通知等待者。"""
            自身.activity.通知(团队标识(根.id))#通知
        自身.journal=团队日志(上下文,提交时)#日志
    ...
    def _接线监听(自身,上下文):#接线监听
        ...
        上下文.监听('session/event',观察事件)#观察投递确认
        def 会话启动(载荷,*_其余):#会话启动恢复
            """调度恢复。"""
            自身._调度恢复(载荷['agent'])#恢复
        上下文.监听('agent/session-start',会话启动)#会话启动恢复
        def 状态变化(载荷,*_其余):#状态变化
            """通知等待者。"""
            关系=自身.roster.试成员关系(载荷['agent'])#试解析成员
            if 关系 is not None:#是成员
                自身.activity.通知(关系['id'])#通知等待者
        上下文.监听('agent/status',状态变化)#status 监听
        def 寿命效果():#生命周期 effect
            """注册投影并在拆除时拆除运行时。"""
            卸投影=上下文.根.sessionProjections.register(团队投影定义)#注册投影
            def 卸除():#卸除
                """先拆除运行时再卸投影。"""
                try:#先拆除
                    自身._拆除运行时()#拆除
                    ...
                    卸投影()#卸投影
            return 卸除#卸除器
        上下文.副作用(寿命效果,'agentTeams.runtimeLifecycle()')#effect 名
    ...
    def _调度恢复(自身,智能体):#调度恢复
        ...
        def 微任务():#微任务
            """执行恢复。"""
            if 自身.lifecycle.已拆除:#已拆除则跳过
                return#跳过
            try:#恢复
                自身._执行恢复(智能体)#恢复
            except Exception as 错误:#roster/mailbox 恢复可能抛团队错误/持久化错误，契约未定所以收不窄
                if 自身.lifecycle.已拆除:#拆除中静默
                    return#静默
                自身.ctx.日志.警告('Agent Teams recovery for "'+str(智能体.id)+'" failed: '+错误文案(错误))#记警告
    ...
    def _拆除运行时(自身):#拆除运行时
        ...
        自身.activity.关闭()#拆除等待者
        失败列表=[]#收集失败
        自身.lifecycle.结算(自身.roster.列出待创建(),失败列表)#结算创建
        自身.lifecycle.结算(自身.mailbox.列出待投递(),失败列表)#结算投递
        for 根,子标识列表 in 自身.roster.按根分组活子().items():#按根停子
            try:#停 teammate
                自身.roster.停止队友(根,子标识列表)#停止
            except Exception as 错误:#停止队友可能抛团队错误/会话错误，契约未定所以收不窄
                失败列表.append(错误)#收集
        if len(失败列表)>0:#汇总失败
            raise 聚合错误(失败列表,'Agent Teams runtime disposal failed')#汇总
