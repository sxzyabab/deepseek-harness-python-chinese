...
def _模型(智能体):#读模型
    ...
    if 智能体 is None:#无 agent
        return None#无
    选项=智能体.options#选项映射
    if 选项 is None or 'model' not in 选项:#无选项或无模型
        return None#无
    return 选项['model']#模型
...
class 团队名册:#成员表
    ...
    def _充实成员行(自身,根,成员):#充实一行
        ...
        行={#成员行
            ...
            'diagnostics':[] if 'error' not in 成员 else [成员['error']],#诊断
        }#行骨架
        if 模型 is not None:#有模型
            行['model']=模型#展开
        return 行#行
    ...
    def 按根分组活子(自身):#按根分组子代
        ...
        for 智能体 in 自身.ctx.agents.list():#扫全部
            ...
            if not any(成员['id']==智能体.id for 成员 in 自身._日志.状态(根)['members']):#非 roster
                continue#下一
            子标识列表=团队表[根] if 根 in 团队表 else None#桶
            if 子标识列表 is None:#新建
                子标识列表=[]#空
                团队表[根]=子标识列表#挂上
            子标识列表.append(智能体.id)#追加
        return 团队表#分组
    ...
    def _已准入创建(自身,调用方,请求):#已准入创建
        ...
        信号=合成中止(请求['signal'] if 'signal' in 请求 else None,自身._生命周期.信号)#合并取消
        若已中止则抛出(信号)#已取消
        根=关系['root']#Lead
        名字=自身._成员名(请求['name'])#校验名
        描述=必填文本(请求['description'],'description',200)#描述
        子标识=str(uuid.uuid4())#预留子 id
        成员={#供应快照
            'id':子标识,#子 id
            'name':名字,#名
            'description':描述,#描述
            'provider':必填文本(请求['provider'],'provider',200),#provider
    ...
    def _判定供应终态(自身,根,成员,信号):#判定供应终态
        ...
        try:#读子
            ...
            头=已载['header']#头
            头父=头['parentSession'] if 'parentSession' in 头 else None#父
            if (头父==根.id and 描述符 is not None
                    and 描述符['mode']=='continuable'
                    and 描述符['provider']==成员['provider'] and 已接受初始):#匹配
                阶段='active'#活跃
            else:#不匹配
