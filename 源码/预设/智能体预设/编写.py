...
from ...工具.原子写入 import 原子写文件#原子写文件
from ...工具.主目录路径 import 展开家目录路径#展开 ~
...
class 预设不可写错误(Exception):
    """在部署不允许编写的地方尝试了编写。"""
    def __init__(自身,预设标识,原因):
        """记下试图改动的对象与原因。"""
        super().__init__('agent-presets: preset "'+预设标识+'" cannot be written: '+原因)#诊断
        自身.presetId=预设标识#试图改动的 id

def 可写根(根列表):
    """本地编写预设写入的根：第一个 user 根的绝对路径。根是 dict。"""
    根=None#候选
    for 候选 in 根列表:#找第一个 user
        if 候选['trust']=='user':#用户根
            根=候选#命中
            break#停
    if 根 is None:#没有可写根
        raise 预设不可写错误('','this deployment configures no user-writable preset root')#部署未配置
    return os.path.abspath(展开家目录路径(根['path']))#展开并绝对
...
def 复制组合(根列表,源,标识,名称=None):
    """通过复制已有预设的整个目录来创建预设。源是 dict。"""
    if 预设标识规则.fullmatch(标识) is None:#id 须合法
        raise 非法预设标识错误(标识)#拒绝
    目录=os.path.join(可写根(根列表),标识)#目标目录
    if 已占用(目录):#目标已被占用
        raise 预设已存在错误(标识)#拒绝覆盖
    源目录=os.path.dirname(源['path'])#源目录
    try:#复制、收紧、重写元数据
        ...
        if 名称 is not None:#有展示名
            元['name']=名称#带上
        if 'description' in 源 and 源['description'] is not None:#保留源描述
            元['description']=源['description']#保留
        渲染=渲染预设元数据(元)#渲染
...
def 删除组合(根列表,预设):
    """删除本地编写的预设。附带预设会被拒绝。预设是 dict。"""
    if 预设['trust']!='user':#附带不可删
        raise 预设不可写错误(预设['id'],'it ships with the deployment')#属于部署
    目录=os.path.join(可写根(根列表),预设['id'])#可写根下应对应的目录
    路径=预设['path']#组合路径
    if not os.path.isabs(路径) or not 路径.startswith(目录):#必须落在可写根下
        raise 预设不可写错误(预设['id'],'it does not live under the writable preset root')#不在可写根下
