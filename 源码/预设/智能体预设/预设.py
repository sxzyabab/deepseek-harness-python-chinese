...
__all__=[#仅中文公开名
    '预设标识规则','预设错误','未知预设错误','预设挂载错误',
]#公开面结束

预设标识规则=re.compile(r'^[a-z0-9][a-z0-9-]*\Z',re.ASCII)#预设目录名文法

class 预设错误(Exception):
    """智能体预设包的一般错误。"""

class 未知预设错误(Exception):
    """没有任何已配置根提供所请求的预设。"""
    def __init__(自身,预设标识,可用):#构造
        """记下请求 id 与可用列表。"""
        super().__init__('agent-presets: preset "'+预设标识+'" not found (available: '+(', '.join(可用) if len(可用)>0 else 'none')+')')#诊断
