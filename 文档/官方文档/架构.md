...
改动 `源码/` 下的任何内容之前，请先阅读本文。本文假定你已了解 Cordis；如果尚未了解，请先阅读[入门](cordis入门.md)或[教程](cordis教程/前言.md)。

建议使用 agent（智能体）探索代码库并理解其架构。

## Cordis

[Cordis](cordis入门.md) 是 dsh 底层的框架：插件向共享上下文贡献服务、类型化事件和可逆的副作用。产品的每一部分都是插件，包括模型适配器、工具注册表、会话日志，以及 agent loop（智能体循环）本身，因此每一部分都可以从配置替换。
...
[`dsh-base`](../源码/组合包/基础/README.zh.md) 是每个 profile 的第一层：模型适配器、工具、持久化、沙箱与审批策略、设置、凭据、遥测。[`dsh-web-app`](../源码/组合包/web/README.zh.md) 增加浏览器应用；[`dsh-headless`](../源码/组合包/无头/README.zh.md) 增加一次性运行器，且完全不带服务器。
...
组装机制见 [app启动](../源码/启动/app启动/README.zh.md#profiles)；配置字段见生成的[配置目录](配置目录.md)。
...
| 包 | 职责 | `ctx` 键 |
|---|---|---|
| [`core/session`](子系统/会话.md) | 仅追加的 `SessionEvent` 日志和内存存储 | `ctx.sessions` |
| [`core/system-prompt`](子系统/系统提示词组装.md) | 提示词片段与工具 schema 的组装 | `ctx.systemPrompt` |
| [`core/tools`](子系统/工具.md) | 作用域化的工具注册表和带把关的执行流水线 | `ctx.tools` |
| [`core/agent`](子系统/内核.md) | `Agent` 接口、活跃 agent 注册表和 `agent/*` 事件 | `ctx.agents` |
| [`core/agent-loop`](子系统/内核.md) | 实现该接口的默认驱动器 | `ctx.agentLoop` |
| [`core/scope`](子系统/作用域注册.md) | 按 agent 划分作用域的注册原语 | 库，无 ctx 键 |
| [`llm/llm`](子系统/流式输出.md) | 消息与流式词汇表，以及适配器 seam | `ctx.llm` |
...
[事件映射](事件生产方与消费方矩阵.md)列出每个事件的生产方与消费方。
...
详情见[时序图](agent生命周期.md)、[工具流水线](工具执行流水线.md)和[取消与错误恢复](子系统/内核.md#the-agent-handle)。
...
一个 **seam** 是一项可替换能力，包含三种角色：声明接口的 **Service Definition**、实现它的 **Service Provider**，以及使用它的 **Consumer**（通常是面向模型的工具）。一个包可以合并承担多个角色，但单一角色本身不是 seam；添加一项能力意味着把三者一并设计（[能力图](能力seams.md)）。

seam 正是替换一个提供方就能改变整个产品的原因。文件系统与进程提供方共享同一个执行世界，因此把它们指向远程沙箱，也就把 Bash、PTY 和 LSP 一并搬了过去，无需提供方专用 fork。[subagent 提供方](子系统/子智能体.md)在同一个接口之后同样千差万别，从新建一个子 agent，到把一个轮次委派给另一个产品。
...
[扩展实操手册](实操手册/拓展实操手册.md)将功能映射到能力，并索引[包](实操手册/添加包.md)、[工具](实操手册/添加工具.md)、[LLM（大语言模型）适配器](实操手册/添加模型适配器.md)和 [Chat 节点](实操手册/在web对话中添加节点.md)的分步指南。
