...
自动生成的[插件配置目录](../../配置目录.md)列出所有受支持的字段与默认值。[`dsh-llm-pi-ai`](../../../源码/模型后端/llm-pi-ai/README.zh.md) 和 [`dsh-llm-deepseek`](../../../源码/模型后端/llm-deepseek/README.zh.md) 参考文档负责直接 `settings.yaml` 配置、目录解析、推理控制、凭据与适配器错误。
