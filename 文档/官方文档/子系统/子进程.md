...
子进程 seam 分为 Service Definition（[dsh-subprocess](../../源码/子进程/子进程)，`ctx.subprocess`）与 Service Provider（[dsh-subprocess-local](../../源码/子进程/本地子进程)）；它的 Consumer 是其他能力 seam 与进程外后端：[bash 执行器家族](Bash执行器.md)使用收集模式的批量输出，LSP 使用原始协议管道，PTY 后端使用终端原语，ACP（Agent Client Protocol）subagent 后端则使用通过管道传输的 ndjson，并让 stderr 采用 inherit。该 seam 拥有受管的 `DSH_*` 环境命名空间、共享的凭据清除（`scrubbedParentEnv`）与 `CollectedOutput` 形状；[dsh-shell](../../源码/命令/命令) 重导出这套词汇，使 bash 消费方保持单一导入入口。

源码：[`源码/子进程/子进程/src/types.ts`](../../源码/子进程/子进程/src/types.ts) 与 [`源码/子进程/子进程/src/index.ts`](../../源码/子进程/子进程/src/index.ts)
...
抽象的 [`SubprocessRuntime`](../../源码/子进程/子进程/src/index.ts) Service Definition 规定执行世界坐标、可执行文件查找、普通 `spawn` 与 `spawnTerminal`。[`LocalSubprocessRuntime`](../../源码/子进程/本地子进程/src/index.ts) 以 detached 进程树、按处置方式接线、凭据清除、`node-pty`、平台进程检查，以及先终止再等待退出的资源释放提供这些能力。Service Definition 约定见 [`dsh-subprocess`](../../源码/子进程/子进程/README.zh.md)，本地机制见 [`dsh-subprocess-local`](../../源码/子进程/本地子进程/README.zh.md)。
...
Source: [`源码/e2b/e2b/src/index.ts:74`](../../源码/e2b/e2b/src/index.ts)
...
Source: [`源码/子进程/子进程/src/index.ts:102`](../../源码/子进程/子进程/src/index.ts)
