...
subagent seam 让一个 agent（智能体）将工作委派给子 agent。与 [bash](Bash执行器.md) 一样，它是**一项可选能力**，不属于 agent loop（智能体循环），因此其类型定义在此而非 [内核.md](内核.md) 中。它不同于其他能力 seam，因为**同一上下文中可共存多个提供方实现**，并按名称注册（`ctx.subagents`），而 bash 只允许一个执行器。该注册表遵循 [LLM（大语言模型）适配器注册表](流式输出.md)，而非单服务的 bash 执行器。

Service Definition：[dsh-subagent](../../源码/子智能体/子智能体)（`ctx.subagents` + 下文词汇）。Service Provider 是六个兄弟包：`dsh-subagent-spawn-in-process`、`-fork`、`-acp`、`-codex`、`-claude-code`、`-dsh-sdk`；面向模型的 Consumer 包括 [dsh-tool-subagent](../../源码/子智能体/子智能体工具)（按提供方委派）、[dsh-tool-subagent-control](../../源码/子智能体/子智能体控制工具)（可选的全局 `send_message`、`interrupt_agent` 与 `list_agents` 控制工具）和 [dsh-tool-subagent-report](../../源码/子智能体/子智能体上报工具)（可选的 child 作用域 `report` 返回通道）。同一个 `ctx.subagents` 服务通过内部激活管理器负责可继续子 agent 编排，并直接基于会话存储和可选的会话持久化提供只读的 child 与后代发现。产品提供方设计理由见 [Codex 与 Claude Code Agent Note](../../.agents/notes/已实现/功能/2026-08-04-claude-code与codex-subagent后端.md)；通用 seam 的设计理由见 [subagent Agent Note](../../.agents/notes/已实现/功能/2026-06-21-subagent能力接缝.md)、[可继续 subagent Agent Note](../../.agents/notes/已实现/功能/2026-07-28-可续跑subagent对话.md)、[report 工具 Agent Note](../../.agents/notes/已实现/功能/2026-07-30-可续跑subagent汇报工具.md)、[持久化目录 Agent Note](../../.agents/notes/已实现/功能/2026-07-22-持久subagent目录与列出agent.md)、[列表身份投影 Agent Note](../../.agents/notes/已实现/架构/2026-08-06-subagent列表身份投影.md)和[服务合并 Agent Note](../../.agents/notes/已实现/简化/2026-07-26-合并subagent控制服务.md)。

源码：[`源码/子智能体/子智能体/src/types.ts`](../../源码/子智能体/子智能体/src/types.ts)、[`源码/子智能体/子智能体/src/index.ts`](../../源码/子智能体/子智能体/src/index.ts)和 [`源码/子智能体/子智能体/src/continuation.ts`](../../源码/子智能体/子智能体/src/continuation.ts)
...
`signal` 是就绪前后唯一的取消通道。[subagent 组合控制 Agent Note](../../.agents/notes/已实现/功能/2026-07-12-subagent人格工具过滤与深度.md)规定 persona、live 全局工具过滤、绝对深度以及「可见性而非权限」的设计理由。
...
`running` 表示 Agent 拥有活跃的准入或轮次，或正在唤醒收件箱工作；`waiting` 表示它已完全停稳，但仍拥有至少一个尚未完成 dispose 的子 Activation；`settled` 表示已完全停稳且其拥有的每个子级都已 dispose，此时管理器会 dispose [`AgentHandle`](内核.md#creation-and-ownership) 并移除该 Activation。管理器根据 Agent 的完全停稳状态与其拥有的子级集合推导这些内部条件，而非维护第二套执行状态机。
...
`SubagentStopReason` 是一个[可合并扩展的派生联合类型](内核.md#the-map--derived-union-pattern)——后端可以添加变体，因此消费方应对已知 case 分支处理，将未知的终态原因视为失败：
...
- **Fork 种子注入**使用 [`CreateAgentOptions.seed`](内核.md#creation-and-ownership)（一个 `SessionEvent[]` 前缀，经由 `AgentLoop.createAgent` → `ctx.sessions.prepare({ seed })` 传递，与 `ctx.agents.resume()` 使用的原语相同）。fork 后端传入父级日志的一段*平衡的已完成轮次前缀*——父级事件直到并包括其最后一个 `turn/end`——因此种子从 0 连续，[invariants](../../源码/runtime-diagnostics/invariants) 回放可以接受它（进行中的、未平衡的轮次被排除在外）。
...
Types: [Agent](内核.md) · [ContentBlock](流式输出.md) · [MessageId](流式输出.md) · [SessionId](内核.md)

Source: [`源码/子智能体/子智能体/src/index.ts:171`](../../源码/子智能体/子智能体/src/index.ts)
...
Source: [`源码/子智能体/子智能体/src/index.ts:166`](../../源码/子智能体/子智能体/src/index.ts)
...
Source: [`源码/子智能体/子智能体/src/index.ts:140`](../../源码/子智能体/子智能体/src/index.ts)
...
Source: [`源码/子智能体/子智能体/src/index.ts:146`](../../源码/子智能体/子智能体/src/index.ts)
...
Source: [`源码/子智能体/子智能体/src/index.ts:157`](../../源码/子智能体/子智能体/src/index.ts)
