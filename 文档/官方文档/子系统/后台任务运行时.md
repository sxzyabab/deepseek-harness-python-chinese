...
长时间运行的生产方、`ctx.jobs` 与任务控制命令共用的类型。[运行时 Agent Note](../../.agents/notes/已实现/架构/2026-06-20-通用长时工具运行时.md) 负责设计；本页记录 [`源码/后台任务/任务/src/types.ts`](../../源码/后台任务/任务/src/types.ts) 中的确切字段和变体。

## ID 与状态

`JobId` 是按 `<kind>-N` 生成的[品牌化 id](内核.md#branded-ids)。访问控制依赖拥有者授权，而非 id 的保密性。`JobKind` 派生自可合并扩展的 map；注册表将各个 kind 视为不透明的 id 命名空间。
...
Source: [`源码/后台任务/任务/src/index.ts:62`](../../源码/后台任务/任务/src/index.ts)
