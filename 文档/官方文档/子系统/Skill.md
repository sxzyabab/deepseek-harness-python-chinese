...
[skill（技能）能力族](../../源码/技能) 包含 Service Definition（[dsh-skill](../../源码/技能/技能)，`ctx.skills`）、本地 Service Provider（[dsh-skill-filesystem](../../源码/技能/技能-文件系统)）、可选的随包徽章提供方（[dsh-skill-badge](../../源码/技能/技能徽章)）和 Consumer（[dsh-tool-skill](../../源码/技能/工具-技能)）。注册表在其宿主层与各 scope 层之间合并各提供方的目录；提供方贡献本地或随包 skill；Consumer 拥有初始目录和替换目录，以及面向模型的 `skill` 工具。skill 是可选的指令而非会话事件，因此其词汇定义在此处而非 [内核.md](内核.md)。

源码：[`源码/技能/技能/src/index.ts`](../../源码/技能/技能/src/index.ts)、[`源码/技能/技能-文件系统/src/index.ts`](../../源码/技能/技能-文件系统/src/index.ts)、[`源码/技能/技能徽章/src/index.ts`](../../源码/技能/技能徽章/src/index.ts) 与 [`源码/技能/工具-技能/src/index.ts`](../../源码/技能/工具-技能/src/index.ts)。

## 提供方注册表

`ctx.skills` 组合本地、内嵌、远程或其他提供方。注册是同步的；远程初始化与发现属于 `list()` 的 await 阶段。提供方对象、选项与候选项以只读方式借用，语义字段会被校验。

注册表采用宿主 + 按 scope 的分层结构，即[工具注册表](工具.md)在 [dsh-scope](../../源码/内核/作用域) 之上确立的形态：注册会落入调用方上下文 scope 对应的层——宿主行与 repository 插件落入全局层，由 agent（智能体） preset 常驻组合挂载的插件落入该 preset 的层——提供方名称在每层内唯一，而非进程级唯一。读取时将全局层与观察 scope 的链合并：最近层的条目直接赢得重名 skill，下文的 rank 顺序只在单层内裁决重名。发现缓存以解析后的 scope 链为键，因此重设 scope 父级（空会话重组）无需注册表变更即可被下一次读取看到。
...
Source: [`源码/技能/技能/src/index.ts:357`](../../源码/技能/技能/src/index.ts)
...
Source: [`源码/技能/技能/src/index.ts:297`](../../源码/技能/技能/src/index.ts)
