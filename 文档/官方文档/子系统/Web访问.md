...
Web 访问 seam 是一个[能力 seam](../../.agents/notes/已实现/架构/2026-06-24-web能力接缝.md)，在同一个 `ctx.web` 服务上横跨**两项操作**（search 与 fetch），并拆分到多个包：Service Definition（[dsh-web](../../源码/web/web)，`ctx.web` + 提供方注册表）、Service Provider（[dsh-web-search-exa](../../源码/web/web搜索-exa)、[dsh-web-search-perplexity](../../源码/web/web搜索-perplexity)、[dsh-web-search-deepseek](../../源码/web/web搜索-deepseek)、[dsh-web-fetch-http](../../源码/web/http请求)）与 Consumer（[dsh-tool-web](../../源码/web/tool-web)，即 `web_search`/`web_fetch` 工具 schema）。Web 是**一项可选能力**，不属于 agent loop（智能体循环）主干，因此其词汇定义在此而非 [内核.md](内核.md) 中。更换 search 提供方不会改变模型提交查询的方式，更换 fetch 提供方也不会改变模型请求 URL 的方式。

源码：[`源码/web/web/src/types.ts`](../../源码/web/web/src/types.ts)
...
Source: [`源码/web/web/src/index.ts:74`](../../源码/web/web/src/index.ts)
