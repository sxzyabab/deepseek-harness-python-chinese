...
[scope 包](../../源码/内核/作用域)提供身份、载体与作用域层词汇，使同一注册上下文同时表达每个 agent（智能体）的可见性和共享生命周期所有权。它是库原语，而不是 Cordis 服务；生命周期设计理由由 [agent-scope 运行时设计 Agent Note](../../.agents/notes/已实现/架构/2026-07-12-agent作用域运行时设计.md#作用域路由一个不透明键选择一层)规定，注册表层决策由[共享存储 Agent Note](../../.agents/notes/已实现/架构/2026-07-12-作用域层存储.md)规定，可调用 API 与过滤语义则由包 [README](../../源码/内核/作用域/README.zh.md)规定。

源码：[`源码/内核/作用域/src/index.ts`](../../源码/内核/作用域/src/index.ts) 与 [`源码/内核/作用域/src/store.ts`](../../源码/内核/作用域/src/store.ts)。
