...
[dsh-credentials](../../源码/凭据/凭据) 的凭据 seam 把机密挡在配置之外：settings 分节与 `cordis.yml` 条目携带的是*引用*（环境变量名），值归 [dsh-credentials-local](../../源码/凭据/本地凭据) 这类提供方所有，消费方每个操作解析一次引用——LLM（大语言模型）适配器每次模型请求解析一次，因此轮换后的凭据无需任何重启即可作用于紧随其后的下一次请求。一条 seam 级规则约束每个提供方：空的存储值在任何地方都视为不存在。

来源：[`源码/凭据/凭据/src/index.ts`](../../源码/凭据/凭据/src/index.ts)
...
Source: [`源码/凭据/凭据/src/index.ts:60`](../../源码/凭据/凭据/src/index.ts)
...
Source: [`源码/凭据/凭据/src/types.ts:29`](../../源码/凭据/凭据/src/types.ts)
