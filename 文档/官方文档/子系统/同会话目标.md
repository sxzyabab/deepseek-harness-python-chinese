...
事件溯源目标服务及其策略消费方共享的类型。[目标领域 Agent Note](../../.agents/notes/已实现/功能/2026-07-19-持久同会话目标领域.md) 负责记录持久化与激活决策；本页记录 [`源码/目标/目标/src/types.ts`](../../源码/目标/目标/src/types.ts) 中的确切字段和变体。

## 标识与生命周期

`GoalId` 是[品牌化 id](内核.md#branded-ids)。调用方通过 `GoalRef` 修改一个确切修订版本；每次获准的持久变更都会递增修订号。
...
[`GoalService`](../../源码/目标/目标/src/index.ts) 解析创建默认值、从持久 `goal/change` 事件执行严格回放折叠、校验传入的 agent（智能体）是注册表中的确切活跃实例、以比较并设置方式执行变更，并发出 `goal/changed` 通知；监听器故障会被隔离。包 [README](../../源码/目标/目标/README.zh.md) 定义可调用 API 和面向模型的约定。
...
Source: [`源码/目标/目标/src/index.ts:183`](../../源码/目标/目标/src/index.ts)
...
Types: [Agent](内核.md) · [Scoped](作用域注册.md)

Source: [`源码/目标/目标/src/domain.ts:114`](../../源码/目标/目标/src/domain.ts)
