...
会话投影 seam 是一项[能力 seam](../能力seams.md)：领域 host 插件经由它向客户端载体供给按会话的日志派生状态的当前全量值；三方分别是 Service Definition 与注册表（[dsh-session-projection](../../源码/会话/会话投影)，`ctx.sessionProjections`）、领域贡献方（每个领域注册一个纯单元）与载体（[dsh-host-apiproxy](../../源码/host/apiproxy) 的历史尾页与 `session/projection` 推送帧）。它是一项可选能力，不属于 agent loop（智能体循环）主干。框架负责驱动，领域负责计算：注册表只订阅一次 `session/event`，并把每个已提交事件折叠进每个单元；领域不持有任何订阅，客户端也从不折叠领域事件——它们收到的是成品值。设计权威：[session-projection RFC](../../.agents/notes/提案/架构/2026-07-27-会话投影与命令日志.md)；驱动、缓存与变更流约定：[包 README](../../源码/会话/会话投影/README.zh.md)。

源码：[`源码/会话/会话投影/src/index.ts`](../../源码/会话/会话投影/src/index.ts)
...
Types: [Session](会话.md) · [SessionHeader](会话持久化.md) · [SessionId](内核.md)

Source: [`源码/会话/会话投影缓存/src/index.ts:71`](../../源码/会话/会话投影缓存/src/index.ts)
...
Source: [`源码/会话/会话投影/src/index.ts:171`](../../源码/会话/会话投影/src/index.ts)
