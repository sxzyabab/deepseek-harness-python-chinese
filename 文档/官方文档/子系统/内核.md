...
**核心**子系统即 [`源码/内核`](../../源码/内核/README.zh.md)，包含每个组合都会启动的包：事件溯源的会话日志、系统提示词组装、工具注册表、agent（智能体）类型，以及驱动它们的具体循环。本页说明 `agent`/`agent-loop` 这对包所声明的内容：agent 如何被创建与拥有，以及 `Agent` 句柄的投递、取消与拦截约定；本页还说明每个子系统都遵循的两个类型模式。该组的专属页面与目录其余部分见[子系统 README](README.md)。

## 主干逐包速览

一个轮次按同一条循环流经六个包：[`agent-loop`](../../源码/内核/智能体循环) 中的 driver 认领一条排队的提示词，在[会话日志](会话.md)（`ctx.sessions`）上开启轮次，通过 [system-prompt](系统提示词组装.md)（`ctx.systemPrompt`）组装请求前缀并从日志派生历史，经 [LLM（大语言模型） seam](流式输出.md) 流式获取模型响应，经[工具注册表](工具.md)（`ctx.tools`）分发工具调用，并把每个模型可见的事实追加回日志，供下一步派生。循环搬运的对话词汇——`Message`、`ContentBlock`、`StreamChunk`、模型请求——由 [`源码/模型后端`](../../源码/模型后端/README.zh.md) 声明，记录在 [流式输出.md](流式输出.md)。

| 包 | 负责内容 | 页面 |
|---|---|---|
| `session/` | 仅追加的 `SessionEvent` 日志与内存 store——唯一真源（`ctx.sessions`） | [会话.md](会话.md) |
| `system-prompt/` | 提示词段落与工具 schema 组装（`ctx.systemPrompt`） | [系统提示词组装.md](系统提示词组装.md) |
| `tools/` | 带作用域的工具注册表与受保护的执行流水线（`ctx.tools`） | [工具.md](工具.md) |
| `agent/` | `Agent` 接口、实时注册表、发起者作用域与 `agent/*` 事件词汇（`ctx.agents`） | 本页 |
| `agent-loop/` | 实现公开 `Agent` 约定的具体 driver（`ctx.agentLoop`） | 本页 |
| `scope/` | 注册表与循环用于构建按 agent 作用域的注册原语 | [作用域注册.md](作用域注册.md) |
...
源码：[`源码/内核/智能体/src/index.ts`](../../源码/内核/智能体/src/index.ts)
...
源码：[`源码/内核/智能体/src/types.ts`](../../源码/内核/智能体/src/types.ts)
...
interface Agent {
   ...
   * claims ([cancel-convergence wake latch](../../.agents/notes/已实现/bug修复/2026-08-07-取消收敛唤醒闩锁.md)).
...
`running` 描述整个驱动器的排空区间，可能跨越连续的排队轮次；它不能证明某个轮次仍然打开。dispose 会把 agent 从注册表移除并发出 `agent/disposed`；它不是一个终态 status 值。`followup()` 不返回句柄：其 `MessageId` 标识的是持久的 inbox 插入、认领与丢弃事实，而非之后的助手输出或轮次结束。`whenIdle()` 观察的是整个 agent，因此只有当调用方明确拥有从回执到空闲的这段区间时，才能把它称为一次 run（[决策](../../.agents/notes/已实现/架构/2026-07-30-后续入队与自有运行.md)）。
...
`ctx.agents` 携带的进程本地 initiator 就是上面的确切 `Agent`，不是单独的 frame 或复制的标识。环境中存在该值既不能证明存活，也不代表授权；[initiator 作用域决策](../../.agents/notes/已实现/架构/2026-07-15-agent发起方作用域.md)定义其生命周期和作用域规则。
...
`SessionEvent` 信封的确切条件字段、十二种事件变体（`turn/start`、`turn/end`、`step/start`、`step/end`、`user/message`、`assistant/chunk`、`assistant/message`、`tool/call`、`tool/result`、`steering/message`、`todo/write`、`request/header`）、`deriveMessages()` 投影规则、`TurnTrigger`/`TurnEndReason` 原因以及执行封闭和独立事件规则都在 **[会话.md](会话.md)** 中。日志如何持久化——`SessionPersistence` 接口、JSONL/SQLite 后端、`session/flush` 检查点、崩溃恢复与 `SessionHeader`——则在 **[会话持久化.md](会话持久化.md)** 中。
...
其完整字段、`defineTool`/`ValueSchemaSpec`/`ParameterSchemaSpec` 类型化 schema DSL、`ToolExecution`/`ToolExecutionResult` waterfall 类型，以及工具展示 UI 类型都在 **[工具.md](工具.md)** 中。
...
| Map | 包 | 派生 | 目录 |
|---|---|---|---|
| `ContentBlockMap` | dsh-llm | `ContentBlock` | [流式输出.md](流式输出.md#content-blocks-and-messages) |
| `MessageSourceMap` | dsh-llm | `MessageSource` | [流式输出.md](流式输出.md#content-blocks-and-messages) |
| `FinishReasonMap` | dsh-llm | `FinishReason` | [流式输出.md](流式输出.md#the-model-request-and-result) |
| `TurnTriggerMap` | dsh-session | `TurnTrigger` | [会话.md](会话.md) |
| `TurnEndReasonMap` | dsh-session | `TurnEndReason` | [会话.md](会话.md) |
| `SessionEventMap` | dsh-session | `SessionEvent` | [会话.md](会话.md) |
...
`Branded<B>` 原语位于独立的纯类型包 [dsh-brand](../../源码/工具/品牌) 中（没有运行时代码，也不依赖 harness 包），因此任何包都能品牌化其拥有的 id，而无需依赖无关的能力包。

源码：[`源码/工具/品牌/src/index.ts`](../../源码/工具/品牌/src/index.ts)
...
两个核心 ID 是 `CallId`（关联工具调用及其结果；dsh-llm）和 `SessionId`（活跃 agent 与持久会话共享的标识；dsh-session）。能力包也会品牌化各自的 id，例如 [后台任务运行时.md](后台任务运行时.md) 中的 `JobId`。
...
Source: [`源码/内核/智能体-默认模型/src/index.ts:64`](../../源码/内核/智能体-默认模型/src/index.ts)
...
Types: [SessionHeader](会话持久化.md)

Source: [`源码/内核/智能体循环/src/index.ts:296`](../../源码/内核/智能体循环/src/index.ts)
...
Types: [ScopeKey](作用域注册.md)

Source: [`源码/预设/智能体预设/src/index.ts:82`](../../源码/预设/智能体预设/src/index.ts)
...
Source: [`源码/内核/智能体/src/index.ts:256`](../../源码/内核/智能体/src/index.ts)
...
Types: [Scoped](作用域注册.md)

Source: [`源码/内核/智能体/src/runtime-types.ts:159`](../../源码/内核/智能体/src/runtime-types.ts)
...
Source: [`源码/内核/智能体/src/runtime-types.ts:168`](../../源码/内核/智能体/src/runtime-types.ts)
...
Source: [`源码/内核/智能体/src/runtime-types.ts:290`](../../源码/内核/智能体/src/runtime-types.ts)
...
Types: [Scoped](作用域注册.md) · [UserMessage](会话.md)

Source: [`源码/内核/智能体/src/runtime-types.ts:197`](../../源码/内核/智能体/src/runtime-types.ts)
...
Source: [`源码/内核/智能体/src/runtime-types.ts:205`](../../源码/内核/智能体/src/runtime-types.ts)
...
Source: [`源码/内核/智能体/src/runtime-types.ts:186`](../../源码/内核/智能体/src/runtime-types.ts)
...
Source: [`源码/内核/智能体/src/runtime-types.ts:231`](../../源码/内核/智能体/src/runtime-types.ts)
...
Types: [LlmCallConfig](流式输出.md) · [Scoped](作用域注册.md)

Source: [`源码/内核/智能体/src/runtime-types.ts:244`](../../源码/内核/智能体/src/runtime-types.ts)
...
Types: [LlmFailure](流式输出.md) · [ResolvedRetryPolicy](流式输出.md) · [Scoped](作用域注册.md)

Source: [`源码/内核/智能体/src/runtime-types.ts:260`](../../源码/内核/智能体/src/runtime-types.ts)
...
Source: [`源码/内核/智能体/src/runtime-types.ts:217`](../../源码/内核/智能体/src/runtime-types.ts)
...
Source: [`源码/内核/智能体/src/runtime-types.ts:178`](../../源码/内核/智能体/src/runtime-types.ts)
...
Source: [`源码/内核/智能体/src/runtime-types.ts:278`](../../源码/内核/智能体/src/runtime-types.ts)
...
Source: [`源码/内核/智能体循环/src/index.ts:183`](../../源码/内核/智能体循环/src/index.ts)
...
Source: [`源码/预设/智能体预设/src/types.ts:13`](../../源码/预设/智能体预设/src/types.ts)
