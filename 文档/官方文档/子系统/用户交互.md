...
[dsh-user-questions](../../源码/交互/用户提问) 的用户交互 seam。它是工具或权限插件需要人类回答后 agent（智能体）才能继续时所使用的、提供方无关的词汇。UI 界面提供活跃的 `UserQuestionProvider`；host 运行时把请求转发给其连接的客户端。

源码：[`源码/交互/用户提问/src/index.ts`](../../源码/交互/用户提问/src/index.ts)
...
Source: [`源码/交互/用户提问/src/index.ts:51`](../../源码/交互/用户提问/src/index.ts)
