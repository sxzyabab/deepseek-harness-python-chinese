...
结构化的跨会话引用请求与准备后的消息上下文。[包约定](../../源码/上下文/会话引用) 定义规范 URI、当前表层投影、标签安全的 JSON 与字节保留、稳定错误和不可信的模型提示词。宿主适配器使用这些类型，而不会把各自 UI 的提及语法传入 agent（智能体）核心。

来源：[`源码/上下文/会话引用/src/types.ts`](../../源码/上下文/会话引用/src/types.ts)
...
Types: [Agent](内核.md) · [ContentBlock](流式输出.md)

Source: [`源码/上下文/会话引用/src/index.ts:70`](../../源码/上下文/会话引用/src/index.ts)
