...
[dsh-settings](../../源码/配置/配置) 的用户设置 seam 持有一份按 namespace 分节的用户文档，并把每个已注册 namespace 解析为：schema 默认值，然后注册方的组合 `base`，最后用户分节。[dsh-settings-file](../../源码/配置/配置文件) 这类提供方存储原始文档并推送外部编辑；消费方插件注册 schema 后读取或观察解析值。组合配置仍留在 `cordis.yml`——namespace 只承载用户可编辑子集。

来源：[`源码/配置/配置/src/index.ts`](../../源码/配置/配置/src/index.ts)
...
Source: [`源码/配置/配置/src/index.ts:350`](../../源码/配置/配置/src/index.ts)
...
Source: [`源码/配置/配置/src/types.ts:48`](../../源码/配置/配置/src/types.ts)
...
Source: [`源码/配置/配置/src/types.ts:35`](../../源码/配置/配置/src/types.ts)
