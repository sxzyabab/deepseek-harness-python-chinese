<!-- 英文源文件由 scripts/gen-persistence-catalog.ts 生成；本中文文件是通过双语配对维护的经评审对侧。
     更新时先运行 `pnpm run gen-persistence-catalog` 更新英文，再更新本文件并运行 `pnpm run verify-translation-pairing --write 文档/会话持久化.md` 重新记录配对。 -->

# 会话持久化事件目录

会话持久事件日志中可能出现的所有事件类型：完整持久化的 `SessionEvent` 信封，以及可通过合并扩展的 `SessionEventMap` 中的每个成员，包括 `@deepseek-ai/dsh-session` 所属的词汇和本仓库中每个插件对 `@deepseek-ai/dsh-session/types` 的声明合并，并附有源 JSDoc、完整 payload 声明、surface 标记和声明位置。本文档是 [会话.md](子系统/会话.md)（surface 排序与 `deriveMessages()` 投影）、[会话持久化.md](子系统/会话持久化.md)（如何让日志持久化）和 [会话.md](子系统/会话.md#cordis-surface) 中生成区域（实时总线接线；日志事件**不是** cordis 事件，它通过唯一的 `session/event` emit 到达监听器）的补充。

英文源文件根据源码生成（`scripts/gen-persistence-catalog.ts`），并由 `pnpm run verify-persistence-catalog`（`doc-sync`（文档同步门禁）的一部分）验证新鲜度；本中文文件作为经评审对侧通过双语配对维护。声明块保留源码声明和嵌套属性的 JSDoc，只移除其所在接口／模块带来的缩进，并使用 `ts persistence-catalog` 围栏（doc-typecheck 会跳过这些围栏，因为声明引用了其所属模块中的类型）。payload 中的类型名称会链接到记录该类型的页面。参见 [persistence-log-catalog Agent Note](../.agents/notes/已归档/流程/2026-07-04-持久化日志目录.md)。

以下信封声明组合了每个事件的 `type`、单调递增的 `seq`、以 epoch 毫秒表示的 `time`、`data`、可选的未知类型跳过标记 `ignorable`，以及条件字段 `surfaceOp`／`sourceEventSeqs`。**surface** 表示 `SurfaceEventType` 成员：它会生成一条 LLM（大语言模型）消息，并声明该事件如何加入 surface 列表。**log-only** 表示其他所有事件：这类记录可持久化、可回放，但不参与派生历史。每个 payload 均可进行 JSON 序列化（在 `Session.append` 处强制执行），整个格式固定为 `SESSION_FORMAT_VERSION = 0`：这是预发布格式，不暗示任何兼容性（参见[版本立场](子系统/会话持久化.md)）。范围仅限本仓库中的包；下游插件可以继续合并其他事件类型，而这些类型按设计不属于本目录。
...
来源：[`源码/内核/会话/src/types.ts:336`](../源码/内核/会话/src/types.ts) · [`源码/内核/会话/src/types.ts:343`](../源码/内核/会话/src/types.ts) · [`源码/内核/会话/src/types.ts:372`](../源码/内核/会话/src/types.ts) · [`源码/内核/会话/src/types.ts:404`](../源码/内核/会话/src/types.ts)
...
来源：[`源码/内核/智能体/src/types.ts:19`](../源码/内核/智能体/src/types.ts)
...
来源：[`源码/预设/智能体预设/src/session.ts:26`](../源码/预设/智能体预设/src/session.ts)
...
来源：[`源码/交互/用户审批/src/index.ts:44`](../源码/交互/用户审批/src/index.ts)
...
来源：[`源码/交互/用户审批/src/index.ts:55`](../源码/交互/用户审批/src/index.ts)
...
来源：[`源码/交互/用户审批/src/index.ts:67`](../源码/交互/用户审批/src/index.ts)
...
类型：[StreamChunk](子系统/流式输出.md)

来源：[`源码/内核/会话/src/types.ts:266`](../源码/内核/会话/src/types.ts)
...
类型：[TokenUsage](子系统/流式输出.md)

来源：[`源码/内核/会话/src/types.ts:273`](../源码/内核/会话/src/types.ts)
...
来源：[`源码/交互/命令/src/types.ts:95`](../源码/交互/命令/src/types.ts)
...
来源：[`源码/交互/命令/src/types.ts:88`](../源码/交互/命令/src/types.ts)
...
来源：[`源码/压缩/压缩/src/types.ts:71`](../源码/压缩/压缩/src/types.ts)
...
来源：[`源码/压缩/压缩/src/types.ts:81`](../源码/压缩/压缩/src/types.ts)
...
来源：[`源码/压缩/压缩/src/types.ts:23`](../源码/压缩/压缩/src/types.ts)
...
类型：[ContentBlock](子系统/内核.md) · [TokenUsage](子系统/流式输出.md)

来源：[`源码/压缩/压缩/src/types.ts:33`](../源码/压缩/压缩/src/types.ts)
...
来源：[`源码/反馈/指令反馈/src/index.ts:62`](../源码/反馈/指令反馈/src/index.ts)
...
来源：[`源码/目标/目标/src/domain.ts:66`](../源码/目标/目标/src/domain.ts)
...
来源：[`源码/钩子/钩子协议/src/types.ts:19`](../源码/钩子/钩子协议/src/types.ts)
...
来源：[`源码/钩子/钩子协议/src/types.ts:31`](../源码/钩子/钩子协议/src/types.ts)
...
来源：[`源码/模型后端/llm-retry/src/types.ts:9`](../源码/模型后端/llm-retry/src/types.ts)
...
来源：[`源码/模型后端/llm-retry/src/types.ts:11`](../源码/模型后端/llm-retry/src/types.ts)
...
来源：[`源码/交互/权限预设/src/index.ts:50`](../源码/交互/权限预设/src/index.ts)
...
来源：[`源码/plan模式/plan模式/src/index.ts:53`](../源码/plan模式/plan模式/src/index.ts)
...
来源：[`源码/内核/会话/src/types.ts:309`](../源码/内核/会话/src/types.ts)
...
来源：[`源码/内核/会话/src/types.ts:304`](../源码/内核/会话/src/types.ts)
...
来源：[`源码/沙盒/沙盒策略/src/session-mode.ts:33`](../源码/沙盒/沙盒策略/src/session-mode.ts)
...
来源：[`源码/日程/日程/src/types.ts:219`](../源码/日程/日程/src/types.ts)
...
来源：[`源码/内核/会话/src/types.ts:332`](../源码/内核/会话/src/types.ts)
...
类型：[SessionTitleEventData](子系统/会话标题.md)

来源：[`源码/会话/会话标题/src/index.ts:100`](../源码/会话/会话标题/src/index.ts)
...
类型：[SessionTitleLlmRequestEventData](子系统/会话标题.md)

来源：[`源码/会话/会话标题-llm/src/index.ts:43`](../源码/会话/会话标题-llm/src/index.ts)
...
来源：[`源码/内核/会话/src/types.ts:256`](../源码/内核/会话/src/types.ts)
...
来源：[`源码/内核/会话/src/types.ts:254`](../源码/内核/会话/src/types.ts)
...
来源：[`源码/子智能体/子智能体/src/descriptor.ts:37`](../源码/子智能体/子智能体/src/descriptor.ts)
...
类型：[TodoItem](子系统/会话.md)

来源：[`源码/内核/会话/src/types.ts:299`](../源码/内核/会话/src/types.ts)
...
来源：[`源码/内核/会话/src/types.ts:279`](../源码/内核/会话/src/types.ts)
...
来源：[`源码/内核/工具/src/types.ts:56`](../源码/内核/工具/src/types.ts)
...
来源：[`源码/内核/工具/src/types.ts:40`](../源码/内核/工具/src/types.ts)
...
来源：[`源码/内核/会话/src/types.ts:291`](../源码/内核/会话/src/types.ts)
...
来源：[`源码/工作流/工具-工作流/src/types.ts:57`](../源码/工作流/工具-工作流/src/types.ts)
...
来源：[`源码/工作流/工具-工作流/src/types.ts:52`](../源码/工作流/工具-工作流/src/types.ts)
...
来源：[`源码/工作流/工具-工作流/src/types.ts:62`](../源码/工作流/工具-工作流/src/types.ts)
...
来源：[`源码/工作流/工具-工作流/src/types.ts:47`](../源码/工作流/工具-工作流/src/types.ts)
...
类型：[TurnEndReason](子系统/会话.md)

来源：[`源码/内核/会话/src/types.ts:252`](../源码/内核/会话/src/types.ts)
...
来源：[`源码/内核/会话/src/types.ts:243`](../源码/内核/会话/src/types.ts)
...
来源：[`源码/内核/会话/src/types.ts:264`](../源码/内核/会话/src/types.ts)
...
来源：[`源码/web/web搜索-deepseek/src/provider.ts:83`](../源码/web/web搜索-deepseek/src/provider.ts)
