...
API 各层按 `remotes → gateway → connection → webserver` 组织。BFF 与 Typert RPC 层位于 `源码/api`；Connection 与 WebServer 位于 `源码/客户端/连接` 和 `源码/host/webserver`。位于 `源码/host/apiproxy` 的 API Proxy 处理没有 Remote 描述符的 endpoint。
