<!-- 英文源文件由 scripts/gen-tool-catalog.ts 生成；本中文文件是通过双语配对维护的经评审对侧。
     更新时先运行 `pnpm run gen-tool-catalog` 更新英文，再更新本文件并运行 `pnpm run verify-translation-pairing --write 文档/工具目录.md` 重新记录配对。 -->

# 工具 Schema 目录

已发布插件向 `ctx.tools` 提供的所有面向模型的工具：模型通过系统提示词组装获得的 `name`、`description` 和 JSON Schema `parameters`。本目录是[子系统页面](子系统/内核.md)（类型及每页生成的 `cordis-surface` 接线区域）的补充；本页列出的是向 agent（智能体）提供的*工具*。

英文源文件由系统**生成**，并通过 `pnpm run verify-tool-catalog`（`doc-sync`（文档同步门禁）的一部分）验证新鲜度；本中文文件作为经评审对侧通过双语配对维护。与 Cordis 目录（纯源码 AST 处理）不同，英文生成器会在真实上下文中**启动**每个工具插件并读取 `ctx.tools.schemas()`，因为工具 schema 无法通过静态分析完全确定，例如运行时展开的枚举、拼接的描述、由配置决定的名称以及使用原始 JSON Schema 的 MCP 工具。完整性守卫会扫描 `源码/` 各分组下的已发布工具包；如果生成器的启动 manifest（元数据清单）遗漏任何包，检查就会失败，因此新工具不会在无人察觉的情况下缺少文档。参见[工具 schema 目录 Agent Note](../.agents/notes/已实现/流程/2026-07-02-工具schema目录.md)。

范围：`源码/` 各分组下已发布的产品工具，每个工具均使用其**默认**配置启动；但如果某个 Config 字段是**必填项**且没有默认值，生成器就必须作出选择，对应包的说明会记录本页展示的是哪个分支。注册的工具**名称**可以是加载时配置，例如 `tool-subagent` 的 `toolName`，因此部署可能以不同名称或额外名称提供某个包；如果存在随产品发布的别名，对应包的说明会予以记录。`示例/` 中的演示工具（例如 `echo`）不在范围内，这与 Cordis 目录仅涵盖包的范围一致。
...
| `@deepseek-ai/dsh-tool-cordis` | `cordis_define`、`cordis_inspect_list`、`cordis_inspect_query`、`cordis_inspect_self`、`cordis_run`、`cordis_stop`、`cordis_undefine` | `ctx.tools`、`ctx.dynamicCordisRunner` | `tool/call`、`tool/result`、`process-local dynamic package lifecycle` | - | 不在任何随产品发布的树中，需要显式选择启用；动态 Package 代码可以访问真实运行时，见 `2026-07-08-self-referential-cordis-toolset.md`。该工具集注入 `@deepseek-ai/dsh-cordis-host-runner` 提供的 `ctx.dynamicCordisRunner`，后者拥有定义注册表和 vm 沙箱；组合缺少它时这些工具不会激活。运行中的 Package 在停止、undefine 或 DSH 重启前可以注册**额外的**模型可见工具；发生这类工具集变化时，系统会记录完整且有变动的请求头。 |
...
来源：[`源码/交互/工具-询问用户/src/index.ts`](../源码/交互/工具-询问用户/src/index.ts)
...
来源：[`源码/内核/工具/src/code-mode.ts`](../源码/内核/工具/src/code-mode.ts)
...
来源：[`源码/plan模式/plan模式/src/index.ts`](../源码/plan模式/plan模式/src/index.ts)
...
来源：[`源码/命令/bash工具/src/index.ts`](../源码/命令/bash工具/src/index.ts)
...
来源：[`源码/命令/powershell工具/src/index.ts`](../源码/命令/powershell工具/src/index.ts)
...
来源：[`源码/拓展/cordis工具/src/index.ts`](../源码/拓展/cordis工具/src/index.ts)
...
不在任何随产品发布的树中，需要显式选择启用；动态 Package 代码可以访问真实运行时，见 `2026-07-08-self-referential-cordis-toolset.md`。该工具集注入 `@deepseek-ai/dsh-cordis-host-runner` 提供的 `ctx.dynamicCordisRunner`，后者拥有定义注册表和 vm 沙箱；组合缺少它时这些工具不会激活。运行中的 Package 在停止、undefine 或 DSH 重启前可以注册**额外的**模型可见工具；发生这类工具集变化时，系统会记录完整且有变动的请求头。
...
来源：[`源码/命令/bash工具持久化/src/index.ts`](../源码/命令/bash工具持久化/src/index.ts)
...
来源：[`源码/文件系统/字符串替换工具/src/index.ts`](../源码/文件系统/字符串替换工具/src/index.ts)
...
来源：[`源码/文件系统/文件系统工具/src/index.ts`](../源码/文件系统/文件系统工具/src/index.ts)
...
来源：[`源码/文件系统/文件搜索工具/src/index.ts`](../源码/文件系统/文件搜索工具/src/index.ts)
...
来源：[`源码/终端/终端工具/src/index.ts`](../源码/终端/终端工具/src/index.ts)
...
来源：[`源码/目标/工具-目标/src/index.ts`](../源码/目标/工具-目标/src/index.ts)
...
来源：[`源码/日程/日程/src/tools.ts`](../源码/日程/日程/src/tools.ts)
...
来源：[`源码/语言服务器/语言服务器工具/src/index.ts`](../源码/语言服务器/语言服务器工具/src/index.ts)
...
来源：[`源码/工作流/tool-ralph/src/index.ts`](../源码/工作流/tool-ralph/src/index.ts)
...
来源：[`源码/技能/工具-技能/src/index.ts`](../源码/技能/工具-技能/src/index.ts)
...
来源：[`源码/会话检索/工具-会话查询/src/index.ts`](../源码/会话检索/工具-会话查询/src/index.ts)
...
来源：[`源码/子智能体/子智能体工具/src/index.ts`](../源码/子智能体/子智能体工具/src/index.ts)
...
来源：[`源码/子智能体/子智能体控制工具/src/index.ts`](../源码/子智能体/子智能体控制工具/src/index.ts)
...
来源：[`源码/子智能体/子智能体控制工具/src/list-agents.ts`](../源码/子智能体/子智能体控制工具/src/list-agents.ts)
...
来源：[`源码/子智能体/子智能体上报工具/src/index.ts`](../源码/子智能体/子智能体上报工具/src/index.ts)
...
来源：[`源码/后台任务/工具-任务/src/index.ts`](../源码/后台任务/工具-任务/src/index.ts)
...
来源：[`源码/待办/工具-待办/src/index.ts`](../源码/待办/工具-待办/src/index.ts)
...
来源：[`源码/工作流/工具-工作流/src/index.ts`](../源码/工作流/工具-工作流/src/index.ts)
...
来源：[`源码/web/tool-web/src/index.ts`](../源码/web/tool-web/src/index.ts)
