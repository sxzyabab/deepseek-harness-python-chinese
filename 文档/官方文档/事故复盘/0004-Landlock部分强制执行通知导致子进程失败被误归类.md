...
- [`RunnerFailureRule`](../子系统/进程沙箱.md#wrapped-argv-and-classification-dialects) 携带可选的允许退出码、不区分大小写的逐行致命签名，以及按不区分大小写的整行精确匹配排除的信息性行。
- [`dsh-sandbox-local`](../../源码/沙盒/本地沙盒/) 把 Landlock 映射为退出码 125 加一行非通知的 `landlock-run:` 诊断，而 bwrap、Seatbelt 和自定义 runner 仍仅依据签名。
- [`dsh-bash-sandbox`](../../源码/命令/沙盒bash/) 直接 spawn 提供方 argv，因此启动前遭拒时使用 spawn 错误通道，而非本地化的 shell 诊断。已结算的前台与后台执行共用一个返回证据的分类器；致命证据优先于拒绝，前台错误会报告匹配到的致命行，同时保持捕获的 stderr 不变。
- [`dsh-tool-fs-search`](../../源码/文件系统/文件搜索工具/) 通过 `ctx.subprocess` 运行打包的 ripgrep，并继续位于沙箱化 bash seam 之外。
- 原生边界回归用例位于 [`partial-landlock.spec.ts`](../../源码/命令/沙盒bash/tests/partial-landlock.spec.ts)，包括信息性通知、致命证据和前台／后台分类。
- 组装后的产品路径由 [`partial-landlock` 快照组合](../../示例/acp-agent/partial-landlock.cordis.snapshot.yml)固定，独立于文件系统搜索的实现选择。
