...
如何接入一个新的模型提供方。参考实现：`源码/模型后端/llm-deepseek`（直接 HTTP，SSE（Server-Sent Events）由 `eventsource-parser` 分帧）与 `源码/模型后端/llm-pi-ai`（封装 LLM 库）。请先阅读 `源码/模型后端/llm/src/types.ts` 中的 `StreamChunk` 文档——它记录了两个适配器都经过验证的协议约定。
...
让协议格式（wire format）类型、请求序列化、传输解析、分片转换和适配器类分别承担独立职责；[`llm-deepseek`](../../源码/模型后端/llm-deepseek/README.zh.md) 是参考布局。

## 验证

遵循[仓库测试策略](../测试.md)，该策略负责适配器覆盖、真实提供方检查和已发布入口要求。
