...
本记录取代[可继续的后台 subagent](../../implemented/feature/2026-07-21-continuable-background-subagents.md)中由 Task 支撑的继续执行管理器。它保留[将 subagent 控制合并到 subagent 服务](../../已实现/简化/2026-07-26-合并subagent控制服务.md)确立的单一 `ctx.subagents` 服务，以及[以意图命名的 subagent 继续执行操作](../../已实现/简化/2026-07-27-按意图命名的subagent续跑操作.md)确立的 `followup` 操作。
