...
- **部分被取代。** 决策中的 steering 标注条款已不再描述 master：[标注移除决策](../简化/2026-08-10-web去掉转向插话说明.md)删除了 `插话` / `Interjection` 标注，轮次中途的 steer 只能靠它在消息流中的位置辨认。决策中的上下文来源与召回命名仍然有效，`SteeringMessageNode` 投影未变。
