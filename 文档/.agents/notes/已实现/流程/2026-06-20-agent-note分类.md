...
本仓库一贯的倾向是[机械质量门禁优于行文规范](2026-06-11-质量门禁.md)：不被机器检查的约定终将腐烂。因此这里的分类方案必须可强制执行，而非靠自觉的文件头。

## 决策

增加第二个维度，即 Agent Note 的**类别**，并将其编码在路径中：`{lifecycle}/{class}/yyyy-mm-dd-topic.md`。文件夹*就是*标签。文件位置声明其类别；封闭集合限定为「这些文件夹且仅限这些」；既有的 [verify-md-links](2026-06-18-markdown交叉链接检查.md) 门禁已经保护移动文件所需的路径改写。
