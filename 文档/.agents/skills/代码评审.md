...
- [dsh-prose-standard](../行文标准/SKILL.md): required coverage and editorial judgment for comments, docs, prompts, and visible strings.
- [docs/testing.md](../../../docs/testing.md) and the [quality-gates Agent Note](../../notes/已实现/流程/2026-06-11-质量门禁.md): required test tiers and gates.
...
1. **New prose receives semantic review.** Use [dsh-prose-standard](../行文标准/SKILL.md) to critically review every added or changed Markdown passage, JSDoc, comment, prompt, description, diagnostic, and visible string. Verify required coverage, accuracy, placement, and editorial quality against the owning code or behavior; automated checks do not establish those properties.
