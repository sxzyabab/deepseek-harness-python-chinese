...
Chain-of-thought leakage is prose whose vantage is the authoring session rather than the repository: it cites artifacts only that session could see, narrates the change instead of the state, or argues with a reviewer who has left. The fix is never deletion alone when a passage carries factual clauses — restate each so it stands at HEAD, then delete the transcript around it; a passage carrying none (an audit code, control-flow narration) is deleted outright. **REQUIRED BACKGROUND:** [dsh-prose-standard](../行文标准/SKILL.md) owns the complete-proposition rule this skill applies; the [committed-artifact-citations note](../../notes/已实现/流程/2026-08-09-已提交产物引用.md) owns the citation rule's rationale. It is guidance, not a script.
...
1. Scope and exclusions per [dsh-prose-standard](../行文标准/SKILL.md): require an explicit scope; never touch `vendor/`, `.agents/notes/已归档/`, or recorded fixtures and snapshots — recorded model output and sealed history keep their original voice.
