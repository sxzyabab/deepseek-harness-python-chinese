...
class 包含(条目树):
    ...
    def __init__(自身,上下文,配置):
        ...
        自身.文件名=解析配置文件路径(配置['path'],自身.上下文.基准网址)#配置文件路径
        ...
        if not 目录网址.endswith('/'):
            目录网址+='/'#目录网址必须带尾斜杠
        自身.上下文.__dict__['基准网址']=目录网址#子树的相对路径以该目录为基准
        自身.内容=None#上次读到的文本
    ...
    def 写入(自身):
        """把当前根条目数据安排写出。"""
        自身.上下文.广播('loader/config-update')#通知配置更新
