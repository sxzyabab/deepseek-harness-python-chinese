...
插件列表读取器.add_constructor(js标签,_构造表达式节点)#登记 !!js 读取
插件列表写出器.add_representer(dict,_写出映射)#登记映射写出
...
def 应用插件补丁(数据,补丁列表,警告):
    "按补丁列表改写插件配置列表，返回一份脱离原输入的新列表"
    ...
    def 建表(插件配置们):
        ...
        for 插件配置 in 插件配置们:
            ...
            if 编号:
                插件配置表[编号]=插件配置#登记
            if 插件配置.get('group') and isinstance(插件配置.get('config'),list):
                建表(插件配置.get('config'))#递归进组
    建表(数据)#索引现有插件配置
    for 补丁 in 补丁列表:
        编号=补丁.get('id')#目标编号
        插入列表=补丁.get('insert')#要插入的插件配置
        if 插入列表 is not None:
            if _插入插件配置(数据,插件配置表,编号,插入列表,警告):
                建表(插入列表)#插进去了，后续补丁才能改到这些插件配置
            continue#本条补丁结束
        if not 编号:
            警告('补丁缺少 id：只有插入型补丁可以省略 id')#非插入必须有编号
            continue#跳过
        目标=插件配置表.get(编号)#覆盖目标
        if 目标 is None:
            警告('补丁跳过：找不到插件配置 %C',编号)#找不到
            continue#跳过
        名称=补丁.get('name')#期望的插件名
        if 名称 and 名称!=目标.get('name'):
            警告('补丁跳过：插件配置 %C 的插件名是 %C，补丁写的是 %C',编号,目标.get('name'),名称)#名字不符
            continue#跳过
        for 键 in 补丁:
...
def _插入插件配置(数据,插件配置表,编号,插入列表,警告):
    "把插件配置插到根列表或某个组的配置里，返回是否真的插进去了"
    if not 编号:
        数据.extend(插入列表)#插到根列表
        return True#插入成功
    目标=插件配置表.get(编号)#目标组
    if 目标 is None:
        警告('插入补丁跳过：找不到插件配置 %C',编号)#找不到
        return False#没插进去
    if not 目标.get('group'):
        警告('插入补丁跳过：插件配置 %C 不是组',编号)#不是组
        return False#没插进去
...
def _解析日志开关(配置,上下文):
    ...
    if 启用 is not None:
        return 启用#显式配置
    插件配置对象=上下文.纤程.插件配置#所属插件配置
    if 插件配置对象 is not None:
        return bool(插件配置对象.父组.所属树.启用日志)#继承父树
    return False#默认关闭
