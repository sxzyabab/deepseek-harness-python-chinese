...
def 隔离(上下文,配置=None):
    ...
    def 条目初始化(条目对象):
        ...
        字典=条目对象.所属上下文.__dict__#条目上下文自有字典
        字典[符号.隔离]=原型映射(获取内部数据(条目对象.所属上下文,'属性链')[符号.隔离])#以祖先那张表为原型
        字典[符号.拦截]=原型映射(获取内部数据(条目对象.所属上下文,'属性链')[符号.拦截])#以祖先那张表为原型
    上下文.监听('loader/entry-init',条目初始化)

    def 补丁上下文(条目对象,续体):
        "按 isolate 与 intercept 重建隔离表，重载纤程后把实现迁到新标签"
        新表=原型映射(获取内部数据(条目对象.父组.所属上下文,'属性链')[符号.隔离])#以父隔离表为原型
        for 服务名 in (条目对象.选项.get('isolate') or {}):
            新表[服务名]=访问标签(条目对象,服务名,True)#本层隔离标签
        旧表=获取内部数据(条目对象.所属上下文,'属性链')[符号.隔离]#换表前的隔离表
        ...
        for 服务名 in dict.fromkeys(新表.键们()+list(旗标键表)):
            ...
            if 旗标键 is None:
                ...
                旗标键表[服务名]=旗标键#登记
            条目对象.所属上下文.__dict__[旗标键]=隔离标签(服务名+'#'+str(条目对象.选项.get('id')))#本条目旗标
            for 标签 in (旧标签,新标签):
                if 标签 is None:
                    continue#这一侧没有标签
                实现=条目对象.所属上下文.反射.存储.get(标签)#按标签取实现
                if 实现 is None or 实现.纤程 is None:
                    continue#该标签下还没有提供方
                本方旗标=获取内部数据(条目对象.所属上下文,'属性链').get(旗标键)#本条目旗标
                实现方旗标=获取内部数据(实现.纤程.所属上下文,'属性链').get(旗标键)#提供方看到的旗标
                变动[服务名]=(旧标签,新标签,本方旗标,实现方旗标)#记下四元组
                if 本方旗标 is not 实现方旗标:
                    break#提供方在隔离边界外，不再看另一侧
        隔离表=条目对象.所属上下文.__dict__[符号.隔离]#当前隔离表
        拦截表=条目对象.所属上下文.__dict__[符号.拦截]#当前拦截表
        隔离表.设原型(获取内部数据(条目对象.父组.所属上下文,'属性链')[符号.隔离])#重挂到当前父组
        拦截表.设原型(获取内部数据(条目对象.父组.所属上下文,'属性链')[符号.拦截])#重挂到当前父组
        隔离表.替换自有(新表)#换上新隔离标签
        拦截表.替换自有(条目对象.选项.get('intercept'))#换上新拦截配置
        续体()#按新表重载纤程
        存储=条目对象.所属上下文.反射.存储#实现存储
        ...
        条目对象.所属上下文.反射.通知(list(变动),过滤器)#刷新受影响的依赖方
        自有名们=新表.键们()#本层仍在用的服务名
        for 服务名 in list(旗标键表):
            if 服务名 not in 自有名们:
                条目对象.所属上下文.__dict__.pop(旗标键表[服务名],None)#清掉多余旗标
