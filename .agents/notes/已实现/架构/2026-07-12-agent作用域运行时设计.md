...
[仅面向自动化的 ACP 桥接层](../简化/2026-07-23-acp仅自动化协议.md)直接将一个进行中的提示词与其观察到的用户消息轮次关联。它不从日志水位线扫描，也不使用会话状态作为第二个调和预言机。
...
事件目录、服务目录、生产者/消费方矩阵、配置目录、模块图、工具目录、type-equiv 块和作用域事件解析器映射都是从源码生成或受新鲜度门禁约束的。[TypeScript 语义门禁 Agent Note](../流程/2026-07-14-基于typescript程序的语义门禁.md) 拥有 Program 构造、语义事件发现和解析器生成规则。
