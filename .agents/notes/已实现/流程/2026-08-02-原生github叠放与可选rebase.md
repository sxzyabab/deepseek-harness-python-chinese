...
[English](2026-08-02-原生github叠放与可选rebase.md) | 中文
...
merge-forward 和 rebase 都可以作为独立 PR 与官方堆叠 PR 的历史刷新方式，包括评审后。改写远端历史时，必须使用精确 lease 或受 lease 保护的 `gh stack` 推送路径；如果远端已经发生变化，操作必须中止。禁止直接使用 `--force`。[增量更新 base 的决策](2026-07-26-增量pr基线改向.md)仍负责 merge-forward 选项。
