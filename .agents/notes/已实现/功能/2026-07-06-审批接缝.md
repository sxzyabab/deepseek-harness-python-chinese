...
应答者通过[仅面向自动化的 ACP Agent Note](../简化/2026-07-23-acp仅自动化协议.md)描述的桥精确 agent 归属检查进行路由，保留了[多会话 Agent Note](2026-06-14-acp-multi-session.md) 要求的每会话权限归属。
...
- [仅面向自动化的 ACP Agent Note](../简化/2026-07-23-acp仅自动化协议.md)——应答者路由时对会话映射执行的精确 agent 归属检查；[多会话 Agent Note](2026-06-14-acp-multi-session.md)——本设计实现的每会话权限归属阻塞项。
