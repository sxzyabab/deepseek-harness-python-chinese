...
本记录已由[可继续的 subagent](2026-07-28-continuable-subagent-conversations.md)取代——后者以一个持久 Session 加至多一个进程内 Activation（驻留期）替换了其基于 Task 的 activation 模型、路由、取消和持久性语义。其服务放置与提供方功能策略此前已由[将 subagent 控制合并到 subagent 服务](../简化/2026-07-26-合并subagent控制服务.md)和[以意图命名的 subagent 继续执行操作](../简化/2026-07-27-按意图命名的subagent续跑操作.md)取代。仅持久 child 会话与 descriptor 的设计依据仍然有效。
...
**将控制编排放在 `SubagentRuntime` 上。** 这一服务放置方案即[服务合并决策](../简化/2026-07-26-合并subagent控制服务.md)；[意图操作细化](../简化/2026-07-27-按意图命名的subagent续跑操作.md)将提供方 start／resume 分发的复用限制在服务内部，同时将可选的 Task 与持久化工作隔离在注入的内部管理器中。
