...
- Use the Agent Note tree and its [rules](../../notes/README.md) to understand intentional architecture. The most relevant implemented examples are [drop mutable session summary](../../notes/已实现/简化/2026-06-19-去掉可变会话摘要.md), [shared persistence write coordinator](../../notes/已实现/架构/2026-06-18-共享持久化写入协调器.md), [capability seams](../../notes/已实现/架构/2026-06-13-能力接缝.md), and the twin adapter / dual persistence backend Agent Notes.
...
- Hand-rolled code reimplements what a well-maintained external package or a Node builtin at the engine floor already provides, and the swap would delete the implementation plus its dedicated tests ([dependency policy](../../notes/已实现/流程/2026-07-26-依赖优先于手写.md)).
...
Introducing a dependency is a valid simplification move, not a policy exception: the [dependency policy](../../notes/已实现/流程/2026-07-26-依赖优先于手写.md) owns the bar. When surveying, ask of protocol parsers, framers, retry/backoff loops, glob matchers, diff engines, and similar infrastructure: does a well-maintained npm package or a Node builtin at the repo's engine floor already do this?
...
Use [`dsh-archive-agent-notes`](../智能体归档.md) for retention judgment and archive mechanics. Low-future-value implemented notes move as frozen triplets to `已归档/{kind}`; proposed notes are never archived; rejected notes that no longer prevent a tempting mistake are deleted. Do not edit an archived note while simplifying current prose or code.
