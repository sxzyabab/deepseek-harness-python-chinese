...
- [Archived Agent Notes](../../notes/已归档/AGENTS.md) — frozen historical snapshots excluded from editorial maintenance and evolving documentation gates.
...
5. In `已实现/` Agent Notes, remove migration plans, acceptance-task checklists, and future-tense spec language. Keep concise verification contracts that identify the behaviors and tiers pinning the shipped decision, plus named coverage gaps.
6. If removing prose changes a promised behavior rather than its explanation, use a proposed Agent Note first (follow [dsh-find-simplifications](../代码简化.md)).

Exclude `.agents/notes/已归档/` from corpus audits and edits. Active prose may repair, redirect, or delete an inbound link, but never follow an archive-wide cleanup into the frozen target.
