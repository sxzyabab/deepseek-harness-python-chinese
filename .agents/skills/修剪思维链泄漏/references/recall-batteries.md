...
- Exclusions go last so a later include cannot re-admit them: `--glob '!vendor/**' --glob '!node_modules/**' --glob '!.agents/notes/已归档/**' --glob '!.agents/skills/修剪思维链泄漏/**'` (the skill's own files quote leaked wording as calibration), plus recorded fixture and snapshot directories in scope. The [owning note](../../../notes/已实现/流程/2026-08-09-已提交产物引用.md) also self-hits through its quoted evidence; judge it as evidence, not usage.
