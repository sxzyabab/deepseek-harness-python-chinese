...
1. Move the complete `foo.md`, `foo.zh.md`, and `foo.i18n.yaml` triplet from `已实现/<kind>/` to `已归档/<kind>/`; `已实现` is deliberately absent from the archive path.
...
Run the archive verifier's focused test, `pnpm run verify-archived-agent-notes`, `pnpm run doc-sync`, `pnpm run lint`, and `git diff --check`; select any additional evidence through [dsh-pre-push-checks](../推送前检查.md).
